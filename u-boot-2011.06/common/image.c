...
//MStar Start
void confirm_LX_address_by_mmap(unsigned int u32KernelLxInfo)
{
    unsigned int u32EnvLxInfo=0;
    if(getenv("E_LX_MEM_ADR")!=NULL && getenv("no_compare_kernel_lx")==NULL)
    {
        u32EnvLxInfo=(unsigned int)strtol(getenv("E_LX_MEM_ADR"),NULL,16);
        if( (u32KernelLxInfo&0xFF00000) != u32EnvLxInfo)
        {
             printf("\033[0;31m[ERROR] linux lx address is different from SN's\033[0m\n");
             printf("\033[0;31msn    lx address = 0x%x \033[0m\n",u32EnvLxInfo);
             printf("\033[0;31mlinux lx address = 0x%x \033[0m\n",(u32KernelLxInfo&0xFFF0000));
             printf("\033[0;31msetenv no_compare_kernel_lx 1 >> for skip this while\033[0m\n");
             printf("\033[0;31m========== while(1) ===========\033[0m\n");
             while(1);
        }            
    }
}
...
void image_print_contents (const void *ptr)
{
    ...
    // MStar Start
    confirm_LX_address_by_mmap(image_get_load(hdr));
    // Mstar End
#if CONFIG_MINIUBOOT
#else
 ...
 */
    ...
    ulong ramdisk_start,ramdisk_len;
#endif
        ...
        uint8_t arch, ulong *rd_start, ulong *rd_end)
{
    ...
    *rd_end = 0;
...
#ifdef CONFIG_ANDROID_BOOT_IMAGE
    else if (genimg_get_format(images->ahdr) == IMAGE_FORMAT_ANDROID){
    ...
    debug ("   ramdisk start = 0x%08lx, ramdisk end = 0x%08lx\n",
        ...
        ramdisk_start =rd_data;   
        ramdisk_len = rd_len;   
        #if 0
        //save ramdisk start addr to 0x100510/0x100512
        *( ( volatile u16 * ) (0x1f000000+0x100510*2)) = (rd_data)&0xffff;
        *( ( volatile u16 * ) (0x1f000000+0x100512*2)) = ((rd_data)>> 16);

        //save ramdisk end addr to 0x100514/0x100516
        *( ( volatile u16 * ) (0x1f000000+0x100514*2)) = (rd_data+rd_len)&0xffff;
        *( ( volatile u16 * ) (0x1f000000+0x100516*2)) = ((rd_data+rd_len)>> 16 );
        #endif
