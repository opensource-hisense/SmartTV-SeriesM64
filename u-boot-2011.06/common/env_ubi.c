...
extern int ubi_create_vol(char *volume, unsigned long long size, int dynamic);
...
int saveenv(void)
{
	...
	else
	{
		...
		ret = ubi_create_vol(ENV_VOL_NAME, (unsigned long long)env_vol_sz, 1);
		if(ret)
		{
	...
	if(last_ubi_partname[0] && strcmp((const char *) last_ubi_partname, (const char *) env_partition)){
