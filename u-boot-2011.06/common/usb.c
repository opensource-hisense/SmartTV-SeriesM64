			...
			void *buffer, int transfer_len, int *actual_length, int interval)
{
	 ...
	 *actual_length = dev->act_len;
	 USB_PRINTF("usb_submit_int_msg:act_len:%d\n", dev->act_len);
	 return 0;
}
...
//#ifdef CONFIG_LEGACY_USB_INIT_SEQ
	...
	SetControl1MaxPacketEx(desc->bMaxPacketSize0, UsbPortSelect);
	...
	if (!((dev->descriptor.idVendor == 0x0E8D) && (dev->descriptor.idProduct==0x76A0)))  // MTK 7662
    	wait_ms(200);
...
int usb_scan_devices(void)
{
	...
	//printf("[USB] idVendor = %x, idProduct = %x\n", dev->descriptor.idVendor, dev->descriptor.idProduct);
	 ...
	 if ((dev->descriptor.idVendor == 0x0E8D) && (dev->descriptor.idProduct==0x76A0))  // MTK 7662
	 {
		printf("MTK 7662 BT+WIFI\n");
		USB_Bulk_InitEx(dev, UsbPortSelect);
		USB_Interrupt_InitEx(dev, UsbPortSelect);
		return 0;
	 }
	...
	USB_PRINTF("scan end\n");
#endif
    if (dev->config.if_desc[0].desc.bInterfaceClass == 8)        //mass storage
    //do mass storage
    {}
#ifdef ENABLE_USB_LAN_MODULE
    else
