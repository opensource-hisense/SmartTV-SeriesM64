...
DECLARE_GLOBAL_DATA_PTR;
	...
	!defined(CONFIG_ENV_IS_IN_NANDUBI)		&& \
 ...
 */
int env_print_rescue(char *name)
{
	char *res = NULL;
	size_t len;
	...
	if (name) {		/* print a single name */
		...
		e.key = name;
		e.data = NULL;
		hsearch_r(e, FIND, &ep, &env_htab_rescue);
		if (ep == NULL)
			return 0;
		len = printf("%s=%s\n", ep->key, ep->data);
		return len;
...
#if CONFIG_RESCUE_ENV
int do_env_print_rescue (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	int i;
	int rcode = 0;
	...
	/* print selected env vars */
	for (i = 1; i < argc; ++i) {
		int rc = env_print_rescue(argv[i]);
		if (!rc) {
			printf("## Error: \"%s\" not defined\n", argv[i]);
			++rcode;
		}
...
int _do_env_set_rescue (int flag, int argc, char * const argv[])
{
	bd_t  *bd = gd->bd;
	int   i, len;
	int   console = -1;
...
//	env_id++;
	...
	if ((argc < 3) || argv[2] == NULL) {
		hdelete_r(name, &env_htab_rescue);
		return 0;
	}
	...
	e.key  = name;
	e.data = value;
	hsearch_r(e, ENTER, &ep, &env_htab_rescue);
	free(value);
	if (!ep) {
...
int setenv_rescue(char *varname, char *varvalue)
{
	char * const argv[4] = { "setenv", varname, varvalue, NULL };
	if ((varvalue == NULL) || (varvalue[0] == '\0'))
		return _do_env_set_rescue(0, 2, argv);
	else
		return _do_env_set_rescue(0, 3, argv);
}

int do_env_set_rescue(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    if (argc < 2)
        return cmd_usage(cmdtp);

    {
        ...
        if(0 == u32Switch)
        {
            printf("%s: BYPASS, '%s': %u, at %d\n", __func__, BRICK_TERMINATOR_SWITCH_STR, u32Switch, __LINE__);
            return 0;
        }
    }
    return _do_env_set_rescue(flag, argc, argv);
}
...
#if defined(CONFIG_CMD_NET) && (1!=CONFIG_MINIUBOOT)
...
int do_clearn_all_env(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    char ipaddr[64];
    char serverip[64];
    char ethaddr[64];
    ...
    if (argc == 1)
    {
        if(hdeleteall_r(&env_htab)<0)
        {
            return -1;
        }
        setenv("mboot_default_env", "1");
        saveenv();
    }
    else if(argc == 2)
    {
        if (strcmp(argv[1], "net") == 0)
        {
            ...
            strcpy(serverip, getenv("serverip"));
...
U_BOOT_CMD(
        cleanallenv, 2, 0,	do_clearn_all_env,
        "cleanall environment variables to persistent storage",
        " - clean all env\n"
        "cleanallenv net - clean all env except the network update env\n"
);

#if CONFIG_RESCUE_ENV
int do_env_save_rescue(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    extern char *env_name_spec;
    ...
    printf("Saving Environment (Rescue) to %s...\n", env_name_spec);

    return saveenv_rescue() ? 1 : 0;
}

U_BOOT_CMD(
	rescue_saveenv, 1, 0,	do_env_save_rescue,
	"save environment variables to the rescue env part of persistent storage",
	""
);
#endif
...
static int do_env_default(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    //Mstar
    #ifdef CONFIG_ENV_IS_IN_NANDUBI
    char mtdstr[128]={0};
...
#if CONFIG_RESCUE_ENV
static int do_env_export_rescue(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    char    buf[32];
    char    *addr, *cmd, *res;
    size_t  size;
    ...
    if (sep) {      /* export as text file */
        len = hexport_r(&env_htab_rescue, sep, &addr, size);
        if (len < 0) {
            error("Cannot export environment: errno = %d\n",
            errno);
            return 1;
...
#if CONFIG_RESCUE_ENV
static int do_env_import_rescue(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    char	*cmd, *addr;
    char	sep = '\n';
    int	chk = 0;
    ...
    if (himport_r(&env_htab_rescue, addr, size, sep, del ? 0 : H_NOCLEAR) == 0) {
        error("Environment import failed: errno = %d\n", errno);
        return 1;
    }
    gd->flags |= GD_FLG_ENV_READY;
...
#if CONFIG_RESCUE_ENV
	U_BOOT_CMD_MKENT(export_rescue, 4, 0, do_env_export_rescue, "", ""),
#endif
#endif
...
#if CONFIG_RESCUE_ENV
	U_BOOT_CMD_MKENT(import_rescue, 5, 0, do_env_import_rescue, "", ""),
#endif
#endif
...
#if CONFIG_RESCUE_ENV
U_BOOT_CMD_COMPLETE(
	rescue_printenv, CONFIG_SYS_MAXARGS, 1,	do_env_print_rescue,
	"print environment variables from rescue env",
	"\n    - print values of all environment variables\n"
	"printenv name ...\n"
	"    - print value of environment variable 'name'",
	var_complete
...
#if CONFIG_RESCUE_ENV
U_BOOT_CMD_COMPLETE(
	rescue_setenv, CONFIG_SYS_MAXARGS, 0,	do_env_set_rescue,
	"set environment variables to rescue env",
	"name value ...\n"
	"    - set environment variable 'name' to 'value ...'\n"
	"setenv name\n"
	"    - delete environment variable 'name'",
