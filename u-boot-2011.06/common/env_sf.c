...
#if (defined(CONFIG_KANO) || defined(CONFIG_CURRY)) // use BDMA instead of memcpy to speed up boot time
    flush_cache(pEnvBuf,CONFIG_ENV_SIZE);
    MDrv_BDMA_FlashCopy2Dram(  CONFIG_ENV_ADDR , (MS_PHYADDR)MsOS_VA2PA(pEnvBuf), CONFIG_ENV_SIZE);
#else
...
#if (defined(CONFIG_KANO) || defined(CONFIG_CURRY)) // use BDMA instead of memcpy to speed up boot time
    flush_cache(pEnvBufBak,CONFIG_ENV_SIZE);
    MDrv_BDMA_FlashCopy2Dram(  CONFIG_ENV_ADDR_BAK , (MS_PHYADDR)MsOS_VA2PA(pEnvBufBak), CONFIG_ENV_SIZE);
#else
