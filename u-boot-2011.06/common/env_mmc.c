			...
			unsigned long offset, const void *buffer)
{
   ...
   char command[100];
   sprintf(command,"mmc write.p.continue 0x%x %s 0x%x 0x%x",(unsigned int)buffer, env_part_name,(unsigned int)(mpart->block_count - (blk_cnt <<1) + (offset >> 9)),(unsigned int)blk_cnt<<9);
			...
			unsigned long offset, const void *buffer)
{
	...
	char command[100];
	sprintf(command,"mmc read.p.continue 0x%x %s 0x%x 0x%x",(unsigned int)buffer, env_part_name,(unsigned int)(mpart->block_count - (blk_cnt <<1) + (offset >> 9)),(unsigned int)blk_cnt<<9);
	ret=run_command((const char *)command, 0);
