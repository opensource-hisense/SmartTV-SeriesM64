...
int eth_initialize(bd_t *bis)
{
	...
	mv6446x_eth_initialize(bis);
		...
		show_boot_progress (-64);
                ...
                if (   NULL != (macaddr = getenv("macaddr"))
                    && strlen(macaddr) == strlen("XX:XX:XX:XX:XX:XX")
                    && ':' == macaddr[2]
                    && ':' == macaddr[5]
                    && ':' == macaddr[8]
                    && ':' == macaddr[11]
                    ...
                    )
                {
                    ...
                    memcpy(pmac, macaddr, strlen("XX:XX:XX:XX:XX:XX"));
                    pmac[2]  = '\0';
                    pmac[5]  = '\0';
                    pmac[8]  = '\0';
                    pmac[11] = '\0';
                    ...
                    MY_MAC[0]   = (u8)simple_strtoul(&(pmac[0]),  NULL, 16);
                    MY_MAC[1]   = (u8)simple_strtoul(&(pmac[3]),  NULL, 16);
                    MY_MAC[2]   = (u8)simple_strtoul(&(pmac[6]),  NULL, 16);
                    MY_MAC[3]   = (u8)simple_strtoul(&(pmac[9]),  NULL, 16);
                    MY_MAC[4]   = (u8)simple_strtoul(&(pmac[12]), NULL, 16);
                    MY_MAC[5]   = (u8)simple_strtoul(&(pmac[15]), NULL, 16);

                    free(pmac);
                }
                else
                {
                    printf(" Set MAC default\n");

                #if(LOAD_MAC_FROM_SPI_ENABLE==1)
                    char buffer[64];
                    int ret=0;
                    memset(buffer, 0 , 64);
                    snprintf(buffer, sizeof(buffer), "spi rdc 0x%x 0x%x 0x%x", MAC_DRAMADDR, MAC_SPIADDR, MAC_LEN);
                    printf("=>cmd: %s\n", buffer);
                    ret=run_command(buffer, 0);
                    if(ret)
                    {
                        printf(" Get MAC from spi\n");
                         s=(unsigned char *)MAC_DRAMADDR;
                        for (i = 0; i < MAC_LEN; ++i)
                        {
                            sa[i] = s[i];
                        }
            ...
            //Write ethaddr to emac reg for kernel usage.
