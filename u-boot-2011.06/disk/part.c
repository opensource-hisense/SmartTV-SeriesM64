...
static const struct block_drvr block_drvr[] = {
	...
	{ .name = "sdcard", .get_dev = sd_get_dev, },
#endif
...
void dev_print (block_dev_desc_t *dev_desc)
{
	...
	case IF_TYPE_USB:
		...
		break;
	case IF_TYPE_NAND:
		puts("device type Mstar NAND FTL\n");
		return;
	case IF_TYPE_DOC:
...
void init_part (block_dev_desc_t * dev_desc)
{
	...
	if (test_part_mac(dev_desc) == 0) {
        ...
        if (test_part_emmc(dev_desc) == 0) {
		#if defined(CONFIG_MSTAR_UNFD_FTL)
		dev_desc->part_type = PART_TYPE_NAND;
		#elif defined(CONFIG_MMC)
                dev_desc->part_type = PART_TYPE_EMMC;
		#endif
                return;
        }
#endif
...
void print_part (block_dev_desc_t * dev_desc)
{
	...
	case PART_TYPE_NAND:
		...
		#if defined(CONFIG_MSTAR_UNFD_FTL)
                PRINTF ("## Testing for valid Mstar NAND partition ##\n");
