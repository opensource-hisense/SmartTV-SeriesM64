...
retry:
		...
		res = nand_erase_nand(mtd, &einfo, 1);
            ...
            for(k=0; k<bbtpage; k++){
                ...
                /* Mark it as used */
                #ifdef CONFIG_MTD_UBI_BACKUP_LSB                
                td->pages[chip][k] = (int) (unsigned long)(to >> this->page_shift) + ga_tPairedPageMap[k].u16_LSB;
                #else
                td->pages[chip][k] = (int) (unsigned long)(to >> this->page_shift) + k ;
                #endif
...
//	len += (len >> this->page_shift) * mtd->oobsize;
    len = NAND_MAX_BBT_PAGES * (mtd->writesize + mtd->oobsize);

    #if (ENABLE_MODULE_NAND_FLASH == 1)
    //scan and mark cis block
    cisbuf = vmalloc(mtd->writesize + mtd->oobsize);
    if (!cisbuf) {
