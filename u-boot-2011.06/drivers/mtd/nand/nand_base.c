...
static int nand_default_block_markbad(struct mtd_info *mtd, loff_t ofs)
{
	...
	int block, ret = 0;
    #if defined(CONFIG_ENV_IS_IN_NAND)
	uint32_t u32_env_in_nand_start,u32_env_in_nand_end;
	uint8_t u8_skip_env_in_nand =0;
    #endif

	if(chip->bbt)
		crc = crc32(BBT_CRC32_INIT, chip->bbt, (mtd->size >> (chip->bbt_erase_shift + 2)));
	if ((chip->bbt) && (crc != chip->bbt_crc))
	{
		chip->scan_bbt(mtd);
		printk(KERN_EMERG "NAND bbt in memory has been reread\n");
	}
	...
	if (chip->bbt)
	{
		...
		chip->bbt_crc = crc32(BBT_CRC32_INIT, chip->bbt, (mtd->size >> (chip->bbt_erase_shift + 2)));
	}

	/* Do we have a flash based bad block table ? */
	if (chip->options & NAND_USE_FLASH_BBT)
		ret = nand_update_td(mtd, ofs);

    #if defined(CONFIG_ENV_IS_IN_NAND)
	u32_env_in_nand_start= u16_ENV_Star_Blk*mtd->erasesize;
	u32_env_in_nand_end	=(u16_ENV_Star_Blk + u16_ENV_Blk_Region -1)*mtd->erasesize;    
	if(((ofs >= u32_env_in_nand_start)||(ofs <= u32_env_in_nand_end))
	&& (u32_env_in_nand_start >0) && (u32_env_in_nand_end >0))// not in nand env region
	{
		u8_skip_env_in_nand =1;
	}
    #endif
	...
	*/
    ...
    #if defined(CONFIG_ENV_IS_IN_NAND) 
	if(!u8_skip_env_in_nand)// not in nand env region
	{
		...
		if (realpage != chip->pagebuf || oob) {
			...
			if (unlikely(ops->mode == MTD_OOB_RAW))
				ret = chip->ecc.read_page_raw(mtd, chip,
						bufpoi, page);
#if (ENABLE_MODULE_SPI_NAND_FLASH == 1) && (SPINAND_READ_SUBPAGE == 1)
			else if(!aligned && !oob)
#else
		    ...
		    int allowbbt)
{
	...
	if (chip->options & BBT_AUTO_REFRESH && !allowbbt)
		bbt_masked_page = chip->bbt_td->pages[chipnr][0] & BBT_PAGE_MASK;
	...
	while (len) {
		...
		if (status & NAND_STATUS_FAIL) {
			...
			instr->state = NAND_STATUS_HWFAIL;
		...
		if (len && !(page & chip->pagemask)) {
			...
			if (bbt_masked_page != 0xffffffff &&
				...
				bbt_masked_page = chip->bbt_td->pages[chipnr][0] &
					BBT_PAGE_MASK;
		}
	}
...
static int nand_block_markbad_e(struct mtd_info *mtd, loff_t ofs)
{
	...
	return chip->block_markbad_e(mtd, ofs);
}
...
static void nand_set_defaults(struct nand_chip *chip, int busw)
{
	...
	if (!chip->block_markbad_e)
		chip->block_markbad_e = nand_default_block_markbad_e;    
	if (!chip->write_buf)
                          ...
                          const struct nand_flash_dev *type)
{
	...
	mtd->oobblock = mtd->writesize = pNandDrv->u16_PageByteCnt * pNandDrv->u8_PlaneCnt;
	...
	mtd->ismlc = pNandDrv->u8_CellType;
                          ...
                          const struct nand_flash_dev *type)
{
	...
	mtd->oobblock = mtd->writesize = gtSpiNandDrv.tSpinandInfo.u16_PageByteCnt;
	mtd->oobsize = gtSpiNandDrv.tSpinandInfo.u16_SpareByteCnt;
	mtd->erasesize = gtSpiNandDrv.tSpinandInfo.u16_BlkPageCnt * gtSpiNandDrv.tSpinandInfo.u16_PageByteCnt;
	chip->chipsize = (uint64_t)gtSpiNandDrv.tSpinandInfo.u16_BlkCnt * (uint64_t)gtSpiNandDrv.tSpinandInfo.u16_BlkPageCnt * (uint64_t)gtSpiNandDrv.tSpinandInfo.u16_PageByteCnt;

	if(!mtd->writesize || !mtd->oobsize || !mtd->erasesize)
	{
		int i;
		printf("Unsupported SPI NAND Flash type is detected with ID");
		for(i = 0; i < gtSpiNandDrv.tSpinandInfo.u8_IDByteCnt; i++)
			printf(" 0x%X", gtSpiNandDrv.tSpinandInfo.au8_ID[i]);
		printf("\n");
		return  ERR_PTR(-EINVAL);
	}

	chip->onfi_version = 0;
	chip->cellinfo = gtSpiNandDrv.tSpinandInfo.au8_ID[2];
	mtd->ismlc = 0;
		    ...
		    const struct nand_flash_dev *table)
{
	...
	type = nand_get_flash_type_mstar(mtd, chip, busw, &nand_maf_id, &nand_dev_id, table);
...
int nand_scan_tail(struct mtd_info *mtd)
{
	...
	for (i = 0; i < ARRAY_SIZE(chip->ecc.layout->oobfree); i++)
	{
	    if(chip->ecc.layout->oobfree[i].length)
