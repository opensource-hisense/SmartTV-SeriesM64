...
extern U32 MDrv_SPINAND_ReadBlkByteOffset(U8 *pu8_DestAddr, U16 u16_PBA, U32 u32_StartByte, U32 u32_ByteCnt);
extern U32 MDrv_SPINAND_WriteBlkByteOffset(U8 *pu8_DestAddr, U16 u16_PBA, U32 u32_StartByte, U32 u32_ByteCnt);
#endif
...
int nand_erase_env(nand_info_t *meminfo, const nand_erase_options_t *opts)
{
	...
	buf = malloc(meminfo->writesize+meminfo->oobsize);
    if(buf ==NULL)
	    return 1;	
	...
	{ 
		...
		chip->ops.datbuf  = buf;
		chip->ops.oobbuf  = &buf[meminfo->writesize];
		chip->ops.ooboffs = 0;
        ...
        else
        {
            ...
            {
                memset(buf, '\0' , meminfo->writesize+meminfo->oobsize);
				chip->ops.ooblen = meminfo->oobsize;
				chip->ops.len    = meminfo->writesize;
				chip->ops.datbuf = buf;
				chip->ops.oobbuf = &buf[meminfo->writesize];
...
//	int need_skip;
    ...
    //always check bad block for handling write fails
    #if 0
	...
	while (left_to_write > 0) {
		...
		if (rval != 0) {
            ...
            p_buffer -= write_size;
            //mark bad
            rval = nand->block_markbad(nand, offset & ~(nand->erasesize - 1));
            if(rval != 0)
            {
                printf("block 0x%08llx NOT marked "
                    "as bad! ERROR %d\n",
                    offset & ~(nand->erasesize - 1), rval);
                return rval;
            }
            offset += nand->erasesize - block_offset;
            continue;
...
//	int need_skip;
    ...
    */
	...
	while (left_to_read > 0) {
        ...
        {
            if(nand->ismlc)
    		    nand_ReadDisturbance_BigImg((offset & ~(nand->erasesize - 1))/nand->writesize, 0, nand_MTDMarkBad, 0xFF, 0xFF);
        }
        #endif
			...
			u_char *buffer, int withoob)
{
	...
	u_char *p_tmpbuf = pNandDrv->pu8_PageDataBuf;
	ulong u32_Row;
	...
	memset(p_tmpbuf, 0xa5, nand->writesize);	
		       ...
		       u_char *buffer, int refresh)
{
	...
	if(((*length) & (nand->writesize - 1)) != 0)
	{
		PageCnt += 1;
		aligned = 0;
        left_to_read = PageCnt << pNandDrv->u8_PageByteCntBits;
	}
	
	p_Spare = pNandDrv->pu8_PageSpareBuf;
		...
		if(pNandDrv->u8_CellType)
		{
			...
			for(i = 0; i < read_length >> pNandDrv->u8_PageByteCntBits; i++)
			{
				...
				else
				{
                    ...
                    if(p_Spare[0x5] == 0xDE && p_Spare[0x6] == 0xAD)
                        OverThreshold = 1;
                    #endif                    
				}
			...
			if(ReadRetryHappens == 1)
				nand_ReadDisturbance_BigImg(u32_Row, 1, nand_MTDMarkBad, 0xFF, 0xFF);
			//Trigger Too many correctable ecc bits handling
			if(OverThreshold == 1)
				nand_ReadDisturbance_BigImg(u32_Row, 0, nand_MTDMarkBad, 0xFF, 0xFF);
			...
			u_char *buffer, int withoob)
{
	...
	while (left_to_write > 0) {
        ...
        rval = MDrv_SPINAND_WriteBlkByteOffset(p_buffer, u16_PBA, block_offset, write_size);
		loffset += write_size;
		p_buffer += write_size;
			...
			u_char *buffer)
{
	...
	while (left_to_read > 0) {
        ...
        rval = MDrv_SPINAND_ReadBlkByteOffset(p_buffer, u16_PBA, block_offset, read_size);
		loffset += read_size;
		p_buffer += read_size;
