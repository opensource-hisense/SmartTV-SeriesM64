...
#define MAX_SECTOR_SIZE 1024//2048
#define SECTOR_SIZE FS_BLOCK_SIZE

#define FS_BLOCK_SIZE (cur_dev->blksz)  //512
...
#define FATBUFBLOCKS	8
#define MAX_FATBUFSIZE (MAX_SECTOR_SIZE*FATBUFBLOCKS)
#define FATBUFSIZE	(FS_BLOCK_SIZE*FATBUFBLOCKS)
#define FAT12BUFSIZE	((FATBUFSIZE*2)/3)
#define FAT16BUFSIZE	(FATBUFSIZE/2)
#define FAT32BUFSIZE	(FATBUFSIZE/4)

#define FATMEMBUFBLOCKS 8192
#define FATMEMBUFSIZE (0x200*FATMEMBUFBLOCKS/4)
...
#define TOHIGHER(c)	if((c) >= 'a' && (c) <= 'z'){(c)-=('a' - 'A');}
...
typedef struct {
    ...
    __u32 entnum;        /* max FAT entry number */ 
    __u32 partfatlength; /* Length of FAT in sectors in buffer */
    __u16	fat_sect;	/* Starting sector of the FAT */
    __u32	rootdir_sect;	/* Start sector of root directory */
    __u16	clust_size;	/* Size of clusters in sectors */
    __u32	data_begin;	/* The sector of the first cluster, can be negative */
    int    rootdir_size;   /* Size of rootdir */
    __u32 fatbufsize; /* Soze of part FAT in buffer */
    __u32 maxfatentry;  /* max cluster record in FAT */
    __u32 endfatentry;  /* end cluser record in FAT */
} fsdata;
...
typedef long	(file_write_func)(const char *filename, void *buffer,
				 unsigned long maxsize);
...
#define MAXFILEBUFFER 16
#define MODECREATE 1
#define MODEAPPEND 2
#define MODENEW 3

struct filedescipter {
	long          fd;           //file num
	long          mode;         //mode
	__u32         ent_cluser;   //file entry cluster
	int           ent_offset;   //file entry offset
	dir_entry     dirEnt;       //file entry in dir
	char filename[13]; //only support short name
};
...
file_write_func		file_fat_writefile;
...
long file_fat_filesize(const char *filename);
long file_fat_part_read(const char *filename, void *buffer, unsigned long offset, unsigned long maxsize);
long file_fat_writefile(const char *filename, void *buffer, unsigned long maxsize);
...
int file_fat_list(const char *dir,void *buffer, unsigned long maxsize);
