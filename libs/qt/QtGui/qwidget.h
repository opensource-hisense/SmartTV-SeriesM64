...
public:
#if defined(MSTAR_WEBGL) && MSTAR_WEBGL
    //  if DFB surface size-change, reset EGLSurface
    virtual void ResetEGLSurface(){}
    // MWB crash when closing window
    virtual void DestroyContext(){}
    // 2012-09-14, david.peng: add interface to set surface's opacity
    virtual void setSurfaceOpacity(qreal){}
#endif

    // Add interface to adjust stacking of DFB window directly
    void windowRaise();
    void windowLower();
    void windowRaiseToTop();
    void windowLowerToBottom();
    void windowPutAtop(QWidget* lower);
    void windowPutBelow(QWidget* upper);
    void windowSetStackingClassMIDDLE();
    void windowSetStackingClassUPPER();
    void windowSetStackingClassLOWER();
...
public:
    ...
    void changeResolution(int width, int height, int size);
...
public:
    // [2012/11/02] Jie added.
    // Reduce memory usage by resizing 2D DFB window.
    void restoreWindowSurfaceSize();
    void minimizeWindowSurfaceSize();
};
