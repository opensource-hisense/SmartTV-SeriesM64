...
typedef enum ne_session_flag_e {
    ...
    // MStar Patch Begin
    NE_SESSFLAG_DISABLE_TE, /*disable this flag to send TE field*/
    // MStar Patch End
...
/* RR1358 : Check if CA is revoked for youtube. */
/* set the flag of revoke mode to determine if we should check ssl CA revoke for the session */
void ne_session_set_ssl_revoke_mode(ne_session *sess, int nRevokeMode);

/* get the flag of revoke mode to determine if we should check ssl CA revoke for the session */
int ne_session_get_ssl_revoke_mode(ne_session *sess);

/* set the CA path of client */
void ne_session_set_Client_CApath(ne_session *sess, const char *szCAPath);

/* get the CA path of client */
const char *ne_session_get_Client_CApath(ne_session *sess);
