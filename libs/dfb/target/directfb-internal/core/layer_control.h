...
DFBResult dfb_layer_set_hv_mirror_enable( CoreLayer *layer, 
                                              bool HEnable,
                                              bool VEnable );
DFBResult dfb_layer_set_lbcouple_enable( CoreLayer *layer, 
                                              bool LBCoupleEnable);
DFBResult dfb_layer_set_GOPDstByPass_enable( CoreLayer *layer, bool ByPassEnable);

DFBResult dfb_layer_set_hv_scale( CoreLayer *layer, int HScale,int VScale );

DFBResult dfb_layer_Set_GopDst(DFBDisplayLayerID  id, DFBDisplayLayerGOPDST gop_dst);

DFBResult dfb_layer_Set_ForceWrite( CoreLayer *layer, bool force_write);
DFBDisplayLayerGOPDST dfb_layer_Get_GopDst(DFBDisplayLayerID  id);

u8 dfb_layer_Get_GopIndex(DFBDisplayLayerID  id);

DFBResult dfb_layer_Set_Boot_Logo_Patch(DFBDisplayLayerID  id, int miusel);

DFBResult dfb_layer_get_screen (CoreLayer *layer, CoreScreen **ret_screen);
