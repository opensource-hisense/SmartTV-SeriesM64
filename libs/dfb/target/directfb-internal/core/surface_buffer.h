...
struct __DFB_CoreSurfaceAllocation {
     ...
     CoreGraphicsSerial        gfxSerial;    /*last graphic operation serial NO about this surface, including GPU write & read*/
...
struct __DFB_CoreSurfaceBufferLock {
     ...
     void                    *addr;               /* " */
     unsigned long            phys;               /* " */
     CoreSurfaceBufferPhyType phys_type;          /* tlb or phy*/
     unsigned char            miu;                 /* whichi miu*/
#ifdef DFB_ION_TLB     
     unsigned long            tlb_table_addr;  /*tlb base addr in this miu */
#endif
...
DFBResult dfb_surface_buffer_allocations_destroy( CoreSurfaceBuffer *buffer );
DFBResult dfb_surface_buffer_allocations_sync( CoreSurfaceBuffer *buffer,  DFBSurfaceSyncFlags flag);
