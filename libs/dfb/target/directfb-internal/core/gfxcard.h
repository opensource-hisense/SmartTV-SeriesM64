...
#undef UFO_GFX_TRIANGLE
#undef UFO_GFX_SPAN

#ifdef HAVE_UFO_H
#include "UFO.h"

#if defined(UFO_GFX_TRIANGLE) && defined(UFO_GFX_SPAN)
#define USE_GFX_EXTENSION
#endif
...
typedef enum {
     EXT_API_FILLTRIANGLE = 0x00000001,
     EXT_API_FILLSPAN = 0x00000002,
} ExtensionAPI;
#endif
...
typedef struct {
     ...
     unsigned int            surface_hegiht_alignment;//used  only by tilemode
} CardLimitations;
...
typedef struct _GraphicsDeviceFuncs {
     ...
     bool (*FillTriangle)  ( void *driver_data, void *device_data,
                             DFBTriangle *tri );
#if defined(USE_GFX_EXTENSION)
     bool (*FillTriangleEx)  ( void *driver_data, void *device_data,
                             DFBTriangle *tri, DFBRegion *clip );

     bool (*FillSpan)  ( void *driver_data, void *device_data,
                             DFBSpan *span, int num_spans, int y, DFBRegion *clip );
#endif
     bool (*FillTrapezoid)(void *driver_data, void *device_data, DFBTrapezoid *dtrapezoid );

     bool (*DrawOval)(void *driver_data, void *device_data, DFBOval *oval );
     ...
     bool (*BlitTrapezoid)( void *driver_data, void *device_data,
            DFBRectangle *srect, DFBTrapezoid *dtrapezoid );
     ...
     /*
     * stretchblitEX function
     */
     bool (*StretchBlitEx) (void *driver_data, void *device_data,
                            DFBRectangle *srect, DFBRectangle *drect, const DFBGFX_ScaleInfo *scale_info);

    /*
    * begin/end a sequence of hw operations
    */
     void (*BeginDraw)();
     void (*EndDraw)();
#if defined(USE_GFX_EXTENSION)
     bool (*CheckExtension)  ( struct _GraphicsDeviceFuncs *funcs, ExtensionAPI ext_api);
#endif

     bool (*BlitGLES2)            ( void *driver_data, void *device_data, CardState *state,
                               DFBRectangle *rect, int dx, int dy );
 ...
 */
DFBResult dfb_gfxcard_fillrectangles         ( const DFBRectangle   *rects,
                                          int                   num,
                                          CardState            *state );

DFBResult dfb_gfxcard_drawovals         ( const DFBOval   *ovals,
                                          int                   num,
                                          CardState            *state );

DFBResult dfb_gfxcard_drawrectangle          ( DFBRectangle         *rect,
                                          CardState            *state );

DFBResult dfb_gfxcard_drawlines              ( DFBRegion            *lines,
                                          int                   num_lines,
                                          CardState            *state );

DFBResult dfb_gfxcard_fillspans              ( int                   y,
                                          DFBSpan              *spans,
                                          int                   num_spans,
                                          CardState            *state );

DFBResult dfb_gfxcard_filltriangles          ( const DFBTriangle    *tris,
                                          int                   num,
                                          CardState            *state );

DFBResult dfb_gfxcard_blit                   ( DFBRectangle         *rect,
                                          int                   dx,
                                          int                   dy,
                                          CardState            *state );

DFBResult dfb_gfxcard_blit_gles2               ( DFBRectangle         *rect,
                                          int                   dx,
                                          int                   dy,
                                          CardState            *state );

DFBResult dfb_gfxcard_trapezoidblit(DFBRectangle *srect,
                                               DFBTrapezoid *trapezoid,
                                               CardState *state );

DFBResult dfb_gfxcard_filltrapezoid(DFBTrapezoid *trapezoid,
                                   CardState *state );


DFBResult dfb_gfxcard_batchblit              ( DFBRectangle         *rects,
                                          DFBPoint             *points,
                                          int                   num,
                                          CardState            *state );

DFBResult dfb_gfxcard_tileblit               ( DFBRectangle         *rect,
                                          int                   dx1,
                                          int                   dy1,
                                          int                   dx2,
                                          int                   dy2,
...
DFBResult dfb_gfxcard_stretchblitEx( DFBRectangle         *srect,
                                          DFBRectangle         *drect,
                                          CardState            *state ,
                                          const DFBGFX_ScaleInfo  *scaleinfo);

DFBResult dfb_gfxcard_texture_triangles      ( DFBVertex            *vertices,
                                          int                   num,
                                          DFBTriangleFormation  formation,
                                          CardState            *state );

DFBResult dfb_gfxcard_drawstring             ( const u8             *text,
                                          int                   bytes,
...
DFBResult dfb_gfxcard_drawglyph              ( CoreGlyphData       **glyph,
                                          int                   x,
                                          int                   y,
                                          CoreFont             *font,
                                          unsigned int          layers,
...
DFBResult dfb_gfxcard_drawglyphs              ( unsigned int          *index,
                                          int                  *x,
                                          int                  *y,
                                          CoreFont             *font,
                                          unsigned int         layers,
...
DFBResult dfb_gfxcard_sync_surface( CardState   *state, DFBSurfaceSyncFlags flags);
