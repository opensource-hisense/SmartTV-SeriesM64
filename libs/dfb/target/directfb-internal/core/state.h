...
typedef enum {
     ...
     SMF_ALPHA_CMP          =    0x00100000,
     SMF_BLIT_NEARESTMODE  =    0x00200000,
     SMF_BLIT_XMIRROR    = 0x00400000,
     SMF_BLIT_YMIRROR    = 0x00800000,
     SMF_BLINK             = 0x10000000,
#ifdef PATCH_MSTAR_GE_BLEND_OPT
     SMF_MSTAR_PATCH_BLEND = 0x01000000,
     SMF_ALL               = 0x11F33FFF
#else
     SMF_ALL               = 0x10F33FFF
#endif     
...
struct _CardState {
     ...
     DFBColor                 color2;         /* color for gradient draw end color*/
     unsigned int             color_index;   /* index to color in palette */
     unsigned int             color2_index;   /* index to color2 in palette */
...
#ifdef PATCH_MSTAR_GE_BLEND_OPT
     DFBSurfaceBlendFunction  mstar_patchBlendParam;
#endif
     DFB_ACmpOp               cmp_mode;     //extra alpha mode

     bool                     blit_nearestmode_enabled;
     bool                     blit_xmirror_enabled;
     bool                     blit_ymirror_enabled;
...
#define dfb_state_set_color2_index(state,index)    _dfb_state_set_checked( color2_index,    \
                                                                          COLOR,          \
                                                                          state, index )
...
#ifdef PATCH_MSTAR_GE_BLEND_OPT
#define dfb_state_set_mstar_patch_blend(state,blend)      _dfb_state_set_checked( mstar_patchBlendParam,      \
                                                                          MSTAR_PATCH_BLEND,      \
                                                                          state, blend )
#endif                                                                          


#define dfb_state_set_alpha_cmp_mode(state,cmpmode)      _dfb_state_set_checked( cmp_mode,      \
                                                                          ALPHA_CMP,      \
                                                                          state, cmpmode )

#define dfb_state_set_blit_nearestmode_enabled(state,nearestmode_enabled)      _dfb_state_set_checked( blit_nearestmode_enabled,      \
                                                                          BLIT_NEARESTMODE,      \
                                                                          state, nearestmode_enabled )

#define dfb_state_set_blit_xmirror_enabled(state,xmirror_enabled)      _dfb_state_set_checked( blit_xmirror_enabled,      \
                                                                          BLIT_XMIRROR,      \
                                                                          state, xmirror_enabled )

#define dfb_state_set_blit_ymirror_enabled(state,ymirror_enabled)      _dfb_state_set_checked( blit_ymirror_enabled,      \
                                                                          BLIT_YMIRROR,      \
                                                                          state, ymirror_enabled )
...
static inline void dfb_state_set_color2( CardState *state, const DFBColor *color )
{
     D_MAGIC_ASSERT( state, CardState );
     D_ASSERT( color != NULL );

     if (! DFB_COLOR_EQUAL( state->color2, *color )) {
          state->color2    = *color;
          state->modified = (StateModificationFlags)( state->modified | SMF_COLOR );//share the same flag with color
     }
}
