...
typedef enum {
     ...
     CSCONF_PREALLOCATED_IN_VIDEO = 0x00000020,

     CSCONF_PREALLOCATED_IN_VIDEO_NO_VA = 0x00000040, /* the preallcate by the other process, VA can't share over different process,
                    so, use new flags to identify this preallocate don't get VA.*/

     CSCONF_ALL          = 0x00000077
...
typedef enum {
     ...
     CSTF_PREALLOCATED_IN_VIDEO = 0x00000800,  /* preallocated memory in video memory */

     CSTF_MIU0           = 0x00001000,/* allocated video memory in MIU0*/
     CSTF_MIU1           = 0x00002000,/* allocated video memory in MIU1*/
     CSTF_DECLARABLE       = 0x00004000,/* should been allocated in spec pool*/
     CSTF_DEDICATED      = 0x00008000, /* should be allocated from devmem for imageprovider gopc & vec*/

     CSTF_MIU2           = 0x00010000,/* allocated video memory in MIU2*/
...
DFBResult dfb_surface_destroy_allocations( CoreSurface *surface );

DFBResult dfb_surface_sync_allocations( CoreSurface *surface, DFBSurfaceSyncFlags flag);
                               ...
                               DFBSurfaceCapabilities *caps )
{
     switch (policy) {
          case CSP_SYSTEMONLY:
               *caps = (DFBSurfaceCapabilities)((*caps & ~(DSCAPS_VIDEOONLY|DSCAPS_VIDEOHIGH)) | DSCAPS_SYSTEMONLY);
               break;

          case CSP_VIDEOONLY:
               *caps = (DFBSurfaceCapabilities)((*caps & ~(DSCAPS_SYSTEMONLY|DSCAPS_VIDEOHIGH)) | DSCAPS_VIDEOONLY);
               break;

          case CSP_VIDEOHIGH:
               *caps = (DFBSurfaceCapabilities)((*caps & ~(DSCAPS_SYSTEMONLY | DSCAPS_VIDEOONLY)) | DSCAPS_VIDEOHIGH);
               break;
