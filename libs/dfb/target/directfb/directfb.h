...
//section for devive DFB ioctl request ID
// use 'D' as magic number
#define MDRV_DFB_IOC_MAGIC      'D'

#define DFB_LINUX_INPUT_GET_PHY_KB_CNT 1
#define DFB_LINUX_INPUT_GET_PHY_MS_CNT 2
#define DFB_LINUX_INPUT_SET_MOUSE_SENSITIVITY_NUM 3
#define DFB_LINUX_INPUT_SET_MOUSE_SENSITIVITY_DEN 4
#define DFB_LOOPBACK_INPUT_SEND_EVENT 5
#define DFB_MSTARKEYPAD_SET_INDEX 6
#define DFB_MSTARKEYPAD_SET_ENABLE 7
#define DFB_STOP_INPUT      8
#define DFB_START_INPUT     9
#define DFB_MSTARIR_SET_REPEAT_TIME     10
#define DFB_MSTARIR_GET_REPEAT_TIME     11
#define DFB_LOOPBACK_INPUT_SET_DEVICE_ID     12




#define DFB_DEV_IOC_GET_PHY_KEYBOARD_CNT                     _IOR(MDRV_DFB_IOC_MAGIC,   DFB_LINUX_INPUT_GET_PHY_KB_CNT, void*)
#define DFB_DEV_IOC_GET_PHY_MOUSE_CNT                          _IOR(MDRV_DFB_IOC_MAGIC,   DFB_LINUX_INPUT_GET_PHY_MS_CNT, void*)
#define DFB_DEV_IOC_SET_REL_AXIS_SENSE_NUMERATOR                   _IOW(MDRV_DFB_IOC_MAGIC,   DFB_LINUX_INPUT_SET_MOUSE_SENSITIVITY_NUM, void*)
#define DFB_DEV_IOC_SET_REL_AXIS_SENSE_DENOMINATOR                   _IOW(MDRV_DFB_IOC_MAGIC,   DFB_LINUX_INPUT_SET_MOUSE_SENSITIVITY_DEN, void*)
#define DFB_DEV_IOC_SEND_LOOPBACK_EVENT                   _IOW(MDRV_DFB_IOC_MAGIC,   DFB_LOOPBACK_INPUT_SEND_EVENT, void*)
#define DFB_DEV_IOC_SET_FAKE_DEVICE_ID                   _IOW(MDRV_DFB_IOC_MAGIC,   DFB_LOOPBACK_INPUT_SET_DEVICE_ID, void*)
#define DFB_DEV_IOC_SET_KEYPAD_INDEX                   _IOW(MDRV_DFB_IOC_MAGIC,   DFB_MSTARKEYPAD_SET_INDEX, void*)
#define DFB_DEV_IOC_SET_KEYPAD_ENABLE                   _IOW(MDRV_DFB_IOC_MAGIC,   DFB_MSTARKEYPAD_SET_ENABLE, void*)
#define DFB_IOC_STOP_INPUT                              _IOW(MDRV_DFB_IOC_MAGIC,   DFB_STOP_INPUT, void*)
#define DFB_IOC_START_INPUT                              _IOW(MDRV_DFB_IOC_MAGIC,   DFB_START_INPUT, void*)
#define DFB_DEV_IOC_SET_MSTARIR_REPEAT_TIME                   _IOW(MDRV_DFB_IOC_MAGIC,   DFB_MSTARIR_SET_REPEAT_TIME, void*)
#define DFB_DEV_IOC_GET_MSTARIR_REPEAT_TIME                   _IOW(MDRV_DFB_IOC_MAGIC,   DFB_MSTARIR_GET_REPEAT_TIME, void*)
...
/*
 * 1st parameter: keymapping callback
 * 2nd parameter: keypad input interval
*/
 ...
 * 2nd parameter: keypad input interval
*/
extern void driver_set_keypad_conf2(DFBInputDeviceKeySymbol2 (*func)( u32 index, int keycode), unsigned long usec);
//mstar patch end

/*
 * 1st parameter: keymapping callback
 * 2nd parameter: network rc input interval
*/
extern void driver_set_networkir_conf(int (*func)( int keycode ), unsigned long usec);

typedef enum
{
    E_DFB_IPRC_RET_SUCCESSFUL = 0,          /* 0 Successful */

    E_DFB_IPRC_RET_UNKNOW_FAILURE,          /* 1 Unknown failure */

    E_DFB_IPRC_RET_FAIL_KEYCODE_FORMAT,     /* 2 Failure.Remote key code format is not correct. */

    E_DFB_IPRC_RET_FAIL_KEYCODE_NOT_EXIST,  /* 3 Failure.Received key code does not exist. */

    E_DFB_IPRC_RET_FAIL_TV_BOOTING,         /* 4 TV could not do function defined in key code
                                                 because TV is on booting, need to send other acceptable key codes. */

    E_DFB_IPRC_RET_FAIL_TV_ACTIVE_STANDBY,  /* 5 TV could not do function defined in key code
                                                 because TV is in Active Standby state (TV system is working but display panel is power OFF), need to send other acceptable key codes.
...
} DFB_IPRC_RET; ///Need to sync with //AEGIS/ConnectAP/iPhoneRemoteController/iprc_server/remote.c

typedef enum
{
    E_DFB_IPRC_TV_STATE_BOOTING = 0,       /* 0 TV is on booting */
    E_DFB_IPRC_TV_STATE_RUNNING,           /* 1 TV is in Running state */
    E_DFB_IPRC_TV_STATE_ACTIVE_STANDBY,    /* 2 TV is in Active Standby state */
    E_DFB_IPRC_TV_STATE_MAX,

}DFB_IPRC_TV_STATE_MODE;

/*
 * 1st parameter: tv_state
...
/*
 * Horizontial or vertical Trapezoid specified by end point of left edge and up/buttom edge
 */
typedef struct {
     DFBBoolean   horiz_direct;
     int            x0, y0;   /* X coordinate of its top-left point */
     int            x1, y1;   /* Y coordinate of its buttom-left point */
     int            top_edge_width;   /* width of top edge */
     int            bottom_edge_width;   /* width of buttom edge */
} DFBTrapezoid;
...
/*
 * Oval Defines with Line-width
 */
typedef struct {
     DFBRectangle   oval_rect;   /* rect which host oval */
     int            line_width;   /* line width of Oval Implement */
     DFBColor       line_color;  /* line width color of Oval Implement */
} DFBOval;
...
typedef struct
{
    bool DFB_VMirrorEnable;
    bool DFB_HMirrorEnable;

}DFBMirrorInfo;

typedef enum
{
   DFB_FORCEWRITE_DISABLE =0X0,
   DFB_FORCEWRITE_ENABLE = 0X1,
   DFB_FORECWRITE_USING = 0X80000000,
}DFBForceWriteFlag;

/*
 * Layer GOP Destination.
 */
typedef enum
{
    ///IP0 path.
...
DFBResult DirectFBGetMIUCPUOffset(
                                  DFBMIUIdentifier identifer,
                                  unsigned long *puloffset
                                  );

DFBResult DirectFBGetMIUHALOffset(
                                  DFBMIUIdentifier identifier,
                                  unsigned long * puloffset
                                  );
...
/* Dynamic set gop destination, contrast to the static setting in dfbrc.ini. 
this API give a chance to change the gop destination setting.
there is on constraint: when used this API, it should be called before calling dfb->GetDisplayLayer()
*/
DFBResult
DirectFBSetGOPDst(DFBDisplayLayerID         id, DFBDisplayLayerGOPDST gop_dst );


/* Get the gop destination of specified layer */
DFBDisplayLayerGOPDST DirectFBGetGOPDst(DFBDisplayLayerID         id);

/*Get the gop index of specified layer */
u8 DirectFBGetGOPIndex(DFBDisplayLayerID         id);

DFBResult
DirectFBSetBootLogoPatch(DFBDisplayLayerID id, int miusel );

/*Set the GOP ForceWrite for boot time issue */
DFBResult
...
typedef enum {
     ...
     DSDESC_PALETTE      = 0x00000020,  /* Initialize the surfaces palette
                                           ...
                                           description. */
     DSDESC_ORIG_WIDTH   = 0x00000040, /* Valid the  real jpeg width*/
     DSDESC_ORIG_HEIGHT  = 0x00000080, /* Valid the  real jpeg height*/
     ...
     DSDESC_PREALLOCATED_IN_VIDEO = 0x00000400,  /* Surface uses data that has been
                                           preallocated by the application and provided by physical address
                                           instead of virtual address, and this pool will support GPU Accel.
                                           The field array 'preallocated'
                                           has to be set using the first
                                           element for the front buffer
                                           and eventually the second one
     ...
     DSDESC_VIDEOMEMORY_IN_MIU0 = 0X00001000,/*Surface buffer will be allocated in the
                                                    video memory pool in MIU0*/


     DSDESC_VIDEOMEMORY_IN_MIU1 = 0X00002000,/*Surface buffer will be allocated in the video
                                                     memory pool in MIU1*/

     DSDESC_VIDEOMEMORY_DECLARABLE = 0X00004000,/*Surface buffer which are declarable should been allocated in spec pool*/

     DSDESC_PREALLOCATED_IN_VIDEO_NO_VA = 0x00000800,  /* the preallcate by the other process, VA can't share over different process,
                                            so, use new flags to identify this preallocate don't get VA.*/

     DSDESC_ALL          = 0x00007FFF   /* all of these */
...
typedef enum {
     ...
     DSCAPS_VIDEOHIGH     = 0x00000008,  /* Prefer to have surface data stored in video memory.*/
     ...
     DSCAPS_STATIC_POS  = 0x00000400,  /* surface can not been moved during fragment merge */

     DSCAPS_PREALLOCATED  = 0x00000800,  /* preallocated surface */
     ...
     DSCAPS_ALL           = 0x01111FFF,  /* All of these. */
...
typedef enum {
     ...
     DSDRAW_COLOR_GRADIENT_X = 0x00000040, /*draw with x-gradient */
     DSDRAW_COLOR_GRADIENT_Y = 0x00000080 /*draw with y-gradient */
...
typedef enum {
     ...
     DSBLIT_BLINK_BACKGROUND   = 0x00400000, /* F/B = 1, show  FG
                                                F/B = 0, blink FG -> BG -> FG -> BG      */
     DSBLIT_BLINK_FOREGROUND   = 0x00800000, /* F/B = 1, blink FG -> BG -> FG -> BG
                                                F/B = 0, show  BG                        */
     /*
     (FA1, FA0, BA1, BA0)
     (0, 0, x, x) : use FG[5:0], but no alpha (solid color)
     (1, 0, x, x) : use FG[5:0], with alpha
     (1, 1, x, x) : Transparent
     (0, 1, 0, 0) : N frames use FG[5:0] (solid), the other N frames use BG[5:0] (solid)
     (0, 1, 1, 0) : N frames use FG[5:0] (solid), the other N frames use BG[5:0] (with alpha)
     (0, 1, 1, 1) : N frames use FG[5:0] (solid), the other N frames are transparent
     */
     DSBLIT_FA_SOLID           = 0x01000000, /* (FA1, FA0) = 00  solid       */
     DSBLIT_FA_FLASH           = 0x02000000, /* (FA1, FA0) = 01  flash       */
     DSBLIT_FA_TRANSLUCENT     = 0x04000000, /* (FA1, FA0) = 10  translucent */
     DSBLIT_FA_TRANSPARENT     = 0x08000000, /* (FA1, FA0) = 11  transparent */
     DSBLIT_BA_SOLID           = 0x10000000, /* (BA1, BA0) = 00  solid       */
     DSBLIT_BA_NOUSE           = 0x20000000, /* (BA1, BA0) = 01  on use      */
     DSBLIT_BA_TRANSLUCENT     = 0x40000000, /* (BA1, BA0) = 10  translucent */
     DSBLIT_BA_TRANSPARENT     = 0x80000000, /* (BA1, BA0) = 11  transparent */
...
typedef enum {
     ...
     DFXL_DRAWOVAL        = 0x00000020,  /* Draw Oval is accelerated. */
     ...
     DFXL_TRAPEZOID_BLIT = 0x00080000,/* BlitTrapezoid() is accelerated. */
...
typedef enum {
     ...
     DWOP_SCALE_WINDOW_AUTOFIT_LAYER = 0x00020000, 
     ...
     DWOP_ALL            = 0x0073307F   /* all possible options */
} DFBWindowOptions;
...
typedef enum {
     ...
     DWSC_LOWER          = 0x00000002,   /* Window is always below windows
      ...
      DWSC_PRI0          =  0x00000100,/*increase some  extra stacking class which above the DWSC_UPPER*/
      DWSC_PRI1          =  0x00000101,/*when window is assigned a new stacking class ,the effect is */
      DWSC_PRI2          =  0x00000102,/*not showned immediately, that will be shown only after restack window */
      DWSC_PRI3          =  0x00000103,
      DWSC_PRI4          =  0x00000104,
      DWSC_PRI5          =  0x00000105,
      DWSC_PRI6          =  0x00000106,
      DWSC_PRI7          =  0x00000107,
      DWSC_PRI8          =  0x00000108,
      DWSC_PRI9          =  0x00000109,
...
typedef enum {
     ...
     DFDESC_BOLD_STRENGTH     = 0x00000200,  /*Bold strength is set */
     DFDESC_KERNING_DISTANCE     = 0x00000400,  /*Bold strength is set */
...
typedef struct {
     ...
     int                                bold_strength;     /* set as the strength of embold*/
     int                                kerning_advance;
} DFBFontDescription;
...
typedef enum {
     ...
     /* 16 bit   YUV (4 byte/ 2 pixel, macropixel contains CbYCrY [31:0]) */
     DSPF_YVYU      = DFB_SURFACE_PIXELFORMAT( 31, 16, 0, 0, 0, 2, 0, 0, 0, 0, 0 ),

     /* 4 bit   LUT (1 byte/ 2 pixel, 4 bit color and alpha lookup from palette) */
     DSPF_LUT4      = DFB_SURFACE_PIXELFORMAT( 32,  4, 0, 1, 4, 0, 1, 0, 0, 1, 0 ),

     /* 1 bit   LUT (1 byte/ 8 pixel, 1 bit color and alpha lookup from palette) */
     DSPF_LUT1      = DFB_SURFACE_PIXELFORMAT( 33,  1, 0, 1, 1, 0, 7, 0, 0, 1, 0 ),

               /* 32 bit  ARGB (4 byte, alpha 8@24, blue 8@16, green 8@8, red 8@0) */
     DSPF_ABGR      = DFB_SURFACE_PIXELFORMAT( 34, 24, 8, 1, 0, 4, 0, 0, 0, 0, 0 ),
    /* 16 bit  BLINK12355 (2 byte, 1 1@15, NA 2@13, Blink 2@11, F/B 1@10, FG 5@5, BG 5@0) */
    DSPF_BLINK12355 = DFB_SURFACE_PIXELFORMAT( 35, 16, 0, 1, 0, 2, 0, 0, 0, 0, 0 ),

    /* 16 bit  BLINK12355 (2 byte, FA 2@14, BA 2@12, FG 6@6, BG 6@0) */
    DSPF_BLINK2266  = DFB_SURFACE_PIXELFORMAT( 36, 16, 0, 1, 0, 2, 0, 0, 0, 0, 0 ),
 ...
 */
typedef enum {
     DSUF_SYNC_GPU_READ   = 0x00000001,  /* wait any GPU read to this surface finished*/
     DSUF_SYNC_GPU_WRITE   = 0x00000002,  /* wait any GPU write to this surface finished*/
     DSUF_SYNC_FB_2_DCACHE = 0x00000004, /*invalid CPU data cache*/
     DSUF_SYNC_DCACHE_2_FB = 0x00000008, /*flush CPU data cache*/
     DSUF_SYNC_ONLY_LOCKED = 0x10000000, /*skip any sync if allocation is not locked at all*/
     DSUF_SYNC_ROUGH            = 0x20000000, /*sync roughly to save cpu time*/
} DFBSurfaceSyncFlags;
...
typedef struct {
     ...
     unsigned int             video_memory_available; /*the video memory size which is available*/
 ...
 * The field of contrast is in the range 0 to 100.
 * When the value of  contrast is 50, it means  no change for contrast.
 * More than 50, the value of contrast is greater,the contrast of image is brighter.
 * Below 50, the value of contrast is smaller, the contrast of image is darker.
 */
typedef struct {
...
/*
*  set the primary surface tile mode
*
*/
    DFBResult (*SetTileMode) (
        IDirectFB                   *thiz,
        bool                        tilemode
    );
/*
*
* set primary surface alpha mode
*/
    DFBResult (*SetAlphaMode) (
        IDirectFB                   *thiz,
        DFBDisplayLayerOptions         flag
    );
/*
*
*set primary surface opacity
*/
...
/*
*do suspend all layers
*/
  DFBResult (*SuspendLayers)(
     IDirectFB          *thiz
  );


/*
*do resume all layers
*/
  DFBResult (*ResumeLayers)(
     IDirectFB          *thiz
  );

/*
*do suspend all input
*/
  DFBResult (*SuspendInput)(
     IDirectFB          *thiz
  );
...
#define DIDID_MSTAR_NETWORKIR 0x000E    /* primary mstar networkir */
#define DIDID_MSTARIR       0x000F   /* primary mstar IR control */
#define DIDID_MSTARKEY        0x0004   /* primary mstar Key control */
...
/*
 * desk top display mode , for supporting mstar muf 3D UI
 */
typedef enum {
     DLDM_NORMAL            = 0, /* normal desktop */
     DLDM_STEREO_LEFTRIGHT_3DUI,    /* duplicate desktop in left-right direct */
     DLDM_STEREO_TOPBOTTOM_3DUI,    /* duplicate desktop in top-bottom direct */
     DLDM_STEREO_FRAME_PACKING,    /*Full resolution  frames Packet Ex . 1280x( 720x2 + 30) or 1920x( 1080x2 + 45)*/
     DLDM_STEREO_LEFTRIGHT_TOPHALF_3DUI,/*toshiba special side by side ,duplicate the desktop to left_top && right_top */
     DLDM_STEREO_TOPHALF_3DUI,/* special frame packing ,stretchblit the desktop to top half*/
     DLDM_STEREO_FRAME_PACKING_FHD,/* special frame packing ,stretchblit the desktop to FHD*/
} DFBDisplayLayerDeskTopDisplayMode;
...
// Define Layer V-Stretch mode.
typedef enum
{
    ///linear mode.
    DISPLAYER_VSTRCH_LINEAR = 0,
    /// duplicate mode.
    DISPLAYER_VSTRCH_DUPLICATE = 1,
    /// nearest mode.
    DISPLAYER_VSTRCH_NEAREST = 2,
    /// nearest mode.
    DISPLAYER_VSTRCH_LINEAR_GAIN0 = 3,
    DISPLAYER_VSTRCH_LINEAR_GAIN1 = 4,
    /// Linear
    DISPLAYER_VSTRCH_LINEAR_GAIN2 = 5,
} DISPLAYER_STRETCH_VMODE;

// Define Layer H-Stretch mode.
typedef enum
{
    /// 6-tape (including nearest) mode
    DISPLAYER_HSTRCH_6TAPE         = 0,
    /// duplicate mode.
    DISPLAYER_HSTRCH_DUPLICATE     = 1,
    /// 6-tape (Linear mode)
    DISPLAYER_HSTRCH_6TAPE_LINEAR  = 2,
    /// 6-tape (Nearest mode)
    DISPLAYER_HSTRCH_6TAPE_NEAREST = 3,
    /// 6-tape (Gain0)
    DISPLAYER_HSTRCH_6TAPE_GAIN0   = 4,
    /// 6-tape (Gain1)
    DISPLAYER_HSTRCH_6TAPE_GAIN1   = 5,
    /// 6-tape (Gain2)
    DISPLAYER_HSTRCH_6TAPE_GAIN2   = 6,
    /// 6-tape (Gain3)
    DISPLAYER_HSTRCH_6TAPE_GAIN3   = 7,
    /// 6-tape (Gain4)
    DISPLAYER_HSTRCH_6TAPE_GAIN4   = 8,
    /// 6-tape (Gain5)
    DISPLAYER_HSTRCH_6TAPE_GAIN5   = 9,
    /// 4-tap filer
    DISPLAYER_HSTRCH_4TAPE         = 0xA,
    ///2-tape
    DISPLAYER_HSTRCH_2TAPE         = 0xB,
} DISPLAYER_STRETCH_HMODE;

/// Define Layer Transparent color Stretch Mode.
typedef enum
{
    DISPLAYER_TRANSPCOLOR_STRCH_DUPLICATE = 0,
    DISPLAYER_TRANSPCOLOR_STRCH_ASNORMAL = 1,
} DISPLAYER_STRCH_TRANSPCOLORMODE;


typedef enum
{
    DLSM_UNKNOWN    =   0x00000000,
    DLSM_H_MODE     =   0x00000001,
    DLSM_V_MODE     =   0x00000002,
    DLSM_T_MODE     =   0x00000004,
}DFBDisplayLayerStretchMode;

typedef struct
{
    DFBDisplayLayerStretchMode flag;
    DISPLAYER_STRETCH_HMODE  h_mode;
    DISPLAYER_STRETCH_VMODE  v_mode;
    DISPLAYER_STRCH_TRANSPCOLORMODE t_mode;


}DFBDisplayLayerStretchSetting;

/*
 * Query ID
 */
typedef enum {
     DISPLAYER_FORCE_FULLUPDATE = 0,  // force full update window stack
} DFBDisplayLayerQueryID;
...
DEFINE_INTERFACE(   IDirectFBDisplayLayer,
   ...
   /** Rotation **/
     ...
     *
     */
     DFBResult (*SetDeskDisplayMode)(IDirectFBDisplayLayer         *thiz,
     ...
     *
     */
      DFBResult (*MoveCursorTo)(IDirectFBDisplayLayer         *thiz,
                                                                                      int x, int y
        );
     ...
     *
     */
      DFBResult (*GetActiveWindowNum)(IDirectFBDisplayLayer         *thiz,
                                                                                      int *ret_num
        );
     ...
     *
     */
      DFBResult (*ReactiveLayer)(IDirectFBDisplayLayer         *thiz);

     /*
       * API to bind the shadowlayer
         */

      DFBResult (*BindShadowLayer)(IDirectFBDisplayLayer         *thiz,
                                    IDirectFBDisplayLayer         *other);

    /*
        API to unbind shadowlayer
      */
      DFBResult (*UnbindShadowLayer)(IDirectFBDisplayLayer         *thiz,
                                    IDirectFBDisplayLayer         *other);
    ...
    DFBResult (*ForceFullUpdateWindowStack)(IDirectFBDisplayLayer         *thiz,
                                            bool                   force_fullupdate);

    /*
    API to polling layer state
      */

    DFBResult (*QueryState)(IDirectFBDisplayLayer         *thiz,
    ...
    DFBResult (*SetHVMirrorEnable)(IDirectFBDisplayLayer         *thiz,
                                            bool                   HEnable,
                                            bool                   VEnable);
    ...
    DFBResult (*SetLBCoupleEnable)(IDirectFBDisplayLayer         *thiz,
                                            bool                   LBCoupleEnable);
    ...
    DFBResult (*SetGOPDstByPassEnable)(IDirectFBDisplayLayer         *thiz,
                                            bool                   ByPassEnable);
    ...
    DFBResult (*SetHVScale)(IDirectFBDisplayLayer         *thiz,
                                            int                   HScale,
                                            int                   VScale);
    /*
   API  GOP Set Force Write
     */
      
    DFBResult (*SetForceWrite)(IDirectFBDisplayLayer         *thiz,
                                            bool                   force_write);
...
typedef enum {
     ...
     DSTF_BOLD           = 0x00000020,  /*enable the bold*/
     DSTF_BLEND_FUNCS    = 0x00000040,  /*use the blend function set by userspace*/
...
/******************The following define is used for supporting MSTAR blend patch*********************/
typedef enum
{
    /// Aconst
    MST_ABL_TYPE_ZERO = 0,
    ...
    MST_ABL_TYPE_SEVEN = 7,
    /// 1 - Aconst
    MST_ABL_TYPE_EIGHT = 8,
    /// 1 - Asrc
    MST_ABL_TYPE_NINE = 9,
    /// 1 - Adst
    MST_ABL_TYPE_TEN = 10,
    /// Adst * Asrc * Aconst + Adst * (1 - Asrc * Aconst)
    MST_ABL_TYPE_ELEVEN = 11,
    /// Asrc * Aconst * Adst + Asrc * Aconst * (1 - Adst)
    MST_ABL_TYPE_TWELVE = 12,
    ///(1 - Adst) * Asrc * Aconst + Adst * (1 - Asrc * Aconst)
    MST_ABL_TYPE_THIRTEEN = 13,
    ///Asrc * Asrc * Aconst + Adst * (1 - Asrc * Aconst)
    MST_ABL_TYPE_FOURTEEN = 14,
    ///Asrc * (1 - Asrc * Aconst) + Adst * Asrc * Aconst
    MST_ABL_TYPE_FIFTEEN = 15,

}MSTAR_GE_ALPHA_SRC_FROM;

typedef enum
{
    /// Csrc
    MST_COEF_TYPE_ZERO     =  0,
    /// Csrc * Aconst + Cdst * (1 - Aconst)
    MST_COEF_TYPE_ONE   =  1,
      ///  Csrc * Asrc + Cdst * (1 - Asrc)
    MST_COEF_TYPE_TWO    =  2,
        /// Csrc * Adst + Cdst * (1 - Adst)
    MST_COEF_TYPE_THREE    =  3,
        /// Cdst
    MST_COEF_TYPE_FOUR = 4,
        /// Csrc * (1 - Aconst) + Cdst * Aconst
    MST_COEF_TYPE_FIVE = 5,
     /// Csrc * (1 - Asrc) + Cdst * Asrc
    MST_COEF_TYPE_SIX =6,
        ///  Csrc * (1 - Adst) + Cdst * Adst
    MST_COEF_TYPE_SEVEN = 7,
     /// ((Asrc * Aconst) * Csrc + (1-(Asrc *Aconst)) * Cdst) / 2
    MST_COEF_TYPE_EIGHT    =  8,
    /// ((Asrc * Aconst) * Csrc + Adst * Cdst * (1-(Asrc * Aconst))) / (Asrc * Aconst) + Adst * (1- Asrc * Aconst))
    MST_COEF_TYPE_NINE =  9,
    /// ((Asrc * Aconst) * Csrc * (1-Adst) + Adst * Cdst) / (Asrc * Aconst) * (1-Adst) + Adst)
    MST_COEF_TYPE_TEN  =  10,

    /// Csrc * Aconst
    ///@note
    /// <b>[URANUS] <em>It does not support COEF_CONST_SRC</em></b>
    MST_COEF_TYPE_ELEVEN  =  11,
    /// Csrc * (1 - Aconst)
    ///@note
    /// <b>[URANUS] <em>It does not support COEF_1_CONST_SRC</em></b>
    MST_COEF_TYPE_TWELVE = 12,
    /// Csrc * Adst * Asrc * Aconst + Cdst * Adst * (1 - Asrc * Aconst)
    ///@note
    /// <b>[URANUS] <em>It does not support COEF_SRC_ATOP_DST</em></b>
    MST_COEF_TYPE_THIRTEEN = 13,
    /// Cdst * Asrc * Aconst * Adst + Csrc * Asrc * Aconst * (1 - Adst)
    ///@note
    /// <b>[URANUS] <em>It does not support COEF_DST_ATOP_SRC</em></b>
    MST_COEF_TYPE_FOURTEEN = 14,
    /// (1 - Adst) * Csrc * Asrc * Aconst + Adst * Cdst * (1 - Asrc * Aconst)
    ///@note
    /// <b>[URANUS] <em>It does not support COEF_SRC_XOR_DST</em></b>
    MST_COEF_TYPE_FIFTEEN = 15,

}MSTAR_GE_BLEND_COEF;

#define MSTAR_MADE_BLEND_PARAM(alphaSrc, geBlendCOEF) \
    ((DFBSurfaceBlendFunction)((u16)((u16)(alphaSrc)+(((u16)geBlendCOEF)<<4))|0x100))

#define GE_MSTAR_GE_BLEND_COEF(mstarBlendOpt)   ((MSTAR_GE_BLEND_COEF)((((u16)(mstarBlendOpt))>>4)&0x0f))
#define GE_MSTAR_GE_ALPHA_SRC(mstarBlendOpt)   ((MSTAR_GE_ALPHA_SRC_FROM)(((u16)(mstarBlendOpt))&0x0f))
#define IS_MSTAR_GE_BLEND_PARAM(mstarBlendOpt)  \
    ( (((u16)(mstarBlendOpt))&0x100)  && (GE_MSTAR_GE_BLEND_COEF(mstarBlendOpt)<=0x0f))

/*
#define GE_MSTAR_BLEND_NEED_READ_DST(mstarBlendOpt) \
    (GE_MSTAR_GE_BLEND_COEF(mstarBlendOpt)!=MST_COEF_ONE || \
    GE_MSTAR_GE_ALPHA_SRC(mstarBlendOpt)==MST_ABL_FROM_ADST ||  \
    GE_MSTAR_GE_ALPHA_SRC(mstarBlendOpt)==MST_ABL_ASRCxCONSTxADST || \
    GE_MSTAR_GE_ALPHA_SRC(mstarBlendOpt)==MST_ABL_ADST_MINUS_ADSTxASRCxCONST || \
    GE_MSTAR_GE_ALPHA_SRC(mstarBlendOpt)==MST_ABL_ASRCxCONST_MINUS_ADSTxASRCxCONST || \
    GE_MSTAR_GE_ALPHA_SRC(mstarBlendOpt)==MST_ABL_ASRCxCONST_MINUS_ADSTxASRCxCONST_PLUS_ADST)
*/


/**************************** End MSTAR blend patch definations***********************************/
...
typedef enum {
    DFB_GFXDRAW_FLAG_DEFAULT        = 0x0,
    DFB_GFXDRAW_FLAG_SCALE          = 0x1,
    DFB_GFXDRAW_FLAG_DUPLICAPE      = 0x2,
    DFB_GFXDRAW_FLAG_TRAPEZOID_X    = 0x4,
    DFB_GFXDRAW_FLAG_TRAPEZOID_Y    = 0x8,
}DFBGfxDrawFlags;


typedef struct
{
    unsigned long u32DeltaX;
    unsigned long u32DeltaY;
    unsigned long u32InitDelatX;
    unsigned long u32InitDelatY;
    DFBGfxDrawFlags u32GFXDrawFlag;
}DFBGFX_ScaleInfo;
...
DEFINE_INTERFACE(   IDirectFBSurface,
   ...
   /** Drawing/blitting control **/
     ...
     DFBResult (*SetAlphaCmpMode) (
          IDirectFBSurface         *thiz,
          DFB_ACmpOp           cmpmode
     );

    DFBResult (*GetAlphaCmpMode) (
          IDirectFBSurface         *thiz,
          DFB_ACmpOp *          pcmpmode
     );

    DFBResult (*SetBlitNearestMode) (
          IDirectFBSurface         *thiz,
          bool           enable
     );

    DFBResult (*SetBlitMirror) (
          IDirectFBSurface         *thiz,
          bool           xmirror_enable,
          bool           ymirror_enable
     );
      ...
      */
     DFBResult (*TrapezoidBlit) ( IDirectFBSurface   *thiz,
                              IDirectFBSurface   *source,
                              const DFBRectangle *source_rect,
      ...
      * following the specified flags.
      */
     DFBResult (*FillTrapezoid) ( IDirectFBSurface *thiz,
                                const DFBTrapezoid *trapezoid
     );
      ...
      */
    DFBResult (*DrawOvals) ( IDirectFBSurface *thiz,
          const DFBOval *ovals,
          unsigned int  num
     );
    ...
    DFBResult (*Sync)(
      ...
      *
      */
     DFBResult (*SetGradientColor2)(
                IDirectFBSurface *thiz,
                u8 r, u8 g, u8 b, u8 a );

     /*Get Onscreen Rectangle*/
     DFBResult (*GetOnScreenRect)(IDirectFBSurface *thiz, DFBRectangle *pRect);
...
DEFINE_INTERFACE(   IDirectFBInputDevice,
   ...
   /** Specialized queries **/
     ...
     /*
      * Utility function for app to directly control dfb device driver behavious.
      *
      * device driver defines the special protocals it supports
      */
     DFBResult (*IOCtl) (
         IDirectFBInputDevice *thiz,
 ...
 **************************/
typedef struct {
    u32     u32_readbuf_phy_addr;
    u32     u32_readbuf_length;
    u32     u32_interbuf_phy_addr;
    u32     u32_interbuf_length;
    u32     u32_outbuf_phy_addr;
    u32     u32_outbuf_length;
    bool    bEnable;
}DFBHWJPEGDecoderSetting_t;
...
typedef enum {
     DFBGOPC_NONE            = 0x00000000,  /* None of these.            */
     DFBGOPC_X      = 0x00000001,   /* revise  the DFBGOP capture x*/
     DFBGOPC_Y      = 0x00000002,   /*revise the DFBGOP capture y */
     DFBGOPC_WIDTH  = 0x00000004,   /*revise the DFBGOP width*/
     DFBGOPC_HEIGHT = 0x00000008,   /*revise the DFBGOP height*/
...
DEFINE_INTERFACE(   IDirectFBImageProvider,
    ...
    */
     DFBResult (*SetHWDecoderParameter)(
           IDirectFBImageProvider *thiz,
           void                   *hw_decoder_setting
     );
)
