...
public:
    ...
    // return true is the page is using audio or video element
    bool hasMediaPlayer() const;

    // Set the database used by MBAL sqlite backend.
    // Do nothing if MBAL using another backend.
    bool setBackendDatabasePath(const QString&);

    // Dump GraphicsLayers and RenderLayers debug information
    void dumpGraphicsLayers(bool showDebugInfo = false);
    void dumpRenderLayers(bool showDebugInfo = false);
    ...
    // set the size of the hardware layer (as used in ChromeClientQt::rootViewToHardwareLayer)
    static void setHardwareLayerSize(const QSize& geom);

    void setDeviceScaleFactor(float);

    void setEnforceCachePolicyHistoryBuffer(bool);
public Q_SLOTS:
...
Q_SIGNALS:
    ...
    // triggered when page starts/stop audio/video (needed for PIP)
    void mediaPlayerStateChanged();

    // triggered when entering/leaving fullscreen playback
    void mediaPlayerFullScreenChanged(bool isFullScreen);
