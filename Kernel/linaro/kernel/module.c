...
#ifdef CONFIG_MP_DEBUG_TOOL_MODULE_CHECK
static const char _vermagic[] = _VERMAGIC_STRING;
#endif
...
#ifdef CONFIG_MP_DEBUG_TOOL_MODULE_CHECK
extern enum module_check_mode module_check_mode;
static inline int mstar_same_magic(struct module *mod, 
		const char* modmagic, bool has_crcs)
{
	int cmplen = strlen(_vermagic); /* without timestamp */
	int tscmplen = strlen(vermagic); /* with timestamp */
	bool print = false, reject = false;


	switch (module_check_mode) {
	case MODULE_CHECK_ORIG:
		if (!same_magic(modmagic, vermagic, has_crcs)) {
			printk(KERN_ERR "%s: version magic '%s' should be '%s'\n",
					mod->name, modmagic, vermagic);
			return false;
		}
		return true;
	case MODULE_CHECK_STRICT: 
		reject = true;
	case MODULE_CHECK_PROMPT: 
		print = true;
	case MODULE_CHECK_SILENT: 
		cmplen = tscmplen;
		break;
	default:
		printk(KERN_ERR "unsupported module checking policy: %d\n", 
				module_check_mode);
	}

	if (strncmp(modmagic, vermagic, cmplen)) { /* if not match */
		if (print)
			printk(KERN_ERR "%s: version magic '%s' should be '%s'\n",
					mod->name, modmagic, vermagic);
...
static int check_modinfo(struct module *mod, struct load_info *info, int flags)
{
	...
	if (!modmagic) {
		...
		if (err)
			return err;
#ifdef CONFIG_MP_DEBUG_TOOL_MODULE_CHECK
	} else if (!mstar_same_magic(mod, modmagic, info->index.vers)) {
#else
