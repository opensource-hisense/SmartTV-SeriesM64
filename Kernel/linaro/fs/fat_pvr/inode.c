...
static void fat_put_super(struct super_block *sb)
{
	...
	remove_inode_hash(sbi->fattab_inode);
	iput(sbi->fattab_inode);
...
extern int fat_table_init(struct inode *inode);
...
#if 1
	//For fat table optimization
	error = -ENOMEM;
	sbi->fattab_inode = new_inode(sb);
	if(!sbi->fattab_inode)
		goto out_fail;
	//sbi->fattab_inode->i_ino = MSDOS_FATT_INO;
	sbi->fattab_inode->i_ino = iunique(sb, MSDOS_ROOT_INO);
	sbi->fattab_inode->i_version = 1;
	error = fat_table_init(sbi->fattab_inode);
	if(error < 0)
		goto out_fail;
	error = -ENOMEM;
	insert_inode_hash(sbi->fattab_inode);
