...
#ifdef CONFIG_MP_INPUT_PATCH_STOP_SAMEKEYREPEAT_YOU_START
         static __u16 pre_code;
#endif
...
#ifdef CONFIG_MP_INPUT_PATCH_STOP_SAMEKEYREPEAT_YOU_START
                                //memorize this key used in start_autorepeat.
                                pre_code=v->code; 
#endif
                        }
			else{
...
#ifdef CONFIG_MSTAR_IR_BOOST

#include "chip_dvfs_calibrating.h"

// the frequency in khz of cpu overclocking
atomic_t ir_boost_cpu_freq = ATOMIC_INIT(MAX_CPU_FREQ);
int read_ir_boost_cpu_freq(void);
void write_ir_boost_cpu_freq(int);

// the duration in ms of cpu overclocking
