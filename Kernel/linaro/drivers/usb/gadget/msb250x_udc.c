...
//int work_index;
/* the OTG IRQ number */
//#define INT_OTG (INT_MS_OTG/*INT_MSB250X_OTG*/ - IRQ_BASE)
//#define INT_MS_OTG 204//fix
	...
	printk("#######======>>>>>hello_bus_reset\n");
        ...
        if(using_dma && dev->DmaRxMode == DMA_RX_MODE1)
        {
            ...
            *(DMA_CNTL_REGISTER(MAX_USB_DMA_CHANNEL))=USB_Read_DMA_Control(MAX_USB_DMA_CHANNEL)&0xfe;
			...
			//udc_write16(0x4000,MSB250X_USBCREG(0x82));
        }
#else
...
void InitUSBVar(USB_INFO_st *pUsbInfo)
{
    ...
    for (u8EpNum = 0; u8EpNum<MAX_EP_NUMBER; u8EpNum++)
    {
...
void USB_Set_ECO(void)
{
#ifdef AHB_WDATA_CHANGES_WITHOUT_HREADY_ECO
	USB_REG_WRITE8(0x8d*2-1, USB_REG_READ8(0x8d*2-1)|0x40);
#endif
#ifdef SHORTPKT_STATUS_CLEAR_ECO
	USB_REG_WRITE8(0x83*2-1, USB_REG_READ8(0x83*2-1)|0x40);
#endif
#ifdef DMA_RXTX_INT_LAST_DONE_ECO
	USB_REG_WRITE8(0x83*2-1, USB_REG_READ8(0x83*2-1)|0x80);
#endif
#ifdef XIU_ACCESS_FAILED_WITH_ECO
	USBC_REG_WRITE8(0x01*2-1, USBC_REG_READ8(0x01*2-1)|0x08);
#endif
#ifdef MONKEY_TEST_ECO
	UTMI_REG_WRITE8(0x10*2, UTMI_REG_READ8(0x10*2)|0x40);
#endif
#ifdef ENABLE_TX_RX_RESET_ECO
	UTMI_REG_WRITE8(0x04*2, UTMI_REG_READ8(0x04*2)|0x20);
#endif
}
...
void UTMI_Init(void)
{	
	unsigned int UTMI_base;

	printk("+UTMI\n");

	UTMI_base=UTMI_BASE_ADDR;
...
PUSB_INFO_st USBInit(unsigned char u8DeviceClass, unsigned char u8DeviceCap)
{
	...
	// sync code from eCos
	{
	unsigned int UTMI_base=UTMI_BASE_ADDR;		
	u16 reg_t;
	// sync code from eCos		
...
static int msb250x_udc_probe(struct platform_device *pdev)
{
    ...
    retval = request_irq(r->start/*INT_MSB250X_OTG*/, msb250x_udc_irq,
...
static int msb250x_udc_remove(struct platform_device *pdev)
{
    ...
    {
    	//DBG("free irq: %d \n", udc->irq);
        free_irq(udc->irq/*INT_MSB250X_OTG*/, udc);
        udc->got_irq = 0;
    }
...
static int __init msb250x_udc_init(void)
{
	...
	//platform_device_register(&ms_udc_device);//sean
