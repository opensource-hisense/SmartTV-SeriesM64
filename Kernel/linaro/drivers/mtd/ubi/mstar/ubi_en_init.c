...
{
	printk("table record\t compact %d\n", etr.compact);
	printk("table record\t copy_flag %d\n", etr.copy_flag);
	printk("table record\t data_crc %X\n", be32_to_cpu(etr.data_crc));
	printk("table record\t data_pad %d\n", be32_to_cpu(etr.data_pad));
	printk("table record\t data_size %d\n", be32_to_cpu(etr.data_size));
	printk("table record\t ec %lld\n", be64_to_cpu(etr.ec));
	printk("table record\t has_vid %d\n", etr.has_vid);
	printk("table record\t image_seq %d\n", be32_to_cpu(etr.image_seq));
	printk("table record\t is_bad %d\n", etr.is_bad);
	printk("table record\t lnum %d\n", be32_to_cpu(etr.lnum));
	printk("table record\t pnum %d\n", be16_to_cpu(etr.pnum));
	printk("table record\t sqnum %lld\n", be64_to_cpu(etr.sqnum));
	printk("table record\t used_ebs %d\n", be32_to_cpu(etr.used_ebs));
	printk("table record\t vol_id %d\n", be32_to_cpu(etr.vol_id));
	printk("table record\t vol_type %d\n", (etr.vol_type));
	return 0;
}
...
__u32 bfind_first_empty_page(struct ubi_device * ubi, __u32 blk_idx, int blk_page_count)
{
	...
	u32_err = ctrl_mtd->_read(ctrl_mtd, blk_idx << ctrl_mtd->writesize_shift,
		ctrl_mtd->writesize, &read, buf);
	if(u32_err)
	{
		if (u32_err!= -EUCLEAN && u32_err!= (-EUCLEAN - 1) && u32_err != (-EUCLEAN - 2))
		{
	...
	u32_err = ctrl_mtd->_read(ctrl_mtd, (blk_idx + blk_page_count - 1) << ctrl_mtd->writesize_shift,
		ctrl_mtd->writesize, &read, buf);
	if(u32_err)
	{
		if (u32_err!= -EUCLEAN && u32_err!= (-EUCLEAN - 1) && u32_err != (-EUCLEAN - 2))
	...
	while( p1 < p2 -1)
	{
		pn = (p1+p2)/2;
		//read page pn
		u32_err = ctrl_mtd->_read(ctrl_mtd, pn << ctrl_mtd->writesize_shift,
		ctrl_mtd->writesize, &read, buf);
		if(u32_err)
		{
			if (u32_err!= -EUCLEAN && u32_err!= (-EUCLEAN - 1) && u32_err != (-EUCLEAN - 2))
...
int update_next_avail_ctrl_page(struct ubi_device *ubi)
{
	...
	if(ubi->ebr->next_avail_bit_pages >= (ctrl_mtd->size >> ctrl_mtd->writesize_shift))
		ubi->ebr->next_avail_bit_pages = 0;

	if((ubi->ebr->next_avail_bit_pages & (blk_page_count - 1)) == 0)
	{
		retry:
		err = ctrl_mtd->_block_isbad(ctrl_mtd,
			ubi->ebr->next_avail_bit_pages << ctrl_mtd->writesize_shift);
		if(err == 1)
		{
		...
		err = ctrl_mtd->_read(ctrl_mtd,
			ubi->ebr->next_avail_bit_pages << ctrl_mtd->writesize_shift, ctrl_mtd->writesize, &read,
			pu8_DataBuf);
		/*
		if(err < 0)
		{
		...
		*/
		...
		if(!check_buf_all0xff(pu8_DataBuf, ctrl_mtd->writesize))
		{
			...
			ei.addr = (ubi->ebr->next_avail_bit_pages / blk_page_count)
				<< ctrl_mtd->erasesize_shift;
			ei.len = ctrl_mtd->erasesize;
			ei.callback = NULL;

			err = ctrl_mtd->_erase(ctrl_mtd, &ei);

			if(err == -EIO)
			{
				eninit_msg("erase fail at %llX",ei.addr);
					err = ctrl_mtd->_block_markbad(ctrl_mtd,
						(ubi->ebr->next_avail_bit_pages / blk_page_count)
						<< ctrl_mtd->erasesize_shift);
					if(err)
					{
...
int update_next_avail_tbl_page(struct ubi_device *ubi)
{
	...
	if(ubi->ebr->next_avail_tbl_pages >= (tbl_mtd->size >> tbl_mtd->writesize_shift))
		ubi->ebr->next_avail_tbl_pages = 0;

	retry:
	if((ubi->ebr->next_avail_tbl_pages & (blk_page_count - 1)) == 0)
	{
		err = tbl_mtd->_block_isbad(tbl_mtd,
			ubi->ebr->next_avail_tbl_pages << tbl_mtd->writesize_shift);
		if(err == 1)
		{
	...
	err = tbl_mtd->_read(tbl_mtd,
		ubi->ebr->next_avail_tbl_pages << tbl_mtd->writesize_shift, tbl_mtd->writesize, &read,
		pu8_DataBuf);
	/*
	if(err < 0)
	{
	...
	*/
	...
	if(!check_buf_all0xff(pu8_DataBuf, tbl_mtd->writesize))
	{
		...
		ei.addr = (ubi->ebr->next_avail_tbl_pages / blk_page_count)
			<< tbl_mtd->erasesize_shift;
		ei.len = tbl_mtd->erasesize;
		ei.callback = NULL;

		err = tbl_mtd->_erase(tbl_mtd, &ei);

		eninit_msg("avail : %d", ubi->ebr->avail_tbl_blk_num);
		if(err == -EIO)
			{
				err = tbl_mtd->_block_markbad(tbl_mtd,
					(ubi->ebr->next_avail_tbl_pages / blk_page_count)
					<< tbl_mtd->erasesize_shift);
				if(err)
				{
...
int find_first_empty_page(struct ubi_device * ubi, int blk_idx, int blk_page_count)
{
	...
	while(--pn >= blk_idx)
	{
		...
		//ubi->mtd->read(ctrl_mtd, addr, ctrl_mtd->writesize, &read, buf);
		err = ctrl_mtd->_read(ctrl_mtd, pn << ctrl_mtd->writesize_shift,
		ctrl_mtd->writesize, &read, buf);
		if(err)
		{
			if(err != -EUCLEAN  && err!= (-EUCLEAN - 1) && err != (-EUCLEAN - 2))
			{
				kfree(buf);
				return -1;
			}
...
int erase_ctrl_blocks(void)
{
	...
	{
		err = ctrl_mtd->_block_isbad(ctrl_mtd, u32_i << ctrl_mtd->writesize_shift);
		if(err == 1)
		{
...
int erase_all_blocks(void)
{
	...
	blk_page_count = (tbl_mtd->erasesize/tbl_mtd->writesize);
	cb_count = tbl_mtd->size >> tbl_mtd->writesize_shift;

	for(u32_i = 0; u32_i < cb_count; u32_i += blk_page_count)
	{
		err = tbl_mtd->_block_isbad(tbl_mtd, u32_i << tbl_mtd->writesize_shift);
		if(err == 1)
		{
			eninit_msg("Skip bad block %X", u32_i);
			continue;
		...
		memset(&ei, 0, sizeof(struct erase_info));
		ei.mtd = tbl_mtd;
		ei.addr = (u32_i / blk_page_count) << tbl_mtd->erasesize_shift;
		ei.len = tbl_mtd->erasesize;
		ei.callback = NULL;
		...
		{
			err = tbl_mtd->_block_markbad(tbl_mtd,
					(u32_i / blk_page_count)<< tbl_mtd->erasesize_shift);
			if(err < 0)
				return err;
		}
		else if(err < 0 && err != -EIO)
...
int process_ctrl_record(struct ubi_device *ubi, __u8 * pu8_DataBuf)
{
	...
	{
		err = ctrl_mtd->_block_isbad(ctrl_mtd, i << ctrl_mtd->writesize_shift);
		if(err > 0)
		{
		...
		if( first_empty == i)
			continue;
		//read first_empty -1;
		err = ctrl_mtd->_read(ctrl_mtd, (first_empty - 1) << ctrl_mtd->writesize_shift,
			ctrl_mtd->writesize, &read, pu8_DataBuf);
		if(err)
		{
			if(err != -EUCLEAN && err!=( -EUCLEAN - 1) && err != (-EUCLEAN - 2))	//ecc fail
			{
				erase_all_blocks();
				//err = ctrl_mtd->block_markbad(ctrl_mtd,
				//	((first_empty - 1) / blk_page_count) << ctrl_mtd->erasesize_shift);
				eninit_msg("read fail at %X",(first_empty - 1) << ctrl_mtd->writesize_shift);
				memset(pu8_DataBuf, 0, ctrl_mtd->writesize);
				ebr->bh = (struct ubi_en_bitmap_hdr*) pu8_DataBuf;
				ebr->bitmap = (__u8*) (pu8_DataBuf + sizeof(struct ubi_en_bitmap_hdr));
				ebr->bh->sqnum =  cpu_to_be64(seq + 1);
	...
	if(bitmap_block != -1)
	{
		...
		err = ctrl_mtd->_read(ctrl_mtd, (bitmap_page) << ctrl_mtd->writesize_shift,
			ctrl_mtd->writesize, &read, pu8_DataBuf);
...
int process_valid_tbl_record(struct ubi_device *ubi, struct ubi_attach_info *ai, struct ubi_en_bitmap_record *ebr)
{
	int i, j, v, pagecount, total_tbl_page, blk_page_count;
	__u8 tmp, *pu8_DataBuf;
	size_t read;
	__u32 err= 0, err2 = 0, page_idx;
	total_tbl_page = tbl_mtd->size >> tbl_mtd->writesize_shift;
	blk_page_count = tbl_mtd->erasesize/tbl_mtd->writesize;
	pagecount = 0;
	...
	for(i = 0; i < tbl_mtd->writesize - sizeof(struct ubi_en_bitmap_hdr); i ++)
	{
		...
		for(j = 7; j >= 0 ; j --)
		{
			...
			if(v)
			{
				...
				err = tbl_mtd->_read(tbl_mtd, page_idx << tbl_mtd->writesize_shift,
					tbl_mtd->writesize, &read, pu8_DataBuf);
				if((int)err < 0)
				{
					if((int)err != -EUCLEAN && (int)err!= (-EUCLEAN - 1) && (int)err != (-EUCLEAN - 2))	//ecc fail
					{
						erase_all_blocks();
						memset(ubi->ebr->bitmap, 0, ctrl_mtd->writesize - sizeof(struct ubi_en_bitmap_hdr));
						ebr->next_avail_bit_pages = -1;
				...
				err = process_tbl_page(ubi, ai, pu8_DataBuf, page_idx);
				if((int) err < 0)
				{
					erase_all_blocks();
					memset(ubi->ebr->bitmap, 0, ctrl_mtd->writesize - sizeof(struct ubi_en_bitmap_hdr));
...
int update_ctrl_content(struct ubi_device *ubi, __u32 prv_idx, __u32 nxt_idx)
{
	...
	if(prv_idx != 0xffffffff)
	{
		...
		ubi_assert(prv_idx >= 0 && prv_idx < (tbl_mtd->size >> tbl_mtd->writesize_shift));
		prv_idx -= 0;
	...
	if(nxt_idx != 0xffffffff)
	{
		...
		ubi_assert(nxt_idx >= 0 && nxt_idx < (tbl_mtd->size >> tbl_mtd->writesize_shift));
		nxt_idx -= 0;
	...
	for(j = 0; j < tsm->groups_num; j ++)
	{
		if(!tsm->valid_groups[j] || all_sync)
		{
			...
			for(i = j * n ; i < end_idx ; i ++)
			{
				...
				{
					err = tbl_mtd->_write(tbl_mtd, ubi->ebr->next_avail_tbl_pages <<  tbl_mtd->writesize_shift,
						tbl_mtd->writesize, &written, pu8_DataBuf);
					if(err)
					{
						eninit_msg("write fail at %X, mark it bad",ubi->ebr->next_avail_tbl_pages <<  tbl_mtd->writesize_shift);
						tbl_mtd->_block_markbad(tbl_mtd, (ubi->ebr->next_avail_tbl_pages/blk_page_count)
							<< tbl_mtd->erasesize_shift);
						if(ubi->tsm->val_tbl_blk[(ubi->ebr->next_avail_tbl_pages) / blk_page_count])
						{
							ubi->ebr->avail_tbl_blk_num --;
...
retry:
	...
	err = ctrl_mtd->_write(ctrl_mtd, page_idx << ctrl_mtd->writesize_shift, ctrl_mtd->writesize, &written, pu8_DataBuf);
	if(err)
	{
		eninit_msg("write fail at %X, mark it bad",page_idx << ctrl_mtd->writesize_shift);
		ctrl_mtd->_block_markbad(ctrl_mtd, (page_idx/blk_page_count) << ctrl_mtd->erasesize_shift);
		ubi->ebr->avail_ctrl_blk_num --;
...
set_dirty_group
=====================================================
set the group, which pnum belongs to, dirty.
*/
int set_dirty_group(struct ubi_device *ubi, int pnum)
{
	struct tbl_storage_manager *tsm = ubi->tsm;
	int group, err = 0;
	__u32 prv_idx = 0;
...
check_dirty_group
=====================================================
check the group, which pnum belongs to, dirty, for write back
*/
int check_dirty_group(struct ubi_device *ubi, int pnum)
{
	struct tbl_storage_manager *tsm = ubi->tsm;
	int group, err = 0;
