...
#define FBP_DELAY 50 // 50s
// close delay scrub if scrub number less than SCRUB_DELAY_LIMIT
#define SCRUB_DELAY_LIMIT 8
#endif
...
#if defined(CONFIG_MTD_UBI_FULL_BLOCK_PROGRAM) && (MP_NAND_UBI == 1)
void enable_full_progblk_scrub_fn(unsigned long p)
{
	struct ubi_device *ubi = (struct ubi_device *)p;
	ubi->fbp_scrub_en = 1;
}
#endif
...
static int do_work(struct ubi_device *ubi)
{
	...
	struct ubi_work *wrk;
       ...
       #if defined(CONFIG_MTD_UBI_FULL_BLOCK_PROGRAM) && (MP_NAND_UBI == 1)
	if (!(ubi->fbp_scrub_en) &&
		 (ubi->scrub_count > SCRUB_DELAY_LIMIT))
	{
		return 0;
	}
       #endif
...
#if defined(CONFIG_MTD_UBI_BITFLIPS) && (MP_NAND_UBI == 1)
	ubi_msg("schedule PEB %d for scrubbing tree %d", pnum, scrubtree);
#else
...
#if defined(CONFIG_MTD_UBI_FULL_BLOCK_PROGRAM) && (MP_NAND_UBI == 1)
	ubi->fbp_scrub_en = 0;
	init_timer(&ubi->fbp_timer);
	ubi->fbp_timer.expires = jiffies + FBP_DELAY * HZ;
	ubi->fbp_timer.data = (unsigned long)ubi;
	ubi->fbp_timer.function = enable_full_progblk_scrub_fn;
	add_timer(&(ubi->fbp_timer));
#endif
