...
static int part_read_slc(struct mtd_info *mtd, loff_t from, size_t len,
		size_t *retlen, u_char *buf)
{
	struct mtd_part *part = PART(mtd);
	struct mtd_ecc_stats stats;
	...
	stats = part->master->ecc_stats;
	res = part->master->_read_slc(part->master, from + part->offset, len,
				  retlen, buf);
	if (unlikely(mtd_is_eccerr(res)))
		mtd->ecc_stats.failed +=
			part->master->ecc_stats.failed - stats.failed;
...
#if defined(CONFIG_MTD_SLC_WRITE) && (MP_NAND_MTD == 1)
static int part_write_slc(struct mtd_info *mtd, loff_t to, size_t len,
		size_t *retlen, const u_char *buf)
{
	struct mtd_part *part = PART(mtd);
	return part->master->_write_slc(part->master, to + part->offset, len,
				    retlen, buf);
}
#endif
...
#if defined(CONFIG_MTD_SLC_WRITE) && (MP_NAND_MTD == 1)
    slave->mtd._read_slc = part_read_slc;
    slave->mtd._write_slc = part_write_slc;
#endif
