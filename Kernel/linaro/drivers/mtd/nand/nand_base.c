 ...
 * Get the flash and manufacturer id and lookup if the type is supported
 */
static struct nand_flash_dev *nand_get_flash_type_mstar_spi(struct mtd_info *mtd,
						  struct nand_chip *chip,
						  int busw,
						  int *maf_id, int* dev_id, struct nand_flash_dev *type)
{
	...
	mtd->writesize = gtSpiNandDrv.tSpinandInfo.u16_PageByteCnt;
	...
	//chip->onfi_version = 0;
	*maf_id = gtSpiNandDrv.tSpinandInfo.au8_ID[0];
	*dev_id = gtSpiNandDrv.tSpinandInfo.au8_ID[1];
		    ...
		    struct nand_flash_dev *table)
{
	...
	if(chip->options & NAND_IS_SPI)
	{
		type = nand_get_flash_type_mstar_spi(mtd, chip, busw, &nand_maf_id, &nand_dev_id, table);
		i = 1;
	}
#endif
#else
...
#if defined(CONFIG_MSTAR_NAND) && (MP_NAND_MTD == 1)
    if(mtd->writebufsize == 0)
        mtd->writebufsize = mtd->erasesize >> 2;
    else      
#endif
