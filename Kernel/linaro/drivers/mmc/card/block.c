	...
	unsigned int user_cmd, unsigned long user_arg)
{
	...
	{
        if(_IOC_NR(user_cmd)==3)//rpmb request  command  
        {
        ...
        else if(_IOC_NR(user_cmd)==4)//rpmb auth data write request
        {
            buff = (unsigned char *)__get_free_pages(GFP_KERNEL, get_order(_IOC_SIZE(user_cmd)<<9));
            if (!buff)
            {
                printk(KERN_ERR "alloc 0x400 bytes buffer failed!\n");
                err = -ENOMEM;
                goto cmd_done;
            }
            ...
            if(copy_from_user((void *)buff,(void __user*) user_arg, _IOC_SIZE(user_cmd)<<9))
            {
                printk(KERN_ERR "copy user space to kernel space failed!\n");
                err = -EFAULT;
                free_pages((unsigned char)buff, get_order(_IOC_SIZE(user_cmd)<<9));
                goto cmd_done;
            }
	...
	{
        if(_IOC_NR(user_cmd)==5)//Auth data read
        {
            buff = (unsigned char *)__get_free_pages(GFP_KERNEL, get_order(_IOC_SIZE(user_cmd)<<9));
            if (!buff)
            {
...
{
	int res;
#if (!defined CONFIG_MSTAR_SDMMC) && (!defined (CONFIG_MSTAR_FCIE_HOST))
    U16 u16_regval = 0;
