...
extern volatile int show_debug_info;	// check if we are in a allocation hang, deciding if use special oom killer
int count_order_free_mem(int start_order)
{
    int free_mem = 0;
	struct zone *zone;
	...
	for_each_populated_zone(zone) {
        ...
        for (order = start_order; order < MAX_ORDER; order++) {
...
static int lowmem_shrink(struct shrinker *s, struct shrink_control *sc)
{
...
#if CONFIG_MP_CMA_PATCH_OOM_KILLER
	...
	if (selected) {
		lowmem_print(1, "Killing '%s' (%d), adj %hd,\n" \
				...
				"   Free memory is %ldkB above reserved, free_cma=%ldkB\n",
