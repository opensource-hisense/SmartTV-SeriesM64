...
#define CMA_FLAG_MAP_KERNEL (1<<5)//Be notice This CMA_FLAG_MAP_KERNEL only kernel utopia use,user utopia not use!

#if defined(CONFIG_ARM) || defined(CONFIG_MIPS)
#ifdef CONFIG_MP_NEW_UTOPIA_32BIT
/* this is for 32-bit kernel and new_utopia(utopia2) */
struct cma_alloc_args {
    unsigned long long offset_in_heap;
...
struct cma_heap_info {
    ...
    unsigned long long heap_miu_start_offset;
