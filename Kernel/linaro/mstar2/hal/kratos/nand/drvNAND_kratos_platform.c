...
U32 nand_pads_switch(U32 u32EnableFCIE)
{
    ...
    // kratos doesn't have mode, pad is configured by chip config automatically
    REG_SET_BITS_UINT16(reg_sync_pe, BIT5|BIT4);
    REG_SET_BITS_UINT16(reg_s_gpio_pe, BIT12|BIT7|BIT2); 
...
void nand_DumpPadClk(void)
{
    ...
    nand_debug(0, 1, " reg_sync_pe(0x%08X): 0x%04X\n", reg_sync_pe,  REG(reg_sync_pe));
    nand_debug(0, 1, " reg_s_gpio_pe(0x%08X): 0x%04X\n", reg_s_gpio_pe,  REG(reg_s_gpio_pe));   
}
...
{
	_dma_cache_wback_inv(dma_DMAAddr, u32_ByteCnt);

	REG_CLR_BITS_UINT16(NC_MIU_DMA_SEL,BIT_MIU0_SELECTn);

	if( dma_DMAAddr >= MIPS_MIU1_BASE)
	{
		REG_SET_BITS_UINT16( NC_MIU_DMA_SEL, BIT_MIU1_SELECT);
		dma_DMAAddr -= MIPS_MIU1_BASE;
	}
	else
	{
		REG_CLR_BITS_UINT16( NC_MIU_DMA_SEL, BIT_MIU0_SELECTn);
		dma_DMAAddr -= MIPS_MIU0_BASE;
	}
...
U32 NC_CheckBlankPage(U8 * pu8_Buffer)
{
    ...
    {
        nand_debug(UNFD_DEBUG_LEVEL_LOW, 0, "Buffer 0x%X is blank page\n", (U32)pu8_Buffer);
        return 1;
    }
    return 0;
}
