...
extern  u64     is_paging_init;
#define UART_REG(addr) *((volatile unsigned int*)(0x1f201300 + ((addr)<< 3)))
#define DIRECT_UART_READ_REG(reg, addr) do{ if(is_paging_init ==0  ) \
                                            reg = *((volatile unsigned int*)(0x1f201300 + ((addr)<< 3))); \
                                        else \
                                            reg = *((volatile unsigned int*)(0xffffffc003001300 + ((addr)<< 3))); \
                                        }while(0)

#define DIRECT_UART_WRITE_REG(reg, addr) do{ if(is_paging_init ==0  ) \
                                             *((volatile unsigned int*)(0x1f201300 + ((addr)<< 3))) = reg; \
                                         else \
                                             *((volatile unsigned int*)(0xffffffc003001300 + ((addr)<< 3))) = reg; \
                                         }while(0)
