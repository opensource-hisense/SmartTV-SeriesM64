...
extern int Mstar_udc_platform_init(void);
#endif
#if defined(CONFIG_USB_MS_OTG)
extern int Mstar_otg_platform_init(void);
#endif
#ifdef CONFIG_WRITE_TO_ANDROID_MISC_NAND
...
#endif
#if defined(CONFIG_USB_MSB250X)
	Mstar_udc_platform_init();
#endif
#if defined(CONFIG_USB_MS_OTG)
	Mstar_otg_platform_init();
#endif
...
int Write_Mtd0_recovery(__u8* DataBuf,unsigned long u32Size)
{
    ...
    err = ctrl_mtd->_write(ctrl_mtd, 0, ctrl_mtd->writesize, &written, pu8_DataBuf);
...
int Read_Mtd0_recovery(void)
{
    ...
    err = ctrl_mtd->_read(ctrl_mtd,0, 10, &read, pu8_DataBuf);
