...
#endif
                        {
                            if((*(volatile U16 *)(CONFIG_REGISTER_BASE_ADDRESS + (0x002010 << 1)) & 0x01) != 0)
                            {
                                if(hMstarDvfsInfo.bPowerControlLevel == CONFIG_DVFS_POWER_CONTROL_FREEZE)
                                {
                                    dwRegisterValue = (hMstarDvfsInfo.dwVidSetting & 0x00FF);
                                    if((dwRegisterValue > 0) && (dwRegisterValue < CONFIG_DVFS_CPU_POWER_DEFAULT))
                                    {
                                        MHalDvfsCpuPowerAdjustment(CONFIG_DVFS_CPU_POWER_INIT_MAX_LOWER_BOUND_CARDINALS);
                                    }
                                    else
                                    {
                                        MHalDvfsCpuPowerAdjustment(CONFIG_DVFS_CPU_POWER_INIT_MAX_UPPER_BOUND_CARDINALS);
                                    }
                                }
                                else
                                {
                                    ...
                                    if((dwRegisterValue > 0) && (dwRegisterValue < CONFIG_DVFS_CPU_POWER_DEFAULT))
                                    {
                                        MHalDvfsCpuPowerAdjustment(CONFIG_DVFS_CPU_POWER_MAX_LOWER_BOUND_CARDINALS);
                                    }
                                    else
                                    {
                                        MHalDvfsCpuPowerAdjustment(CONFIG_DVFS_CPU_POWER_MAX_UPPER_BOUND_CARDINALS);
                                    }
                                }
                            }
                            else
...
void MHalDvfsPowerControlStateUpdate(void)
{
    if(hMstarDvfsInfo.bPowerControlLevel == CONFIG_DVFS_POWER_CONTROL_FREEZE)
    {
        if(hMstarDvfsInfo.dwFinalCpuTemperature >= CONFIG_DVFS_POWER_CONTROL_UPPER_BOUND)
        {
    ...
    {
        if(hMstarDvfsInfo.dwFinalCpuTemperature < CONFIG_DVFS_POWER_CONTROL_LOWER_BOUND)
        {
