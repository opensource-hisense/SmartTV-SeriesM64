...
U32 nand_pads_switch(U32 u32EnableFCIE)
{
    ...
    {
        REG_SET_BITS_UINT16(reg_pcm_ad_pe, 0xFF);
    }
...
void nand_pads_release(void)
{
    ...
    REG_CLR_BITS_UINT16(reg_pcm_ad_pe, 0xFF);
}
...
void nand_pll_clock_setting(U32 u32EmmcClkParam)
{
    ...
    static U32 u32_OldClkParam=0xFFFF;

    if(u32EmmcClkParam == u32_OldClkParam)
        return;
    else
        u32_OldClkParam = u32EmmcClkParam;
...
void nand_dll_setting(U32 u32_ClkParam)
{
    volatile U16 u16_reg;
    static U32 u32_OldClkParam=0xFFFF;

    if(u32_ClkParam == u32_OldClkParam)
        return ;
    else
        u32_OldClkParam = u32_ClkParam;
...
U32 nand_clock_setting(U32 u32ClkParam)
{
    ...
    if(pNandDrv->u16_Reg58_DDRCtrl & BIT_DDR_MASM )
    {
        ...
        switch(u32ClkParam)
        {
            ...
            default:
            nand_debug(UNFD_DEBUG_LEVEL, 0," error u32ClkParam %08X\n",u32ClkParam);                
                nand_die();
                break;
        }
...
void nand_DumpPadClk(void)
{
    ...
    nand_debug(0, 1, " reg_pcm_ad_pe(0x%08X): 0x%04X\n", reg_pcm_ad_pe, REG(reg_pcm_ad_pe));
