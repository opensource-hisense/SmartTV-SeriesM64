...
U32 eMMC_DMA_MAP_address(uintptr_t ulongBuffer, U32 u32_ByteCnt, int mode)
{
    dma_addr_t dma_addr;
...
void eMMC_DMA_UNMAP_address(uintptr_t ulongDMAAddr, U32 u32_ByteCnt, int mode)
{
    #if defined(CONFIG_ENABLE_EMMC_ACP) && CONFIG_ENABLE_EMMC_ACP

    dma_unmap_single(&sg_mstar_emmc_device_st.dev, ulongDMAAddr, u32_ByteCnt, DMA_ACP);

    #else

    if(mode == 0)   //write
    {
        dma_unmap_single(&sg_mstar_emmc_device_st.dev, ulongDMAAddr, u32_ByteCnt, DMA_TO_DEVICE);
    }
    else
    {
        dma_unmap_single(&sg_mstar_emmc_device_st.dev, ulongDMAAddr, u32_ByteCnt, DMA_FROM_DEVICE);
    }
...
U32 eMMC_pads_switch(U32 u32Mode)
{
    ...
    REG_FCIE_CLRBIT(REG_GPIO_0x3F,BIT9);                    //make emmc reset as gpio high when emmc mode is cleared
    ...
    switch(u32Mode)
    {
        ...
        case FCIE_eMMC_HS200:
            ...
            REG_FCIE_SETBIT(reg_emmcpll_0x03, 4<<4);          // defaut skew2
...
U32 eMMC_FCIE_DetectHS400Timing_Ex(U32 u32_RXDLLResult)
{
    ...
    if((u32_RXDLLResult & ((1<<8*2)-1)) == ((1<<8*2)-1) )
    {
        ...
        g_eMMCDrv.TimingTable_t.Set[0].u8_Cell=7;
        u16_cnt=16;
    }
    else
    {
        do{
            ...
            if(0xFFFF != u16_EndBit)
            {
                ...
                {
                    g_eMMCDrv.TimingTable_t.Set[0].u8_Cell=u16_StartBit+ ((u16_EndBit - u16_StartBit)/2);
    ...
    g_eMMCDrv.TimingTable_t.Set[0].u8_CellCnt = u16_cnt;
    eMMC_debug(0,0," BestDelayLine:0x%x\n",g_eMMCDrv.TimingTable_t.Set[0].u8_Cell);
    return u16_cnt;
}
...
U32 eMMC_FCIE_DetectHS400Timing_Skew2_Ex(U32 u32_SKew2Result)
{
    ...
    {
        eMMC_debug(eMMC_DEBUG_LEVEL,0," all\n");
        g_eMMCDrv.TimingTable_t.Set[0].u8_Skew2=4;
    }
    else
    {   
        do{
...
//        eMMC_debug(0, 1, "u16_StartBit 0x%X, u16_EndBit 0x%X\n", u16_StartBit, u16_EndBit);
        if(0xFFFF != u16_EndBit)
        {
            g_eMMCDrv.TimingTable_t.Set[0].u8_Skew2=u16_StartBit+ ((u16_EndBit - u16_StartBit)/2);            
            u16_cnt= u16_EndBit - u16_StartBit+1;
        }
    }
    eMMC_debug(0,0,"BestSkew2:0x%x\n",g_eMMCDrv.TimingTable_t.Set[0].u8_Skew2);
    return u16_cnt;
}
...
{
    U32 u32_i = 0, u32_err = 0, u32_RXDLLResult=0, u32_Skew2Result = 0;
    U16 u16_RXDLLCnt=0, u16_Skew2Cnt = 0;
    U16 u16_cnt = 0;
    U32 u32_ret;
    ...
    printk("u32_RXDLLResult=%08X\n", u32_RXDLLResult);

    u16_cnt = eMMC_FCIE_DetectHS400Timing_Ex(u32_RXDLLResult);
    ...
    if(u16_cnt >=8)
    {
        ...
        eMMC_FCIE_SetDelayLatch(g_eMMCDrv.TimingTable_t.Set[0].u8_Cell);
    }
    else
    ...
    for(u32_i = 0; u32_i < 9; u32_i ++)
    {
        ...
        {   //u32_err = eMMC_FCIE_Init();
            ...
            if(g_eMMCDrv.TimingTable_t.Set[0].u8_Reg2Ch)
            {
                ...
                REG_FCIE_SETBIT(reg_emmcpll_0x6c, BIT7);       //skew4 inverse for muji
            }
            else
            {
                REG_FCIE_CLRBIT(reg_emmcpll_0x02, BIT14);       //skew4 inverse for muji
                REG_FCIE_CLRBIT(reg_emmcpll_0x6c, BIT7);       //skew4 inverse for muji                
            }
    ...
    u16_cnt = eMMC_FCIE_DetectHS400Timing_Skew2_Ex(u32_Skew2Result);
    if(u16_cnt ==0)
        return eMMC_ST_ERR_NO_OK_DDR_PARAM;
    
    REG_FCIE_CLRBIT(reg_emmcpll_0x03, BIT_SKEW2_MASK);
    REG_FCIE_SETBIT(reg_emmcpll_0x03, g_eMMCDrv.TimingTable_t.Set[0].u8_Skew2<<4);    
