...
{
	MHal_LD_WriteByteMask(REG_LD_LDFW, ((u8Height-1) << 5), REG_LD_LDFH_BITS_L);
	MHal_LD_WriteByteMask(REG_LD_LDFH, ((u8Height-1) >> 3), REG_LD_LDFH_BITS_H);   
}

// h03[9:5] reg_bl_height
U16 MHal_LD_GetLDFBacklightHeight(void)
{
	return (((MHal_LD_Read2Byte(REG_LD_LDFW) & REG_LD_LDFH_BITS) >> 5)+1);
}
...
{
	MHal_LD_WriteByteMask(REG_LD_LDBLW, ((u8Height-1) << 5), REG_LD_LDBLH_MSK_L);
	MHal_LD_WriteByteMask(REG_LD_LDBLH, ((u8Height-1) >> 3), REG_LD_LDBLH_MSK_H);  
}

// h63[13:8] reg_bl_height_led
U8 MHal_LD_GetLDBBacklightHeight(void)
{
    return (((MHal_LD_Read2Byte(REG_LD_LDBLW) & REG_LD_LDBLH_MSK) >> 5)+ 1);
}
