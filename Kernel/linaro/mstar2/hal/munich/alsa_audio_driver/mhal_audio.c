 ...
 * ============================================================================
 */
#define _MAD_CHIP_MODEL    "MAD MUNICH"
...
#define _MAD_PCM_PLAYBACK2_BASE_OFFSET    (0x16D000 + _MAD_DSP2_TO_COMMON_DRAM_OFFSET)
...
#define _MAD_PCM_CAPTURE1_BASE_OFFSET   (0x20000 + _MAD_DSP2_TO_COMMON_DRAM_OFFSET)
#define _MAD_PCM_CAPTURE2_BASE_OFFSET   (0x2C000 + _MAD_DSP2_TO_COMMON_DRAM_OFFSET)
...
static int _mhal_alsa_pcm_playback2_init(void)
{
	...
	if ((pcm_playback->initialized_status != MAD_TRUE) || (pcm_playback->str_mode_info.status != E_RESUME)) {
		...
		/* convert Bus Address to Virtual Address */
		audio_pcm_dmaRdr_base_va = (unsigned int)ioremap_nocache(audio_pcm_dmaRdr_base_ba, _MAD_PCM_PLAYBACK2_BUF_SIZE);
		if (audio_pcm_dmaRdr_base_va == 0) {
...
static int _mhal_alsa_pcm_playback2_start(void)
{
	...
	_mhal_alsa_write_mask_byte(0x2C67, 0x9F, 0x82); /* CH6 sel to SW DMA Reader */
	...
	if (pcm_playback->channel_mode == E_MONO) {
		for (loop = 0; loop < copy_lr_sample; loop++) {
			...
			*(pcm_playback->buffer.w_ptr++) = sample_hi;
			...
			if (pcm_playback->buffer.w_ptr >= (pcm_playback->buffer.addr + _MAD_PCM_PLAYBACK2_BUF_SIZE))
				pcm_playback->buffer.w_ptr = pcm_playback->buffer.addr;
		}
	}
	else {
...
static int _mhal_alsa_pcm_playback2_set_channel_mode(unsigned int channel_mode)
{
	...
	buffer_size = ((pcm_playback->sample_rate << pcm_playback->channel_mode) * _MAD_PCM_PLAYBACK2_QUEUE_SIZE) / 1000;
	if ((buffer_size % _MAD_BYTES_IN_LINE))
		buffer_size += (_MAD_BYTES_IN_LINE - (buffer_size % _MAD_BYTES_IN_LINE));

	pcm_playback->period_size = buffer_size >> 2;
	pcm_playback->buffer.size = (pcm_playback->channel_mode == E_MONO) ? (buffer_size * 2) : buffer_size;
