 ...
 * ============================================================================
 */
#define _MAD_CHIP_MODEL    "MAD EINSTEIN"
...
#define _MAD_DSP1_DRAM_SIZE    0x180000
#define _MAD_DSP2_DRAM_SIZE    0x180000
#define _MAD_DSP1_TO_COMMON_DRAM_OFFSET    _MAD_DSP1_DRAM_SIZE
#define _MAD_DSP2_TO_COMMON_DRAM_OFFSET    (_MAD_DSP1_DRAM_SIZE + _MAD_DSP2_DRAM_SIZE)

#define _MAD_DMA_READER_BASE_OFFSET    (0x00000000 + _MAD_DSP1_TO_COMMON_DRAM_OFFSET)
...
#define _MAD_PCM_PLAYBACK2_BASE_OFFSET    (0x20000 + _MAD_DSP1_TO_COMMON_DRAM_OFFSET)
...
#define _MAD_PCM_CAPTURE1_BASE_OFFSET   (0x30000 + _MAD_DSP1_TO_COMMON_DRAM_OFFSET)
#define _MAD_PCM_CAPTURE2_BASE_OFFSET   (0x3C000 + _MAD_DSP1_TO_COMMON_DRAM_OFFSET)
...
static int _mhal_alsa_dma_reader_init(void)
{
	...
	if ((dma_reader->initialized_status != MAD_TRUE) || (dma_reader->str_mode_info.status != E_RESUME)) {
		...
		audio_pcm_dmaRdr_base_pa += (((_mhal_alsa_read_reg(0x2A40) & 0x0F) << 16) * _MAD_ADDR_CONVERTOR);
...
static int _mhal_alsa_pcm_playback2_init(void)
{
	...
	if ((pcm_playback->initialized_status != MAD_TRUE) || (pcm_playback->str_mode_info.status != E_RESUME)) {
		audio_pcm_dmaRdr_base_pa = (((unsigned int)_mhal_alsa_read_reg(0x2A10) * _MAD_ADDR_CONVERTOR) + _MAD_PCM_PLAYBACK2_BASE_OFFSET);
		audio_pcm_dmaRdr_base_pa += (((_mhal_alsa_read_reg(0x2A40) & 0x0F) << 16) * _MAD_ADDR_CONVERTOR);
		audio_pcm_dmaRdr_base_ba = audio_pcm_dmaRdr_base_pa + _MAD_PHYSICAL_ADDR_TO_BUS_ADDR;
...
static int _mhal_alsa_pcm_capture1_init(void)
{
	...
	if ((pcm_capture->initialized_status != MAD_TRUE) || (pcm_capture->str_mode_info.status != E_RESUME)) {
		...
		audio_pcm_capture_base_pa += (((_mhal_alsa_read_reg(0x2A40) & 0x0F) << 16) * _MAD_ADDR_CONVERTOR);
