...
void HalFcie_Platform_InitChiptop(void)
{
	...
	FCIE_RIU_16_ON(CHIPTOP_5Ah, REG_SD_MODE_2);
}
...
S32 HalFcie_GetCardDetect(void)
{
		...
		// 215C back side socket, only rework in my board

		FCIE_RIU_16_ON(PMGPIO_REG_08, PMGPIO8_OEN); // oen = 1 for input

		if (FCIE_RIU_R16(PMGPIO_REG_08) & PMGPIO8_IN)
		{
			return 0;
		}
		else
...
void HalFcie_SetCardPower(U8 u8OnOff)
{
		...
		// 215C back side socket, only rework in my board
		FCIE_RIU_16_OF(PMGPIO_REG_0B, PMGPIO11_OEN); // oen = 0 for output
		if(u8OnOff)
		{
			FCIE_RIU_16_OF(PMGPIO_REG_0B, PMGPIO11_OUT); // power on
		}
		else
		{
			FCIE_RIU_16_ON(PMGPIO_REG_0B, PMGPIO11_OUT); // power off
		}
