...
U8 gu8FcieSilenceTuning;
U32 gu32BestFcieSkew4;
...
extern void HalFcie_GetSBootGPIOConfig(void);
...
void HalFcie_Init(void)
{
	...
	HalFcie_SwitchPad(SDIO_PAD_SDR12);
...
void HalFcie_DumpDebugBus(void)
{
	...
	for(u16_j=0; u16_j<=0xF; u16_j++)
	{
		...
		for(u16_i=0; u16_i<4; u16_i++)
		{
			...
			printk("0x15 = %Xh, ", REG_FCIE(FCIE_TEST_MODE));
...
void HalFcie_DumpRegister(void)
{
	...
	printk("\n\nfcie reg:");
	HalFcie_DumpRegisterBank(RIU_BANK_SDIO0, 0, 0x40);
    ...
    printk("\n\nCRC reg:");
	HalFcie_DumpRegisterBank(RIU_BANK_SDIO1, 0x40, 0x10);

    printk("\n\nchiptop reg:");
	HalFcie_DumpRegisterBank(RIU_BANK_CHIPTOP, 0, 0x80);

    printk("\n\nclockgen reg:");
	HalFcie_DumpRegisterBank(RIU_BANK_CLKGEN0, 0, 0x80);

	#ifdef SDIO_SUPPORT_SD30
    	printk("\n\nsdio_pll reg:");
		HalFcie_DumpRegisterBank(RIU_BANK_SDIO_PLL, 0, 0x80);
	#endif
...
void HalFcie_ClearEvents(void)
{
	...
	REG_FCIE_W1C(FCIE_SD_STATUS, BIT_SD_FCIE_ERR_FLAGS); // W1C
		...
		HalFcie_ClearWaitQueue();
	#endif
}

E_IO_STS HalFcie_ParsingErrStatus(U16 u16_reg)
{
...
E_IO_STS HalFcie_SendCommand(CMD_RSP_INFO * pCmdRspInfo)
{
    ...
    {
		if(i==0)
			HalFcie_CmdFifoSet(i, pCmdRspInfo->Command.Array[i+3]|TRANSMIT_BIT); // add host transmitter bit
		else
			HalFcie_CmdFifoSet(i, pCmdRspInfo->Command.Array[i+3]);
    }
	...
	if( pCmdRspInfo->Command.Cmd.Index==17 /*&& pCmdRspInfo->DataPath==PATH_R2N*/ )
	{
		...
		HalFcie_DumpRegisterBank(RIU_BANK_SDIO0, 0, 0x80);
	}
    ...
    else // data transfer command
    {
		if(pCmdRspInfo->ReadWriteDir==DIR_R) // read adtc
		{
			...
			else if(IoSts==IO_TIME_OUT)
			{
				...
				for(i=0; i<0x10; i++)
				{
					printk("%04Xh ", HalFcie_ReadCRCFifo(i));
					if(i%8==7) printk("\n");
				}
				printk("\n");
			#endif
		...
		else // write adtc
		{
			...
			else // debug use only, write command OK, check response here
			{
				...
				{
					printk("%02X ", HalFcie_CmdFifoGet(i));
				}
				printk("\n");
			}
			#endif
			...
			//HalFcie_DumpRegisterBank(RIU_BANK_SDIO0, 0, 0x80); // debug use, dump reg before write
	...
	#endif
	IoSts = HalFcie_ParsingErrStatus(u16_reg);
    ...
    {
		#if 0
			IoSts = HalFcie_WaitD0High(TIMEOUT_NORMAL);
		#else
			IoSts = HalFcie_DetectBusyEnd(TIMEOUT_NORMAL);
		#endif
        if(IoSts)
        {
    ...
    if(u8RspSize)
    {
        ...
        for(i=0; i<u8RspSize; i++) // last CRC didn't copy to CIFC
        {
			pCmdRspInfo->Response.Array[i+3] = HalFcie_CmdFifoGet(i);
			#ifdef PRINT_CMD_RSP
...
ErrorHandle:
	...
	if(!gu8FcieSilenceTuning)
	{
		...
		#endif
		#if 0
		u32CardStatusR1  = ((U32)HalFcie_CmdFifoGet(1)) << 24;
		u32CardStatusR1 += ((U32)HalFcie_CmdFifoGet(2)) << 16;
		u32CardStatusR1 += ((U32)HalFcie_CmdFifoGet(3)) <<  8;
		u32CardStatusR1 += ((U32)HalFcie_CmdFifoGet(4)) <<  0;
...
U32 HalFcie_WaitCIFD_Event(U16 u16_WaitEvent, U32  u32_MicroSec)
{
	...
	for (u32_Count=0; u32_Count < u32_MicroSec; u32_Count++)
	{
		...
		HalFcieDelayUs(HW_TIMER_DELAY_1us);
	}
...
E_IO_STS HalFcie_WaitGetCIFD(U32 * pu32_DataBuf, U32 u32_ByteCnt)
{
	...
	if (HalFcie_WaitCIFD_Event(BIT_WBUF_FULL, HW_TIMER_DELAY_500ms)== (HW_TIMER_DELAY_500ms ))
	{
...
E_IO_STS HalFcie_WaitSetCIFD(U32 * pu32_DataBuf, U32 u32_ByteCnt)
{
	...
	if (HalFcie_WaitCIFD_Event(BIT_RBUF_EMPTY, HW_TIMER_DELAY_500ms)== (HW_TIMER_DELAY_500ms ))
	{
...
*
*********************************************************************/
E_IO_STS HalFcie_WaitD0High(U32 timeout)
{
	...
	printk("FCIE Err: wait D0 H timeout: %04Xh\n", FCIE_RIU_R16(FCIE_SD_STATUS));
...
E_IO_STS HalFcie_DetectBusyEnd(U32 timeout)
{
	...
	if(HalFcie_WaitMieEvent(BIT_BUSY_END_INT, timeout))
	{
...
E_IO_STS HalFcie_SetBusWidth(U8 * pu8BusWidth, E_DATA_BUS DataBus)
{
    ...
    switch(DataBus)
    {
        ...
        default:
            printk("wrong para in HalFcie_SetBusWidth()\r\n");
            return IO_ERROR;
    }
    return IO_SUCCESS;
}
...
{
	if(u8Count>=16) {
		printk("FCIE Err: wrong shift counter\n");
		return;
	}
	...
	{
		printk("FCIE Err: wrong shft type\n");
	}
...
U8 HalFcie_SendClocknWaitD0High(void)
{
    ...
    if(IoSts)
    {
		printk("FCIE Err: wait D0 high timeout in HalFcie_SendClocknWaitD0High()\n");
        goto ErrorHandle;
    }
...
ADMA_INFO g_FcieADMAInfo __attribute__((aligned(0x80)));
...
U32 HalFcie_PrepareDescriptors(struct mmc_data *data)
{
	...
	//printk("descriptor addr %08Xh", g_FcieADMAInfo.DescriptorAddr);
	//u32DescptAddr = virt_to_phys((void *)g_FcieADMAInfo.DescriptorAddr); // virtual to bus address
	//printk(" -- virt_to_phys --> %08Xh\n", u32DescptAddr);

	// setup descriptor
	memset(&g_FcieADMAInfo, 0, sizeof(ADMA_INFO)-4); // clear
	...
	for(i=0; i<data->sg_len; i++) {
		...
		if( u32BusAddr >= MSTAR_MIU1_BUS_BASE) {
			...
			g_FcieADMAInfo.Descriptor[i].adma_miu_sel = 1;
		...
		#endif
		g_FcieADMAInfo.Descriptor[i].adma_miu_addr = u32DeviceAddr;
		g_FcieADMAInfo.Descriptor[i].adma_job_cnt = u32DmaLeng/data->blksz;
		g_FcieADMAInfo.Descriptor[i].adma_length = u32DmaLeng;

	}

	g_FcieADMAInfo.Descriptor[data->sg_len-1].adma_end_flag = 1; // must give a end mark

	#if defined(DEBUG_SG_ELEMENT) && DEBUG_SG_ELEMENT
	printk("= %d\n", u32TotalLength/data->blksz);
	#endif

	//MsOS_Dcache_Flush(g_FcieADMAInfo.Descriptor, sizeof(ADMA_INFO));

	//Chip_Clean_Cache_Range_VA_PA(g_FcieADMAInfo.DescriptorAddr, u32DescptAddr, sizeof(ADMA_INFO));

	//eMMC_dump_mem((U8 *)&g_FcieADMAInfo, 32); // debug use
...
void HalFcie_SetupDescriptorAddr(CMD_RSP_INFO * pCmdRspInfo)
{
	...
	//u32DescptMiuAddr = eMMC_translate_DMA_address_Ex(g_FcieADMAInfo.DescriptorAddr, sizeof(ADMA_INFO));
	//printk("mstar_mci_setup_descriptor_addr1 --> %08X\n", g_FcieADMAInfo.DescriptorAddr);
	// kernel

	if(!DescriptorVirtAddr)
	{
		DescriptorVirtAddr = (uintptr_t *)&g_FcieADMAInfo;
		g_FcieADMAInfo.DescriptorBusAddr = virt_to_phys((void *)DescriptorVirtAddr); // virtual to bus address
	}

	// bus addr = virtual addr in mboot
	//u32DescptAddr = ;
	//flush_cache(u32DescptAddr, sizeof(g_FcieADMAInfo));

	Chip_Clean_Cache_Range_VA_PA((uintptr_t)&g_FcieADMAInfo, (uintptr_t)virt_to_phys(&g_FcieADMAInfo), sizeof(ADMA_INFO));

	//printf("mstar_mci_setup_descriptor_addr2 --> %08X\n", u32DescptAddr);
	//printf("Descpt Addr: %08Xh --> %08Xh", g_FcieADMAInfo.DescriptorAddr, u32DescptMiuAddr);
	...
	//printk("mstar_mci_setup_descriptor_addr3 --> %08X\n", g_FcieADMAInfo.DescriptorBusAddr);

	pCmdRspInfo->DataPath = PATH_ADMA;
	pCmdRspInfo->BusAddr = g_FcieADMAInfo.DescriptorBusAddr;
