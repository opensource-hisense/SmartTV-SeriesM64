...
static int mstar_pm_enter(suspend_state_t state)
{
...
#if 0
    {
        extern void test_direct_resume(unsigned long wakeup_addr_phy);
        test_direct_resume(mstar_virt_to_phy((void*)pWakeup));
    }
#endif
    //__asm__ volatile(
    //    "SUSPEND_WAIT:\n"
...
#ifndef CONFIG_MP_TRUSTZONE_PATCH //here errata patch will move to mboot
        /* Begin Workaround for CPU latch up issue, see CL#1241084 */
        "mrc    p15, 0, r9, c15, c0, 1\n"
        "orr    r9, r9, #0x1000\n"
        "mcr    p15, 0, r9, c15, c0, 1\n"
        /* End Workaround for CPU latch up issue */
#endif
    ...
    flush_pmd_entry((void*)saved_pmd_entry);
    local_flush_bp_all();
    local_flush_tlb_all();
