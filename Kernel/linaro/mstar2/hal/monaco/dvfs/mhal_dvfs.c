    ...
    if(dwRegisterValue == CONFIG_DVFS_ENABLE_PATTERN)
    {
        ...
        if(hMstarDvfsInfo.bCpuClockLevel == CONFIG_DVFS_STATE_INIT)
        {
            if(hMstarDvfsInfo.bPowerControlLevel == CONFIG_DVFS_POWER_CONTROL_FREEZE)
            {
                ...
                {
                    MHalDvfsCpuPowerAdjustment(CONFIG_DVFS_CPU_POWER_INIT_L0_LOWER_BOUND);
                }
                else
                {
                    MHalDvfsCpuPowerAdjustment(CONFIG_DVFS_CPU_POWER_INIT_L0_UPPER_BOUND);
                }
            }
            else
            {
...
//                  else
               ...
               (hMstarDvfsInfo.bPowerControlChange != 0))
            {
                ...
                //Adjust Special CPU Clock by Specific Register 0x1005_0E[15:0]
                {
                        ...
                        if(dwCornerChipType == CONFIG_DVFS_FF_CORNER_CHIP_MAX)
                        {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    dwSiddValue = *(volatile U16 *)(CONFIG_REGISTER_BASE_ADDRESS + (0x00200C << 1));
                                    dwSiddValue >>= 12;
                                    dwSiddValue &= 0x0F;
                                    dwSiddValue |= ((*(volatile U16 *)(CONFIG_REGISTER_BASE_ADDRESS + (0x00200C << 1)) & 0x3F) << 4);

                                    //Get OSC Value (Bank1[11:2] = 0x00200C[11:2]
                                    dwOscValue = *(volatile U16 *)(CONFIG_REGISTER_BASE_ADDRESS + (0x00200C << 1));
                                    dwOscValue >>= 2;
                                    dwOscValue &= 0x3FF;

                                    if((dwSiddValue <= 234) || (dwOscValue <= 140))
                                    {
                                        MHalDvfsCpuPowerAdjustment(CONFIG_DVFS_CPU_POWER_MAX_UPPER_BOUND);
                                    }
                                    else if(((dwSiddValue > 234) && (dwSiddValue <= 305)) || \
                                            ((dwOscValue > 140) && (dwOscValue <= 143)))
                                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    dwSiddValue >>= 12;
                                    dwSiddValue &= 0x0F;
                                    dwSiddValue |= ((*(volatile U16 *)(CONFIG_REGISTER_BASE_ADDRESS + (0x00200E << 1)) & 0x3F) << 4);
                    ...
                    else if(dwRegisterValue > CONFIG_DVFS_CPU_CLOCK_L1_2)
                    {
                        if(hMstarDvfsInfo.bPowerControlLevel == CONFIG_DVFS_POWER_CONTROL_FREEZE)
                        {
                            dwRegisterValue = (hMstarDvfsInfo.dwVidSetting & 0x00FF);
                    ...
                    else if(dwRegisterValue > CONFIG_DVFS_CPU_CLOCK_L1_1)
                    {
                        if(hMstarDvfsInfo.bPowerControlLevel == CONFIG_DVFS_POWER_CONTROL_FREEZE)
                        {
                            dwRegisterValue = (hMstarDvfsInfo.dwVidSetting & 0x00FF);
                            if((dwRegisterValue > 0) && (dwRegisterValue < CONFIG_DVFS_CPU_POWER_DEFAULT))
                            {
                                MHalDvfsCpuPowerAdjustment(CONFIG_DVFS_CPU_POWER_INIT_L1_2_LOWER_BOUND);
                            }
                            else
                            {
                                MHalDvfsCpuPowerAdjustment(CONFIG_DVFS_CPU_POWER_INIT_L1_2_UPPER_BOUND);
                            }
                        }
                        else
                        {
                            ...
                            if((dwRegisterValue > 0) && (dwRegisterValue < CONFIG_DVFS_CPU_POWER_DEFAULT))
                            {
                                MHalDvfsCpuPowerAdjustment(CONFIG_DVFS_CPU_POWER_L1_2_LOWER_BOUND);
                            }
                            else
                            {
                                MHalDvfsCpuPowerAdjustment(CONFIG_DVFS_CPU_POWER_L1_2_UPPER_BOUND);
                            }
                        }
                    }
                    else if(dwRegisterValue > CONFIG_DVFS_CPU_CLOCK_L1)
                    {
                        if(hMstarDvfsInfo.bPowerControlLevel == CONFIG_DVFS_POWER_CONTROL_FREEZE)
                        {
                            ...
                            if((dwRegisterValue > 0) && (dwRegisterValue < CONFIG_DVFS_CPU_POWER_DEFAULT))
                            {
                                MHalDvfsCpuPowerAdjustment(CONFIG_DVFS_CPU_POWER_INIT_L1_1_LOWER_BOUND);
                            }
                            else
                            {
                                MHalDvfsCpuPowerAdjustment(CONFIG_DVFS_CPU_POWER_INIT_L1_1_UPPER_BOUND);
                            }
                        }
                        else
                        {
                            ...
                            if((dwRegisterValue > 0) && (dwRegisterValue < CONFIG_DVFS_CPU_POWER_DEFAULT))
                            {
                                MHalDvfsCpuPowerAdjustment(CONFIG_DVFS_CPU_POWER_L1_1_LOWER_BOUND);
                            }
                            else
                            {
                                MHalDvfsCpuPowerAdjustment(CONFIG_DVFS_CPU_POWER_L1_1_UPPER_BOUND);
                            }
                        }
                    }
                    else if(dwRegisterValue >= CONFIG_DVFS_CPU_CLOCK_L0)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                MHalDvfsCpuPowerAdjustment(CONFIG_DVFS_CPU_POWER_L0_UPPER_BOUND);
                            }
                        }
                    }
                    else if(dwRegisterValue <= CONFIG_DVFS_CPU_CLOCK_MIN)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                //MHalDvfsCpuPowerAdjustment(CONFIG_DVFS_CPU_POWER_L0_UPPER_BOUND - CONFIG_DVFS_CPU_POWER_DROP_UPPER_BOUND);
                                MHalDvfsCpuPowerAdjustment(CONFIG_DVFS_CPU_POWER_PROTECTION_UPPER_BOUND);
                            }
                        }
                    }
