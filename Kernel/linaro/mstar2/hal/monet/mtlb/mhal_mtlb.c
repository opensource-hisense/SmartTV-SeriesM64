...
static void __iomem * virt_addr_miu0 = 0;
static void __iomem * virt_addr_miu1 = 0;
static void __iomem * virt_addr_miu2 = 0;
...
//-------------------------------------------------------------------------------------------------
static int _getAddrByMiu(u8 u8miu, u32 *miu_bus_base, phys_addr_t * addr_tlb_table)
{   
    /* check miu, monaco has 3 mius: 0 1 2*/
    if(u8miu > 2)
    {
        MTLB_ERR("invalid miu %u\n", (unsigned int)u8miu);
        return -EINVAL;
    };   
    ...
    if(u8miu == 0)
    {
       ...
       *addr_tlb_table = phys_addr_miu0;
    }
    else if(u8miu == 1)
    {
       *miu_bus_base = ARM_MIU1_BUS_BASE;
       *addr_tlb_table = phys_addr_miu1;
    }
    else if(u8miu == 2)
    {
       *miu_bus_base = ARM_MIU2_BUS_BASE;
       *addr_tlb_table = phys_addr_miu2;
    }
...
void MHal_MTLB_Get_HWInfo(mtlb_hardware_info * hwinfo)
{
    ...
    hwinfo->TLB_MAPTO_MIU = (E_MTLB_MIU_0|E_MTLB_MIU_1);
...
{    
    ...
    if(phys_addr_miu0 > 0)
    {
        virt_addr_miu0 = ioremap(phys_addr_miu0, REVERVE_MEMORY_SIZE);
        if(!virt_addr_miu0)
           goto IOREAMP_PHYS_MIU0_FAIL;
        memset((void *)(virt_addr_miu0+REVERVE_TABLE_SIZE), 0, PAGE_SIZE);
    }

    if(phys_addr_miu1 > 0)
    {
        virt_addr_miu1 = ioremap(phys_addr_miu1, REVERVE_MEMORY_SIZE);
        if(!virt_addr_miu1)
           goto IOREAMP_PHYS_MIU1_FAIL;
        memset((void *)(virt_addr_miu1+REVERVE_TABLE_SIZE), 0, PAGE_SIZE);
    }

    if(phys_addr_miu2 > 0)
    {
        virt_addr_miu2 = ioremap(phys_addr_miu2, REVERVE_MEMORY_SIZE);
        if(!virt_addr_miu2)
           goto IOREAMP_PHYS_MIU2_FAIL;
        memset((void *)(virt_addr_miu2+REVERVE_TABLE_SIZE), 0, PAGE_SIZE);
    }
...
IOREAMP_PHYS_MIU2_FAIL:
    if(virt_addr_miu1 > 0)
        iounmap(virt_addr_miu1);  

IOREAMP_PHYS_MIU1_FAIL:
    if(virt_addr_miu0 > 0)
        iounmap(virt_addr_miu0);

IOREAMP_PHYS_MIU0_FAIL:
    mutex_lock(&mhal_mtlb_mutex);
    return -ENOMEM;    
}
...
int MHal_MTLB_Reserve_Memory(void)
{
    ...
    if(phys_addr_miu1 > 0)
    {
...

        {
            ...
            phys_addr_miu0 = phys_addr_miu1 = 0;
            return -EINVAL;
        }
    }
...
void MHal_MTLB_Mapping_End(unsigned long * flag, unsigned long start, unsigned long size)
{
    //u32 fstart = 0xa0000000 + (TLB_W_HEADER << 17) + (start << 4);
    //u32 fsize = (size << 4);

    /*todo:  Flush cache and miu pipe to push data to sram*/
    //Chip_Flush_Cache_Range(fstart, fsize);
...
int MHal_MTLB_Mapping(u8 u8miu, u32 u32va, u32 u32pa, bool paInc, u32 u32size)
{
    ...
    if(_getAddrByMiu(u8miu, &miu_bus_base, &addr_tlb_table));
        return -EINVAL;

    mb();
    while(u32size != 0)
    {
        u32header = addr_tlb_table + (u32va>>12) * TLB_Entry_BYTE_SIZE;
