 ...
 * ============================================================================
 */
#define _MAD_CHIP_MODEL    "MAD MANHATTAN"
...
#define _MAD_DMA_READER2_BASE_OFFSET    (0x20000 + _MAD_DSP2_TO_COMMON_DRAM_OFFSET)
...
static int _mhal_alsa_dma_reader_init(void)
{
	...
	if ((dma_reader->initialized_status != MAD_TRUE) || (dma_reader->str_mode_info.status != E_RESUME)) {
		...
		audio_pcm_dmaRdr_base_va = (ptrdiff_t)ioremap_nocache(audio_pcm_dmaRdr_base_ba, audio_pcm_dmaRdr_bufSz);
...
static int _mhal_alsa_pcm_capture1_init(void)
{
	...
	if ((pcm_capture->initialized_status != MAD_TRUE) || (pcm_capture->str_mode_info.status != E_RESUME)) {
		...
		audio_pcm_capture_base_va = (ptrdiff_t)ioremap_nocache(audio_pcm_capture_base_ba, pcm_capture->buffer.size);
