...
U32 NC_Init(void)
{
    ...
    nand_debug(UNFD_DEBUG_LEVEL_LOW,1, "NC_ReadID\n");
...
void NC_Config(void)
{
    ...
    #if defined(NC_HWCMD_DELAY) && NC_HWCMD_DELAY
    REG_SET_BITS_UINT16(NC_RAND_W_CMD, pNandDrv->u16_Reg56_Rand_W_Cmd &BIT_NC_HW_DELAY_CNT_MASK);
    #endif
}
...
//    NC_ResetFCIE();
    ...
    while(1)
    {
        ...
        if(u32_Cnt == 0x1000)
        {
            nand_debug(UNFD_DEBUG_LEVEL_ERROR, 1, "ResetFCIE, Register checking fails\n");
            nand_die();
        }
        u32_Cnt ++;
    }
...
U32 NC_WritePages(    U32 u32_PhyRowIdx, U8 *pu8_DataBuf, U8 *pu8_SpareBuf, U32 u32_PageCnt )
{
    ...
    if (NC_WaitComplete(BIT_NC_JOB_END, WAIT_WRITE_TIME) == WAIT_WRITE_TIME)
    {
        ...
        NC_CLR_DDR_MODE();
        NC_ResetNandFlash();
        nand_DMA_UNMAP_address(dma_SpareMapAddr, pNandDrv->u16_SpareByteCnt, WRITE_TO_NAND);            
        nand_DMA_UNMAP_address(dma_DataMapAddr, pNandDrv->u16_PageByteCnt * u32_PageCnt, WRITE_TO_NAND);
...
U32 NC_ReadPages(U32 u32_PhyRowIdx, U8 *pu8_DataBuf, U8 *pu8_SpareBuf, U32 u32_PageCnt)
{
	...
	if( (u16_Tmp & (BIT_NC_JOB_END)) == (BIT_NC_JOB_END) )
		u8_EnableIRQ = 1;
	...
	if(u8_EnableIRQ == 1)
		REG_CLR_BITS_UINT16(NC_MIE_INT_EN, BIT_NC_JOB_END);	//disable irq for read
      ...
      (WAIT_READ_TIME * u32_PageCnt ))
    {
        ...
        NC_Config();
        NC_ResetNandFlash();
        nand_DMA_UNMAP_address(dma_SpareMapAddr, pNandDrv->u16_SpareByteCnt, READ_FROM_NAND);
        nand_DMA_UNMAP_address(dma_DataMapAddr, pNandDrv->u16_PageByteCnt * u32_PageCnt, READ_FROM_NAND);
        ...
        else
        {
            ...
            #endif
            {
 			...
 			if(u8_EnableIRQ == 1)
				REG_WRITE_UINT16(NC_MIE_INT_EN, BIT_NC_JOB_END);	//disable irq for read
    ...
    nand_flush_cache_post_read((uintptr_t)pu8_DataBuf, pNandDrv->u16_PageByteCnt * u32_PageCnt);
    
    if(pu8_SpareBuf)
        nand_flush_cache_post_read((uintptr_t)pu8_SpareBuf, pNandDrv->u16_SpareByteCnt);
    else        
        nand_flush_cache_post_read((uintptr_t)pNandDrv->PlatCtx_t.pu8_PageSpareBuf, pNandDrv->u16_SpareByteCnt);
    ...
    if (u16_Tmp & BIT_NC_ECC_FAIL)
    {
        ...
        if( u8_RetryCnt_ECCFail < u8_MaxReadRetryCount)
        {
            ...
            if(!pNandDrv->u8_RequireRandomizer)
            {
                ...
                nand_debug(UNFD_DEBUG_LEVEL_LOW, 1, "Blk: %Xh  Page: %X\r\n",
    ...
    if(u8_RequireReadRetry && 0!=u8_RetryCnt_ECCFail && u8_RetryCnt_ECCFail < u8_MaxReadRetryCount)
    {
        //should not be all 0xFF, if read retry
        if(pNandDrv->u8_RequireRandomizer && (REG(NC_LFSR_CTRL) & BIT_LFSR_ENABLE) == BIT_LFSR_ENABLE)
            REG_READ_UINT16(NC_ZERO_CNT_SCRAMBLE,u16_Tmp);
        else
            REG_READ_UINT16(NC_ZERO_CNT,u16_Tmp);
...
U32 NC_PageCopy(U32 u32_SrcPhyRowIdx, U32 u32_DstPhyRowIdx, U8 *pu8_DataBuf, U8 *pu8_SpareBuf, U32 u32_PageCnt )
{
      ...
      ((WAIT_WRITE_TIME + WAIT_READ_TIME) * u32_PageCnt))
    {
        ...
        NC_ResetNandFlash();
        nand_DMA_UNMAP_address(dma_SpareMapAddr, pNandDrv->u16_SpareByteCnt, BIDIRECTIONAL);
        nand_DMA_UNMAP_address(dma_DataMapAddr, pNandDrv->u16_PageByteCnt * u32_PageCnt, BIDIRECTIONAL);          
        #else
        //nand_stop();
        #endif
        return UNFD_ST_ERR_R_TIMEOUT;
...
U32  NC_ReadSectors_MTD(U32 u32_PhyRowIdx, U8 u8_SectorInPage, U8 *pu8_DataBuf, U8 *pu8_SpareBuf, U32 u32_SectorCnt)
{
    ...
    if (u32_SectorCnt_Tmp)
    {
        ...
        if (NC_WaitComplete(BIT_NC_JOB_END, WAIT_READ_TIME) == (WAIT_READ_TIME))
        {
            ...
            NC_Config();
            NC_ResetNandFlash();
            nand_DMA_UNMAP_address(dma_DataMapAddr, pNandDrv->u16_SectorByteCnt * u32_SectorCnt_Tmp, READ_FROM_NAND);
            nand_DMA_UNMAP_address(dma_SpareMapAddr, pNandDrv->u16_SpareByteCnt, READ_FROM_NAND);
            if( ++u8_RetryCnt_Events < NAND_TIMEOUT_RETRY_CNT )
        ...
        nand_flush_cache_post_read((uintptr_t)pu8_DataBuf, pNandDrv->u16_SectorByteCnt * u32_SectorCnt_Tmp);        
        if(pu8_SpareBuf)
            nand_flush_cache_post_read((uintptr_t)pu8_SpareBuf, pNandDrv->u16_SpareByteCnt);
        else        
            nand_flush_cache_post_read((uintptr_t)pNandDrv->PlatCtx_t.pu8_PageSpareBuf, pNandDrv->u16_SpareByteCnt);
...
U32  NC_WriteSectors(     U32 u32_PhyRowIdx, U8 u8_SectorInPage, U8 *pu8_DataBuf, U8 *pu8_SpareBuf, U32 u32_SectorCnt )
{
    ...
    if(u8_SectorInPage + u32_SectorCnt > pNandDrv->u16_PageSectorCnt)
    {
        ...
        nand_debug(UNFD_DEBUG_LEVEL_ERROR, 1, "%X %X %lX %lX %X\n",
            u32_PhyRowIdx, u8_SectorInPage, (uintptr_t)pu8_DataBuf, (uintptr_t)pu8_SpareBuf, u32_SectorCnt);
    ...
    if (NC_WaitComplete(BIT_NC_JOB_END, WAIT_WRITE_TIME) == WAIT_WRITE_TIME)
    {
        ...
        NC_ResetNandFlash();
        nand_DMA_UNMAP_address(dma_SpareMapAddr, pNandDrv->u16_SpareByteCnt, WRITE_TO_NAND);            
        nand_DMA_UNMAP_address(dma_DataMapAddr, pNandDrv->u16_SectorByteCnt * u32_SectorCnt, WRITE_TO_NAND);
...
U32  NC_ReadSector_RIUMode(     U32 u32_PhyRowIdx, U8 u8_SectorInPage, U8 *pu8_DataBuf, U8 *pu8_SpareBuf )
{
    ...
    {   // read data
        ...
        u32_Err = NC_WaitGetCIFD((U8*)((uintptr_t)pu8_DataBuf + (u16_j << 6)), u16_Tmp, WAIT_READ_TIME);
    ...
    {   // read data
        ...
        u32_Err = NC_WaitGetCIFD( (U8*)((uintptr_t)pu8_SpareBuf + (u16_j << 6)), u16_Tmp, WAIT_READ_TIME);
        if(u32_Err != UNFD_ST_SUCCESS)
        {
            REG_CLR_BITS_UINT16(NC_FUN_CTL, BIT_R2N_MODE_EN);
            NC_ResetFCIE();
...
U32  NC_WritePage_RIUMode(U32 u32_PhyRowIdx, U8 *pu8_DataBuf, U8 *pu8_SpareBuf )
{
    ...
    for(u16_i = 0; u16_i <  pNandDrv->u16_PageSectorCnt; u16_i ++)
    {
        ...
        {   
            ...
            u32_Err = NC_WaitSetCIFD( (U8*)( (uintptr_t) pu8_DataBuf + (u16_i << pNandDrv->u8_SectorByteCntBits) + (u16_j << 6)), u16_Tmp);
            if(u32_Err != UNFD_ST_SUCCESS)
            {
        ...
        {   
            ...
            u32_Err = NC_WaitSetCIFD((U8*)((uintptr_t)pu8_SpareBuf + (u16_i *  pNandDrv->u16_SectorSpareByteCnt) + (u16_j << 6)),  u16_Tmp);
            if(u32_Err != UNFD_ST_SUCCESS)
            {
...
U32 NC_Read_RandomIn (     U32 u32_PhyRow, U32 u32_Col, U8 *pu8DataBuf, U32 u32DataByteCnt )
{
    ...
    {   // read data
        ...
        NC_WaitGetCIFD((U8*)((uintptr_t)pu8DataBuf + (u16_j << 6)),  u16_Tmp, WAIT_READ_TIME);
        if(u32_Err != UNFD_ST_SUCCESS)
        {
            REG_CLR_BITS_UINT16(NC_FUN_CTL, BIT_R2N_MODE_EN);
            NC_ResetFCIE();
    ...
    if (NC_WaitComplete(BIT_NC_JOB_END, WAIT_READ_TIME) == WAIT_READ_TIME)
    {
        ...
        NC_ResetNandFlash();
        #if !(defined(RANDOM_WR_THR_CIFD) && RANDOM_WR_THR_CIFD)
        nand_DMA_UNMAP_address(dma_DataMapAddr, u32_Tmp, READ_FROM_NAND);
        #endif
        #else
        //nand_stop();
        #endif
...
U32 NC_Write_RandomOut (U32 u32_PhyRow, U32 u32_Col, U8 *pu8_DataBuf, U32 u32_DataByteCnt )
{
    ...
    {   
        ...
        u32_Err = NC_WaitSetCIFD( (U8*)( (uintptr_t) pu8_DataBuf + (u16_j << 6)),  u16_Tmp);
        if(u32_Err != UNFD_ST_SUCCESS)
        {
            nand_debug(0, 1, "Transfer Spare Byte %X\n", (u16_j << 6));
            REG_CLR_BITS_UINT16(NC_FUN_CTL, BIT_R2N_MODE_EN);
    ...
    if (NC_WaitComplete(BIT_NC_JOB_END, WAIT_WRITE_TIME) == WAIT_WRITE_TIME)
    {
        ...
        #if !(defined(RANDOM_WR_THR_CIFD) && RANDOM_WR_THR_CIFD)
        nand_DMA_UNMAP_address(dma_DataMapAddr, u32_Tmp, WRITE_TO_NAND);
        #endif
        #else
        //nand_stop();
        #endif
...
static U32 NC_RandomIn_1Cmd1Addr(U8 u8_Cmd, U8 u8_Addr, U8 u8_Remove2ndByte, U8 u8_ByteCnt, U8 *pu8_DataBuf)
{
    ...
    {   // read data
        ...
        u32_Err = NC_WaitGetCIFD((U8*)((uintptr_t)pu8_DataBuf + (u16_j << 6)),  u16_Tmp, WAIT_PROBE_TIME);
        if(u32_Err != UNFD_ST_SUCCESS)
        {   
...
static U32 NC_ProbeIfONFIDDR_Ex(void)
{
    //NAND_DRIVER *pNandDrv = (NAND_DRIVER*)drvNAND_get_DrvContext_address();
    U32 u32_Err;
    U8 u8_i, au8_buf[sizeof(sgau8_ONFI)*4]={0};
...
static U32 NC_ProbeIfToggleDDR_Ex(void)
{
    //NAND_DRIVER *pNandDrv = (NAND_DRIVER*)drvNAND_get_DrvContext_address();
    U32 u32_Err;
    U8 u8_i, au8_buf[sizeof(sgau8_JEDEC)*5]={0};
...
void NC_CheckECC(int *s32ECCStatus)
{
	...
	#else
	{
		...
		{
		    if( 2 == gu8_ReadRetryHappens)  //if read retry happens and bit flip count > 3/4 max correctable bit count => issue the refresh process.
		    {
		        gu8_ReadRetryHappens = 0;
		        *s32ECCStatus = 1;
...
)
{
    ...
    if (NC_WaitComplete(BIT_NC_JOB_END, WAIT_WRITE_TIME) == WAIT_WRITE_TIME)
    {
        ...
        nand_DMA_UNMAP_address(dma_SpareMapAddr, pNandDrv->u16_SpareByteCnt, WRITE_TO_NAND);            
        nand_DMA_UNMAP_address(dma_DataMapAddr, 2 * pNandDrv->u16_PageByteCnt * u32_PageCnt, WRITE_TO_NAND);
...
(
    U32 u32_PlaneRowIdx, U8 *pu8_DataBuf, U8 *pu8_SpareBuf, U32 u32_PageCnt,  struct write_info *write
)
{
    U16 u16_Tmp;
    NAND_DRIVER *pNandDrv = (NAND_DRIVER*)drvNAND_get_DrvContext_address();
...
U32 NC_ADMAReadPages(U32 u32_PhyRowIdx, U8 *pu8_DataBuf, U8 *pu8_SpareBuf, U32 u32_PageCnt, U8 u8_IsZdec)
{
    U16 u16_Tmp=0;
    NAND_DRIVER *pNandDrv = (NAND_DRIVER*)drvNAND_get_DrvContext_address();
    U32 u32_DataDMAAddr;
