...
//
//  Design Notes:
//    1. 2009/08/20 - UNFD logical layer, connect to file system,
//                    implement L2P Map, BBM, WL and Read-Disturbance.
//    2. 2009/08/20 - for Cyrus / EAON-eCos
//
//=======================================================================
#include "drvNAND.h"
...
static U8 au8_PData[8192] UNFD_ALIGN1;
static U8 au8_PSpare[512] UNFD_ALIGN1;

static U32  nand_WritePages_Raw(U32 u32_PhyRowIdx, U8 *pu8_DataBuf, U8 *pu8_SpareBuf, U32 u32_PageCnt)
{
    NAND_DRIVER *pNandDrv = drvNAND_get_DrvContext_address();
    U8 *au8_PageBuf = au8_PData;
    U8 *au8_SpareBuf = au8_PSpare;
    U32 u32_i, u32_Err = 0;
    U16 u16_SectorSpareFree = pNandDrv->u16_SectorSpareByteCnt - pNandDrv->u16_ECCCodeByteCnt;	
    U8 u8_FTLSpareLen = UNFD_FTL_SPARE_BYTE_CNT;
	...
	if (u8_FTLSpareLen > u16_SectorSpareFree && NULL!=pu8_SpareBuf)
	{
		memcpy(u8_SpareBufTmp, pu8_SpareBuf, UNFD_FTL_SPARE_BYTE_CNT);
					
		for (i = 0; u8_FTLSpareLen > 0; i++)
		{
			memcpy(&au8_SpareBuf[i*pNandDrv->u16_SectorSpareByteCnt], &u8_SpareBufTmp[i*u16_SectorSpareFree], u16_SectorSpareFree);

			u8_FTLSpareLen -= u8_FTLSpareLen>u16_SectorSpareFree ? u16_SectorSpareFree:u8_FTLSpareLen;
		...
		au8_SpareBuf = pu8_SpareBuf;

#if defined(CONFIG_MIPS)
    if( ((U32)pu8_DataBuf) >= 0xC0000000 || ((U32)pu8_DataBuf) % UNFD_CACHE_LINE )   // For
#elif defined(CONFIG_ARM)
    if(!virt_addr_valid((U32)pu8_DataBuf) || !virt_addr_valid((U32)pu8_DataBuf + (u32_PageCnt << pNandDrv->u8_PageByteCntBits)- 1) ) // For High MEM (ARM)  ||virt_to_phys((void*)buf) >= MSTAR_MIU1_BUS_BASE
#elif defined(CONFIG_ARM64)
    if(!virt_addr_valid((uintptr_t)pu8_DataBuf) || !virt_addr_valid((uintptr_t)pu8_DataBuf + (u32_PageCnt << pNandDrv->u8_PageByteCntBits) - 1) ) // For High MEM (ARM)  ||virt_to_phys((void*)buf) >= MSTAR_MIU1_BUS_BASE    
#endif
    {
        for(u32_i = 0; u32_i < u32_PageCnt; u32_i ++)
        {
            memcpy(au8_PageBuf, (void*)(pu8_DataBuf + pNandDrv->u16_PageByteCnt * u32_i), pNandDrv->u16_PageByteCnt);
            u32_Err = NC_WritePages(u32_PhyRowIdx + u32_i, au8_PageBuf, au8_SpareBuf, 1);
        ...
        return NC_WritePages(u32_PhyRowIdx, pu8_DataBuf, au8_SpareBuf, u32_PageCnt);
}
...
static U32  nand_ReadPages_Raw(U32 u32_PhyRowIdx, U8 *pu8_DataBuf, U8 *pu8_SpareBuf, U32 u32_PageCnt)
{
...
#endif
    {
        for(u32_i = 0; u32_i < u32_PageCnt; u32_i ++)
        {
            u32_Err = NC_ReadPages(u32_PhyRowIdx + u32_i, au8_PageBuf, pu8_SpareBuf, 1);
            if(u32_Err != UNFD_ST_SUCCESS)
                return u32_Err;
            
            memcpy((void*)(pu8_DataBuf + pNandDrv->u16_PageByteCnt * u32_i), au8_PageBuf, pNandDrv->u16_PageByteCnt);
            
			if(u8_FTLSpareLen > u16_SectorSpareFree && pu8_SpareBuf != NULL)
			{
				u8_FTLSpareLen -= u8_FTLSpareLen>u16_SectorSpareFree ? u16_SectorSpareFree:u8_FTLSpareLen;			
                
				for (i=1; u8_FTLSpareLen>0; i++)
				{
					memcpy(&pu8_SpareBuf[i*u16_SectorSpareFree], &pu8_SpareBuf[i*pNandDrv->u16_SectorSpareByteCnt], u16_SectorSpareFree);
...
#if 0
static U32  nand_WriteSectors_Raw                               //the buffer of write sector is the buffer of nand driver, no need to handle high memory or unaligned cache
(
...
{
    NAND_DRIVER *pNandDrv = drvNAND_get_DrvContext_address();
    U8 *au8_PageBuf = (U8*)au8_PData;
	U8 *au8_SpareBuf = (U8*)au8_PSpare;
    ...
    if( ((U32)pu8_DataBuf) >= 0xC0000000 || ((U32)pu8_DataBuf) % UNFD_CACHE_LINE )   // For
#elif defined(CONFIG_ARM)
    if(!virt_addr_valid((U32)pu8_DataBuf) || !virt_addr_valid((U32)pu8_DataBuf + (u32_SectorCnt << pNandDrv->u8_SectorByteCntBits)- 1) ) // For High MEM (ARM)  ||virt_to_phys((void*)buf) >= MSTAR_MIU1_BUS_BASE
#elif defined(CONFIG_ARM64)
    if(!virt_addr_valid((uintptr_t)pu8_DataBuf) || !virt_addr_valid((uintptr_t)pu8_DataBuf + (u32_SectorCnt << pNandDrv->u8_SectorByteCntBits) - 1) ) // For High MEM (ARM)  ||virt_to_phys((void*)buf) >= MSTAR_MIU1_BUS_BASE    
#endif
    {
		if (u8_FTLSpareLen > u16_SectorSpareFree && (0 == u8_SectorInPage))
		{
			u32_Err = NC_ReadPages(u32_PhyRowIdx, au8_PageBuf, au8_SpareBuf, 1);
			if(u32_Err != UNFD_ST_SUCCESS)
				return u32_Err;

	        memcpy((void*)pu8_DataBuf, (void*)au8_PageBuf, pNandDrv->u16_SectorByteCnt*u32_SectorCnt);
			
            if(pu8_SpareBuf)
			for (i=0; u8_FTLSpareLen>0; i++)
			{
				memcpy(&pu8_SpareBuf[i*u16_SectorSpareFree], &au8_SpareBuf[i*pNandDrv->u16_SectorSpareByteCnt], u16_SectorSpareFree);
				u8_FTLSpareLen -= u8_FTLSpareLen>u16_SectorSpareFree ? u16_SectorSpareFree:u8_FTLSpareLen;
			}			
		}
		else
        ...
        {
            u32_Err = NC_ReadSectors_MTD(u32_PhyRowIdx, u8_SectorInPage + u32_i,  au8_PageBuf, pu8_SpareBuf, 1);
            if(u32_Err != UNFD_ST_SUCCESS)
                return u32_Err;
                
            memcpy((void*)(pu8_DataBuf + pNandDrv->u16_SectorByteCnt * u32_i), au8_PageBuf, pNandDrv->u16_SectorByteCnt);
        }
		}
    }
    else
	{
        ...
        return u32_Err;
			...
			memcpy((void*)pu8_DataBuf, au8_PageBuf, pNandDrv->u16_SectorByteCnt*u32_SectorCnt);
	        ...
	        u32_Err = NC_ReadSectors_MTD(u32_PhyRowIdx, u8_SectorInPage, pu8_DataBuf, pu8_SpareBuf, u32_SectorCnt);
	}
	return u32_Err;
}
...
static void nand_ECC_ErrorHandler(U16 u16_ErrPBA)
{
    ...
    for(u16_i=0; u16_i<pNandDrv->u16_BlkPageCnt; u16_i++)
    {
        ...
        if(UNFD_ST_SUCCESS != u32_Err)
        {
            ...
            {   
                nand_debug(UNFD_DEBUG_LEVEL_ERROR, 1, "Err: may be HW (PCB) error around NAND: %X\n", u32_Err);
                nand_die();
            }
        }
    }
...
U32 nand_WriteFlow(U32 u32_StartSectorIdx, U32 u32_SectorCnt, U32 u32_RWBufferAddress)
{
    ...
    while(u32_SectorCnt > 0)
    {
        ...
        while(u16_Stg2SectorCnt)
        {
            ...
            if(u32_StartSectorIdx & pNandDrv->u16_PageSectorCntMask)
            {
                ...
                {
                    u32_Ret = nand_WritePages_Raw(u32_Row, pNandDrv->PlatCtx_t.pu8_PageDataBuf,
    ...
    #if UNFD_FTL_WBQ
    drvNAND_TaskBGTrequest_fn(NAND_MSG_FORCE_WRITE_BACK);
...
static void drvNAND_CheckMIUSignature(volatile U32 *pu32_Addr0, volatile U32 *pu32_Addr1)
{
    if(pu32_Addr0[0] == NAND_DMA_RACING_PATTERN0 || pu32_Addr1[0] == NAND_DMA_RACING_PATTERN1)
    {
        ...
        if(pu32_Addr0[0] == NAND_DMA_RACING_PATTERN0 || pu32_Addr1[0] == NAND_DMA_RACING_PATTERN1)
        {
            ...
            nand_debug(UNFD_DEBUG_LEVEL,1,"u32_Addr0(%X) u32_Addr1(%X) u32_Addr0Dat(%X) u32_Addr1Dat(%X)\n", u32_Addr0, u32_Addr0Dat, u32_Addr1, u32_Addr1Dat);
            nand_die();
            #else
            nand_Invalidate_data_cache_buffer((U32)pu32_Addr0, sizeof(U32));
            nand_Invalidate_data_cache_buffer((U32)pu32_Addr1, sizeof(U32));
...
//    U16  u16_Cnt;
    ...
    pu16_InitBBArray =(U16*) vmalloc(506);
...
static void nand_erase_and_set_FreeBlk(U16 u16_PBA)
{
    ...
    if(1 == drvNAND_ChkRdy(0)) // ignore when building LUT
    {
        ...
        {
            if(u16_PBA < pNandDrv->u16_FirstPBA_Zone1) // zone 0
                drvNAND_TaskBGTrequest_fn(NAND_MSG_WEAR_LEVELING);
                //drvNAND_SendMsg2BkgndTask(NAND_MSG_WEAR_LEVELING);
            else
                //drvNAND_SendMsg2BkgndTask(NAND_MSG_WEAR_LEVELING1);
                drvNAND_TaskBGTrequest_fn(NAND_MSG_WEAR_LEVELING1);
        }
...
static void nand_dump_FreeBlk(U8 u8_ZoneID)
{
    ...
    nand_debug(0,1, "head:%X, tail:%X, cnt:%X, max:%X, buf:%lX \n",
        u8_head, u8_tail, u8_cnt, u8_last, (uintptr_t)pu16_buf);
...
static void nand_dump_DiskInfo(U8 u8_DiskCnt)
{
    ...
    for(u8_i=0; u8_i<u8_DiskCnt; u8_i++)
        nand_debug(0,1,"LP[%0X], LBA:%04X, Cnt: %04X\n",
...
U32 nand_get_InitBBT(U16 *pu16_InitBBArray,U16 *u16_Cnt)
{
    ...
    INIT_BBT_t *pInitBBT = (INIT_BBT_t *)pNandDrv->PlatCtx_t.pu8_PageDataBuf;
    DATA_BLK_SPARE_t *pDataBlkSpare_t = (P_DATA_BLK_SPARE_t)pNandDrv->PlatCtx_t.pu8_PageSpareBuf;
    ...
    for(u8_i=0; u8_i<10; u8_i++)
    {
        ...
        if(UNFD_ST_SUCCESS == u32_Ret)
        {
            ...
            {
                nand_debug(UNFD_DEBUG_LEVEL_WARNING, 1, "get IBBT @ PBA:%Xh, ChkSum: %Xh, BBCnt: %u\n",
...
static U32 nand_init_parameters(void)
{
    ...
    pNandDrv->u16_FirstPBA_Zone1 = pNandDrv->u16_FirstPBA_LogiArea + //pNandDrv->u16_FirstPBA_Zone0 + 
