...
static int MDev_GMAC_update_linkspeed (struct net_device *dev)
{
    ...
    {               
        ...
        MHal_GMAC_read_phy(1, MII_LPA, &LocPtrA);
...
static void MDev_GMAC_set_rx_mode (struct net_device *dev)
{
    ...
    {   
        ...
        {
            MDev_GMAC_sethashtable(ha->addr);
        }
        uRegVal |= GMAC_MTI;
    }
...
#ifdef GMAC_CHIP_FLUSH_READ
    #if defined(CONFIG_MIPS)
        Chip_Read_Memory_Range((unsigned int)(&(dlist->descriptors[LocPtr->rxBuffIndex].addr)) & 0x0FFFFFFFUL, sizeof(dlist->descriptors[LocPtr->rxBuffIndex]));
    #elif defined(CONFIG_ARM)
...
static int MDev_GMAC_rx (struct net_device *dev)
{
    ...
    do
    {
        ...
        if( ((dlist->descriptors[LocPtr->rxBuffIndex].addr) & GMAC_DESC_DONE) == 0 )
        {
            break;
        }
    #if GMAC_RX_THROUGHPUT_TEST
...
void MDev_GMAC_HW_init(void)
{
    ...
    // Init RX --------------------------------------------------------------
    memset((u8*)GMAC_RAM_VA_PA_OFFSET + GMAC_RX_BUFFER_BASE, 0x00UL, GMAC_RX_BUFFER_SIZE);

    MHal_GMAC_Write_BUFF((GMAC_RX_BUFFER_BASE | GMAC_RX_BUFFER_SEL) - GMAC_MIU0_BUS_BASE);
    MHal_GMAC_Write_RDPTR(0x00000000UL);
    MHal_GMAC_Write_WRPTR(0x00000000UL);
...
#ifndef GMAC_RX_SOFTWARE_DESCRIPTOR
    //Add Write Protect
    MHal_GMAC_Write_Protect(alloRAM_PA_BASE - GMAC_MIU0_BUS_BASE, alloRAM_SIZE);
#endif
    memset((u32 *)alloRAM_VA_BASE,0x00UL,alloRAM_SIZE);
    
    GMAC_RAM_VA_BASE       = ((u32)alloRAM_VA_BASE + sizeof(struct GMAC_private) + 0x3FFFUL) & ~0x3FFFUL;   // IMPORTANT: Let lowest 14 bits as zero.
    GMAC_RAM_PA_BASE       = ((u32)alloRAM_PA_BASE + sizeof(struct GMAC_private) + 0x3FFFUL) & ~0x3FFFUL;   // IMPORTANT: Let lowest 14 bits as zero.
...
static u32 MDev_GMAC_get_drvinfo(struct net_device *dev, struct ethtool_drvinfo *info)
{
    u32	u32data;
    struct GMAC_private *LocPtr =(struct GMAC_private *) netdev_priv(dev);
...
#ifdef GMAC_NAPI
    netif_napi_add(gmac_dev, &LocPtr->napi_str, MDev_GMAC_napi_poll, 10000);
#endif
