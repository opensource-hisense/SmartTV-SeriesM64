...
static int mali_mem_os_alloc_pages(mali_mem_allocation *descriptor, u32 size)
{
	...
	for (i = 0; i < remaining; i++) {
		...
		new_page = alloc_page(GFP_USER     | __GFP_ZERO | __GFP_REPEAT | __GFP_NOWARN | __GFP_COLD | __GFP_NORMAL_MIU0);    /* MStar: allocate GPU memory from MIU0 */
#elif defined(CONFIG_ZONE_NORMAL_MIU1)
		new_page = alloc_page(GFP_HIGHUSER | __GFP_ZERO | __GFP_REPEAT | __GFP_NOWARN | __GFP_COLD | __GFP_NORMAL_MIU1);    /* MStar: allocate GPU memory from MIU1 */
#else
		new_page = alloc_page(GFP_HIGHUSER | __GFP_ZERO | __GFP_REPEAT | __GFP_NOWARN | __GFP_COLD);
#endif
...
#ifdef MSTAR
		u32 phys = page_private(page) - PHYS_TO_BUS_ADDRESS_ADJUST;
#else
