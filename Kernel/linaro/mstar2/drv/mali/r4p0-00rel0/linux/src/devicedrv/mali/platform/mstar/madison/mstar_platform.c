...
/* Use bit 0 of REG_BRI_DUMMY_REG1 as SW change flag */
#define   REG_BRI_DUMMY_REG1    0x69
#define     REG_SW_CHANGE       0x0001
...
#define GPU_CLOCK_MAX 432
#define GPU_CLOCK_MIN 216
#define GPU_CLOCK_STEP 6
...
void mstar_platform_init(void)
{
    ...
    /* GPU clock has been set by MBoot. */
    /* Mali kernel driver only saturates it to a reasonable extent if necessary. */
    if (G3D_REG(REG_BRI_DUMMY_REG1) | REG_SW_CHANGE)
    {
        if ((G3D_REG(REG_GPUPLL_CTRL1_LOW)&0xFF) > (GPU_CLOCK_MAX/GPU_CLOCK_STEP))
        {
            mali_gpu_clock = GPU_CLOCK_MAX;
        }
        else if ((G3D_REG(REG_GPUPLL_CTRL1_LOW)&0xFF) < (GPU_CLOCK_MIN/GPU_CLOCK_STEP))
        {
            mali_gpu_clock = GPU_CLOCK_MIN;
        }
        else
        {
            mali_gpu_clock = (G3D_REG(REG_GPUPLL_CTRL1_LOW)&0xFF)*GPU_CLOCK_STEP;
        }
    }
    /* GPU clock is only controlled by Mali kernel driver */
    else
    {
        mali_gpu_clock = GPU_CLOCK/GPU_CLOCK_STEP*GPU_CLOCK_STEP;
    }
