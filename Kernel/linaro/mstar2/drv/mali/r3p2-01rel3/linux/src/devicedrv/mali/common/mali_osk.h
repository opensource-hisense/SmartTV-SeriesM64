...
/**
 * @brief Private data type for use in RIU accesses to/from devices.
 *
 * This represents some range that is accessible through the RIU.
 *
 * Access to this range must be made through the _mstar_riu_read() and _mstar_riu_write() functions.
 */
#if (MSTAR_RIU_ADDRESS_TYPE == 32)
typedef u8* mstar_riu_address;
#elif (MSTAR_RIU_ADDRESS_TYPE == 16)
typedef u16* mstar_riu_address;
#else
#error "No supported RIU address type defined"
#endif

/**
 * @brief Read Mali registers via RIU.
 *
 * This function reads values from Mali registers via RIU. 
 *
 * @param base The register base to read values from
 * @param offset The register offset
 *
 * @return Register value
 */ 
u32 _mstar_riu_read(volatile mstar_riu_address base, u32 offset);

/**
 * @brief Write Mali registers via RIU.
 *
 * This function writes values to Mali registers via RIU.
 *
 * @param base The register base to write values to
 * @param offset The register offset
 * @param value The value to write to registers
 */ 
void _mstar_riu_write(volatile mstar_riu_address base, u32 offset, u32 value);
 ...
 * @brief Read Mali registers via RIU.
 *
 * This function reads values from Mali registers via RIU without locking.
 *
 * @param base The register base to read values from
 * @param offset The register offset
 *
 * @return Register value
 */ 
u32 _mstar_riu_read_no_lock(volatile mstar_riu_address base, u32 offset);

/**
 * @brief Write Mali registers via RIU.
 *
 * This function writes values to Mali registers via RIU without locking.
 *
 * @param base The register base to write values to
 * @param offset The register offset
 * @param value The value to write to registers
 */ 
void _mstar_riu_write_no_lock(volatile mstar_riu_address base, u32 offset, u32 value);

#endif /* MSTAR_RIU_ADDRESS_TYPE != 32 */
...
/** @brief Retrun is current session SurfaceFlinger.
 *
 * @return Is current session SurfaceFlinger.
 */
int _mali_osk_is_surfaceflinger(void);
#endif
