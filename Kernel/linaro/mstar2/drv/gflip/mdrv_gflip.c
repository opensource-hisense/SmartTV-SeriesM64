...
typedef struct
{
    ...
    unsigned short SC_OPBlending[4];
#ifdef GOP_MIU_REG
...
#if (!defined(CONFIG_MSTAR_URANUS4) && !defined(CONFIG_MSTAR_KRONUS) && !defined(CONFIG_MSTAR_KAISERIN) && !defined(CONFIG_MSTAR_KENYA) && !defined(CONFIG_MSTAR_KERES) && !defined(CONFIG_MSTAR_KIRIN) && !defined(CONFIG_MSTAR_KRIS) \
                                    && !defined(CONFIG_MSTAR_KRATOS) && !defined(CONFIG_MSTAR_KIWI) && !defined(CONFIG_MSTAR_KENTUCKY))
    ...
    for(u32GopIdx=0; u32GopIdx<MAX_GOP_SUPPORT; u32GopIdx++)
    {
        for(u32GwinIdx=0; u32GwinIdx<MAX_GOP_GWIN; u32GwinIdx++)
        {
            ...
            else //no kick off, try kick off
            {
                ...
                MS_TLB_GFLIP_MULTIINFO* pTLBMultiFlipInfo =(MS_TLB_GFLIP_MULTIINFO* ) _GFlipInfo[u32GopIdx][u32GwinIdx][_u32GFlipInfoReadPtr[u32GopIdx][u32GwinIdx]].pdata;
...
MS_BOOL MDrv_GFLIP_WaitForVsync(MS_U32 u32GopIdx)
{
    ...
    switch(u32GopIdx)
    {
        ...
        case 5:
            ...
            break;
        default :
            printk("[%s][%d]u32GopIdx =%ld is out of case",__FUNCTION__,__LINE__, u32GopIdx);
            break;
    }
    return bRet;
}
...
int MDrv_GFLIP_Suspend(void)
{
    ...
    _gflip_regs_save.SC_OPBlending[2] = SC_REG(GOP_SC_IP2SC);
    _gflip_regs_save.SC_OPBlending[3] = SC_REG(GOP_SC_GOPBLENDING_H);
#endif
#ifdef GOP_MIU_REG
...
#ifdef GOP_SC1_GOPEN
    SC_REG(GOP_SC_BANKSEL) = GOP_SC1_GOPEN;
    SC_REG(GOP_SC_IP2SC) = _gflip_regs_save.SC_OPBlending[2];
    SC_REG(GOP_SC_GOPBLENDING_H) = _gflip_regs_save.SC_OPBlending[3];
#endif
#ifdef GOP_MIU_REG
