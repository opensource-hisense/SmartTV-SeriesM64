...
void enable_cache(MS_BOOL bEnable)
{
    ...
    if (bEnable) //Before enable L1 cache, we need to invalidate L2.
        Chip_L2_cache_inv(0, 0xFFFFFFFF);
    ...
    if (!bEnable) //After disable L1 cache, we need to flush and invalidate L2.
