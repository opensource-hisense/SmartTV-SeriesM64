...
{
	int value = MDrv_GPIO_Pad_Read(offset);
	printk(KERN_EMERG "[%s] gpio_%d: value=%d\n", __FUNCTION__, offset, value);
	return value;
}
...
static void mstar_gpio_set(struct gpio_chip *chip, unsigned offset, int value)
{
	if (value) //high
	{
		MDrv_GPIO_Set_High(offset);
	}
	else
	{
		MDrv_GPIO_Set_Low(offset);
	}
}

static int mstar_direction_input(struct gpio_chip *chip, unsigned offset)
{
	MDrv_GPIO_Pad_Odn(offset);
	return 0;
}

static int mstar_direction_output(struct gpio_chip *chip, unsigned offset, int value)
{
...
static struct gpio_chip mstar_gpio_chip = {
	.label			= "gpio",
	.direction_input	= mstar_direction_input,
	.direction_output	= mstar_direction_output,
	.set			= mstar_gpio_set,
	.get			= mstar_gpio_get,
	.base			= 0,
	.ngpio			= 200,
