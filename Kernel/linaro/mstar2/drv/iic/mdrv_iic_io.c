...
static int mod_iic_init(void)
{
    ...
    IICClass = class_create(THIS_MODULE, MOD_IIC_NAME);
    if (IS_ERR(IICClass))
    {
        IIC_PRINT("IIC class create error.\n");
        return PTR_ERR(IICClass);
    }
    ...
    device_create(IICClass, NULL, dev, NULL, "iic");
...
static int __init mstar_iic_drv_init_module(void)
{
    int retval=0;
    IIC_PRINT("[ki2c] %s\n",__FUNCTION__);
    platform_device_register(&Mstar_iic_device);
...
static void __exit mstar_iic_drv_exit_module(void)
{
    ...
    platform_device_unregister(&Mstar_iic_device);
}
