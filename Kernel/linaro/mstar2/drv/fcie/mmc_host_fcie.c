...
void mmc_fcie_request(struct mmc_host *host, struct mmc_request *req)
{
    ...
    down(&PfModeSem); // mark because system driver not ready
    ...
    up(&PfModeSem); // mark because system driver not ready
...
static int mmc_fcie_probe(struct platform_device *pdev)
{
	...
	mmc_add_host(host);
    	...
    	request_irq(FCIE_INT_VECTOR, HalFcie_KernelIrq, IRQF_SHARED, "mstar fcie", host);
	#endif
