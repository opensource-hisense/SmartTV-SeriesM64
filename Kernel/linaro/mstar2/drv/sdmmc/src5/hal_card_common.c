...
void Hal_CARD_PowerOff(PADEmType ePAD, U16_T u16DelayMs)
{
    ...
    {
        //EVB board
        //FCIE_RIU_16_OF(GET_CARD_REG_ADDR(A_PMU_BANK,0x1C), BIT00);        //reg_ldoper2_a_en
...
U32_T Hal_CARD_TransMIUAddr(IPEmType eIP, U32_T u32Addr)
{
    ...
    //FCIE_RIU_16_ON(A_MMA_PRI_REG(eIP), BIT04|BIT05); // burst 8

    if( u32Addr >= MSTAR_MIU1_BUS_BASE)
    {
		//printk("miu 1 addr: %08Xh - %08Xh = ", u32Addr, MSTAR_MIU1_BUS_BASE);
    ...
    {
		//printk("miu 0 addr: %08Xh - %08Xh = ", u32Addr, MSTAR_MIU0_BUS_BASE);
