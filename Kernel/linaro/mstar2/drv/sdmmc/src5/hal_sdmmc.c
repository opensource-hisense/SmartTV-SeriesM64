...
#define M_SD_MIEEVENT				BIT_ALL_CARD_INT_EVENTS
#define M_REG_SDMIEEvent(IP)        (FCIE_RIU_R16(A_MIE_EVENT_REG(IP)) & M_SD_MIEEVENT)
...
#define A_RD_SBIT_TIMER(IP)			GET_CARD_REG_ADDR(A_SD_REG_BANK(IP), 0x18)
...
static RetEmType _REG_ClearMIEEvent(IPEmType eIP, U8_T u8Retry)
{
	...
	do
	{
		FCIE_RIU_R16(A_MIE_EVENT_REG(eIP)) = M_SD_MIEEVENT;
...
static RetEmType _REG_WaitMIEEvent(IPEmType eIP, U16_T u16ReqEvent, U32_T u32WaitMs)
{
	...
	{
		u8Result = Hal_CARD_INT_WaitMIEEvent(eIP, u16ReqEvent, u32WaitMs);
		if ( u8Result == 0 )
		{
			printk("wait mie event %04Xh timeout\n", u16ReqEvent);
			return EV_FAIL;
		}
...
static void _REG_ResetIP(IPEmType eIP)
{
	...
	{
		panic("SdErr: FCIE/SDIO(%d) reset fail %04Xh\n", eIP, FCIE_RIU_R16(A_FCIE_RST(eIP)));
	}
...
void HalFcie_DumpRegister(void)
{
	...
	HalFcie_DumpRegisterBank(A_FCIE2_0_BANK, 0x80); // FCIE
...
{
	//#define MONITOR_STATE_MACHINE

	#ifdef MONITOR_STATE_MACHINE
		U16_T u16StateMachineArray[20];
		U16_T u16_i;
	#endif
	...
	FCIE_RIU_16_ON(A_TEST_MODE(eIP), 2<<BIT_DEBUG_MODE_SHIFT); // 2: card_data1_dbus = {xxxxx, Wrstate, RDstate}
		...
		for(u16_i=0; u16_i<20; u16_i++)
		{
			u16StateMachineArray[u16_i] = FCIE_RIU_R16(A_DEBUG_BUS_0(eIP));
			if(u16_i>10) udelay(1);
		}

		if(u16StateMachineArray[0]&0x0FFF) // Brian: check bit 0~11, RDstate, Wrstate, xxxxx(DMA?)
		{
			printk(LIGHT_RED"SdWarn: state machine not clean %04Xh, reset IP %s\n"NONE, FCIE_RIU_R16(A_DEBUG_BUS_0(eIP)), (eIP==0)?"SDIO":"FCIE");
			printk("RDState: ");
			for(u16_i=0; u16_i<20; u16_i++)
			{
				printk("%04Xh ", u16StateMachineArray[u16_i]);
			}
			printk("\n");
			_REG_ResetIP(eIP);
		}
...
static RspErrEmType _SDMMC_ErrProcess(IPEmType eIP, CmdEmType eCmdType, RspErrEmType eRspErr, BOOL_T bCloseClk, int Line)
{
	...
	//printk("sd status:%x\n", FCIE_RIU_R16(A_SD_STS_REG(eIP)));
	if (FCIE_RIU_R16(A_SD_STS_REG(eIP)) & 0x17)
	{
...
RspErrEmType Hal_SDMMC_SendCmdAndWaitProcess(IPEmType eIP, TransEmType eTransType, CmdEmType eCmdType, SDMMCRspEmType eRspType, BOOL_T bCloseClk, BOOL_T stop_clk )
{
	...
	FCIE_RIU_R16(A_SD_CTL_REG(eIP)) = (eRspType>>12) | (eCmdType>>4);
	...
	if(eCmdType==EV_CMDREAD) // read, start command & dtrx at the same time
	{
		#if(EN_BIND_CARD_INT)
			#if ENABLE_ERROR_DET_INT
				Hal_CARD_INT_SetMIEIntEn(eIP, EV_INT_SD, BIT_DMA_END|BIT_ERR_STS);
			#else
		...
		#else
			#if ENABLE_ERROR_DET_INT
				FCIE_RIU_16_ON(A_SD_CTL_REG(eIP), BIT_ERR_DET_ON);
				FCIE_RIU_16_ON(A_RD_SBIT_TIMER(eIP), BIT_RD_SBIT_TIMER_EN); // 1.3 ms in default
				//SDSC need calc
				//SDHC fixed 100ms, 50MHz
			#endif
	...
	else // "simple command without data transfer" or "write start command first"
	{
		#if(EN_BIND_CARD_INT)
			#if ENABLE_ERROR_DET_INT
				Hal_CARD_INT_SetMIEIntEn(eIP, EV_INT_SD, BIT_SD_CMD_END|BIT_ERR_STS);
			#else
		...
		if ( _REG_WaitMIEEvent(eIP, BIT_SD_CMD_END, u32WaitMS) )
		{
			#if ENABLE_ERROR_DET_INT
				// patch FCIE5 R3 bug
				if( (eRspType == EV_R3) && ((FCIE_RIU_R16(A_SD_STS_REG(eIP))&BIT_SD_ERR_BITS) == BIT_SD_RSP_CRC_ERR) )
				{
					//printk("R3 & only RSP CRC error\n");
				}
				else
				{
					return _SDMMC_ErrProcess(eIP, eCmdType, EV_STS_MIE_TOUT, bCloseClk, __LINE__);
...
//	FCIE_RIU_R16(A_SD_MODE_REG(eIP)) = _REG_GetPortSetting(eIP);
	...
	//if ( FCIE_RIU_R16(A_SD_STS_REG(eIP)) & R_SD_WR_PRO_N )
