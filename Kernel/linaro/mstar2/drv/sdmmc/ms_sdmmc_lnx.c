...
static U16_T _PreDataBufferMapping(TransEmType eTransType, struct mmc_data *data, struct ms_sdmmc_slot *sdmmchost, volatile U32_T *pu32AddrArr)
{
	...
	//printk("dmalen:%x\n",dmalen);
        ...
        if( dmaaddr >= MSTAR_MIU1_BUS_BASE) // MIU1
        {
            ...
            gSDAdmaDesc[i].u32_MiuSel = 1;
        }
        else // MIU0
        {
            dmaaddr -= MSTAR_MIU0_BUS_BASE;
            gSDAdmaDesc[i].u32_MiuSel = 0;
        }

        gSDAdmaDesc[i].u32_Address = dmaaddr;
        gSDAdmaDesc[i].u32_DmaLen = dmalen;
        gSDAdmaDesc[i].u32_JobCnt = (dmalen / data->blksz );
...
static U16_T _PreDataBufferMapping(TransEmType eTransType, struct mmc_data *data, struct ms_sdmmc_slot *sdmmchost, volatile U32_T *pu32AddrArr)
{
	...
	if(eTransType == EV_DMA)
	{
...
#endif
		{
			...
			BUG_ON(p_sg_list->dma_address&0xFFFFFFFF00000000); // mstar bus address only 32 bits only
...
static int _RequestErrProcess(CmdEmType eCmdType, RspErrEmType eErrType, struct ms_sdmmc_slot *p_sdmmc_slot, struct mmc_data *data)
{
	...
	{
		//printk(LIGHT_RED"eErrType--> %04Xh\n"NONE, (U16_T)eErrType);
		...
		switch( (U16_T)eErrType )
		{
			...
			case EV_STS_RSP_CERR:
				...
				break;
			default:
				printk(LIGHT_RED"translate me %d to kernel error code\n"NONE, eErrType);
				nErr = -EIO;
				break;
		}
	}
	...
	{
		if (p_sdmmc_slot->mmc->f_max >=48000000)
		{
			printk("48M down to 32M\n");
			p_sdmmc_slot->currRealClk =32000000;
			// p_sdmmc_slot->currRealClk = _SetClock(eSlot, p_sdmmc_slot->currClk, gu32_PassLVLSlot[eSlot], p_sdmmc_slot->currDownLevel);
			Drv_Switch_Bypass();
			Hal_CARD_SetClock(eIP, 32000000);
			Hal_SDMMC_SetNcrDelay(eIP, 32000000);
		}
		#if 0
...
static irqreturn_t ms_sdmmc_cdzint(int irq, void *p_dev_id)
{
	...
	if(Hal_CARD_GPIOIntFilter((GPIOEmType)eSlot))
	{
		...
		//tasklet_schedule(&p_sdmmc_slot->hotplug_tasklet);
		 schedule_work(&p_sdmmc_slot->workqueue_cd);
...
#else
static void ms_sdmmc_hotplug(struct work_struct *work)
{
	struct ms_sdmmc_slot *p_sdmmc_slot = container_of(work, struct ms_sdmmc_slot, workqueue_cd);
...
#ifdef  SDBUS
static U8_T TuningPattern[64]={ 0xff, 0x0f, 0xff, 0,
							  0xff, 0xcc, 0xc3, 0xcc,
							  0xc3,0x3c, 0xcc, 0xff,
							  0xfe, 0xff, 0xfe, 0xef,
							  0xff, 0xdf, 0xff, 0xdd,
...
static int sd_execute_tuning(struct mmc_host *mmc, u32 opcode)
{
	...
	for (j=0 ; j <=0x11 ; j++)
	{
      ...
      eErr=Hal_SDMMC_SendCmdAndWaitProcess(eIP, EV_DMA, EV_CMDREAD, EV_R1, TRUE, TRUE);
	  ...
	  if (DataErr==FALSE)
	  {
	  	max=j;
	  	if (j<min) min=j;
	  	printk(" \nScan %x Data is Correct\n",j);
	  }
	  else printk("\nScan %x data is incorrect\n",j);
	}
	//printk("Proper scan idx:%x\n",(min+max)/2);
	gScan=0x0a ;
...
static void ms_sdmmc_request(struct mmc_host *p_mmc_host, struct mmc_request *p_mmc_req)
{
	...
	BOOL_T bCloseClock = FALSE,auto_stop_clk;
	U8_T u8CMD = 0;
...
//	printk(">> [sdmmc_%u] CMD_%u (0x%08X)\n", eSlot, u8CMD, u32Arg);
	...
	{
		if(u8CMD==11)
			eErr = Hal_SDMMC_SendCmdAndWaitProcess(eIP, EV_EMP, EV_CMDRSP, _TransRspType(mmc_resp_type(cmd)), FALSE, FALSE);
		else
			eErr = Hal_SDMMC_SendCmdAndWaitProcess(eIP, EV_EMP, EV_CMDRSP, _TransRspType(mmc_resp_type(cmd)), TRUE, FALSE);
	}
	else // R/W SD Command
	{
	...
	//	if((u16BlkCnt == 1) && (u16BlkSize<=512))
		...
		eErr = Hal_SDMMC_SendCmdAndWaitProcess(eIP, eTransType, eCmdType, _TransRspType(mmc_resp_type(cmd)), bCloseClock, auto_stop_clk);
...
LABEL_SD_ERR:
	...
	{
		//if (eErr & 0x2)
		//	printk("cmd 59 should have error:%x\n",eErr);
		//For CMD 59, there is no CRC status from the device after each block data, so it always make R_SD_DAT_STSERR error.
		eErr &=0x1d;
	}
	...
	if (cmd->error ==-ENOMEDIUM)			//no retry
	{
		cmd->retries = 0;
		if (data)
		{
	...
	//printk("mmc request done cmd->error = %d\n", cmd->error);
...
static int ms_sdmmc_init_slot(unsigned int slotNo, struct ms_sdmmc_host *p_sdmmc_host)
{
 ...
 //   p_mmc_host->f_max = 50000000;	//104M
	...
	if(!gb_HotplugSlot[eSlot])
	{
		...
		INIT_WORK(&p_sdmmc_slot->workqueue_cd, ms_sdmmc_hotplug);
...
static void ms_sdmmc_remove_slot(unsigned int slotNo, struct ms_sdmmc_host *p_sdmmc_host)
{
	...
	if(gb_HotplugSlot[eSlot])
	{
		...
		destroy_work_on_stack(&p_sdmmc_slot->workqueue_cd);
