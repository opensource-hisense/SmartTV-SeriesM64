...
// HAL APIs
//========================================================
U32 eMMC_FCIE_WaitEvents(uintptr_t ulongRegAddr, U16 u16_Events, U32 u32_MicroSec)
{
	return (*sgpFn_eMMC_FCIE_WaitEvents_Ex)(ulongRegAddr, u16_Events, u32_MicroSec);
}

U32 eMMC_FCIE_PollingEvents(uintptr_t ulongRegAddr, U16 u16_Events, U32 u32_MicroSec)
{
	...
	if(u32_i == u32_MicroSec)
	{
		eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1,
			...
			u32_MicroSec*u32_DelayX,
			(ulongRegAddr-FCIE_MIE_EVENT)>>REG_OFFSET_SHIFT_BITS,
			u16_val, u16_Events);
...
void eMMC_FCIE_DumpDebugBus(void)
{
	...
	eMMC_debug(eMMC_DEBUG_LEVEL, 0, "=== FCIE5 DEBUG BUS ===\n");
	eMMC_debug(eMMC_DEBUG_LEVEL, 0, "FCIE[0x15] = %04Xh\n", REG_FCIE(FCIE_TEST_MODE));
	eMMC_debug(eMMC_DEBUG_LEVEL, 0, "FCIE[0x39] = %04Xh\n", REG_FCIE(FCIE_EMMC_DEBUG_BUS1));
	eMMC_debug(eMMC_DEBUG_LEVEL, 0, "FCIE[0x38] = %04Xh\n", REG_FCIE(FCIE_EMMC_DEBUG_BUS0));
	eMMC_debug(eMMC_DEBUG_LEVEL, 0, "=======================\n");
...
U32 eMMC_FCIE_Reset(void)
{
    ...
    eMMC_clock_setting(BIT_FCIE_CLK_48M); // speed up FCIE reset done
	...
	REG_FCIE_CLRBIT(FCIE_MIE_FUNC_CTL, BIT_FUNC_MASK);
	REG_FCIE_SETBIT(FCIE_MIE_FUNC_CTL, BIT_EMMC_EN);
	...
	{	// check status
		...
		if(u16_reg & BIT_SD_RSP_TIMEOUT)
		{
			if(u8_retry_fcie < eMMC_CMD_API_ERR_RETRY_CNT)
			{
				u8_retry_fcie++;
				eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1,
					"eMMC WARN: CMD1 SD_STS: %04Xh, Retry: %u\n", u16_reg, u8_retry_fcie);
				eMMC_FCIE_ErrHandler_Retry();
				goto LABEL_SEND_CMD;
			}
			...
			u32_err = eMMC_ST_ERR_CMD1;
			eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1,
				"eMMC Err: CMD1 SD_STS: %04Xh, Retry: %u\n", u16_reg, u8_retry_fcie);
			eMMC_FCIE_ErrHandler_Stop();
		}
		else
		{	// CMD1 ok, do things here
...
U32 eMMC_CMD8_MIU(U8 *pu8_DataBuf)
{
	...
	u32_DataMapAddr = eMMC_DMA_MAP_address((uintptr_t)pu8_DataBuf, eMMC_SECTOR_512BYTE,1);
...
//	REG_FCIE_W(FCIE_CIFD_WORD_CNT, 0);
	...
	if(IF_TEST_RETRY(31) || eMMC_ST_SUCCESS != u32_err)
	{
	    ...
	    {   // read data
		    u32_err = eMMC_WaitGetCIFD((U8*)((uintptr_t)pu8_DataBuf + (u16_i << 6)), 0x40);
		    if(u32_err != eMMC_ST_SUCCESS)
		    {
			    eMMC_FCIE_ErrHandler_Stop();
			    goto LABEL_END;
...
U32 eMMC_CMD18_MIU(U32 u32_eMMCBlkAddr, U8 *pu8_DataBuf, U16 u16_BlkCnt)
{
	...
	u32_DataMapAddr = eMMC_DMA_MAP_address((uintptr_t)pu8_DataBuf,eMMC_SECTOR_512BYTE*u16_BlkCnt,1);
    ...
    #endif
	u32_err = eMMC_FCIE_WaitEvents(FCIE_MIE_EVENT,
	    BIT_DMA_END, TIME_WAIT_n_BLK_END*(1+(u16_BlkCnt>>11)));
    #else
    u32_err = eMMC_FCIE_PollingEvents(FCIE_MIE_EVENT,
	    BIT_DMA_END, TIME_WAIT_n_BLK_END*(1+(u16_BlkCnt>>11)));
    #endif
...
U32 eMMC_CMD25_MIU(U32 u32_eMMCBlkAddr, U8 *pu8_DataBuf, U16 u16_BlkCnt)
{
	...
	eMMC_FCIE_ClearEvents();
	REG_FCIE_W(FCIE_JOB_BL_CNT, u16_BlkCnt);
    u32_DataMapAddr = eMMC_DMA_MAP_address((uintptr_t)pu8_DataBuf, eMMC_SECTOR_512BYTE*u16_BlkCnt,0);
    u32_DataDMAAddr = eMMC_translate_DMA_address_Ex(u32_DataMapAddr, eMMC_SECTOR_512BYTE*u16_BlkCnt);
...
U32 eMMC_CMD24_MIU(U32 u32_eMMCBlkAddr, U8 *pu8_DataBuf)
{
	...
	eMMC_FCIE_ClearEvents();
	REG_FCIE_W(FCIE_JOB_BL_CNT, 1);
    u32_DataMapAddr = eMMC_DMA_MAP_address((uintptr_t)pu8_DataBuf, eMMC_SECTOR_512BYTE,0);
	u32_DataDMAAddr = eMMC_translate_DMA_address_Ex(u32_DataMapAddr, eMMC_SECTOR_512BYTE);
	...
	if(IF_TEST_RETRY(11) || eMMC_ST_SUCCESS != u32_err || (u16_reg & (BIT_SD_W_FAIL|BIT_SD_W_CRC_ERR)))
	{
		if(u8_retry_fcie < eMMC_CMD_API_ERR_RETRY_CNT && 0==eMMC_IF_TUNING_TTABLE())
		{
			u8_retry_fcie++;
			eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1,
				"eMMC Warn: CMD24 %s SD_STS: %04Xh, Err: %Xh, Retry: %u, Arg: %Xh\n",
		...
		eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1,
			"eMMC Err: CMD24 %s SD_STS: %04Xh, Err: %Xh, Retry: %u, Arg: %Xh\n",
			(eMMC_ST_SUCCESS!=u32_err)?"TO ":"", u16_reg, u32_err, u8_retry_fcie, u32_arg);
		eMMC_FCIE_ErrHandler_Stop();
		goto LABEL_END;
	}
...
U32 eMMC_CMD24_CIFD(U32 u32_eMMCBlkAddr, U8 *pu8_DataBuf)
{
	...
	for (u16_i=0; u16_i< (eMMC_SECTOR_512BYTE >> 6); u16_i++)
	{
		u32_err = eMMC_WaitSetCIFD( (U8*)( (uintptr_t) pu8_DataBuf + (u16_i << 6)), 0x40);
		if(u32_err != eMMC_ST_SUCCESS)
		{
			eMMC_FCIE_ErrHandler_Stop();
			goto LABEL_END;
	...
	if(IF_TEST_RETRY(23) || eMMC_ST_SUCCESS != u32_err || (u16_reg & (BIT_SD_W_FAIL|BIT_SD_W_CRC_ERR)))
	{
		if(u8_retry_fcie < eMMC_CMD_API_ERR_RETRY_CNT && 0==eMMC_IF_TUNING_TTABLE())
		{
			u8_retry_fcie++;
			eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1,
				"eMMC WARN: CMD24 TO SD_STS: %04Xh, Err: %Xh, Retry: %u, Arg: %Xh\n", u16_reg, u32_err, u8_retry_fcie, u32_arg);
			eMMC_FCIE_ErrHandler_ReInit();
			eMMC_FCIE_ErrHandler_Retry();
			goto LABEL_SEND_CMD;
		}
