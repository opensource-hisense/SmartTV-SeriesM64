...
#ifdef CONFIG_COMPAT        
static long compat_fusion_ioctl(struct file *filp, unsigned int cmd, unsigned long arg);
static int compat_lounge_ioctl(struct file *filp, unsigned int cmd, unsigned long arg);
static int compat_messaging_ioctl(struct file *filp, unsigned int cmd, unsigned long arg);
static int compat_get_fusion_send_message_allocation_data(CompatFusionSendMessage __user *data32,FusionSendMessage __user *data);
static int compat_call_ioctl(struct file *filp, unsigned int cmd, unsigned long arg);
static int compat_get_fusion_call_new_allocation_data(CompatFusionCallNew __user *data32,FusionCallNew __user *data);
static int compat_put_fusion_call_new_allocation_data(CompatFusionCallNew __user *data32,FusionCallNew __user *data);
static int compat_get_fusion_call_execute_allocation_data(CompatFusionCallExecute __user *data32,FusionCallExecute __user *data);
static int compat_put_fusion_call_execute_allocation_data(CompatFusionCallExecute __user *data32,FusionCallExecute __user *data);
static int compat_ref_ioctl(struct file *filp, unsigned int cmd, unsigned long arg);
static int compat_skirmish_ioctl(struct file *filp, unsigned int cmd, unsigned long arg);
static int compat_property_ioctl(struct file *filp, unsigned int cmd, unsigned long arg);
static int compat_reactor_ioctl(struct file *filp, unsigned int cmd, unsigned long arg);
static int compat_get_fusion_reactor_dispatch_allocation_data(CompatFusionReactorDispatch  __user *data32, FusionReactorDispatch  __user *data);
static int compat_get_fusion_reactor_set_callback_allocation_data(CompatFusionReactorSetCallback  __user *data32, FusionReactorSetCallback  __user *data);
static int compat_shmpool_ioctl(struct file *filp, unsigned int cmd, unsigned long arg);
static int compat_get_fusion_shmpool_new_allocation_data(CompatFusionSHMPoolNew __user *data32,FusionSHMPoolNew  __user *data);
static int compat_get_fusion_shmpool_attach_allocation_data(CompatFusionSHMPoolAttach __user *data32,FusionSHMPoolAttach  __user *data);
static int compat_put_fusion_shmpool_new_allocation_data(CompatFusionSHMPoolNew __user *data32,FusionSHMPoolNew  __user *data);
static int compat_put_fusion_shmpool_attach_allocation_data(CompatFusionSHMPoolAttach __user *data32,FusionSHMPoolAttach  __user *data);
#endif
...
static int fusiondev_init(FusionDev * dev)
{
	...
	if (ret)
		goto error_call;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 23)	
     proc_create_data("stat", 0, dev->proc_dir,
	     ...
	     unsigned int cmd, unsigned long arg)
{
    ...
    case _IOC_NR(FUSION_PLATFORM_GET_INFO):

        env_info.miu0_bus_base = MSTAR_MIU0_BUS_BASE;
        env_info.miu1_bus_base = MSTAR_MIU1_BUS_BASE;

		if (copy_to_user((FusionEnvInfo *) arg, &env_info, sizeof(env_info)))
			return -EFAULT;
...
static int compat_get_fusion_shmpool_attach_allocation_data(
			CompatFusionSHMPoolAttach __user *data32,
			FusionSHMPoolAttach  __user *data)
{
      int err;
      compat_int_t  s32;
      compat_uptr_t compat_ptr;
	err = get_user(s32, &data32->pool_id);
	err |= put_user(s32, &data->pool_id);
	err |= get_user(s32, &data32->size);
	err |= put_user(s32, &data->size);
	err |= get_user(compat_ptr, &data32->addr_base);
	err |= put_user(compat_ptr, &data->addr_base);    

	return err;  
}

static int compat_put_fusion_shmpool_attach_allocation_data(
			CompatFusionSHMPoolAttach __user *data32,
			FusionSHMPoolAttach  __user *data)
{
      int err;
      compat_int_t  s32;
      compat_uptr_t compat_ptr;
	err = get_user(s32, &data->pool_id);
	err |= put_user(s32, &data32->pool_id);
	err |= get_user(s32, &data->size);
	err |= put_user(s32, &data32->size);
	err |= get_user(compat_ptr, &data->addr_base);
	err |= put_user(compat_ptr, &data32->addr_base);    

	return err;  
}
static int compat_get_fusion_shmpool_new_allocation_data(
			CompatFusionSHMPoolNew __user *data32,
			FusionSHMPoolNew  __user *data)
{
      int err;
      compat_int_t  s32;
      compat_uptr_t compat_ptr;
	err = get_user(s32, &data32->max_size);
	err |= put_user(s32, &data->max_size);
	err |= get_user(s32, &data32->pool_id);
	err |= put_user(s32, &data->pool_id);
	err |= get_user(compat_ptr, &data32->addr_base);
	err |= put_user(compat_ptr, &data->addr_base);    
...
static int compat_put_fusion_shmpool_new_allocation_data(
			CompatFusionSHMPoolNew __user *data32,
			FusionSHMPoolNew  __user *data)
{
      int err;
      compat_int_t  s32;
      compat_uptr_t compat_ptr;
	err = get_user(s32, &data->max_size);
	err |= put_user(s32, &data32->max_size);
	err |= get_user(s32, &data->pool_id);
	err |= put_user(s32, &data32->pool_id);
	err |= get_user(compat_ptr, &data->addr_base);
	err |= put_user(compat_ptr, &data32->addr_base);    
...
static int
compat_shmpool_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	int err,ret;
	
	switch (_IOC_NR(cmd)) 
	{
		case _IOC_NR(COMPAT_FUSION_SHMPOOL_NEW):
             {
			      CompatFusionSHMPoolNew __user *data32;
		            FusionSHMPoolNew __user *data;
		            ...
		            err = compat_get_fusion_shmpool_new_allocation_data(data32, data);
		            if (err)
		                return err;            
		    		
		    		ret = filp->f_op->unlocked_ioctl(filp, FUSION_SHMPOOL_NEW, (unsigned long)data);
				err = compat_put_fusion_shmpool_new_allocation_data(data32, data);
				return ret ? ret : err;	
		}
		case _IOC_NR(COMPAT_FUSION_SHMPOOL_ATTACH):	
             {
			      CompatFusionSHMPoolAttach __user *data32;
		            FusionSHMPoolAttach __user *data;
		            data32 = compat_ptr(arg);
		            ...
		            err = compat_get_fusion_shmpool_attach_allocation_data(data32, data);
		            if (err)
		                return err;            
		    		
		    		ret = filp->f_op->unlocked_ioctl(filp, FUSION_SHMPOOL_ATTACH, (unsigned long)data);
				err = compat_put_fusion_shmpool_attach_allocation_data(data32, data);
				
				return ret ? ret : err;		
		}
		case _IOC_NR(COMPAT_FUSION_SHMPOOL_DETACH):
		case _IOC_NR(COMPAT_FUSION_SHMPOOL_DISPATCH):
		case _IOC_NR(COMPAT_FUSION_SHMPOOL_DESTROY):
...
static int compat_get_fusion_reactor_set_callback_allocation_data(
			CompatFusionReactorSetCallback  __user *data32,
			FusionReactorSetCallback  __user *data)
{
      int err;
      compat_int_t  s32;
      compat_uptr_t compat_ptr;
	err = get_user(s32, &data32->reactor_id);
	err |= put_user(s32, &data->reactor_id);
	err |= get_user(s32, &data32->call_id);
	err |= put_user(s32, &data->call_id);
	err |= get_user(compat_ptr, &data32->call_ptr);
	err |= put_user(compat_ptr, &data->call_ptr);    

	return err;  
}

static int compat_get_fusion_reactor_dispatch_allocation_data(
			CompatFusionReactorDispatch  __user *data32,
			FusionReactorDispatch  __user *data)
{
      int err;
      compat_int_t  s32;
      compat_uptr_t compat_ptr;
	err = get_user(s32, &data32->reactor_id);
	err |= put_user(s32, &data->reactor_id);
	err |= get_user(s32, &data32->channel);
	err |= put_user(s32, &data->channel);
	err |= get_user(s32, &data32->self);
	err |= put_user(s32, &data->self);
	err |= get_user(s32, &data32->msg_size);
	err |= put_user(s32, &data->msg_size);
	err |= get_user(compat_ptr, &data32->msg_data);
	err |= put_user(compat_ptr, &data->msg_data);    
...
static int
compat_reactor_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	int err;
	
	switch (_IOC_NR(cmd)) 
	{
		case _IOC_NR(COMPAT_FUSION_REACTOR_NEW):
		case _IOC_NR(COMPAT_FUSION_REACTOR_ATTACH):	
		case _IOC_NR(COMPAT_FUSION_REACTOR_DETACH):
		case _IOC_NR(COMPAT_FUSION_REACTOR_DESTROY):
		    		return filp->f_op->unlocked_ioctl(filp, cmd, (unsigned long)compat_ptr(arg));	
		
		case _IOC_NR(COMPAT_FUSION_REACTOR_DISPATCH):
		{
		            CompatFusionReactorDispatch __user *data32;
		            FusionReactorDispatch __user *data;
		            data32 = compat_ptr(arg);
		            ...
		            err = compat_get_fusion_reactor_dispatch_allocation_data(data32, data);
		            if (err)
		                return err;            
		    		
		    		return filp->f_op->unlocked_ioctl(filp, FUSION_REACTOR_DISPATCH, (unsigned long)data);
		}						
		case _IOC_NR(COMPAT_FUSION_REACTOR_SET_DISPATCH_CALLBACK):
             {
			      CompatFusionReactorSetCallback __user *data32;
		            FusionReactorSetCallback __user *data;
		            data32 = compat_ptr(arg);
		            ...
		            err = compat_get_fusion_reactor_set_callback_allocation_data(data32, data);
		            if (err)
		                return err;            
		    		
		    		return filp->f_op->unlocked_ioctl(filp, FUSION_REACTOR_SET_DISPATCH_CALLBACK, (unsigned long)data);
		}	
	}
...
static int
compat_property_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	switch (_IOC_NR(cmd)) 
	{
                 case _IOC_NR(COMPAT_FUSION_PROPERTY_NEW):
		    case _IOC_NR(COMPAT_FUSION_PROPERTY_LEASE):
		    case _IOC_NR(COMPAT_FUSION_PROPERTY_PURCHASE):
		    case _IOC_NR(COMPAT_FUSION_PROPERTY_CEDE):
		    case _IOC_NR(COMPAT_FUSION_PROPERTY_HOLDUP):
		    case _IOC_NR(COMPAT_FUSION_PROPERTY_DESTROY):
		    		return filp->f_op->unlocked_ioctl(filp, cmd, (unsigned long)compat_ptr(arg));	
	}
...
static int
compat_skirmish_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	switch (_IOC_NR(cmd)) 
	{
                case _IOC_NR(COMPAT_FUSION_SKIRMISH_NEW):
		   case _IOC_NR(COMPAT_FUSION_SKIRMISH_PREVAIL):
		   case _IOC_NR(COMPAT_FUSION_SKIRMISH_SWOOP):
		   case _IOC_NR(COMPAT_FUSION_SKIRMISH_DISMISS):
		   case _IOC_NR(COMPAT_FUSION_SKIRMISH_DESTROY):
		   case _IOC_NR(COMPAT_FUSION_SKIRMISH_LOCK_COUNT):
		   case _IOC_NR(COMPAT_FUSION_SKIRMISH_WAIT):
		   case _IOC_NR(COMPAT_FUSION_SKIRMISH_NOTIFY):
		    		return filp->f_op->unlocked_ioctl(filp, cmd, (unsigned long)compat_ptr(arg));	
	}
...
static int
compat_ref_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	switch (_IOC_NR(cmd)) 
	{
		case _IOC_NR(COMPAT_FUSION_REF_NEW):
		case _IOC_NR(COMPAT_FUSION_REF_UP):
		case _IOC_NR(COMPAT_FUSION_REF_UP_GLOBAL):
		case _IOC_NR(COMPAT_FUSION_REF_DOWN):
		case _IOC_NR(COMPAT_FUSION_REF_DOWN_GLOBAL):
		case _IOC_NR(COMPAT_FUSION_REF_ZERO_LOCK):
		case _IOC_NR(COMPAT_FUSION_REF_ZERO_TRYLOCK):
		case _IOC_NR(COMPAT_FUSION_REF_UNLOCK):
		case _IOC_NR(COMPAT_FUSION_REF_STAT):
		case _IOC_NR(COMPAT_FUSION_REF_WATCH):
		case _IOC_NR(COMPAT_FUSION_REF_INHERIT):
		case _IOC_NR(COMPAT_FUSION_REF_DESTROY):
	    		return filp->f_op->unlocked_ioctl(filp, cmd, (unsigned long)compat_ptr(arg));	
	}
...
static int compat_put_fusion_call_execute_allocation_data(
			CompatFusionCallExecute __user *data32,
			FusionCallExecute __user *data)
{
      int err;
      compat_int_t  s32;
	err = get_user(s32, &data->ret_val);
	err |= put_user(s32, &data32->ret_val);

	return err;
}
static int compat_get_fusion_call_execute_allocation_data(
			CompatFusionCallExecute __user *data32,
			FusionCallExecute __user *data)
{  
      int err;
      compat_int_t  s32;
      compat_uptr_t compat_ptr;
	err = get_user(s32, &data32->ret_val);
	err |= put_user(s32, &data->ret_val);
	err |= get_user(s32, &data32->call_id);
	err |= put_user(s32, &data->call_id);    
	err |= get_user(s32, &data32->call_arg);
	err |= put_user(s32, &data->call_arg);   
	err |= get_user(compat_ptr, &data32->call_ptr);
	err |= put_user(compat_ptr, &data->call_ptr);   
	err |= get_user(s32,&data32->flags);
	err |= put_user(s32,&data->flags);  
	
	return err;  
}


static int compat_put_fusion_call_new_allocation_data(
			CompatFusionCallNew __user *data32,
			FusionCallNew __user *data)
{
      int err;
      compat_int_t  s32;
	err = get_user(s32, &data->call_id);
	err |= put_user(s32, &data32->call_id);

	return err;
    
}
static int compat_get_fusion_call_new_allocation_data(
			CompatFusionCallNew __user *data32,
			FusionCallNew __user *data)
{
      int err;
	...
	err = get_user(s32, &data32->call_id);
	err |= put_user(s32, &data->call_id);
	err |= get_user(compat_ptr, &data32->handler);
	err |= put_user(compat_ptr, &data->handler);    
	err |= get_user(compat_ptr, &data32->ctx);
	err |= put_user(compat_ptr, &data->ctx);   
...
static int
compat_call_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	int ret,err;
	
	switch (_IOC_NR(cmd)) 
	{
		case _IOC_NR(COMPAT_FUSION_CALL_NEW):
		{
	            CompatFusionCallNew __user *data32;
	            FusionCallNew __user *data;
	            data32 = compat_ptr(arg);
	            ...
	            err = compat_get_fusion_call_new_allocation_data(data32, data);
	            if (err)
	                return err;            
	    		
	    		ret = filp->f_op->unlocked_ioctl(filp, FUSION_CALL_NEW, (unsigned long)data);
                    err = compat_put_fusion_call_new_allocation_data(data32, data);
							
	            return ret ? ret : err;
		}		
		case _IOC_NR(COMPAT_FUSION_CALL_EXECUTE):
		{
	            CompatFusionCallExecute __user *data32;
	            FusionCallExecute __user *data;
	            data32 = compat_ptr(arg);
	            ...
	            err = compat_get_fusion_call_execute_allocation_data(data32, data);
	            if (err)
	                return err;            
	    		
	    		ret = filp->f_op->unlocked_ioctl(filp, FUSION_CALL_EXECUTE, (unsigned long)data);
				
                   return compat_put_fusion_call_execute_allocation_data(data32, data);
		}					
		case _IOC_NR(COMPAT_FUSION_CALL_RETURN):  			
		case _IOC_NR(COMPAT_FUSION_CALL_DESTROY):    
...
static int compat_get_fusion_send_message_allocation_data(
			CompatFusionSendMessage __user *data32,
			FusionSendMessage __user *data)
{
      int err;
      compat_ulong_t u32;
      compat_int_t  s32;
      compat_uptr_t compat_ptr;
	err = get_user(u32, &data32->fusion_id);
	err |= put_user(u32, &data->fusion_id);  
	err |= get_user(s32, &data32->msg_id);
	err |= put_user(s32, &data->msg_id);
	err |= get_user(s32, &data32->msg_channel);
	err |= put_user(s32, &data->msg_channel);
	err |= get_user(s32, &data32->msg_size);
	err |= put_user(s32, &data->msg_size);
	err |= get_user(compat_ptr, &data32->msg_data);
	err |= put_user(compat_ptr, &data->msg_data);    
...
static int
compat_messaging_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
       int err;
	   
	switch (_IOC_NR(cmd)) 
	{
		case _IOC_NR(COMPAT_FUSION_SEND_MESSAGE):
		{
	            CompatFusionSendMessage __user *data32;
	            FusionSendMessage __user *data;
	            data32 = compat_ptr(arg);
	            ...
	            err = compat_get_fusion_send_message_allocation_data(data32, data);
	            if (err)
	                return err;            
	    		
	    		return filp->f_op->unlocked_ioctl(filp, FUSION_SEND_MESSAGE, (unsigned long)data);
		}		
	}
...
static int compat_get_fusion_fork_allocation_data(
			CompatFusionFork __user *data32,
			FusionFork __user *data)
{
      int err;
      compat_ulong_t u32;
	err = get_user(u32, &data32->fusion_id);
...
static int compat_put_fusion_fork_allocation_data(
			CompatFusionFork __user *data32,
			FusionFork __user *data)
{
      int err;
      compat_ulong_t u32;
	err = get_user(u32, &data->fusion_id);
	err |= put_user(u32, &data32->fusion_id);
	
	return err;  
    
}

static int compat_get_fusion_kill_allocation_data(
			CompatFusionKill __user *data32,
			FusionKill __user *data)
{
      int err;
      compat_int_t  s32;
      compat_ulong_t u32;
	err = get_user(s32, &data32->signal);
	err |= put_user(s32, &data->signal);
	err |= get_user(s32, &data32->timeout_ms);
	err |= put_user(s32, &data->timeout_ms);
	err |= get_user(u32, &data32->fusion_id);
	err |= put_user(u32, &data->fusion_id);
...
static int compat_get_fusion_enter_allocation_data(
			CompatFusionEnter __user *data32,
			FusionEnter __user *data)
{
      int err;
      compat_int_t  s32;
      compat_ulong_t u32;
      err = get_user(s32, &data32->api.major); 
      err |= put_user(s32, &data->api.major);
      err |= get_user(s32, &data32->api.minor);
      err |= put_user(s32, &data->api.minor);
      err |= get_user(u32, &data32->fusion_id);
      err |= put_user(u32, &data->fusion_id);
...
static int compat_put_fusion_enter_allocation_data(
			CompatFusionEnter __user *data32,
			FusionEnter __user *data)
{
      int err;
      compat_int_t  s32;
      compat_ulong_t u32;
      err = get_user(s32, &data->api.major);
      err |= put_user(s32, &data32->api.major);
      err |= get_user(s32, &data->api.minor);
      err |= put_user(s32, &data32->api.minor);
      err |= get_user(u32, &data->fusion_id);
      err |= put_user(u32, &data32->fusion_id);
...
static int
compat_lounge_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
      int err,ret;
	  
	switch (_IOC_NR(cmd)) {
		case _IOC_NR(COMPAT_FUSION_ENTER):
		{
		      CompatFusionEnter __user *data32;
	            FusionEnter __user *data;
	            data32 = compat_ptr(arg);
	            ...
	            err = compat_get_fusion_enter_allocation_data(data32, data);
	            if (err)
	                return err;            
	    		
	    	      ret = filp->f_op->unlocked_ioctl(filp, FUSION_ENTER, (unsigned long)data);					
	            err = compat_put_fusion_enter_allocation_data(data32, data);
	            return ret ? ret : err;
		}		

		case _IOC_NR(COMPAT_FUSION_UNBLOCK):	
		      return filp->f_op->unlocked_ioctl(filp, cmd, (unsigned long)compat_ptr(arg));


		case _IOC_NR(COMPAT_FUSION_KILL):
		{
		      CompatFusionKill __user *data32;
	            FusionKill __user *data;
	            data32 = compat_ptr(arg);
	            ...
	            err = compat_get_fusion_kill_allocation_data(data32, data);
	            if (err)
	                return err;            
	    		
	    		return filp->f_op->unlocked_ioctl(filp, FUSION_KILL, (unsigned long)data);
		}		

		case _IOC_NR(COMPAT_FUSION_ENTRY_SET_INFO):
			
		      return filp->f_op->unlocked_ioctl(filp, FUSION_ENTRY_SET_INFO, (unsigned long)compat_ptr(arg));

		case _IOC_NR(COMPAT_FUSION_ENTRY_GET_INFO):
			
		      return filp->f_op->unlocked_ioctl(filp, FUSION_ENTRY_GET_INFO, (unsigned long)compat_ptr(arg));

		case _IOC_NR(COMPAT_FUSION_FORK):
		{
	            CompatFusionFork __user *data32;
	            FusionFork __user *data;
	            data32 = compat_ptr(arg);
	            ...
	            err = compat_get_fusion_fork_allocation_data(data32, data);
	            if (err)
	                return err;            
	    		
	    		ret = filp->f_op->unlocked_ioctl(filp, FUSION_FORK, (unsigned long)data);				
	             err = compat_put_fusion_fork_allocation_data(data32, data);
				
	            return ret ? ret : err;
		}
		
		case _IOC_NR(COMPAT_FUSION_PLATFORM_GET_INFO):
				return filp->f_op->unlocked_ioctl(filp, FUSION_PLATFORM_GET_INFO, (unsigned long)compat_ptr(arg));
...
static long
compat_fusion_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{    
	...
	{
	    case FT_LOUNGE:
			return compat_lounge_ioctl(filp, cmd, arg);
	    case FT_MESSAGING:
			return compat_messaging_ioctl(filp, cmd, arg);
	    case FT_CALL:
			return compat_call_ioctl(filp, cmd, arg);
	    case FT_REF:
			return compat_ref_ioctl(filp, cmd, arg);
	    case FT_SKIRMISH:
			return compat_skirmish_ioctl(filp, cmd, arg);
	    case FT_PROPERTY:
			return compat_property_ioctl(filp, cmd, arg);
	    case FT_REACTOR:
			return compat_reactor_ioctl(filp, cmd, arg);
	    case FT_SHMPOOL:
			return compat_shmpool_ioctl(filp, cmd, arg);
	}	
	  return 	 -ENOSYS;
...
#endif
#ifdef CONFIG_COMPAT        
        .compat_ioctl =         compat_fusion_ioctl,
#endif
	.mmap = fusion_mmap
};
