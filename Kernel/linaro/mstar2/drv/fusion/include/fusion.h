...
#ifdef CONFIG_COMPAT
typedef compat_ulong_t CompatFusionID;
#endif
...
typedef struct {
     struct {
          compat_int_t            major;         /* Must be set to FUSION_API_MAJOR before entering. */
          compat_int_t            minor;         /* Must be set to FUSION_API_MINOR before entering. */
     } api;

     CompatFusionID            fusion_id;     /* Returns the fusion id of the entering process. */
} CompatFusionEnter;
#endif
...
typedef struct {
    unsigned long long miu0_bus_base;
    unsigned long long miu1_bus_base;
} FusionEnvInfo;


#ifdef CONFIG_COMPAT
typedef struct {
     CompatFusionID            fusion_id;     /* Returns the fusion id of the new (forked) fusionee. */
} CompatFusionFork;
#endif
...
#ifdef CONFIG_COMPAT
typedef struct {
     CompatFusionID            fusion_id;     /* recipient */

     compat_int_t                 msg_id;        /* optional message identifier */
     compat_int_t                 msg_channel;   /* optional channel number */
     compat_int_t                 msg_size;      /* message size, must be greater than zero */
     const compat_uptr_t        msg_data;      /* message data, must not be NULL */
} CompatFusionSendMessage;
#endif
...
#ifdef CONFIG_COMPAT
typedef struct {
     compat_int_t                 reactor_id;    /* id of target reactor */
     compat_int_t                 channel;       /* optional reactor channel (0-1023) */
     compat_int_t                 self;          /* send to ourself if attached */

     compat_int_t                 msg_size;      /* message size, must be greater than zero */
     const compat_uptr_t         msg_data;      /* message data, must not be NULL */
} CompatFusionReactorDispatch;
#endif
...
#ifdef CONFIG_COMPAT
typedef struct {
     compat_int_t                 reactor_id;

     compat_int_t                 call_id;       /* id of the call to execute when a message has been
                                           processed by all recipients of the dispatch */
     compat_uptr_t                call_ptr;      /* optional call parameter, e.g. the pointer of a user
                                           space resource associated with that reference */
} CompatFusionReactorSetCallback;
#endif
...
#ifdef CONFIG_COMPAT
typedef struct {
     compat_int_t                 call_id;       /* new call id returned */

     compat_uptr_t               handler;       /* function pointer of handler to install */
     compat_uptr_t               ctx;           /* optional handler context */
} CompatFusionCallNew;
#endif
...
#ifdef CONFIG_COMPAT
typedef struct {
     compat_int_t                 ret_val;       /* return value of the call */

     compat_int_t                 call_id;       /* id of the requested call, each call has a fixed owner */

     compat_int_t                 call_arg;      /* optional int argument */
     compat_uptr_t               call_ptr;      /* optional pointer argument (shared memory) */

     FusionCallExecFlags flags;         /* execution flags */
} CompatFusionCallExecute;
#endif
...
#ifdef CONFIG_COMPAT
typedef struct {
     compat_uptr_t               handler;       /* function pointer of handler to call */
     compat_uptr_t               ctx;           /* optional handler context */

     compat_int_t                 caller;        /* fusion id of the caller or zero if called from Fusion */
     compat_int_t                 call_arg;      /* optional call parameter */
     compat_uptr_t               call_ptr;      /* optional call parameter */

     compat_uint_t        serial;
} CompatFusionCallMessage;
#endif
...
#ifdef CONFIG_COMPAT
typedef struct {
     CompatFusionID            fusion_id;     /* fusionee to kill, zero means all but ourself */
     compat_int_t                 signal;        /* signal to be delivered, e.g. SIGTERM */
     compat_int_t                 timeout_ms;    /* -1 means no timeout, 0 means infinite, otherwise the
                                           max. time to wait until the fusionee(s) terminated */
} CompatFusionKill;
#endif
...
#ifdef CONFIG_COMPAT
typedef struct {
     compat_int_t                 max_size;      /* Maximum size that this pool will be allowed to grow to. */

     compat_int_t                 pool_id;       /* Returns the new pool id. */
     compat_uptr_t               addr_base;     /* Returns the base of the reserved virtual memory address space. */
} CompatFusionSHMPoolNew;
#endif
...
#ifdef CONFIG_COMPAT
typedef struct {
     compat_int_t                 pool_id;       /* The id of the pool to attach to. */

     compat_uptr_t               addr_base;     /* Returns the base of the reserved virtual memory address space. */
     compat_int_t                 size;          /* Returns the current size of the pool. */
} CompatFusionSHMPoolAttach;
#endif
...
#ifdef CONFIG_COMPAT
#define COMPAT_FUSION_ENTER                         _IOR(FT_LOUNGE,    0x00, CompatFusionEnter)
#define COMPAT_FUSION_UNBLOCK                       _IO (FT_LOUNGE,    0x01)
#define COMPAT_FUSION_KILL                          _IOW(FT_LOUNGE,    0x02, CompatFusionKill)

#define COMPAT_FUSION_ENTRY_SET_INFO                _IOW(FT_LOUNGE,    0x03, FusionEntryInfo)
#define COMPAT_FUSION_ENTRY_GET_INFO                _IOW(FT_LOUNGE,    0x04, FusionEntryInfo)

#define COMPAT_FUSION_FORK                          _IOW(FT_LOUNGE,    0x05, CompatFusionFork)

#define COMPAT_FUSION_PLATFORM_GET_INFO             _IOR(FT_LOUNGE,    0x06, FusionEnvInfo)

#define COMPAT_FUSION_SEND_MESSAGE                  _IOW(FT_MESSAGING, 0x00, CompatFusionSendMessage)

#define COMPAT_FUSION_CALL_NEW                      _IOW(FT_CALL,      0x00, CompatFusionCallNew)
#define COMPAT_FUSION_CALL_EXECUTE                  _IOW(FT_CALL,      0x01, CompatFusionCallExecute)
#define COMPAT_FUSION_CALL_RETURN                   _IOW(FT_CALL,      0x02, FusionCallReturn)
#define COMPAT_FUSION_CALL_DESTROY                  _IOW(FT_CALL,      0x03, int)

#define COMPAT_FUSION_REF_NEW                       _IOW(FT_REF,       0x00, int)
#define COMPAT_FUSION_REF_UP                        _IOW(FT_REF,       0x01, int)
#define COMPAT_FUSION_REF_UP_GLOBAL                 _IOW(FT_REF,       0x02, int)
#define COMPAT_FUSION_REF_DOWN                      _IOW(FT_REF,       0x03, int)
#define COMPAT_FUSION_REF_DOWN_GLOBAL               _IOW(FT_REF,       0x04, int)
#define COMPAT_FUSION_REF_ZERO_LOCK                 _IOW(FT_REF,       0x05, int)
#define COMPAT_FUSION_REF_ZERO_TRYLOCK              _IOW(FT_REF,       0x06, int)
#define COMPAT_FUSION_REF_UNLOCK                    _IOW(FT_REF,       0x07, int)
#define COMPAT_FUSION_REF_STAT                      _IOW(FT_REF,       0x08, int)
#define COMPAT_FUSION_REF_WATCH                     _IOW(FT_REF,       0x09, FusionRefWatch)
#define COMPAT_FUSION_REF_INHERIT                   _IOW(FT_REF,       0x0A, FusionRefInherit)
#define COMPAT_FUSION_REF_DESTROY                   _IOW(FT_REF,       0x0B, int)

#define COMPAT_FUSION_SKIRMISH_NEW                  _IOW(FT_SKIRMISH,  0x00, int)
#define COMPAT_FUSION_SKIRMISH_PREVAIL              _IOW(FT_SKIRMISH,  0x01, int)
#define COMPAT_FUSION_SKIRMISH_SWOOP                _IOW(FT_SKIRMISH,  0x02, int)
#define COMPAT_FUSION_SKIRMISH_DISMISS              _IOW(FT_SKIRMISH,  0x03, int)
#define COMPAT_FUSION_SKIRMISH_DESTROY              _IOW(FT_SKIRMISH,  0x04, int)
#define COMPAT_FUSION_SKIRMISH_LOCK_COUNT           _IOW(FT_SKIRMISH,  0x05, int)
#define COMPAT_FUSION_SKIRMISH_WAIT                 _IOW(FT_SKIRMISH,  0x06, FusionSkirmishWait)
#define COMPAT_FUSION_SKIRMISH_NOTIFY               _IOW(FT_SKIRMISH,  0x07, int)

#define COMPAT_FUSION_PROPERTY_NEW                  _IOW(FT_PROPERTY,  0x00, int)
#define COMPAT_FUSION_PROPERTY_LEASE                _IOW(FT_PROPERTY,  0x01, int)
#define COMPAT_FUSION_PROPERTY_PURCHASE             _IOW(FT_PROPERTY,  0x02, int)
#define COMPAT_FUSION_PROPERTY_CEDE                 _IOW(FT_PROPERTY,  0x03, int)
#define COMPAT_FUSION_PROPERTY_HOLDUP               _IOW(FT_PROPERTY,  0x04, int)
#define COMPAT_FUSION_PROPERTY_DESTROY              _IOW(FT_PROPERTY,  0x05, int)

#define COMPAT_FUSION_REACTOR_NEW                   _IOW(FT_REACTOR,   0x00, int)
#define COMPAT_FUSION_REACTOR_ATTACH                _IOW(FT_REACTOR,   0x01, FusionReactorAttach)
#define COMPAT_FUSION_REACTOR_DETACH                _IOW(FT_REACTOR,   0x02, FusionReactorDetach)
#define COMPAT_FUSION_REACTOR_DISPATCH              _IOW(FT_REACTOR,   0x03, CompatFusionReactorDispatch)
#define COMPAT_FUSION_REACTOR_DESTROY               _IOW(FT_REACTOR,   0x04, int)
#define COMPAT_FUSION_REACTOR_SET_DISPATCH_CALLBACK _IOW(FT_REACTOR,   0x05, CompatFusionReactorSetCallback)

#define COMPAT_FUSION_SHMPOOL_NEW                   _IOW(FT_SHMPOOL,   0x00, CompatFusionSHMPoolNew)
#define COMPAT_FUSION_SHMPOOL_ATTACH                _IOW(FT_SHMPOOL,   0x01, CompatFusionSHMPoolAttach)
#define COMPAT_FUSION_SHMPOOL_DETACH                _IOW(FT_SHMPOOL,   0x02, int)
#define COMPAT_FUSION_SHMPOOL_DISPATCH              _IOW(FT_SHMPOOL,   0x03, FusionSHMPoolDispatch)
#define COMPAT_FUSION_SHMPOOL_DESTROY               _IOW(FT_SHMPOOL,   0x04, int)
#endif
...
#define FUSION_PLATFORM_GET_INFO             _IOR(FT_LOUNGE,    0X06, FusionEnvInfo)
