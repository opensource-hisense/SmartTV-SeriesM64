...
fusionees_proc_show(struct seq_file *m, void *v)
{
	...
	fusion_list_foreach(l, dev->fusionee.list) {
		Fusionee *fusionee = (Fusionee *) l;
               seq_printf(m,
                       "(%5d) 0x%08lx (%4d packets waiting, %7ld received, %7ld sent) - wcq 0x%x\n",
                       fusionee->pid, fusionee->id,
                       fusionee->messages.count, atomic_long_read(&fusionee->rcv_total),
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 23)	
     proc_create_data("fusionees", 0, dev->proc_dir,
		      ...
		      Fusionee * fusionee, void *buf, int buf_size, bool block)
{
	...
	while (fusionee->messages.count) {
		...
		if(is_compat_task())
		{
			if(message->type == FMT_CALL)
			{
				message->size = sizeof(CompatFusionCallMessage);
				bytes = ((message->size + 3) & ~3) + sizeof(header);
		...
		header.msg_size		= message->size;
             ...
             #ifdef CONFIG_COMPAT
		CompatFusionCallMessage cfcm;	 
		if(is_compat_task())
		{	
			FusionCallMessage *fcm; 
			if(message->type == FMT_CALL )
			{
			      fcm = message->data;
				cfcm.handler = (compat_uptr_t)fcm->handler;
				cfcm.ctx = (compat_uptr_t)fcm->ctx;
				cfcm.caller = fcm->caller;
				cfcm.call_arg = fcm->call_arg;
				cfcm.call_ptr = (compat_uptr_t)fcm->call_ptr;
				cfcm.serial = fcm->serial;
				message->data = &cfcm;
		      }
		}
             #endif
