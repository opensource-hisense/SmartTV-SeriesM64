...
int _mdrv_alsa_hook_device(struct MStar_MAD_Info *mad_info)
{
	...
	if ((mad_info == NULL) || (madchip == NULL) || (card == NULL)) {
		MAD_PRINT(KERN_ERR "Error! invalid argument!\n");
		return -EINVAL;
	}
...
static void _mdrv_alsa_playback_monitor(unsigned long device_no)
{
	...
	if (_mdrv_alsa_timer_update(&madchip_runtime->timer, device_no, msecs_to_jiffies(time_interval)) != 0) {
...
static int _mdrv_alsa_system_suspend(struct platform_device *dev, pm_message_t state)
{
	...
	for (device_no = 0; device_no < MAD_MAX_DEVICES; device_no++) {
		...
		if (madchip->pcm_playback[device_no].ops.stop) {
			madchip->pcm_playback[device_no].ops.stop();
		}
	...
	for (device_no = 0; device_no < MAD_MAX_DEVICES; device_no++) {
		...
		if (madchip->pcm_capture[device_no].ops.stop) {
			madchip->pcm_capture[device_no].ops.stop();
		}
...
static int _mdrv_alsa_system_resume(struct platform_device *dev)
{
	...
	for (device_no = 0; device_no < MAD_MAX_DEVICES; device_no++) {
		...
		if (madchip->pcm_playback[device_no].ops.get) {
			madchip->pcm_playback[device_no].ops.get(E_PCM_PLAYBACK_GET_DEVICE_STATUS, &madchip->pcm_playback[device_no].device_status);
		}
		else {
			madchip->pcm_playback[device_no].device_status = MAD_TRUE;
		}

		if (madchip->pcm_playback[device_no].device_status != MAD_TRUE) {
			MAD_PRINT(KERN_ERR "Error! Playback device is not ready yet, try again later!\n");
		}
		else {
			...
			if (madchip->pcm_playback[device_no].ops.set) {
				madchip->pcm_playback[device_no].ops.set(E_PCM_PLAYBACK_SET_SAMPLE_RATE, &madchip->pcm_playback[device_no].sample_rate);
			}

			if (madchip->pcm_playback[device_no].ops.set) {
				madchip->pcm_playback[device_no].ops.set(E_PCM_PLAYBACK_SET_CHANNEL_MODE, &madchip->pcm_playback[device_no].channel_mode);
			}

			if (madchip->pcm_playback[device_no].ops.open) {
				madchip->pcm_playback[device_no].ops.open();
			}

			if (madchip->pcm_playback[device_no].ops.start) {
				madchip->pcm_playback[device_no].ops.start();
			}

			madchip->pcm_playback[device_no].runtime_status = E_START;
		}
	...
	for (device_no = 0; device_no < MAD_MAX_DEVICES; device_no++) {
		...
		if (madchip->pcm_capture[device_no].ops.get) {
			madchip->pcm_capture[device_no].ops.get(E_PCM_CAPTURE_GET_DEVICE_STATUS, &madchip->pcm_capture[device_no].device_status);
		}
		else {
			madchip->pcm_capture[device_no].device_status = MAD_TRUE;
		}

		if (madchip->pcm_capture[device_no].device_status != MAD_TRUE) {
			MAD_PRINT(KERN_ERR "Error! Capture device is not ready yet, try again later!\n");
		}
		else {
			...
			if (madchip->pcm_capture[device_no].ops.open) {
				madchip->pcm_capture[device_no].ops.open();
			}

			if (madchip->pcm_capture[device_no].ops.start) {
				madchip->pcm_capture[device_no].ops.start();
			}

			madchip->pcm_capture[device_no].runtime_status = E_START;
		}
