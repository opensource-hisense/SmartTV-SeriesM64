...
#define FLASH_WAIT_TIME     (DRV_FLASH_MS(100)*0x200UL)
                                          ...
                                          MS_U8 u8OpCfg)
{
    ...
    MS_U16 CpyType = ((BDMA_DEV_FLASH & 0x0FUL) | _LShift((eDstDev &0x0FUL), 8));
...
{
  return  HAL_QPI_Enable( bEnable);
}

MS_BOOL  MDrv_SERFLASH_QPI_REST(MS_BOOL bEnable)
{
  return  HAL_QPI_RESET( bEnable);
}
...
//  WRAPPER FOR CHAKRA
//-------------------------------------------------------------------------------------------------
MS_BOOL MDrv_FLASH_QPI_ENALBE(MS_BOOL bEnable)
{
   return MDrv_SERFLASH_QPI_ENABLE( bEnable);
}

MS_BOOL MDrv_FLASH_QPI_REST(MS_BOOL bEnable)
{
   return MDrv_SERFLASH_QPI_REST( bEnable);
}
