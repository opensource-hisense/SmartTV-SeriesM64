...
void _MDrv_MBX_MsgRecvCb(MS_S32 s32Irq)
{
    ...
    switch(s32Irq)
    {
        ...
        case MBX_INT_PM2MIPS:
            ...
            break;
        default:
            mbxMsg.eRoleID = 2;//_eMbxCpu2Role[E_MBX_CPU_AEON];
            //DRV_MBX_UnLockRecv();
            //return;
    }
...
/*
  ...
  Return Value:
        ...
        MBX_UNKNOW_ERROR-->Other undefined errors
*/
MBX_Result  MDrv_MBX_SendMsg(MS_U32 u32AsyncID, MBX_Msg *pMsg, MS_U8 u8Trigger)
{
    //parameter check:
    if(NULL==pMsg)
    {
    ...
    #ifdef CONFIG_MSTAR_MBX_U2K
    if (pMsg->u8MsgClass < E_MBX_CLASS_MAX)
...
/*
  ...
  Return Value:
        ...
        MBX_UNKNOW_ERROR-->Other undefined errors
*/
MBX_Result  MDrv_MBX_RecvMsg(MS_U32 u32AsyncID, MBX_Class eTargetClass, MBX_Msg *pMsg, MS_U32 u32WaitMillSecs, MS_U32 u32Flag, MS_U8 u8Trigger)
{
    MBX_Result mbxResult = E_MBX_SUCCESS;
    MS_S16 mbxAsyncNotifierID = _MDrv_MBX_GetAsyncNotifierIDByAsyncID(u32AsyncID);
    long ret;
    ...
    DRV_MBX_LockRecv();		
    #ifdef CONFIG_MSTAR_MBX_U2K
         if (eTargetClass < E_MBX_CLASS_MAX)
...
MS_U32 MDrv_MBX_GetAsyncID(void)
{
    MS_U32 u32AMBXAsyncID = 0;
    MS_S16 s16Idx;
