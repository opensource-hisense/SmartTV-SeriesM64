...
#if LED_BLINK
extern int initflag;
extern long repeatkey[];
extern long IRHeadcode0 ;
extern long IRHeadcode1;
	#if CVT_SUPPORT_THIRD_IR
		extern long IR2Headcode0 ;
		extern long IR2Headcode1;
	#endif
#endif
...
int key_dispatch(void *p)
{
    ...
    while(1)
    {
        ...
        {
           //IR_PRINT("xxxxx 4\n");
		   if ((scancode != prev_scancode)||(u8_repeatflag == 0))
           {
               //IR_PRINT("xxxxx 5\n");
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 0, 20)
               rc_keyup(ir->dev);
...
//irqreturn_t _MDrv_IR_ISR(int irq, void *dev_id, struct pt_regs *regs)
{
    ...
    if ((CurrentTime - PreTime > IR_TIMEOUT_CYC/1000))//reset
    {
        ...
        _u8PioneerFirstKey = 0xfaUL;
        _u8PioneerSecondKey = 0xfaUL;
    }
        ...
        if ((bHaveKey=_MDrv_IR_GetKey(&u8Key, &u8System, &u8RepeatFlag)) != 0)
        {
            if(down_trylock(&fantasy_protocol.sem)==0)
            {
                ...
                else
                {
                    ...
                    // //set fantasy class to IR
                    // fantasy_protocol.data |= (0x01UL << 28);
                    U32 CVT_factory_ir=0;
                    fantasy_protocol.data = (u8Key << 8 | u8RepeatFlag);
                    if(user_ir_flag == CVT_FCTORY_HEADER_RECEIVE)
                    // if((u8RepeatFlag&CVT_FCTORY_HEADER_RECEIVE))
                    {
                        //printk("fac  ir repeat = %x\n", u8RepeatFlag);
                        u8RepeatFlag &= (~CVT_FCTORY_HEADER_RECEIVE);
                        CVT_factory_ir=(0x04 << 28);
                        fantasy_protocol.data |= CVT_factory_ir;
                    }
                    #if CVT_SUPPORT_THIRD_IR
                    // else if((u8RepeatFlag & CVT_THIRD_HEAD_RECEIVE))
                    else if(user_ir_flag == CVT_THIRD_HEAD_RECEIVE)
                    {
                        //printk("test ir repeat = %x\n", u8RepeatFlag);
                        u8RepeatFlag &= (~CVT_THIRD_HEAD_RECEIVE);
                        CVT_factory_ir=(0x08 << 28);
                        //printk("test ir CVT_factory_ir = %x\n", CVT_factory_ir);
                        fantasy_protocol.data |= CVT_factory_ir;
                    }
                    #endif
                    else
...
static BOOL _MDrv_IR_GetKey(U8 *pu8Key, U8 *pu8System, U8 *pu8Flag)
{
    ...
    *pu8System = u8IRSwModeBuf[1];//timl
    ...
    if(u8IRSwModeBuf[0] == _u8IR2HeaderCode0)
    {
        if(u8IRSwModeBuf[1] == _u8IR2HeaderCode1)
        {
            if(u8IRSwModeBuf[2] == (u8)~u8IRSwModeBuf[3])
            {
                ...
                *pu8Flag = 0;
                #if CVT_SUPPORT_THIRD_IR
                // *pu8Flag |= CVT_THIRD_HEAD_RECEIVE;
                user_ir_flag = CVT_THIRD_HEAD_RECEIVE;
                #endif
                bRet = TRUE;
                #if (IR_TYPE_SEL == IR_TYPE_HISENSE || IR_TYPE_SEL == IR_TYPE_MSTAR_DTV || IR_TYPE_SEL == IR_TYPE_CHANGHONG)
                _u8PrevKeyCode = *pu8Key;
                ...
                _u8IRRepeateDetect = 1;
                _u8IRHeadReceived = 0;
                _u8IRType = CVT_THIRD_HEAD_RECEIVE;
                #endif
    ...
    if(u8IRSwModeBuf[0] == CVT_DEF_CVTE_FAC_IR_HEAD0)
    {
        if(u8IRSwModeBuf[1] == CVT_DEF_CVTE_FAC_IR_HEAD1)
        {
            if(u8IRSwModeBuf[2] == (u8)~u8IRSwModeBuf[3])
            {
                *pu8Key = u8IRSwModeBuf[2];
                *pu8Flag = 0;
                // *pu8Flag |= CVT_FCTORY_HEADER_RECEIVE;
                user_ir_flag = CVT_FCTORY_HEADER_RECEIVE;
                bRet = TRUE;
                #if (IR_TYPE_SEL == IR_TYPE_HISENSE || IR_TYPE_SEL == IR_TYPE_MSTAR_DTV || IR_TYPE_SEL == IR_TYPE_CHANGHONG)
	                _u8PrevKeyCode = *pu8Key;
	                _u8PrevSystemCode = *pu8System;
	                _u8IRRepeateDetect = 1;
	                _u8IRHeadReceived = 0;
	                _u8IRType = CVT_FCTORY_HEADER_RECEIVE;
                #endif
    ...
    if (bRet && initflag)
    {
        ...
        if(*pu8Flag & 1)
        {
            ...
            for (i=0;i<KEY_NUM;i++)
            {
                ...
                #if 0//CVT_SUPPORT_RC6_AND_NEC_IR
                if(_u8IRType == CVT_FCTORY_HEADER_RECEIVE)
                {
                    if((repeatkey[i]&0x0100)&&((repeatkey[i]&0x00FF) == *pu8Key))
                    {
                        repeatflag = TRUE;
                        break;
                    }
            ...
            //Repeat for Factory IR
            if(_u8IRType == CVT_THIRD_HEAD_RECEIVE)
            {
                for (i=0;i<KEY_NUM;i++)
                {
                    if(((repeatkey[i] & 0xff) == *pu8Key) && (repeatkey[i] & 0x200))
                    {
                        *pu8Flag |= CVT_THIRD_HEAD_RECEIVE;
                         repeatflag = TRUE;
                         break;
...
void MDrv_IR_Init(int bResumeInit)
{
    int result;
#if LED_BLINK
    _u8IRHeaderCode0 = IRHeadcode0;
    _u8IRHeaderCode1 = IRHeadcode1;

    #if CVT_SUPPORT_THIRD_IR
    _u8IR2HeaderCode0 = IR2Headcode0;
    _u8IR2HeaderCode1 = IR2Headcode1;
    #endif
#endif
    ir_irq_depth = 1;
    _u8PrevSystemCode = 0;
