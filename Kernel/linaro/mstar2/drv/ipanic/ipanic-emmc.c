...
static const struct file_operations ipanic_proc_debug_fops = {
	.read		 =  ipanic_proc_debug_read,
	.write 	       =  ipanic_proc_debug_write,
};
...
static int createIpanicProcEntry(void)
{
	...
	proc_entry_parent = proc_mkdir_mode(IPROC_ENTRY_DIR_NAME,0644, NULL);
    ...
    proc_entry_debug = proc_create_data (IPROC_ENTRY_DEBUG_NAME,S_IRUGO,proc_entry_parent,&ipanic_proc_debug_fops,NULL);    
  ...
  if(proc_entry_debug == NULL)
    ...
    proc_entry_upload = proc_create_data (IPROC_ENTRY_UPLOAD_NAME,S_IRUGO,proc_entry_parent,&ipanic_proc_fops,NULL);    
...
{	
	...
	iheader.version = MSTAR_IPANIC_PHDR_VERSION;
      ...
      static char ipanic_kmsg_buffer[MAX_NATIVEINFO];
	kmsg_dump_get_buffer(dumper, true, ipanic_kmsg_buffer,MAX_NATIVEINFO, &iheader.console_length);
	...
	emmc_ipanic_write(ipanic_kmsg_buffer,iheader.console_offset,iheader.console_length, FILEWRITE);
}
