...
#elif (IR_TYPE_SEL==IR_TYPE_XMP1)
#include "IR_MSTAR_XMP1.h"
...
#include "IR_TCL.h"
#elif (defined(IR_TYPE_HWRC5) && ((IR_TYPE_SEL==IR_TYPE_HWRC5)||( IR_TYPE_SEL==IR_TYPE_HWRC5EX)))
#include "IR_MSTAR_HWRC5.h"
#elif (defined(IR_TYPE_PANASONIC) && IR_TYPE_SEL == IR_TYPE_PANASONIC)
#include "IR_PANASONIC_DTV.h"
#elif (defined(IR_TYPE_VESTEL_RC5) && IR_TYPE_SEL==IR_TYPE_VESTEL_RC5)
#include "IR_VESTEL_RC5.h"
#else
#include "IR_MSTAR_DTV.h"
#endif
...
#endif
#define MSTAR_FANTASY_PROTOCOL_SUPPORTED 1
#define SUPPORT_RETURN_KEY_STATES         0

#if SUPPORT_RETURN_KEY_STATES
#ifdef IR_MODE_SEL
#undef IR_MODE_SEL
#endif
#define IR_MODE_SEL             IR_TYPE_SWDECODE_PANASONIC7051_NEC_MODE
#endif
...
#if(IR_TYPE_SEL == IR_TYPE_PANASONIC)
#define IR_RAW_DATA_NUM	        6    //CODE0 CODE1 MAKERPARITY+SYSTEM CATEGORY COMMAND PARITY
#else
...
#if (IR_MODE_SEL == IR_TYPE_SWDECODE_PANASONIC7051_NEC_MODE)
#define IR_SWDECODE_MODE_BUF_LEN        100
static U32  _u32IRData[IR_SWDECODE_MODE_BUF_LEN];
static U32  _u32IRCount=0;
...
static U8   g_u8IrDataBits=0; // for store bit count
static U16  g_wIrPulseCounter=0;
static U8   g_u8IrFail=0;
static U8   g_u8IrPANRepeatCount=0;    // for repeat
static long long  g_llIrData=0;    // for store shift ir data
static U16  g_u16IrReKeyData=0;
static U16  g_u16IrKeyData = 0;
static U32  g_u32Header=0;
static U32  _u32TmpFlag;
#endif
...
#if(IR_MODE_SEL == IR_TYPE_SWDECODE_KON_MODE || IR_MODE_SEL == IR_TYPE_SWDECODE_MODE)// pulse width modulation format: Header(1)+CusterCode(8)+Data(8)+Stop(1)
...
static Ir_Key_Param         oldIrKeyParam =
{
    FALSE, 0xFF, TRUE
};
#endif
...
#if (IR_MODE_SEL == IR_TYPE_SWDECODE_PANASONIC7051_NEC_MODE || defined(SUPPORT_MULTI_PROTOCOL))
 struct
 {
...
#if ((IR_TYPE_SEL != IR_TYPE_CUS08_RC5) && (IR_TYPE_SEL != IR_TYPE_RCMM)  && (IR_TYPE_SEL!= IR_TYPE_XMP1) && (IR_TYPE_SEL != IR_TYPE_VESTEL_RC5) )
static U32  _u32IRData[IR_SWDECODE_MODE_BUF_LEN];
#endif
#else
...
#if (IR_TYPE_SEL == IR_TYPE_PANASONIC)
static U8   _u8IRHeadReceived_Panas=0;
static U8   _u8IRRepeated_Panas=0;
static U8  _u8IRRepeateDetect_panas=0;
#endif
...
#elif (IR_TYPE_SEL == IR_TYPE_CUS08_RC5 || IR_TYPE_SEL == IR_TYPE_VESTEL_RC5)
...
#elif (IR_TYPE_SEL == IR_TYPE_XMP1)
static BOOL _bIsDataPacket = FALSE;
static BOOL _bIsLastDataPacket = FALSE;
static BOOL _bIsLastRegPacket = FALSE;
static U8 _u8DataIndex  = 0;
static U32 _u32RegPacket = 0;
static U32 _u32DataPacket = 0;
static BOOL _bSupportIR = FALSE;
static unsigned long  _ulPrevKeyTime;
//int i_gp = 0;
//U32 Counter[40]={0};
...
#if (defined(IR_TYPE_VESTEL_RC5) && IR_TYPE_SEL==IR_TYPE_VESTEL_RC5)
static U8 _MDrv_IR_VESTEL_ParseKey(U8 u8KeyData);
#else
static U8 __MDrv_IR_ParseKey(U8 u8KeyData);
#endif
#if (IR_MODE_SEL == IR_TYPE_SWDECODE_PANASONIC7051_NEC_MODE)
static BOOL _MDrv_IR_GetKey(U16 *pu16Key, U8 *pu8System, U8 *pu8Flag);
#else
...
static U8 _MDrv_IR_ParseKey(U8 u8KeyData)
{
    ...
    #if (defined(IR_TYPE_VESTEL_RC5) && IR_TYPE_SEL==IR_TYPE_VESTEL_RC5)
        u8RetVal = _MDrv_IR_VESTEL_ParseKey(u8KeyData);
    #else
        u8RetVal = __MDrv_IR_ParseKey(u8KeyData);
    #endif
    return u8RetVal;
}
...
static U8 _MDrv_IR_VESTEL_ParseKey(U8 u8KeyData)
{
    ...
    switch(u8KeyData)
    {
        ...
        case IRKEY_QUICK         : u8RetVal = IRKEY_QUICK;               break;
        ...
        case IRKEY_SUBTITLE        : u8RetVal = IRKEY_SUBTITLE;      	  break;
        case IRKEY_INTERNET        : u8RetVal = IRKEY_INTERNET;      	  break;
        case IRKEY_BACK            : u8RetVal = IRKEY_BACK;               break;
        case IRKEY_LANGUAGE        : u8RetVal = IRKEY_LANGUAGE;      	  break;
        case IRKEY_MYBUTTON        : u8RetVal = IRKEY_MYBUTTON;     	  break;
        case IRKEY_NETFLIX         : u8RetVal = IRKEY_NETFLIX;         	  break;
        case IRKEY_MEDIA           : u8RetVal = IRKEY_MEDIA;           	  break;

	case IRKEY_FAV             : u8RetVal = IRKEY_FAV;           	  break;
	case IRKEY_YOUTUBE         : u8RetVal = IRKEY_YOUTUBE;            break;
	case IRKEY_MYBUTTON2       : u8RetVal = IRKEY_MYBUTTON2;          break;
...
static void _MDrv_IR_Timing(void)
{
    ...
    REG(REG_IR_CKDIV_NUM_KEY_DATA) = IR_CKDIV_NUM+1;   // clock divider for milan chip
    #else
...
int key_dispatch(void *p)
{
    ...
    while(1)
    {
        ...
        else
        {
            ...
            //
            // NOTE:
            // If too small, you will find the repeat function don't work in android. (up immediately)
            //   It will become down->up->down->down.....(not continue down)
            // If too large, continue key pree become long press in android.
...
static void _MDrv_MultiProtocol_Init(void)
{
   ...
   _key_MultiProtocol_timer.expires = jiffies + 0xFFFFFFFF;
...
#if SUPPORT_RETURN_KEY_STATES
inline static void _MDrv_ReleaseKey_RestartTimer(void)
{
    timer_pending(&key_Release_timer);
    mod_timer(&key_Release_timer, jiffies + 150);
}

inline static void _MDrv_ReleaseKey_RestartTimer2(void)
{
    timer_pending(&key_Release_timer);
    mod_timer(&key_Release_timer, jiffies + 10);
}

#if (IR_MODE_SEL == IR_TYPE_SWDECODE_PANASONIC7051_NEC_MODE)
static void _MDrv_ReleaseKey_Cb(unsigned long lparam)
{
    if (down_trylock(&fantasy_protocol.sem) == 0)
    {
        if (fantasy_protocol.data != 0)
        {
            _MDrv_ReleaseKey_RestartTimer2();
        }
        else
        {
            if (oldIrKeyParam.bpress == true)
            {
                //set fantasy class to IR
                if(_u32TmpFlag==1)       //nec
                {
                    fantasy_protocol.data = (oldIrKeyParam.u16KeyCode << 8 | oldIrKeyParam.bRepeat);
                    fantasy_protocol.data |= (g_u32Header << 16);
                }
                else if(_u32TmpFlag==2)  //panasonic
                {
                    fantasy_protocol.data = (oldIrKeyParam.u16KeyCode << 8 | oldIrKeyParam.bRepeat);
                    fantasy_protocol.data |= ((long long)g_u32Header << 24);
                }
                fantasy_protocol.data |= (0x01 << 1); //Release Flag 02
                //printk("fantasy_protocol.data=%llx\n",fantasy_protocol.data);
                //set fantasy class to IR
                oldIrKeyParam.bpress = false;
            }
        }
        up(&fantasy_protocol.sem);
        wake_up_interruptible(&fantasy_protocol.inq);
...
static void _MDrv_ReleaseKey_Cb(unsigned long lparam)
{
    if (down_trylock(&fantasy_protocol.sem) == 0)
    {
        ...
        {
            DEBUG_IR(printk("bpres = [%d] Release Key = 0x%x\n", oldIrKeyParam.bpress, (oldIrKeyParam.u8KeyCode << 8 | oldIrKeyParam.bRepeat)));
            DEBUG_IR(printk("Cb time = %ld \n", _MDrv_IR_GetSystemTime()));
            if (oldIrKeyParam.bpress == true)
            {
                fantasy_protocol.data = (oldIrKeyParam.u8KeyCode << 8 | oldIrKeyParam.bRepeat);
                fantasy_protocol.data |= (0x01 << 1); //Release Flag  02
                fantasy_protocol.data |= (_u16NecHeader << 16);
                oldIrKeyParam.bpress = false;
            }
        }
        up(&fantasy_protocol.sem);
...
#endif
static void _MDrv_ReleaseKey_Init(void)
{
   init_timer(&key_Release_timer);
   key_Release_timer.expires = jiffies + 0xFFFFFFFF;
   key_Release_timer.function = _MDrv_ReleaseKey_Cb;
   add_timer(&key_Release_timer);
}

inline static void _MDrv_ReleaseKey_StartTimer(Ir_Key_Param KeyParam)
{
    timer_pending(&key_Release_timer);
    oldIrKeyParam = KeyParam;
    mod_timer(&key_Release_timer, jiffies + 150);
}

#if 0
static void _MDrv_ReleaseKey_Finalize(void)
{
...
//irqreturn_t _MDrv_IR_ISR(int irq, void *dev_id, struct pt_regs *regs)
{
    ...
    if ((CurrentTime - PreTime > IR_TIMEOUT_CYC/1000))//reset
    {
        ...
        _u8PioneerFirstKey = 0xfa;
        _u8PioneerSecondKey = 0xfa;
    }
    ...
    else
    {
        if(TRUE == _bIRStart)
        {
            ...
            {
                if (tmp > 4500 && tmp > (IR_OFC_LOB) && tmp < (IR_OFC_UPB))
                {
    ...
    if((_u32IRCount==3) && (TRUE == _bskyworth_normal_code))
    {
        if (((_u32IRData[1] > (IR_HDC_LOB)) && (_u32IRData[1] < (IR_HDC_UPB)))&&
            ...
            (_u8BeforeHeader==1)&&((_MDrv_IR_GetSystemTime()-ulPreTime)>500))//repeat key, 150
        {
            _brepeat_flag = TRUE;
        }
    }

	#else //#if ENABLE_PIONEER_IR
	U16 tmp;
    static unsigned long PreTime;
	unsigned long CurrentTime = _MDrv_IR_GetSystemTime();
    if ((CurrentTime - PreTime > IR_TIMEOUT_CYC/1000))//reset
...
#elif(IR_TYPE_SEL == IR_TYPE_PANASONIC)
    static unsigned long ulPreTime;
    U32 tmp = 0;

    if ((_MDrv_IR_GetSystemTime() - ulPreTime > IR_TIMEOUT_CYC/1000))
    {
        _u32IRCount = 0;
        _u8IRRepeated_Panas=0;
        _u8IRHeadReceived_Panas=0;
        _u8IRRepeateDetect_panas=0;
        memset(_u32IRData,0,sizeof(_u32IRData));
    }
    ...
    if( tmp > IR_HDC_LOB)//Head received
	{
		...
		_u32IRCount=0;
		#endif
		_u8IRHeadReceived_Panas=1;
	}

    if(_u8IRHeadReceived_Panas)//begin data received
    {
        _u32IRData[_u32IRCount++] = tmp;
        if(_u8IRRepeateDetect_panas)
		{
			if( tmp>(IR_OFC_RP_LOB) && tmp<(IR_OFC_RP_UPB) )
			{
                if(_u32IRCount<4)//for quickly change ir
                {
                    _u8IRRepeated_Panas=1;
                    _u8IRHeadReceived_Panas=0;
                }
			}
		}
    ...
    else
    {
     ...
     //printk(KERN_EMERG "-- \033[1;37;41m[%s] [line:%d]\033[0m --\n", __func__,__LINE__);
     U32 u32IrCounter;
    ...
    if(i_gp<40)
    {
        Counter[i_gp] = u32IrCounter;
        i_gp ++;
    }*/

    if(u32IrCounter < MIN_PULSE_WIDTH)   //Data Pulse Timing
    {
        //Do nothing
    }
    else if(MIN_PULSE_WIDTH <= u32IrCounter && u32IrCounter < MAX_PULSE_WIDTH)  //Reg Packet or Data Packet
    {
        U32 TmpBuffer = (u32IrCounter - MIN_PULSE_WIDTH + (PPM_CELL/2)) / PPM_CELL;

        if(0 <= TmpBuffer && TmpBuffer < 16)
        {
            _u8DataIndex++;

            if(_bIsDataPacket)
            {
                _u32DataPacket = (_u32DataPacket | TmpBuffer);

                if(_u8DataIndex == 8)    //8 data consists of a data packet
                {
                    _bIsLastDataPacket = TRUE;
                }
                else
                {
                    _u32DataPacket <<= 4;
           ...
           {
                _u32RegPacket = (_u32RegPacket | TmpBuffer);

                if(_u8DataIndex == 8)
                {
                    _bIsLastRegPacket = TRUE;
                }
                else
                {
                    _u32RegPacket <<= 4;
    ...
    else if(MIN_INTER_PACKET_SPACE < u32IrCounter && u32IrCounter < MAX_INTER_PACKET_SPACE)  //Internal Packet space
    {
        if(_u8DataIndex == 8)   //8 data consists of a  reg packet
        {
            _bIsDataPacket = TRUE;
            _u8DataIndex  = 0;
        }
    }
    else  // Packet begin or Packet end
    {
        _bIsDataPacket = FALSE;
        _u8DataIndex  = 0;
        _u32DataPacket = 0;
        _u32RegPacket = 0;
    }
 ...
 //   printk("<<<%d \n",g_u16IrKonCounter);
...
#elif (IR_MODE_SEL == IR_TYPE_SWDECODE_PANASONIC7051_NEC_MODE)
    static unsigned long ulPreTime;
    U32 tmp;
    if ((_MDrv_IR_GetSystemTime() - ulPreTime > IR_TIMEOUT_CYC/1000))//||_u8IRRepeated) //timeout or not handler yet
    {
    ...
    {
        tmp=((REG(REG_IR_SHOT_CNT_H_FIFO_STATUS) & 0xF) << 16) | REG(REG_IR_SHOT_CNT_L);
        if( tmp < IR_LG0_LOB  && _u8IRHeadReceived == 0)//error signal
        {
           _u32IRCount = 0;
           _u8IRRepeateDetect=0;
           _u8IRRepeated=0;
        ...
        if(_u8IRHeadReceived)//begin data received
        {
            g_wIrPulseCounter = tmp;// add p
            if (_u32IRCount == 0)
            {
                _u32TmpFlag = 1;     //nec
                g_u8IrDataBits = 0;
            }
            else if (_u32IRCount == 1)
            {
                if (g_wIrPulseCounter > 1800 && g_wIrPulseCounter< 2200)//panasonic
                {
                    _u32TmpFlag = 2;
                    g_u8IrDataBits = 1;
                }
                else if(g_wIrPulseCounter > 2200 && g_wIrPulseCounter< 3000)
                {
                    _u32TmpFlag = 1;
                }
                else if(g_wIrPulseCounter > 4000)// nec
                {
                    _u32TmpFlag = 1;
                }
            }
            ...
            if(_u32TmpFlag == 2)
            {
                //printk("b:%d P:%d\n",g_u8IrDataBits,g_wIrPulseCounter);
                if (g_u8IrDataBits == 1)
                {
                    g_u8IrDataBits++;
                }
                else if ( g_u8IrDataBits >= 2 && g_u8IrDataBits < (2+48) )
                {
                    if (g_wIrPulseCounter > 1400 && g_wIrPulseCounter < 1700)
                    {
                        g_llIrData <<= 1;
                        g_llIrData |= 1;
                        g_u8IrDataBits++;
                    }
                    else if (g_wIrPulseCounter > 600 && g_wIrPulseCounter < 800)
                    {
                        g_llIrData <<= 1;
                        g_u8IrDataBits++;
                    }
...
//        if(_MDrv_IR_GetInputData(&u32InputData))
        #if(IR_MODE_SEL == IR_TYPE_SWDECODE_PANASONIC7051_NEC_MODE)
        if ((bHaveKey=_MDrv_IR_GetKey(&u16Key, &u8System, &u8RepeatFlag)) != 0)
        {
            if(down_trylock(&fantasy_protocol.sem)==0)
            {
                if(fantasy_protocol.data!=0)
                ...
                else
                {
                    ...
                    if(_u32TmpFlag==1)       //nec
                    {
                        fantasy_protocol.data = (u16Key<<8 | u8RepeatFlag);
                        fantasy_protocol.data |= (g_u32Header << 16);
                        //printk("Key=%x,fantasy_protocol.data=%x\n",u16Key,fantasy_protocol.data);
                    }
                    else if(_u32TmpFlag==2)  //panasonic
                    {
                        fantasy_protocol.data = (u16Key<<8 | u8RepeatFlag);
                        fantasy_protocol.data |= ((long long)g_u32Header << 24);
                        //printk("Header= %x,Key=%x,fantasy_protocol.data=%llx\n",g_u32Header,u16Key,fantasy_protocol.data);
                    }
                }
                up(&fantasy_protocol.sem);
...
#elif defined(IR_TYPE_HAIER) && (IR_TYPE_SEL == IR_TYPE_HAIER)
                    U32 haier_factory_ir=0;
                    if(u8RepeatFlag&FCTORY_HEADER_RECEIVE)//sec ir
                    {
                        u8RepeatFlag &= (~FCTORY_HEADER_RECEIVE);
                        haier_factory_ir=(0x04 << 28);

                    }
                    else if(u8RepeatFlag&PCCOMMAND_HEADER_RECEIVE)//third ir
                    {
                        u8RepeatFlag &= (~PCCOMMAND_HEADER_RECEIVE);
                        haier_factory_ir=(0x08 << 28);
                    }
                    else
                    {
                        haier_factory_ir= (0x01 << 28);
                    }

                    fantasy_protocol.data = (u8Key<<8 | u8RepeatFlag);
                    fantasy_protocol.data |= haier_factory_ir;
                    //printk("fantasy_protocol.data is %lx\n",fantasy_protocol.data);
#elif defined(IR_TYPE_CHANGHONG) && (IR_TYPE_SEL == IR_TYPE_CHANGHONG)//for Changhong need suport 2 ir head code ;
                    U32 changhong_factory_ir=0;
                    if(u8RepeatFlag&FCTORY_HEADER_RECEIVE)//sec ir
                    {
                        u8RepeatFlag &= (~FCTORY_HEADER_RECEIVE);
                        changhong_factory_ir=(0x04 << 28);
                    }
                    else if(u8RepeatFlag&PCCOMMAND_HEADER_RECEIVE)//third ir
                    {
                        u8RepeatFlag &= (~PCCOMMAND_HEADER_RECEIVE);
                        changhong_factory_ir=(0x08 << 28);
                    }
                    else//first ir
                    {
                        changhong_factory_ir=(0x01 << 28);
                    }
                    fantasy_protocol.data = (u8Key<<8 | u8RepeatFlag);
                    fantasy_protocol.data |= changhong_factory_ir;
                    //printk("fantasy_protocol.data is %lx\n",fantasy_protocol.data);
	...
	if ((u8Key == CONFIG_MSTAR_IR_POWER_KEY) && (u8RepeatFlag == 1)) //only judge "power key" & repoeat for long press
	{
		...
		if ( (_MDrv_IR_GetSystemTime() - _u16IRLongPressPrevTime) > CONFIG_MSTAR_IR_LONG_PRESS_TIMEOUT)
		{
			...
			printk(KERN_EMERG "\n[%s]: Power Key Long Press , System is being to reset!!!!!!!\n", __FUNCTION__);
			//system reboot
			kernel_restart(NULL);
		}
...
static BOOL _MDrv_IR_GetKey(U8 *pu8Key, U8 *pu8System, U8 *pu8Flag)
{
    ...
    *pu8Flag = (REG(REG_IR_RC_KEY_COMMAND_ADD)&0x8000)>>15;//repeat
    ...
    if((*pu8Flag==1)&&(_MDrv_IR_GetSystemTime() - _ulPrevKeyTime < IR_TIMEOUT_CYC/1100))
    {
        printk("\r\n repeat key too fast, delay a little");
    ...
    {
        //printk("\r\n repeat key too fast, else repeat:%d,comtime:%ld,pretime:%ld,nowtime:%ld",*pu8Flag,IR_TIMEOUT_CYC/1200,_ulPrevKeyTime,_MDrv_IR_GetSystemTime());
        _ulPrevKeyTime = _MDrv_IR_GetSystemTime();
        *pu8Key = u8KeyCmd;
        bRet=TRUE;
    }
...
static BOOL _MDrv_IR_GetKey(U8 *pu8Key, U8 *pu8System, U8 *pu8Flag)
{
...
#else
#if ((IR_TYPE_SEL != IR_TYPE_CUS21SH) && (IR_TYPE_SEL != IR_TYPE_TOSHIBA))
    #if ((IR_TYPE_SEL == IR_TYPE_CUS08_RC5) || (IR_TYPE_SEL == IR_TYPE_XMP1) || (IR_TYPE_SEL == IR_TYPE_VESTEL_RC5))
    unsigned long i;
    #else
    U32 u8Byte, u8Bit;
    ...
    *pu8System = 0;
	...
	if(_bskyworth_normal_code || _bskyworth_pioneer_code)
    {
        ...
        else if(_bskyworth_pioneer_code) //we have to handle pioneer key
        {
            ...
            if (_u8PioneerFirstKey == _u8PioneerSecondKey)
            {
                ...
                u16DoubleCode = 0x00 << 8 | _u8PioneerFirstKey;
                *pu8Key = _u8PioneerFirstKey;
            }
            else
            {
                if (_u8PioneerFirstKey > 0x50)
                {
                    //pioneer double code
                ...
                else
                {
                    //pioneer single code
                    //printk("==>Pioneer single code = 0x%02x\n", _u8PioneerSecondKey);
                    u16DoubleCode = 0x00 << 8 | _u8PioneerSecondKey;
                }
	...
	if(_bskyworth_normal_code || _bskyworth_shuttle_code)
	{
		...
		else if(_bskyworth_shuttle_code) //we have to handle shuttle key
		{
			...
			if((u8IRSwModeByte&0x03) != 0x01 ) // Customer_Code(01)
			{
				_u32IRCount = 0;
				_bskyworth_shuttle_code = FALSE;
				_bIRStart = FALSE;
			...
			if(((u8IRSwModeByte>>2)&0x07)+((u8IRSwModeByte>>5)&0x07) == 0x07)
			{
				if(0x01 == ((u8IRSwModeByte>>2)&0x07))
				{
					*pu8Key = 0x15;
				}
				else
    ...
    memset(u8IRSwModeBuf,0x0,sizeof(U8)*IR_RAW_DATA_NUM);  //CODE0 CODE1 MAKERPARITY+SYSTEM CATEGORY COMMAND PARITY
    for( u8Byte=0; u8Byte<IR_RAW_DATA_NUM; u8Byte++)
    {
        for( u8Bit=0; u8Bit<8; u8Bit++)
        {
    ...
    //verify data
    /*if((((u8IRSwModeBuf[2]&0x0f) == (((u8IRSwModeBuf[0]>>4)&0x0f)^(u8IRSwModeBuf[0]&0x0f)\
        ^((u8IRSwModeBuf[1]>>4)&0x0f)^(u8IRSwModeBuf[1]&0x0f))))&&
      ((u8IRSwModeBuf[5] ==(((u8IRSwModeBuf[2]>>4)&0x0f)+u8IRSwModeBuf[2]&0x0f)^
        u8IRSwModeBuf[3]^u8IRSwModeBuf[4])))
    {
        printk("verify is ok!\n");
    }*/
    if ((u8IRSwModeBuf[0] == IR_HEADER_CODE0) && (u8IRSwModeBuf[1] == IR_HEADER_CODE1))
    {
        *pu8Key = u8IRSwModeBuf[4];
        *pu8Flag = 0;
        _u8PrevKeyCode = *pu8Key;
        _u8IRRepeateDetect_panas = 1;
        _u8IRHeadReceived_Panas = 0;
        bRet = TRUE;
    }
    else
    ...
    if (bRet) {
        ...
        if (_ePrevKeyProperty == E_IR_KEY_PROPERTY_INIT) {
            ...
            *pu8Flag = 0;
            ...
            if (_u8PrevKeyCode == 0x0B) {//OK key press
    ...
    if(_bIsLastRegPacket)
    {
        if((_u32RegPacket&0xFFFF) == 0x4429)
        {
             _bSupportIR = TRUE;
        }
        _bIsLastRegPacket = FALSE;
    }
    else if(_bIsLastDataPacket && _bSupportIR)
    {
        if((_u32DataPacket&0xFF) == 0x0)    // Handset remote Control
        {
             *pu8Key = (_u32DataPacket&0xFF00) >> 8;
             //Test = *pu8Key;
        }
        else     //Keyboard
        {
        ...
        if(_u32DataPacket&0x800000)
        {
             *pu8Flag = TRUE;
        }
         _bSupportIR = FALSE;
         _bIsLastDataPacket = FALSE;
         bRet = TRUE;
    }
    ...
    *pu8System = u8IRSwModeBuf[1];//timl
        ...
        if(u8IRSwModeBuf[1] == _u8IRHeaderCode1)
        {
            ...
            if(u8IRSwModeBuf[2] == (u8)~u8IRSwModeBuf[3])
            {
                ...
                *pu8Flag = 0;
                ...
                _u8IRType = 0;
                #if SUPPORT_RETURN_KEY_STATES
                _u16NecHeader = (((U16)u8IRSwModeBuf[0]) << 8 | u8IRSwModeBuf[1]);
                Ir_Key_Param KeyParam;
                KeyParam.bpress     = true;
                KeyParam.bRepeat    = 0;
                KeyParam.u8KeyCode  = *pu8Key;
                _MDrv_ReleaseKey_StartTimer(KeyParam);
                #endif
                #endif
                goto done;
            }
...
#if (IR_TYPE_SEL == IR_TYPE_HISENSE || IR_TYPE_SEL == IR_TYPE_CHANGHONG || IR_TYPE_SEL == IR_TYPE_HAIER)
    ...
    if (!bRet)
    {
        _u8IRRepeateDetect = 0;///Get key done but data invalid, no need to detect repeate.
    }
    _u32IRCount = 0;
    return bRet;
#endif
...
static BOOL _MDrv_IR_GetKey(U16 *pu16Key, U8 *pu8System, U8 *pu8Flag)
{
    ...
    if(_u32TmpFlag==1)// nec
    {
        ...
        *pu8System = 0;
        ...
        if(_u8IRRepeated)//repeate key
        {
            ...
            //printk("u8PrevKeyCode==%x\n",_u8PrevKeyCode);
            *pu16Key = _u8PrevKeyCode;
            *pu8Flag = 1;
            bRet = TRUE;
            goto done;
        }
        ...
        if(u8IRSwModeBuf[2] == (u8)~u8IRSwModeBuf[3])
        {
            *pu16Key = u8IRSwModeBuf[2];
            *pu8Flag = 0;
            g_u32Header = (((U16)u8IRSwModeBuf[0]) << 8 | u8IRSwModeBuf[1]);
            bRet = TRUE;
            _u8PrevKeyCode = *pu16Key;
            _u8PrevSystemCode = *pu8System;
            _u8IRRepeateDetect = 1;
            _u8IRHeadReceived = 0;
            _u8IRType = 0;
    ...
    {
        if ( g_u8IrDataBits<(48+2))
        {
            return FALSE;
        }
        else if(g_u8IrDataBits>=(48+2))
        {
            g_u8IrDataBits = 0;
            //printk("g_llIrData=%llx\n",g_llIrData);
            g_u16IrKeyData = g_llIrData & 0xFFFF;
            *pu16Key = g_u16IrKeyData;
            g_u32Header = (g_llIrData>>16) & 0xFFFFFFFF;
            if(g_u16IrReKeyData == g_u16IrKeyData)
            {
                *pu8Flag=1;
            }
            else
            {
                g_u16IrReKeyData = g_u16IrKeyData;
                *pu8Flag=0;
            }
            ...
            else
            {
                 ...
                 _MDrv_ReleaseKey_RestartTimer();
                #endif
                g_u8IrPANRepeatCount++;
                if (g_u8IrPANRepeatCount >= 4)//IR_FILTER_REPEAT_NUM
                {
                    g_u8IrPANRepeatCount = 0;
                    return TRUE;
                }
...
void MDrv_IR_Init(int bResumeInit)
{
    ...
    REG(REG_IR_SEPR_BIT_FIFO_CTRL) = 0xF00;//[10:8]: FIFO depth, [11]:Enable FIFO full
#if (IR_MODE_SEL==IR_TYPE_FULLDECODE_MODE)
...
#elif (IR_MODE_SEL == IR_TYPE_HWRC_MODE)
    //wakeup key sel
//    REG(REG_IR_RC_CTRL) = IR_RC_WKUP_EN;
    ...
    _MDrv_IR_ClearFIFO();
	 ...
	 REG(REG_IR_SEPR_BIT_FIFO_CTRL) |= (0x1 <<14);
	 REG(REG_IR_CKDIV_NUM_KEY_DATA) = 0x00CF ;
	 REG(REG_IR_SEPR_BIT_FIFO_CTRL) |= (0x1 <<6);
	...
	#if (MP_PLATFORM_ENTROPY_FROM_IRQ == 1)
		irqflags |= IRQF_SAMPLE_RANDOM;
	#endif
        #ifdef CONFIG_MSTAR_PM_SWIR
        //result = request_irq(E_FIQ_IR, MDrv_SWIR_PM51_ISR, SA_INTERRUPT, "IR", NULL); //Doyle change ISR
        #elif (IR_MODE_SEL == IR_TYPE_HWRC_MODE)
        result = request_irq(E_FIQEXPL_IR_INT_RC, _MDrv_IR_RC_ISR, irqflags, "IR_RC", NULL);
        #else
        result = request_irq(E_FIQ_IR, _MDrv_IR_ISR, irqflags, "IR", NULL);
        #endif
        ir_irq_depth = 0;
        if (result)
        {
...
unsigned int MDrv_IR_SetMultiCustomerCode(unsigned int CustomerCode, unsigned int IR_Protocol, unsigned char Enable)
{
#define REG_IR_PROTOCOL            (REG_IR_BASE+ (0x11*4)) //cusomter code 2
	#define IR_PROTOCOL_MASK      (BIT12|BIT13)
	#define IR_PROTOCOL_NEC      (BIT12|BIT13)
	#define IR_PROTOCOL_SW       (BIT12)
	#define IR_PROTOCOL_RAW      (BIT13)
#define REG_IR_CCODE_1            (REG_IR_BASE+ (0x20*4)) //cusomter code 2
#define REG_IR_CCODE_1_EN            (REG_IR_BASE+ (0x21*4)) //cusomter code 2 Enable
	#define IR_CCODE_1_EN    (BIT0)


	if (((REG(REG_IR_PROTOCOL) &IR_PROTOCOL_MASK) != IR_PROTOCOL_NEC) || (IR_Protocol != 1))
	{
		printk("Warnning: [%s][line:%d] not NEC protocol\n", __FUNCTION__, __LINE__);
		return 0;
	}

	REG(REG_IR_CCODE_1) = CustomerCode;
	if (Enable == 1)
	    REG(REG_IR_CCODE_1_EN) |=  IR_CCODE_1_EN;
	else
	    REG(REG_IR_CCODE_1_EN) &=  (~IR_CCODE_1_EN);
...
EXPORT_SYMBOL(MDrv_IR_SetMultiCustomerCode);
