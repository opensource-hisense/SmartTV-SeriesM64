...
#if defined(DMA_MODE) && DMA_MODE==0
#define MSTAR_W_R_FUNC                  0 //for RIU mode
#else
#define MSTAR_W_R_FUNC                  1   // 0 for RIU mode
#endif
...
uint64_t	u64_TotalWriteBytes_Hal;
uint64_t	u64_TotalReadBytes_Hal;
...
static int mstar_nand_do_read_ops(struct mtd_info *mtd, loff_t from, struct mtd_oob_ops *ops)
{
	...
	while (1) {
		...
		#endif
		{
			...
			{
                false_alarm = 1;
				//check if false alarm caused by non-all-0xff blank page
			...
			if(pu16_FreePagePos[u16_BlkIdx]	== 0xFFFF || pu16_FreePagePos[u16_BlkIdx] > (realpage % pNandDrv->u16_BlkPageCnt))
			{
				if(NC_CheckBlankPage(bufpoi) &&  aligned)
					pu16_FreePagePos[u16_BlkIdx] = realpage % pNandDrv->u16_BlkPageCnt;
			}
					...
					int buffer_length, int *eof, void *data)
{
	...
	if(offset > 0)
		...
		ret = sprintf(buffer,"TotalWriteBytes %lld GB %lld MB\nTotalReadBytes %lld GB %lld MB\n"
				"Hal TotalWriteBytes %lld GB %lld MB\nHal TotalReadBytes %lld GB %lld MB\n",
				...
				u64_TotalWriteBytes_Hal/1024/1024/1024, (u64_TotalWriteBytes_Hal/1024/1024) % 1024,
				u64_TotalReadBytes_Hal/1024/1024/1024, (u64_TotalReadBytes_Hal/1024/1024) % 1024);
	return ret;
}
#endif
...
{
	unsigned long u32_monitor_read_enable = 0;

	if(sscanf(buf, "%lu", &u32_monitor_read_enable) != 1)
		return -EINVAL;
...
static ssize_t fcie_monitor_write_enable_store(struct device *dev,
	...
	const char *buf, size_t count)
{
	unsigned long u32_monitor_write_enable = 0;

	if(sscanf(buf, "%lu", &u32_monitor_write_enable) != 1)
		return -EINVAL;
	...
	struct device_attribute *attr, char *buf)
{
	...
	if(gu32_monitor_count_enable)
	{
		...
		return sprintf(buf,"fcie_monitor_count_enable=%lu\n"
							"Read Count %lu\n"
							"Write Count %lu\n"
...
static ssize_t fcie_monitor_count_enable_store(struct device *dev,
	...
	const char *buf, size_t count)
{
	unsigned long u32_monitor_count_enable = 0;

	if(sscanf(buf, "%lu", &u32_monitor_count_enable) != 1)
		return -EINVAL;
	...
	struct device_attribute *attr, char *buf)
{
    ...
    u32_off += sprintf(buf, "NAND ID:0x%x ",pNandDrv->au8_ID[0]);
    for(u8_i=1;u8_i< pNandDrv->u8_IDByteCnt;u8_i++)
    {
        u32_off += sprintf(buf+u32_off, ",0x%x ", pNandDrv->au8_ID[u8_i]);
    }
    u32_off += sprintf(buf+u32_off, "\n");
    return u32_off;
}
...
static ssize_t fcie_nand_bad_block_cnt_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
    return sprintf(buf, "Bad block count:%lu\n", gu32_nand_bad_block_count);
}
...
#endif
{
	...
	u64_TotalWriteBytes_Hal = u64_TotalReadBytes_Hal = 0;
