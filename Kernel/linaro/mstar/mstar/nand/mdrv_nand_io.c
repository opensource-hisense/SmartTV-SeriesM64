...
int flush(void)
{
	...
	if(pNandDrv->u8_BL0PBA != 0 && pNandDrv->u8_BL1PBA != 0)
	{
		...
		if(u32_Err != 0)
		{
			printk(KERN_ERR"error: program sboot @ block %lX fail\n", u32_BLPBA0);
			goto ERR_OUT;
		}
		...
		if(u32_Err != 0)
		{
			printk(KERN_ERR"error: program sboot backup @ block %lX fail\n", u32_BLPBA1);
			goto ERR_OUT;
		}
		...
		if(u32_Err != 0)
		{
			printk(KERN_ERR"error: program uboot @ block %lX fail\n", u32_UbootPBA0);
			goto ERR_OUT;			
		}
		...
		if(u32_Err != 0)
		{
			printk(KERN_ERR"error: program uboot backup @ block %lX fail\n", u32_UbootPBA1);			
			goto ERR_OUT;
		}
	...
	else if(pNandDrv->u8_HashPBA[0][0] != 0)
	{
		...
		for(i=0; i<3; i++)
		{
			...
			if(u32_Err != 0)
			{
				printk(KERN_ERR"program hash%d @ block 0x%lX fails\n", i, u32_block_hash[i]);
				goto ERR_OUT;
			}
			//check current block with cis records
			...
			if(u32_Err != 0)
			{
				printk(KERN_ERR"program hash%d backup @ block 0x%lX fails\n", i, u32_block_hash[i]);
				goto ERR_OUT;
			}			
			//check current block with cis records
