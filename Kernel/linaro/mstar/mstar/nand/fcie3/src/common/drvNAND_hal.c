...
)
{
	...
	U32 u32_Ret;	
	U8	u8_IsSpareDMA = ((pNandDrv->u16_Reg48_Spare & BIT_NC_SPARE_DEST_MIU) != 0);
	...
	u32_DataMapAddr = nand_DMA_MAP_address((U32)pu8_DataBuf, pNandDrv->u16_PageByteCnt * u32_PageCnt, WRITE_TO_NAND);
	...
	if(u8_IsSpareDMA)
	{
		...
		if (pu8_SpareBuf)
		{
			u32_SpareMapAddr = nand_DMA_MAP_address((U32)pu8_SpareBuf, pNandDrv->u16_SpareByteCnt, WRITE_TO_NAND);
		...
		else
		{
			...
			u32_SpareMapAddr = nand_DMA_MAP_address((U32)pNandDrv->PlatCtx_t.pu8_PageSpareBuf, pNandDrv->u16_SpareByteCnt, WRITE_TO_NAND);
...
)
{
	...
	u32_DataMapAddr = nand_DMA_MAP_address((U32)pu8_DataBuf, pNandDrv->u16_PageByteCnt * u32_PageCnt, READ_FROM_NAND);
	...
	if(u8_IsSpareDMA)
	{
		...
		if (pu8_SpareBuf)
		{
			u32_SpareMapAddr = nand_DMA_MAP_address((U32)pu8_SpareBuf, pNandDrv->u16_SpareByteCnt, READ_FROM_NAND);
			u32_SpareDMAAddr = nand_translate_DMA_address_Ex(u32_SpareMapAddr, pNandDrv->u16_SpareByteCnt, READ_FROM_NAND);
		}
		else
		{
			u32_SpareMapAddr = nand_DMA_MAP_address((U32)pNandDrv->PlatCtx_t.pu8_PageSpareBuf, pNandDrv->u16_SpareByteCnt, READ_FROM_NAND);
			u32_SpareDMAAddr = nand_translate_DMA_address_Ex(u32_SpareMapAddr, pNandDrv->u16_SpareByteCnt, READ_FROM_NAND);
		}
		REG_WRITE_UINT16(NC_SPARE_DMA_ADR0, u32_SpareDMAAddr & 0xFFFF);
		REG_WRITE_UINT16(NC_SPARE_DMA_ADR1, u32_SpareDMAAddr >>16);
...
//		if(u8_RetryCnt_ECCFail >= (u8_MaxReadRetryCount*3) / 4)
...

	{
		...
		if (pu8_SpareBuf)
		{
			pu8_tmpBuf = pu8_SpareBuf;
		//else
		//	pu8_tmpBuf = pNandDrv->PlatCtx_t.pu8_PageSpareBuf;

			for (i = pNandDrv->u16_SpareByteCnt - 1; i >= 0; i--)
			{
				if (pu8_tmpBuf[i] != 0xFF)
				{
					break;
				}
			}

			if (i < 0)	//Maybe an empty page
			{
				gint_IS_ECC_Fail = 0;

				for (i = pNandDrv->u16_PageByteCnt - 1; i >= 0; i--)
				{
					if (pu8_DataBuf[i] != 0xff)
					{
...
U32  NC_ReadSectors_MTD(U32 u32_PhyRowIdx, U8 u8_SectorInPage, U8 *pu8_DataBuf, U8 *pu8_SpareBuf, U32 u32_SectorCnt)
{
	...
	if (u32_SectorCnt_Tmp)
	{
		...
		u32_DataMapAddr = nand_DMA_MAP_address((U32)pu8_DataBuf, pNandDrv->u16_SectorByteCnt * u32_SectorCnt_Tmp, READ_FROM_NAND);
...
//			if(u8_RetryCnt_ECCFail >= (u8_MaxReadRetryCount*3) / 4)
	...
	#if 1
	{
		...
		if (pu8_SpareBuf)
		{
			for (i = pNandDrv->u16_SectorSpareByteCnt*u32_SectorCnt_Tmp - 1; i >= 0; i--)
			{
				if (pu8_SpareBuf[i] != 0xFF)
				{
					break;
				}
			}
		...
		if(i < 0)
		{
			if(((U32)pu8_DataBuf) % 4 != 0)
			{
				int last_byte = (((U32)pu8_DataBuf) % 4);
				int first_byte = 4 - (((U32)pu8_DataBuf) % 4);
				U8* pu8_tmpBuf;
				for(i = 0; i < first_byte ; i ++)
				{
					if(pu8_DataBuf[i] != 0xFF)
					{
						nand_debug(UNFD_DEBUG_LEVEL_WARNING, 1,"0xff error detect, i=%d,data=0x%x\r\n", i, pu8_DataBuf[i]);
						pu8_DataBuf[i] = 0xFF;
					}
				}
				pu8_tmpBuf = pu8_DataBuf + first_byte;
				for(i = 0; i < (pNandDrv->u16_SectorByteCnt * u32_SectorCnt_Tmp - 4)/4; i ++)
				{
					if(((U32*)pu8_tmpBuf)[i] != 0xFFFFFFFF)
					{
						nand_debug(UNFD_DEBUG_LEVEL_WARNING, 1,"0xff error detect, i=%d,data=0x%lx\r\n", i, ((U32*)pu8_tmpBuf)[i]);
				       	((U32*)pu8_tmpBuf)[i] = 0xffffffff;
					}
				}
				pu8_tmpBuf += pNandDrv->u16_SectorByteCnt * u32_SectorCnt_Tmp - 4;
				for(i = 0; i < last_byte ; i ++)
				{
					if(pu8_tmpBuf[i] != 0xFF)
					{
						nand_debug(UNFD_DEBUG_LEVEL_WARNING, 1,"0xff error detect, i=%d,data=0x%x\r\n", first_byte + pNandDrv->u16_PageByteCnt - 4 + i, pu8_tmpBuf[i]);
						pu8_tmpBuf[i] = 0xFF;
					}
				}
			}
			...
			{
				for (i = 0; i < (pNandDrv->u16_SectorByteCnt * u32_SectorCnt_Tmp /4); i ++)
				{
					if (((U32*)pu8_DataBuf)[i] != 0xffffffff)
			        {
						// the page is not really empty
						nand_debug(UNFD_DEBUG_LEVEL_WARNING, 1,"0xff error detect, i=%d,data=0x%lx\r\n", i, ((U32*)pu8_DataBuf)[i]);
							((U32*)pu8_DataBuf)[i] = 0xffffffff;
					}
			    }
			}
		}
...
// -------------------------------------
	    ...
	    if( ((u16_Tmp & BIT_NC_ECC_MAX_BITS_MASK) >> BIT_NC_ECC_MAX_BITS_SHIFT) >= pNandDrv->u16_BitflipThreshold)
    	{
		    ...
		    if(0 != u16_Tmp && 	pNandDrv->u8_CISBlk != 0xFF)
		    {
			    nand_debug(0,1,"Reg.51h: %X  BitsCnt: %X \n", u16_Tmp, pNandDrv->u8_RD_ECCBitsCnt);
			    nand_debug(0,1,"Blk: %X,  Page: %lX \n", pNandDrv->u16_RD_SrcPBA,
				    u32_PhyRowIdx & pNandDrv->u16_BlkPageCntMask);
...
)
{
	...
	u32_DataMapAddr = nand_DMA_MAP_address((U32)pu8_DataBuf, pNandDrv->u16_SectorByteCnt * u32_SectorCnt, WRITE_TO_NAND);
...
)
{
	...
	#endif
	u32_DataMapAddr = nand_DMA_MAP_address((U32)pu8_DataBuf, (2 * pNandDrv->u16_PageByteCnt) * u32_PageCnt, WRITE_TO_NAND);
	...
	if(u8_IsSpareDMA)
	{
		...
		if (pu8_SpareBuf)
		{
			u32_SpareMapAddr = nand_DMA_MAP_address((U32)pu8_SpareBuf, 2 * pNandDrv->u16_SpareByteCnt, WRITE_TO_NAND);
		...
		else
		{
			...
			u32_SpareMapAddr = nand_DMA_MAP_address((U32)pNandDrv->PlatCtx_t.pu8_PageSpareBuf, 2 * pNandDrv->u16_SpareByteCnt, WRITE_TO_NAND);
			u32_SpareDMAAddr = nand_translate_DMA_address_Ex(u32_SpareMapAddr, 2 * pNandDrv->u16_SpareByteCnt, WRITE_TO_NAND);
		}
		REG_WRITE_UINT16(NC_SPARE_DMA_ADR0, u32_SpareDMAAddr & 0xFFFF);
		REG_WRITE_UINT16(NC_SPARE_DMA_ADR1, u32_SpareDMAAddr >>16);
