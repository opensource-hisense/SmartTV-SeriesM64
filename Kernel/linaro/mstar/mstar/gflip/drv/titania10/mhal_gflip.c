...
{
     return (MS_U8)((GFLIP_GOP_BANKOFFSET * u32GopIdx) + (MS_U8)(u32GopIdx/2) + u16BankIdx);
}
   ...
   // GFLIPHAL_ASSERT(((u32GopIdx==GFLIP_GOP_IDX_4G)||(u32GopIdx==GFLIP_GOP_IDX_2G)), "Invalid GopIdx, Currently Only support Gop_4G and Gop_2G!\n");
    if (u32GopIdx < MAX_GOP_SUPPORT)
    {
        u8Bank = _MHal_GFLIP_GetGopBank();
         ...
         _MHal_GFLIP_SetGopBank((GFLIP_GOP_BANKOFFSET * u32GopIdx) + (MS_U8)(u32GopIdx/2) + u16BankIdx);
...
MS_U32 MHal_GFLIP_GetValidGWinIDPerGOPIdx(MS_U32 u32GopIdx, MS_U32 u32GwinIdx)
{
    switch(u32GopIdx)
    {
        ...
        case GFLIP_GOP_IDX_1GX:
            u32GwinIdx = 1; //only GWin 1 is valid;
            break;
...
MS_BOOL MHal_GFLIP_IsVSyncInt(MS_U32 u32GopIdx)
{
...

    {
        ...
        _MHal_GFLIP_SetGopBank((GFLIP_GOP_BANKOFFSET * (MS_U8)u32GopIdx) + (MS_U8)(u32GopIdx/2) + GFLIP_GOP_BANK_IDX_0);
...
MS_U32 MHal_GFLIP_GetIntGopIdx(void)
{
    ...
    u16RegVal = (GOP_REG(REG_GOP_BANK_SEL) & GOP_INT_FLAG_MASK) | (GOP_REG(REG_GOP_MUX) & GOP_1GX_INT_FLAG_MASK);
...
MS_BOOL MHal_GFLIP_ClearDWINIRQ(GFLIP_DWININT_INFO *pGFlipDWinIntInfo)
{
    ...
    if(!(GOP_REG(REG_GOP_BANK_SEL) & GOPD_INT_FLAG))
    {
        return FALSE;
    }
...
MS_U16 MHal_XC_GetCurrentReadBank(void)
{
    ...
    u16CurrentReadBank = XC_REG( 0x3A ) >> 4 ;
    XC_REG( 0x00 ) = u16Bank;
