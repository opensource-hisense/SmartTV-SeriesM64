...
//have a lot of wrong setting
    ...
    u16RegVal = (GOP_REG(REG_GOP_WR_ACK, 0) & GOP_INT_FLAG_MASK) | (GOP_REG(REG_GOP_MUX, 0) & GOPD_INT_FLAG_MASK);
...
{
    printf("---Warning: %s not implement.\n",__FUNCTION__);
}
...
{
    MS_U16 u16FWR = GOP_REG(REG_GOP_WR_ACK, 0)&GOP_FWR;
...
MS_BOOL MHal_GFLIP_ClearDWINIRQ(GFLIP_DWININT_INFO *pGFlipDWinIntInfo)
{
    ...
    if(!(GOP_REG(REG_GOP_WR_ACK, 0) & GOPD_INT_FLAG))
    {
        return FALSE;
    }
