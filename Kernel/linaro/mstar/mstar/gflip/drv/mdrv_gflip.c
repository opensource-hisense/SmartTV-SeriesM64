...
typedef struct
{
    ...
    unsigned short CKG_GopReg[4];
    unsigned short GS_GopReg;
    unsigned short SC_OPBlending[2];
...
#if (!defined(CONFIG_MSTAR_URANUS4) && !defined(CONFIG_MSTAR_KRONUS) && !defined(CONFIG_MSTAR_KAISERIN) && !defined(CONFIG_MSTAR_KENYA) && !defined(CONFIG_MSTAR_KERES) && !defined(CONFIG_MSTAR_KIRIN))
...
{
    return MHal_GFLIP_SetFlipToGop(u32GopIdx, u32GwinIdx, u32MainAddr, u32SubAddr, bForceWriteIn);
}
...
#ifdef	GFLIP_MULTI_FLIP
MS_BOOL MDrv_GFLIP_SetMultiFlipInfo(MS_GFLIP_MULTIINFO* pMultiFlipInfo)
{
    MS_U32 u32NextWritePtr;
    MS_U32 u32QCnt = 0;
    MS_U32 cur_jiffies;
    ...
    MS_U32 u32MainAddr = 0,u32SubAddr = 0;
    ...
    if(TRUE == _bGFlipInVsyncLimitation[u32GopIdx])
    { //in handling vsync limitation status, just print the error and return true to avoid block app:
        printk("\n@@@@Error!!!! In MDrv_GFLIP_SetMultiFlipInfo, no Signal for GOP Clock Source and GFlip Vsync Interrupt disabled!!!\n");
    }
    mutex_lock(&Semutex_Gflip);
    u32NextWritePtr = (_u32GFlipInfoWritePtr[u32GopIdx][u32GwinIdx]+1) % MAX_FLIP_ADDR_FIFO;
    if(u32NextWritePtr != _u32GFlipInfoReadPtr[u32GopIdx][u32GwinIdx]) //can write
...
MS_BOOL MDrv_GFLIP_WaitForVsync(MS_U32 u32GopIdx)
{
    ...
    switch(u32GopIdx)
    {
        ...
        case 5:
            ...
            break;
        default :
            printk("MDrv_GFLIP_WaitForVsync u32GopIdx =%ld is out of case",u32GopIdx);
            break;
    }
    return bRet;
}
MS_BOOL MDrv_GFLIP_GetVsync(MS_U32 u32GopIdx)
{
    ...
    switch(u32GopIdx)
    {
        ...
        default :
            printk("MDrv_GFLIP_GetVsync u32GopIdx =%ld is out of case",u32GopIdx);
            break;
    }
    return bRet;
}
...
int MDrv_GFLIP_Suspend(void)
{
    ...
    _gflip_regs_save.SC_OPBlending[0] = SC_REG(GOP_SC_IP2SC);
...
#ifdef GOP_SC1_GOPEN
    SC_REG(GOP_SC_BANKSEL) = GOP_SC1_GOPEN;
    SC_REG(GOP_SC_IP2SC) = _gflip_regs_save.SC_OPBlending[0];
