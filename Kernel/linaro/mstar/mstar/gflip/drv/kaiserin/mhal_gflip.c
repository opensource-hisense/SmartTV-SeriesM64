...
{
    MS_U16 u16Bank;
    u16Bank = GOP_REG(REG_GOP_BANK_SEL);
    u16Bank &= ~GOP_BANK_SEL;
    u16Bank |= (u8bank&GOP_BANK_SEL);
    GOP_REG(REG_GOP_BANK_SEL) = u16Bank;
}

MS_U8 _MHal_GFLIP_GetGopBank(void)
{
    return (MS_U8)(GOP_REG(REG_GOP_BANK_SEL)&GOP_BANK_SEL);
}
...
*/
        case GFLIP_GOP_DST_OP0:
            ...
            break;
        case GFLIP_GOP_DST_VE:
            switch(u32GopIdx)
            {
                case GFLIP_GOP_IDX_4G:
                    u16RegAddr = GOP_GOPCLK;
                    u16RegVal = CKG_GOPG0_VECLK;
                    u16RegMask = CKG_GOPG0_MASK;
                    break;
                case GFLIP_GOP_IDX_4GX:
                    u16RegAddr = GOP_GOPCLK;
                    u16RegVal = CKG_GOPG1_VECLK;
                    u16RegMask = CKG_GOPG1_MASK;
                    break;
                case GFLIP_GOP_IDX_1G:
                    u16RegAddr = GOP_GOP23CLK;
                    u16RegVal = CKG_GOPG2_VECLK;
                    u16RegMask = CKG_GOPG2_MASK;
                    break;
                case GFLIP_GOP_IDX_1GX:
                    u16RegAddr = GOP_GOP23CLK;
                    u16RegVal = CKG_GOPG3_VECLK;
                    u16RegMask = CKG_GOPG3_MASK;
                    break;
                default:
                    return FALSE;
...
MS_U32 MHal_GFLIP_GetIntGopIdx(void)
{
    ...
    u16RegVal = (GOP_REG(REG_GOP_BANK_SEL) & GOP_INT_FLAG_MASK);

    if(u16RegVal & GOP4G_INT_FLAG)
    {
        return GFLIP_GOP_IDX_4G;
    }
    else if(u16RegVal & GOP4GX_INT_FLAG)
    {
        return GFLIP_GOP_IDX_4GX;
    }
/*
...
{
    MS_U16 u16GOPWAck = GOP_REG(REG_GOP_BANK_SEL);
...
void MHal_GFLIP_Fire(MS_U32 u32GopBitMask)
{
    ...
    for(i = 0; i < MAX_GOP_SUPPORT; i++)
    {
        ...
        {
            u16GOPWAck = GOP_REG(REG_GOP_BANK_SEL);
            u8BankOffset = MHal_GFLIP_GetBankOffset(i, GFLIP_GOP_BANK_IDX_0);
            if((u16GOPWAck & GOP_BK_WR) == 0)
            {
                GOP_REG(REG_GOP_BANK_SEL) = (u16GOPWAck & (~GOP_BANK_SEL)) | u8BankOffset | GOP_BK_WR;
                GOP_REG(REG_GOP_BANK_SEL) &= ~GOP_BK_WR;
                GOP_REG(REG_GOP_BANK_SEL) = (GOP_REG(REG_GOP_BANK_SEL) & (~GOP_BANK_SEL)) | (u16GOPWAck & GOP_BANK_SEL);//Restore bank
            }
        }
    }
}
...
MS_BOOL MHal_GFLIP_SetFlipToGop(MS_U32 u32GopIdx, MS_U32 u32GwinIdx, MS_U32 u32MainAddr, MS_U32 u32SubAddr, MS_BOOL bForceWriteIn)
{
    ...
    if(bForceWriteIn)
    {
        ...
        {
             GOP_REG(REG_GOP_BANK_SEL) |= GOP_FWR;
             GOP_REG(REG_GOP_BANK_SEL) &= ~GOP_FWR;
        }
    }
    else
...
BOOL msDLCMSTVToolReadWriteValue( void )
{
    ...
    if ( MApi_GFLIP_XC_R2BYTEMSK(REG_SC_BK30_03_L, BIT(0))==BIT(0) )
    {
        ...
        if((u8DataByte>16)|(u8IDValue>181)|((u8IDValue+u8DataByte)>181))
        {
        ...
        {
            for (ucTmp = 0 ,ucTmpEx=0 ; ucTmp < u8DataByte; ucTmp++)
            {
                if(ucTmp>=3)
                {
                    ucTmpEx=1;
                ...
                msWriteByte((REG_SC_BK30_07_H+ucTmp+ucTmpEx), g_ucDLCInitBuffer[u8IDValue+ucTmp] );
            }
        ...
        else//MSTV Tool want to write the data
        {
            ...
            for (ucTmp = 0 ,ucTmpEx=0 ; ucTmp < u8DataByte; ucTmp++)
            {
                ...
                g_ucDLCInitBuffer[u8IDValue+ucTmp] =msReadByte(REG_SC_BK30_07_H+ucTmp+ucTmpEx) ;
