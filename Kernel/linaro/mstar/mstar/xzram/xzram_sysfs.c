...
// echo 1 > /sys/block/xzram0/pool_set_id ==> change the set pool
static ssize_t  pool_set_id_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t len)
{
    int ret;
    u64 id;
	unsigned temp_pool_id;
...
// EX: "echo 1 > /sys/block/xzram0/pool_set_migrate" ==> to migrate pool to EXTERNAL
/***********************************/
// EXT_XVPOOL_INTERNAL_NO_LIMIT 0
// EXT_XVPOOL_EXTERNAL          1
// EXT_XVPOOL_INTERNAL          2
/***********************************/
        ...
        struct device_attribute *attr, const char *buf, size_t len)
{
	...
	if(!xzram->xv_mem_pool[set_pool_id])
		printk("\033[35mFunction = %s, Line = %d, this pool [%d] is not activate, please check it!!\033[m\n", __PRETTY_FUNCTION__, __LINE__, set_pool_id);
    ...
    {
        pr_info("xzram migration is busy, current status: [%d]\n", xzram->pool_migration_stat);
    	up_write(&xzram->lock);
    ...
    switch(migration)
    {
		// migrate to EXT_XVPOOL_INTERNAL (EXT_XVPOOL_INTERNAL_NO_LIMIT is only for pool_0)
        case 0:
        {
		...
		// migrate to EXT_XVPOOL_EXTERNAL
        case 1:
        {
        ...
        default:
            pr_info("using error migration setting: %d, it should be 0 for EXT_XVPOOL_INTERNAL_NO_LIMIT, and 1 for EXT_XVPOOL_EXTERNAL\n", (int) migration);
            goto migration_fail_init;
        break;
    }
	printk("\033[35mFunction = %s, Line = %d, starting migrate\033[m\n", __PRETTY_FUNCTION__, __LINE__); // joe.liu

    tmp_bitmap = ext_xv_create_bitmap(xzram->xv_mem_pool[set_pool_id]);	// ask a memory whose size is as size of xbitmap
    ...
    down_write(&xzram->lock);
    ext_xv_set_migration(xzram->xv_mem_pool[set_pool_id]);					// set this pool is on_migration
    ext_xv_bitmap_duplicate(xzram->xv_mem_pool[set_pool_id], tmp_bitmap);	// copy the xbitmap data form pool to tmp_bitmap
    up_write(&xzram->lock);

	// check the pool, find all used pages and prepare corresponding pages(pointed by tmp_array, but no data in page then)
...
fill_backuplist:
    if(firstrun++)
    {
		...
		// already add cnt backup_page to backuplist
        ext_xv_sub_write_cnt(xzram->xv_mem_pool[set_pool_id], cnt);
    }

    down_write(&xzram->lock);

	// get the cnt first, if cnt > 0, goto fill_backuplist
	// the cnt increases only when migration is set, and having bvec_write --> grow_pool
	...
	// main function, tmp_array is pre-allocate for ori_pool's ext_xv_pool_array, and xzram->lock is locked 
...
migration_end:
	...
	cnt = ext_xv_get_write_cnt(xzram->xv_mem_pool[set_pool_id]);
	if(cnt)
		printk("\033[35mFunction = %s, Line = %d, strange write_cnt, cnt is %d\033[m\n", __PRETTY_FUNCTION__, __LINE__, cnt);
    ext_xv_sub_write_cnt(xzram->xv_mem_pool[set_pool_id], cnt);
    
	cnt = ext_xv_get_read_cnt(xzram->xv_mem_pool[set_pool_id]);
	if(cnt)
		printk("\033[35mFunction = %s, Line = %d, strange read_cnt, cnt is %d\033[m\n", __PRETTY_FUNCTION__, __LINE__, cnt);
        ...
        case XZRAM_INFO_PAGE_USAGE:
        {
            ...
            for(i=1; i<MAX_POOL_NUM; i++)
            {
                if(xzram->pool_info[i].activate)
                {
					...
					pr_info("KVA is = 0x%X\n", ext_xv_show_KVA(xzram->xv_mem_pool[i]));
        ...
        struct device_attribute *attr, const char *buf, size_t len)
{
    ...
    {
        pr_info("Only value 0 is acceptable, val is %d, len is %d\n", len);
        up_write(&xzram->lock);
        return len;
    }
...
// cat /sys/block/xzram0/pool_set_migrate_stat ==> to show the migrate_stat of specified xzram device
...
// (pool_set_id, S_IWUSR, NULL, pool_set_id_store) => (_name, _mode, _show, _store)
// pool_set_id: command used in console
// S_IWUSR    : can use echo (S_IRUGO can use cat)
