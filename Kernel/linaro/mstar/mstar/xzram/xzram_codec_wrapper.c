...
#elif defined(XZRAM_COMPRESS_WKDM)
int compress_func(const char* src_buf, int src_len,char* dst_buf, int* dst_len,  void* work_buf)
{
	#ifdef ZRAM_CHECKSUM	
	*dst_len = (int)WKdm_compress((WK_word*)src_buf, (WK_word*)(dst_buf+CHECKSUM_HEADER_LEN),src_len);
	...
	*dst_len += CHECKSUM_HEADER_LEN;			 
	#else
	*dst_len = (int)WKdm_compress((WK_word*)src_buf, (WK_word*)dst_buf, src_len);
	#endif
	return ZRAM_CODEC_OK;
}
...
{  
	...
	if(header_crc == compute_crc)
	{		
		WKdm_decompress((WK_word*)(src_buf+CHECKSUM_HEADER_LEN), (WK_word*)dst_buf, *dst_len/sizeof(WK_word));
	}
	else
	{
	   	ret = ZRAM_CODEC_ERROR_CHECKSUM;
	...
	#else
	WKdm_decompress((WK_word*)src_buf, (WK_word*)dst_buf, *dst_len/sizeof(WK_word));
	#endif
	return ZRAM_CODEC_OK;
}
#else
