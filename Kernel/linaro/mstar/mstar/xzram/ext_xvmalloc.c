...
static void __iomem *get_ptr_atomic(void __iomem *page, u16 offset, enum km_type type, struct ext_xv_pool *pool)
{
	...
	//printk("\033[35mFunction = %s, Line = %d, PageIndexToOffset is 0x%X\033[m\n", __PRETTY_FUNCTION__, __LINE__, (unsigned int)PageIndexToOffset(page)); // joe.liu
...
static void __iomem * alloc_xpage(struct ext_xv_pool *pool, gfp_t flag)
{
    ...
    {
		// search 32 pages to see if any any free page
        if(*(pool->ext_pool_attr->xbitmap+i) ==0xffffffff)
           continue;

        u32bit = GetRightMostZero(*(pool->ext_pool_attr->xbitmap+i));

		// if (mod != 0) => check if u32bit(0~31) is over our pool size
        if((mod != 0) && (i == bmap_entry_num - 1) && (u32bit >= mod))
            continue;

        if(u32bit >= 0)
        {
            array_offset = i * BITS_PER_LONG + u32bit;
            *(pool->ext_pool_attr->xbitmap + i) |= (1 << u32bit); // set the pool bit_map to specify this page is being used
...
static int grow_pool(struct ext_xv_pool *pool, gfp_t flags)
{
    ...
    {
		// the return page is only a pool_array_index
    ...
    if(pool->type != EXT_XVPOOL_INTERNAL_NO_LIMIT)
    {
        ...
        {
			// while migration (release mutex), some bio_write is coming ==> write_cnt++	
...
struct ext_xv_pool *ext_xv_create_pool(struct ext_xv_pool_attr *attr, u8 pool_id)
{
    ...
    else
    {
        ...
        {
            printk("Create external memory pool for xzram pool_id = %d!! ba_start is 0x%X\n", pool->pool_id, (unsigned int)pool->ext_pool_attr->ba_start);
            ...
            pool->ext_pool_attr->ba_start = ioremap_nocache((unsigned long)pool->ext_pool_attr->ba_start, 
                                                                            attr->ext_pool_size);
			// check if ioremap_nocache success
			if(!pool->ext_pool_attr->ba_start)
			{
				printk("\033[35mFunction = %s, attr->ext_pool_start is at 0x%X, mapping to va failed(0x%X)\033[m\n", __PRETTY_FUNCTION__, (unsigned int)attr->ext_pool_start, (unsigned int)pool->ext_pool_attr->ba_start);
				//return NULL;
			}
        ...
        {
            printk("Create internal memory pool for xzram pool_id = %d!! ba_start is 0x%X\n", pool->pool_id, (unsigned int)pool->ext_pool_attr->ba_start);
        ...
        u32 *offset, gfp_t flags)
{
    ...
    *offset = 0;
	...
	// find a block of given size in pool, if no such block, *page will still NULL
    ...
    if (!*page) {
		...
		// if can not find a suitable block, ask a new page and try find_block again
...
bool ext_xv_is_ext_pool(struct ext_xv_pool *pool)
{
	if(pool == NULL)
	{
		printk("\033[35m[Crirical Error] This pool is NULL!!\033[m\n");
        return false;
	}
...
unsigned int ext_xv_show_KVA(struct ext_xv_pool *pool)
{
    if(pool == NULL)
        return EXT_XV_FAIL;

    return (unsigned int)pool->ext_pool_attr->ba_start;
}
EXPORT_SYMBOL_GPL(ext_xv_show_KVA);
                                                 ...
                                                 unsigned int *tmp_xbitmap, struct ext_xv_backuplist *backuplist)
{
    ...
    spin_lock(&pool->lock); 
    memcpy(tmp_xbitmap, pool->ext_pool_attr->xbitmap, sizeof(u32)*bmap_entry_num);	// re-duplicate xbitmap to tmp_xbitmap, this is for updating bit_map
    spin_unlock(&pool->lock);

    orig_backuplist_num = backuplist->cnt;
   
   	// check if any read/write is done during this migration ==> this make pool->bitmap be different from tmp_array(array[offset] is 0 means this xzram_page is not used)
    for(i = 0; i < bmap_entry_num; i++)
    {
        for(j = 0; j < BITS_PER_LONG; j++)
        {
		    ...
		    if((pool->ext_pool_attr->ext_xv_pool_array[array_offset] != 0) && (tmp_array[array_offset]== 0))  // xzram write request while migration(xzram_page is used, but tmp is not)
            {
                m_wcnt ++;
				
			    if( (pool->type == EXT_XVPOOL_EXTERNAL) && (type == EXT_XVPOOL_INTERNAL) ) // external -> internal
			    {
                    tmp_array[array_offset] = get_page_from_backup_list(backuplist);	// from pre-prepared backuplist to take a free page
                }
                else if( (pool->type == EXT_XVPOOL_INTERNAL) && (type == EXT_XVPOOL_EXTERNAL) ) // internal -> external
    			{
                    tmp_array[array_offset] = (void __iomem *)( pool->ext_pool_attr->ba_start + ( i * BITS_PER_LONG * PAGE_SIZE) + (PAGE_SIZE * j) );
			    }
		    }

            if((pool->ext_pool_attr->ext_xv_pool_array[array_offset] == 0) && (tmp_array[array_offset] != 0))  // xzram read request while migration(xzram_page from used to free, but tmp is still used)
            {
                m_rcnt++;

                if( (pool->type == EXT_XVPOOL_EXTERNAL) && (type == EXT_XVPOOL_INTERNAL))	// release tmp_array page
			    {
    ...
    {
		// pool->xv_migration.read_cnt is usually add when doing ext_xv_free during migration
        printk("XX22 m_rcnt = %d, pool->xv_migration.read_cnt = %d, type = %d\n", m_rcnt, pool->xv_migration.read_cnt, pool->type);
    }

	// here to do migration!!
    if( (pool->type == EXT_XVPOOL_EXTERNAL) && (type == EXT_XVPOOL_INTERNAL) ) // migrate data from external page to internal page
    { 
        spin_lock(&pool->lock);
        ...
        for(i = 0; i < bmap_entry_num; i++)
        {
            ...
            for(j = 0; j < BITS_PER_LONG; j++)
            {
                ...
                if(u32bit >= 0)
                {
                    ...
                    memcpy(va_dest, va_src, PAGE_SIZE);	// migration, copy from src_page to dst_page
        ...
        pool->type = EXT_XVPOOL_INTERNAL;	// migration is done 
        spin_unlock(&pool->lock);
        ...
        for(i = 0; i < bmap_entry_num; i++)
        {
            ...
            for(j = 0; j < BITS_PER_LONG; j++)
            {
                ...
                if(u32bit >= 0)
                {
                    array_offset = i * BITS_PER_LONG + u32bit;
                    tmp_bitmap &= ~(1 <<u32bit);
                    va_dest = (void __iomem *) (pool->ext_pool_attr->ba_start + (i * BITS_PER_LONG*PAGE_SIZE)+(PAGE_SIZE * u32bit));
...
unsigned int * ext_xv_create_bitmap(struct ext_xv_pool *pool)
{
	...
	//printk("\033[35mFunction = %s, Line = %d, create a bitmap, size is %d\033[m\n", __PRETTY_FUNCTION__, __LINE__, bmap_entry_num); // joe.liu
	//printk("\033[35mFunction = %s, Line = %d, create a bitmap, size is %d\033[m\n", __PRETTY_FUNCTION__, __LINE__, sizeof(u32)*bmap_entry_num); // joe.liu
...
void __iomem** ext_xv_create_pool_array(struct ext_xv_pool *pool, u32 type, unsigned int *tmp_xbitmap)
{
    ...
    if(tmp_xbitmap == NULL)
    {
        printk("migration fail, tmp_xbitmap not exist\n");
        return NULL;
    }

	// tmp_array is an array to point to all pool pages
    tmp_array = kzalloc((pool->ext_pool_attr->size >> PAGE_SHIFT)*
                                             sizeof(void __iomem *), GFP_KERNEL|__GFP_REPEAT);

    if(tmp_array == NULL)
    {
        printk("migration fail, create tmp pool array fail\n");
        return NULL;
    }
	...
	// for each bit of bitmap, check if this bit is 1, which means this page is used => need to be migrated
    for(i = 0; i < bmap_entry_num; i++)
    {
        spin_lock(&pool->lock);
        tmp_bitmap = *(tmp_xbitmap + i);
        ...
        for(j = 0; j < BITS_PER_LONG; j++)
        {
			...
			u32bit = GetRightMostOne(tmp_bitmap);	// get the first non-zero bit, which means this bit is used in ori_type
            
            if((i == bmap_entry_num - 1) && (u32bit >= mod) && (mod != 0))	// this case, the non-zero bit is not used for bitmap
                break;

            if(u32bit >= 0)
            {
                array_offset = i * BITS_PER_LONG + u32bit;
                tmp_bitmap &= ~(1 << u32bit);	// set the non-zero bit to be 0

                if(pool->type == EXT_XVPOOL_EXTERNAL)
                {
					// migrate from EXT_XVPOOL_EXTERNAL to EXT_XVPOOL_INTERNAL(allocpage)
                    
					page = alloc_page(GFP_NOIO | __GFP_WAIT | __GFP_HIGHMEM | __GFP_FS);  	// page alloc fail case?  or //page = alloc_page(GFP_ATOMIC); ??

                    if(page == NULL)	// error case, free all allocated pages in tmp_array and free tmp_array
                    {
                ...
                {
					// migrate from EXT_XVPOOL_INTERNAL to EXT_XVPOOL_EXTERNAL 
                    tmp_array[array_offset] = (void __iomem *)(pool->ext_pool_attr->ba_start + (i * BITS_PER_LONG * PAGE_SIZE) + (PAGE_SIZE * u32bit));
                }
                else
                {
                    BUG_ON(1);
...
void ext_xv_delete_pool_array(struct ext_xv_pool *pool, void __iomem **tmp_array)
{
	...
	// only need to free internal pages, if pool->type is internal, then tmp_array is for external(no matter pass or failed migration)
    if(pool->type == EXT_XVPOOL_EXTERNAL)
    {	
        for(i = 0; i <= bmap_entry_num; i++)
        {
...
void __iomem ** ext_xv_pool_migration(struct ext_xv_pool *pool, u32 type, void __iomem **tmp_array, unsigned int *tmp_xbitmap, struct ext_xv_backuplist *backuplist)
{
	...
	// the ret is an array, which is the original xzram_pool
...
// add cnt ext_xv_backup_page to the ext_xv_backuplist
...
{ 
    ...
    spin_lock(&pool->lock);
	//printk("\033[35mFunction = %s, Line = %d, memcpy %d bytes to tmp_xbitmap\033[m\n", __PRETTY_FUNCTION__, __LINE__, sizeof(u32)*bmap_entry_num); // joe.liu
