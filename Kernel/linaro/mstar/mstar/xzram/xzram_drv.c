			  ...
			  u32 index, int offset, struct bio *bio)
{
	...
	// user_mem is the page we will read data from xzram_page to
	user_mem = kmap_atomic(page, KM_USER0);
	if (!is_partial_io(bvec))
		uncmem = user_mem;	// let xzram_page data be uncompressed to user_mem
	clen = PAGE_SIZE;
	...
	// if partial_io, copy data from uncmem to user_mem
	if (is_partial_io(bvec)) {
...
static int xzram_read_before_write(struct xzram *xzram, char *mem, u32 index)
{
    ...
    cmem = xzram_map( (void __iomem *)xzram->table[index].page, KM_USER0, PAGE_SIZE, xzram, index) + xzram->table[index].offset;
                ...
                u32 *offset, gfp_t flags)
{
  ...
  {
      if(ext_xv_malloc(xzram->xv_mem_pool[i], size, page, offset, flags))
      {
         continue;
      }
			   ...
			   int offset)
{
	...
	// find a block with size is "clen + sizeof(*zheader)", and update the xzram->table[index].page to the page of block ==> read data from "a" xzram_page, after add write_data, compress and write into "b" xzram_page
...
memstore:
	...
	cmem = xzram_map((void __iomem *)xzram->table[index].page, KM_USER1, PAGE_SIZE, xzram, index) + xzram->table[index].offset;
	...
	// the compressed data is in xzram_buffer(src)
...
{
	int ret;
	//printk("\033[31mFunction = %s, Line = %d, index is %d, offset is %d\033[m\n", __PRETTY_FUNCTION__, __LINE__, index, offset); // joe.liu
...
static int xzram_make_request(struct request_queue *queue, struct bio *bio)
{
	...
	{
		printk("\033[35mFunction = %s, Line = %d, xzram->init_done is 0 or xzram_init_device failed\033[m\n", __PRETTY_FUNCTION__, __LINE__); // joe.liu
		bio_io_error(bio);
		return 0;
	}
...
static int xzram_create_pools(struct xzram * xzram)
{
    ...
    for(i = 1; i < MAX_POOL_NUM; i++)
    {
		...
		printk("\033[35mFunction = %s, Line = %d, create pool [%d] finished\033[m\n", __PRETTY_FUNCTION__, __LINE__, i); // joe.liu

		// remember the xzram->init_done is set, so u can not change setting or activate it anymore
		if(xzram->xv_mem_pool[i] == NULL)
		{
			printk("\033[35mFunction = %s, Line = %d, ext_xv_create_pool for pool[%d] is failed, do not activate it\033[m\n", __PRETTY_FUNCTION__, __LINE__, i); // joe.liu
			//xzram->pool_info[i].activate = 0;
		}
    }
...
int xzram_init_device(struct xzram *xzram)
{
	...
	// totalram_pages is total low_mem / PAGE_SIZE
