...
static void read_cmd_fis(void *rx_fis, u32 u32cmd_offset , u8 *pfis, u32 misc_base)
{
    ...
    memcpy(pfis, cmd_address, SATA_KA9_RX_FIS_SIZE);
...
static bool sata_mstar_qc_fill_rtf(struct ata_queued_cmd *qc)
{
    ...
    struct sata_mstar_host_priv *host_priv = qc->ap->host->private_data;
    unsigned int tag = sata_mstar_tag(qc->tag);
    u8 FisBuffer[SATA_KA9_RX_FIS_SIZE];
    u32 misc_base = host_priv->misc_base;
    void *rx_fis;

    rx_fis = pp->rx_fis;

    read_cmd_fis(rx_fis,(tag * SATA_KA9_CMD_DESC_SIZE), FisBuffer, misc_base);

    if (qc->tf.protocol == ATA_PROT_PIO && qc->dma_dir == DMA_FROM_DEVICE &&
        !(qc->flags & ATA_QCFLAG_FAILED))
    {
        ata_tf_from_fis((u8 *)FisBuffer + SATA_KA9_RX_PIO_FIS_OFFSET, &qc->result_tf);
        qc->result_tf.command = ((u8 *)FisBuffer + SATA_KA9_RX_PIO_FIS_OFFSET)[15];
    }
    else
    {
        ata_tf_from_fis((u8 *)FisBuffer + SATA_KA9_RX_D2H_FIS_OFFSET, &qc->result_tf);
    }
    return true;
}
