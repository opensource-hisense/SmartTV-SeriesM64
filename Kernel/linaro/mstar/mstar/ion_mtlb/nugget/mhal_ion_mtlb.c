...
static int _getAddrByMiu(u8 u8miu, u32 *miu_bus_base, void ** addr_tlb_table)
{
    ...
    if(u8miu == 0)
    {
       *miu_bus_base = MIU0_BUS_BASE;
       *addr_tlb_table = virt_addr_miu0;
    }
    else if(u8miu == 1)
    {
       *miu_bus_base = MIU1_BUS_BASE;
       *addr_tlb_table = virt_addr_miu1;
    }
    else if(u8miu == 2)
    {
       *miu_bus_base = MIU2_BUS_BASE;
...
{
    u8 val;
    void* address = (void*)((unsigned long)MTLB_RIU_REG_BASE + u32bank*0x100*2+(u32reg << 1) - (u32reg & 1));
...
static void * _mtlb_phys_to_virtul(phys_addr_t phys_addr)
{
    ...
    if(phys_addr < lowmem_limit)
    {
        virt_addr = (void *)(__va(phys_addr));
    }
    else  // in high memroy
    {
        pgprot_t pgprot;
        ...
        page = __pfn_to_page(((unsigned long)((phys_addr) >> PAGE_SHIFT)));
...
int MHal_MTLB_Reserve_Memory(void)
{
    ...
    if(lx_mem_addr != INVALID_PHY_ADDR)
    {
        phys_addr_miu0 = _find_in_range(MIU0_BUS_BASE, MIU1_BUS_BASE, REVERVE_MEMORY_SIZE, PAGE_SIZE);
        if(phys_addr_miu0 > 0)
        {
    ...
    if(lx_mem2_addr != INVALID_PHY_ADDR)
    {
        phys_addr_miu1 = _find_in_range(MIU1_BUS_BASE, MIU2_BUS_BASE, REVERVE_MEMORY_SIZE, PAGE_SIZE);
        if(phys_addr_miu1 > 0)
        {
    ...
    if(lx_mem3_addr != INVALID_PHY_ADDR)
    {
        phys_addr_miu2 = _find_in_range(MIU2_BUS_BASE, MIU3_BUS_BASE, REVERVE_MEMORY_SIZE, PAGE_SIZE);
        if(phys_addr_miu2 > 0)
        {
...
int MHal_MTLB_Mapping_Address_Support(unsigned long * u32pa, unsigned long nents, unsigned char * miu)
{
    ...
    else if(u32pa[0] >= MIU1_BUS_BASE)
    {
        *miu = 1;
        min = MIU1_BUS_BASE;
        max = MIU2_BUS_BASE;
    }
    else if(u32pa[0] >= MIU0_BUS_BASE)
    {
        *miu = 0;
        min = MIU0_BUS_BASE;
        max = MIU1_BUS_BASE;
...
void MHal_MTLB_Mapping_End(unsigned long * flag, u8 u8miu, unsigned long start, unsigned long size)
{
    ...
    _dma_cache_wback_inv((unsigned long)(u32header), size * TLB_Entry_BYTE_SIZE);
