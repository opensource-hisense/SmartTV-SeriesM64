...
#define GMAC_IER_FOR_INT_JULIAN_D   (0x0000E435)
...
#ifdef GMAC_CHIP_FLUSH_READ
    #if defined(CONFIG_MIPS)
        Chip_Read_Memory_Range((unsigned int)(&(dlist->descriptors[LocPtr->rxBuffIndex].addr)) & 0x0FFFFFFF, sizeof(dlist->descriptors[LocPtr->rxBuffIndex]));
    #elif defined(CONFIG_ARM)
...
#ifdef GMAC_CHIP_FLUSH_READ
    #if defined(CONFIG_MIPS)
        Chip_Read_Memory_Range((unsigned int)(&(dlist->descriptors[LocPtr->rxBuffIndex])) & 0x0FFFFFFF, sizeof(dlist->descriptors[LocPtr->rxBuffIndex]));
    #elif defined(CONFIG_ARM)
...
#ifdef GMAC_NAPI
    //netif_napi_add(gmac_dev, &LocPtr->napi_str, MDev_GMAC_napi_poll, 1);  
