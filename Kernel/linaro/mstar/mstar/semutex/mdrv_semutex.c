...
//for sharememory
#define SHAREDMEMORY_LEN	(2 * 1024 * 1024)
...
static int _Cleanup_Mutex(struct file *filp,SemutexModHandle *dev)
{
    ...
    for(i = 0 ; i < MUTEX_SUPPORTED ; i ++)
    {
        ...
        {//the process which locked the mutex cleanup(probably terminated un-normally)
			...
			memset(dev->mutex_info[i].procname, 0, 64);
        ...
        case MDRV_SEMUTEX_LOCK_WITHTIMEOUT:
            {
                ...
                else if(res == 0)
                {
                    ...
                    memset(dev->mutex_info[lockarg.index].procname, 0, 64);
                    strcpy(dev->mutex_info[lockarg.index].procname, current->comm);
        ...
        case MDRV_SEMUTEX_TRY_LOCK:
            {
                ...
                {//already locked!!!
                    ...
                    memset(dev->mutex_info[index].procname, 0, 64);
                    strcpy(dev->mutex_info[index].procname, current->comm);
        ...
        case MDRV_SEMUTEX_UNLOCK:
            {
                ...
                {
                    printk(KERN_EMERG"WARNING2!Try to unlock mutex index %d name %s with pid %d->%s , but not in the lock pid %d->%s\n",
        ...
        case MDRV_SEMUTEX_CREATE_SHAREMEMORY:
            {
				...
				/* 	Allow the case:
					1. /dev/semutex opened in porcess A, B forked from A, create sharememory in B
					2. multi create sharememory in one process
				*/                
				if(filp == pprocinfo->createsharememory_filp)
                {
                    printk("Try to create sharememory again! \n");
                    return 0;
                }
