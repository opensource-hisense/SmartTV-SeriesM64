...
#define WT_DB_PLUG                400       //Waiting time for Insert Debounce
...
#define REG_GPIO(addr) *((volatile unsigned int*)(0xFD000000 + ((addr)<< 1)))
#endif

static irqreturn_t ms_sdmmc_cdzint(int irq, void *p_dev_id)
{
	irqreturn_t irq_t = IRQ_NONE;
	IntSourceStruct* pstIntSource = (IntSourceStruct *)p_dev_id;
...
static void ms_sdmmc_request(struct mmc_host *p_mmc_host, struct mmc_request *p_mmc_req)
{
	...
	/****** Simple SD command *******/
	if(!data)
		eErr = Hal_SDMMC_SendCmdAndWaitProcess(eIP, EV_EMP, EV_CMDRSP, _TransRspType(mmc_resp_type(cmd)), TRUE,TRUE);
	else // R/W SD Command
	{
	...
	//	printk("__[Sgl: %u] (TB: %u)(BSz: %u)", (U16_T)data->sg_len, u16BlkCnt, u16BlkSize);
