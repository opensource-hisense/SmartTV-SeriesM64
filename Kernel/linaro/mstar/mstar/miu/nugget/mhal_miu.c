...
//  Macros
//-------------------------------------------------------------------------------------------------
#define _phy_to_miu_offset(MiuSel, Offset, PhysAddr) if (PhysAddr < MS_MIU_INTERVAL) \
                                                        {MiuSel = E_CHIP_MIU_0; Offset = PhysAddr;} \
                                                     else \
                                                         {MiuSel = E_CHIP_MIU_1; Offset = PhysAddr - MS_MIU_INTERVAL;} \

#define _miu_offset_to_phy(MiuSel, Offset, PhysAddr) if (MiuSel == E_CHIP_MIU_0) \
                                                        {PhysAddr = Offset;} \
                                                     else \
                                                         {PhysAddr = Offset + MS_MIU_INTERVAL;} \
...
static MS_U32 _gMIU_MapBase = 0xBF200000UL;   //default set to arm 32bit platfrom
...
MS_U32 HAL_MIU_BA2PA(MS_U32 u32BusAddr)
{
    ...
    // pa = ba - offset
	if( (u32BusAddr >= MIU0_BUS_BASE) && (u32BusAddr < MIU1_BUS_BASE) )	// MIU0
		u32PhyAddr = u32BusAddr - MIU0_BUS_BASE;	// NUGGET MIU0 is @ 0
	else if( (u32BusAddr >= MIU1_BUS_BASE) && (u32BusAddr < MIU2_BUS_BASE))	// MIU1
		u32PhyAddr = u32BusAddr - MIU1_BUS_BASE + MS_MIU_INTERVAL;
	else
		printk("\033[35mFunction = %s, Line = %d, Error, this u32BusAddr is not valid\033[m\n", __PRETTY_FUNCTION__, __LINE__);
