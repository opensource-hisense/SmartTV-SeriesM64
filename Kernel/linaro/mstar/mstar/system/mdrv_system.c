...
#define PAD_PCM_D_ENABLE()    REG_ADDR(REG_CHIP_PCM_D_PE)   |= REG_CHIP_PCM_D_PE_MASK; \
                              REG_ADDR(REG_CHIP_PCM_D_PE1)  |= REG_CHIP_PCM_D_PE1_MASK;
                              
#define PAD_PCM_A_ENABLE()    REG_ADDR(REG_CHIP_PCM_A_PE)  |= REG_CHIP_PCM_A_PE_MASK;

#define PAD_PCM_D_DISABLE()   REG_ADDR(REG_CHIP_PCM_D_PE)   &= ~REG_CHIP_PCM_D_PE_MASK; \
                              REG_ADDR(REG_CHIP_PCM_D_PE1)  &= ~REG_CHIP_PCM_D_PE1_MASK;

#define PAD_PCM_A_DISABLE()   REG_ADDR(REG_CHIP_PCM_A_PE)  &= ~REG_CHIP_PCM_A_PE_MASK;
...
#define MSTAR_UART_LSR         (5)
	...
	defined(CONFIG_MSTAR_MULAN_BD_FPGA) || \
...
#if defined(CONFIG_MMC_MSTAR_MMC_EMMC) && CONFIG_MMC_MSTAR_MMC_EMMC
static inline BOOL _MDrv_PCMCIA_WaitEMMCDone(U32 u32loopCnt)
{
...
int MDrv_SYS_PCMCIA_WRITE(U32 arg, BOOL bFromUser)
{
...
int MDrv_SYS_PCMCIA_READ(U32 arg, BOOL bFromUser)
{
    PCMCIA_Map_Info_t op ;
    U16 u16Addr, i;
...
int MDrv_SYS_PCMCIA_READ_DATA(U32 arg, BOOL bFromUser)
{
    PCMCIA_Map_Info_t op ;
...
static struct proc_dir_entry *root_kattr_dir;
static struct proc_dir_entry *kattr_dir;
static struct proc_dir_entry *k_entry;

static char kattribute_str[128];

static int kattr_proc_show(struct seq_file *seq, void *v)
{
    seq_printf(seq, "%x\n", (unsigned int)seq->private); 
    return 0;
}

static int kattr_proc_open(struct inode* inode, struct file *file)
{
    return single_open(file, kattr_proc_show, PDE(inode)->data);
}

static int kattr_proc_write(struct file *file, const char __user * buffer,
                         size_t count, loff_t * ppos)
{
     ...
     val = simple_strtol(val_str, (char **)(val_str+count), 16);
...
static const struct file_operations kattr_node_proc_fops = {
        .owner          = THIS_MODULE,
        .open           = kattr_proc_open,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
        .write          = kattr_proc_write,
 };

static __init int MDrv_SYS_KAttribute(char *attribute)
{
    strcpy (kattribute_str, attribute);
    return 0;
}

static int MDrv_SYS_KAttr_Register(char *attribute)
{
    char substr[10], val_str[8];
    int cnt, len=0, idx=0, num_len;
    unsigned long value=0;

    len = strlen(attribute);

    root_kattr_dir = proc_mkdir("kattr", NULL);
    if (!root_kattr_dir)
        return -1;
    ...
    {
        cnt = 0;
        while(attribute[idx] != ':' && idx < len)
        {
            substr[cnt] = attribute[idx];
            cnt ++;
            idx ++;
        }
        ...
        while(attribute[idx+num_len] != ',' && attribute[idx+num_len] != ';' && (idx+num_len)<len)
        {
            num_len++;
        }
    
        strncpy(val_str, attribute+idx, num_len);
        idx+=num_len;
      
        
        val_str[num_len] = '\0';
        value = simple_strtol(val_str, (char **)(val_str+num_len), 16);
         
        memset(val_str, 0, 8); 

        kattr_dir = proc_mkdir(substr, root_kattr_dir);
        if (!kattr_dir)
            return -1;
 
        k_entry = proc_create_data("value", 0666, kattr_dir,
                         &kattr_node_proc_fops, NULL);

        k_entry->data = (void *)value;      
...
int __init mod_MDrv_SYS_KAttr_Init(void)
{
    MDrv_SYS_KAttr_Register(kattribute_str);

    return -1;
}
module_init(mod_MDrv_SYS_KAttr_Init);

early_param("Kattribute", MDrv_SYS_KAttribute);
