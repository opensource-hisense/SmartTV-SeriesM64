...
_mali_osk_errcode_t _mali_osk_pm_dev_ref_add(void)
{
	...
	if (0 > err) {
		...
		if (err == -EACCES)
		{
			/* XXX: When restarting system, the runtime PM of this device may be disabled.
			 * In this case, pm_runtime_get_sync may return -EACCES.
			 * We would only print this message in debug mode, and would not take this as error message.
			 */
			MALI_DEBUG_PRINT(1, ("Mali OSK PM: pm_runtime_get_sync() returned error code %d\n", err));
