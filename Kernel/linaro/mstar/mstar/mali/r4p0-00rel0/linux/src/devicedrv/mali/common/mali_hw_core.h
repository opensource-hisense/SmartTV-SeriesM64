...
struct mali_hw_core {
	...
	mstar_riu_address mapped_registers; /**< Virtual mapping of the registers */
#else
...
MALI_STATIC_INLINE u32 mali_hw_core_register_read(struct mali_hw_core *core, u32 relative_address)
{
	u32 read_val;
#ifdef MSTAR_RIU_ENABLED
	read_val = _mstar_riu_read(core->mapped_registers, relative_address);
#else
...
#ifdef MSTAR_RIU_ENABLED
	_mstar_riu_write(core->mapped_registers, relative_address, new_val);
#else
...
MALI_STATIC_INLINE u32 mali_hw_core_register_read_no_lock(struct mali_hw_core *core, u32 relative_address)
{
	u32 read_val;

	read_val = _mstar_riu_read_no_lock(core->mapped_registers, relative_address);
...
MALI_STATIC_INLINE void mali_hw_core_register_write_no_lock(struct mali_hw_core *core, u32 relative_address, u32 new_val)
{
	MALI_DEBUG_PRINT(6, ("register_write for core %s, relative addr=0x%04X, val=0x%08X\n",
	                      core->description, relative_address, new_val));

	_mstar_riu_write_no_lock(core->mapped_registers, relative_address, new_val);
}
#endif /* defined(MSTAR_RIU_ENABLED) && (MSTAR_RIU_ADDRESS_TYPE != 32) */
