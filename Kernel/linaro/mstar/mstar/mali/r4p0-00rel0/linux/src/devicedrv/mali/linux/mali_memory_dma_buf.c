...
#ifdef MSTAR
			mali_mmu_pagedir_update(pagedir, virt, phys - PHYS_TO_BUS_ADDRESS_ADJUST, size, MALI_MMU_FLAGS_DEFAULT);
#else
...
#ifdef MSTAR
			mali_mmu_pagedir_update(pagedir, virt, guard_phys - PHYS_TO_BUS_ADDRESS_ADJUST, MALI_MMU_PAGE_SIZE, MALI_MMU_FLAGS_DEFAULT);
#else
