...
#include <mst_devid.h>
#ifndef MDRV_MAJOR_UMP
#error MDRV_MAJOR_UMP not defined
#endif
#if MDRV_MAJOR_UMP == 0
#error MDRV_MAJOR_UMP cannot be 0
#endif
#endif
...
unsigned long ump_max_pre_alloc_size = 0;
module_param(ump_max_pre_alloc_size, ulong, S_IRUSR | S_IWUSR | S_IWGRP | S_IRGRP | S_IROTH);
MODULE_PARM_DESC(ump_max_pre_alloc_size, "Max ump pre-alloc size ");
#endif
...
#ifdef UMP_MEMORY_PRE_ALLOCATION
static void parsing_kernel_nv(char *name)
{
	...
	*value++ = 0;
	...
	if (!strcmp(name,"UMP_MAX_PRE_ALLOC")) {
		int err;

		err = kstrtoul(value, 0, &size);
		
		if (err==0)
		{
			ump_max_pre_alloc_size = size;
		}
		else
		{
			DBG_MSG(0, ("UMP_MAX_PRE_ALLOC %s failed converted by kstrtoul\n",value ));
		}
	}
}


void parsing_kernel_cmdline(void)
{
	char cmdline[512];
	char *ptr;
	mm_segment_t old_fs;
	...
	if (filp >= 0) {
		...
		old_fs = get_fs();
		set_fs(KERNEL_DS);
		n = filp->f_op->read(filp, cmdline, 511,&filp->f_pos);
		set_fs(old_fs);
