...
static ssize_t memory_used_read(struct file *filp, char __user *ubuf, size_t cnt, loff_t *ppos)
{
	...
	buff1 = mali_paged_allocated_miu();
	if (buff1 != NULL) buff1_len = strlen(buff1);

	result_len = sizeof(buf)+ buff1_len;
	if(result_len != 0)
	{
		result = _mali_osk_malloc(result_len);
		memset(result, 0, result_len);
		if (result)
		{
			strcat(result, buf);
			if(buff1 != NULL) strcat(result, buff1);
		}
	}
	if(buff1 != NULL) _mali_osk_free(buff1);

	r = simple_read_from_buffer(ubuf, cnt, ppos, result, result_len);

	_mali_osk_free(result);
