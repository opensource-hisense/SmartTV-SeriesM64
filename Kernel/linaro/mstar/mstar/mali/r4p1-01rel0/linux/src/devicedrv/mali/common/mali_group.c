...
/* MSTAR patch begin */
#ifdef MSTAR_EXTEND_JOB_RUNTIME_WHEN_TIMEOUT
			/* XXX: extend the timeout limit for large rendering lists if still rendering */
			u32 core_status = mali_gp_read_core_status(group->gp_core);
...
#ifdef MSTAR_EXTEND_JOB_RUNTIME_WHEN_TIMEOUT
			/* XXX: extend the timeout limit for large rendering lists if still rendering */
			u32 core_status = mali_hw_core_register_read(&core->hw_core, MALI200_REG_ADDR_MGMT_STATUS);

			if (0 != (core_status & MALI200_REG_VAL_STATUS_RENDERING_ACTIVE))
			{
				_mali_osk_timer_mod(group->timeout_timer, _mali_osk_time_mstoticks(mali_max_job_runtime));
			}
			else
