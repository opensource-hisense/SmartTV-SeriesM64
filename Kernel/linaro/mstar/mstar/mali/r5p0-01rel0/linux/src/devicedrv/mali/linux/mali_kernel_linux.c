...
#if defined(CONFIG_MMU_NOTIFIER) || defined(CONFIG_MP_MM_MALI_MMU_NOTIFIER) /* MStar: MMU notifier */
...
extern unsigned long mali_dedicated_mem_start;
module_param(mali_dedicated_mem_start, ulong, S_IRUSR | S_IRGRP | S_IROTH);
#else
...
#if defined(CONFIG_MALI_DVFS)
int dvfs_enable = 1;
module_param(dvfs_enable, int, S_IRUSR | S_IWUSR | S_IWGRP | S_IRGRP | S_IROTH); /* rw-rw-r-- */
MODULE_PARM_DESC(dvfs_enable, "Enable DVFS");
#endif

#if MSTAR_SKIP_GP_AND_PP_JOBS_SWITCH
...
static void mali_mn_release(struct mmu_notifier *mn, struct mm_struct *mm)
{
	...
	/* XXX: the code below is copied from _mali_ukk_close to flush pending work */
...
#ifndef MSTAR_USING_FIXED_DEVID
	/* input validation */
...
#endif
{
	...
	case MALI_IOC_SOFT_JOB_SIGNAL:
		...
		break;
#ifdef MSTAR
	case MALI_IOC_MEM_GET_CPU_USAGE_ADJUST:
		BUILD_BUG_ON(!IS_ALIGNED(sizeof(_mali_uk_get_cpu_usage_adjust), sizeof(u64)));
		err = mem_get_cpu_usage_adjust_wrapper(session_data, (_mali_uk_get_cpu_usage_adjust __user *)arg);
		break;

	case MALI_IOC_MEM_GET_MIU1_BASE:
		BUILD_BUG_ON(!IS_ALIGNED(sizeof(_mali_uk_get_miu1_base), sizeof(u64)));
