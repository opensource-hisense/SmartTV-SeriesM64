...
void set_mali_alloc_strategy_wrapper(unsigned long mali_strategy)
{
#if defined(CONFIG_MP_MM_CMA)
	set_mali_alloc_strategy(0/*not used*/, mali_strategy);
#elif defined(CONFIG_MP_ION_PATCH_MSTAR)
...
int mali_mmap(struct file *filp, struct vm_area_struct *vma)
{
...
#if MSTAR_CONTROL_MIU
	mali_strategy = get_mali_alloc_strategy_wrapper();

	if((session->use_miu < MSTAR_MIU_NUM ) && (mali_strategy != session->use_miu))
	{
		set_mali_alloc_strategy_wrapper(session->use_miu);
		strategy_changed = 1;
	}
#endif
	...
	if (strategy_changed)
	{
		set_mali_alloc_strategy_wrapper(mali_strategy);
	}
#endif
...
#ifdef MSTAR 
u32 _mali_ukk_report_memory_usage_maximum(void)
{
	u32 sum = 0;

	sum += mali_mem_os_stat_maximum();
	/* TODO: Consider the dedicated memory */
	return sum;
}
#endif
