...
#if MSTAR_CONTROL_MIU && defined(CONFIG_MP_MM_CMA)
...
static struct mali_mem_os_allocator {
	...
	atomic_t allocated_pages_max[MSTAR_MIU_NUM];
#else
	struct list_head pool_pages;
	size_t pool_count;

	atomic_t allocated_pages;
	atomic_t allocated_pages_max;
#endif
...
static int mali_mem_os_alloc_pages(mali_mem_allocation *descriptor, u32 size)
{
...
#if defined(MSTAR) || (defined(CONFIG_ARM) && !defined(CONFIG_ARM_LPAE))
		flags |= GFP_HIGHUSER;
#else
	...
	{
		u32 allocated_pages = atomic_read(&mali_mem_os_allocator.allocated_pages[mali_id]);
		u32 allocated_pages_max = atomic_read(&mali_mem_os_allocator.allocated_pages_max[mali_id]);

		if ( allocated_pages  > allocated_pages_max )
		{
			atomic_set(&mali_mem_os_allocator.allocated_pages_max[mali_id], allocated_pages);
		}
	}
	...
	{
		u32 allocated_pages = atomic_read(&mali_mem_os_allocator.allocated_pages);
		u32 allocated_pages_max = atomic_read(&mali_mem_os_allocator.allocated_pages_max);

		if ( allocated_pages  > allocated_pages_max )
		{
			atomic_set(&mali_mem_os_allocator.allocated_pages_max, allocated_pages);
		}
	}
...
static int mali_mem_os_mali_map(mali_mem_allocation *descriptor, struct mali_session_data *session)
{
	...
	list_for_each_entry(page, &descriptor->os_mem.pages, lru) {
		...
		bus_to_phys_address_adjust(&phys);
#else
		dma_addr_t phys = page_private(page);
#endif
...
#ifdef MSTAR_PRE_ALLOC_PAGE_TABLE
#define MALI_MEM_OS_PAGE_TABLE_PAGE_POOL_SIZE 384
#else
...
#ifdef MSTAR_PRE_ALLOC_PAGE_TABLE
static _mali_osk_errcode_t mali_mem_os_pre_alloc_table_page(void)
{
	size_t i;
	size_t count;
	bool ret = _MALI_OSK_ERR_NOMEM;

	for (i = 0; i < MALI_MEM_OS_PAGE_TABLE_PAGE_POOL_SIZE; i++)
	{
		dma_addr_t tmp_phys;
		mali_io_address mapping = NULL;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 8, 0)
		...
		if (NULL != mapping) {
			 ...
			 * wider than 32-bit. */
#ifndef MSTAR
			/* NOTE: the "physical" address is allowed to be wider than 32-bit */
			MALI_DEBUG_ASSERT(0 == (tmp_phys >> 32));
#endif
#endif
			mali_mem_page_table_page_pool.page[i].phys = (mali_dma_addr) tmp_phys;
			mali_mem_page_table_page_pool.page[i].mapping = mapping;
...
#ifdef MSTAR_PRE_ALLOC_PAGE_TABLE
			MALI_PRINTF(("mali_mem_page_table_page_pool is empty. Fallback to dma_alloc_attrs\n"));
#endif
		}
	}
#ifdef MSTAR
...
{
#ifdef MSTAR
	phys_to_bus_address_adjust(&phys);
#endif
	spin_lock(&mali_mem_page_table_page_pool.lock);
...
_mali_osk_errcode_t mali_mem_os_init(void)
{
	...
	for (i = 0; i < MSTAR_MIU_NUM; i++)
	{
		...
		atomic_set(&(mali_mem_os_allocator.allocated_pages_max[i]),0);
	}
#endif
...
#ifdef MSTAR_PRE_ALLOC_PAGE_TABLE
	mali_mem_os_pre_alloc_table_page();
#endif
...
#ifdef MSTAR
u32 mali_mem_os_stat_maximum(void)
{
	int alloc_bytes = 0;
#if MSTAR_CONTROL_MIU
	int i;
	for(i = 0; i < MSTAR_MIU_NUM; i++)
	{
		alloc_bytes += atomic_read(&mali_mem_os_allocator.allocated_pages_max[i]) * _MALI_OSK_MALI_PAGE_SIZE;
	}
#else
	alloc_bytes += atomic_read(&mali_mem_os_allocator.allocated_pages_max) * _MALI_OSK_MALI_PAGE_SIZE;
#endif
	return alloc_bytes;
}
#endif
