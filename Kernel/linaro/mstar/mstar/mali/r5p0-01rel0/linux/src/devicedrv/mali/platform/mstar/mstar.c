...
#define GPU_BASE_ADDRESS 0xFD340000
#else
#define GPU_BASE_ADDRESS 0x00340000
#endif
...
void mali_report_gpu_clock_info( struct mali_gpu_clock **data)
{
    MALI_DEBUG_PRINT(4, ("mali_report_gpu_clock_info() called\n"));

    mstar_platform_report_gpu_clock_info(data);
}

int mali_gpu_get_clock_step(void)
{
    MALI_DEBUG_PRINT(4, ("mali_gpu_get_clock_step() called\n"));

    return mstar_platform_get_gpu_clock_step();
}

int mali_gpu_set_clock_step(int setting_clock_step)
{
    MALI_DEBUG_PRINT(4, ("mali_gpu_set_clock_step() called\n"));

    if (mstar_platform_set_gpu_clock_step(setting_clock_step) == true)
    {
        return MALI_TRUE;
    }
