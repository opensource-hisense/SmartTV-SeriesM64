...
#if defined(CONFIG_ARCH_DMA_ADDR_T_64BIT)
static mali_mem_allocator *mali_mem_block_allocator_create(u32 base_address, u64 cpu_usage_adjust, u32 size)
#else
...
#ifdef MSTAR
		info->all_blocks = _mali_osk_valloc(sizeof(block_info) * num_blocks);
#else
...
#ifdef MSTAR
	_mali_osk_vfree(info->all_blocks);
#else
...
#if defined(CONFIG_ARCH_DMA_ADDR_T_64BIT)
static int mali_mem_block_cpu_map(mali_mem_allocation *descriptor, struct vm_area_struct *vma, u32 mali_phys, u32 mapping_offset, u32 size, u64 cpu_usage_adjust)
#else
...
#if defined(CONFIG_ARCH_DMA_ADDR_T_64BIT)
	u64 cpu_phys = mali_phys + cpu_usage_adjust;
#else
...
#if defined(CONFIG_ARCH_DMA_ADDR_T_64BIT)
_mali_osk_errcode_t mali_memory_core_resource_dedicated_memory(u64 start, u32 size)
#else
...
#ifdef MSTAR
#if defined(CONFIG_ARCH_DMA_ADDR_T_64BIT)
	if (start < MSTAR_MIU1_BUS_BASE) {
		allocator = mali_mem_block_allocator_create(start, PHYS_TO_BUS_ADDRESS_ADJUST /* cpu_usage_adjust */, size);
	}
	else {
		/* TODO:Fix mapping strategy, CPU will not use bus address >= 0x2_8000_0000 for now */
		allocator = mali_mem_block_allocator_create(start, PHYS_TO_BUS_ADDRESS_ADJUST_MIU1 /* cpu_usage_adjust */, size);
	}
#else
	allocator = mali_mem_block_allocator_create(start, PHYS_TO_BUS_ADDRESS_ADJUST /* cpu_usage_adjust */, size);
#endif
