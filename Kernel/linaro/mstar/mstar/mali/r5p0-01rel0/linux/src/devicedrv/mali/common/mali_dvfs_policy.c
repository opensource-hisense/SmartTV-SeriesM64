...
/* MSTAR patch begin */
extern int dvfs_enable;
/** The max fps the same as display vsync default 60, can set by module insert parameter */
#ifndef MALI_MAX_SYSTEM_FPS
#define MALI_MAX_SYSTEM_FPS 60
#endif
int mali_max_system_fps = MALI_MAX_SYSTEM_FPS;
/** A lower limit on their desired FPS default 58, can set by module insert parameter */
#ifndef MALI_DESIRED_FPS
#ifdef MSTAR
#define MALI_DESIRED_FPS 60
#else
#define MALI_DESIRED_FPS 58
#endif
#endif
int mali_desired_fps = MALI_DESIRED_FPS;
/* MSTAR patch end */
...
void mali_dvfs_policy_realize(struct mali_gpu_utilization_data *data, u64 time_period)
{
    ...
    if (0 == dvfs_enable) {
        ...
        if (cur_clk_step == gpu_clk->num_of_steps - 1) {
            MALI_DEBUG_PRINT(5, ("Skip DVFS setting: dvfs_enable = %d\n", dvfs_enable));
            return;
        }
        else {
            clock_changed = true;
            clock_step = gpu_clk->num_of_steps - 1;
            MALI_DEBUG_PRINT(5, ("Disable DVFS and set GPU frequency to be maximum available value, clock_step = %d\n", clock_step));
            goto real_setting;
        }
    }
