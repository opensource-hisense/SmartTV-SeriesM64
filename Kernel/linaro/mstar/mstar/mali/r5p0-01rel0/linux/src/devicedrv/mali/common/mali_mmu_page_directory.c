...
#ifdef MSTAR
static inline struct mali_page_directory* _get_page_table_dir(void)
{
#ifdef MSTAR_VALLOC_PAGE_TABLE_DIR
	struct mali_page_directory* pagedir = _mali_osk_valloc(sizeof(struct mali_page_directory));

	if (NULL != pagedir) {
		memset(pagedir, 0, sizeof(struct mali_page_directory));
	}

	return pagedir;
#else
	return _mali_osk_calloc(1, sizeof(struct mali_page_directory));
#endif
}


static inline void _free_page_table_dir(struct mali_page_directory* pagedir)
{
#ifdef MSTAR_VALLOC_PAGE_TABLE_DIR
	_mali_osk_vfree(pagedir);
#else
