...
#if defined(CONFIG_MP_MM_CMA)
			mali_free_page(pfn_to_page(GET_BUS_ADDRESS(descriptor->block_array[pages_allocated].addr) >> PAGE_SHIFT));
#else
...
#if defined(CONFIG_MP_MM_CMA)
		mali_free_page(pfn_to_page(GET_BUS_ADDRESS(descriptor->block_array[i].addr)>>PAGE_SHIFT));
#else
