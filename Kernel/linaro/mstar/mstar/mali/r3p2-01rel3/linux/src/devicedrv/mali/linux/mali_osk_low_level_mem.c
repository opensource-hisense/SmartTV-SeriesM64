...
#if defined(CONFIG_MP_MM_CMA) || defined(CONFIG_MP_MM_MALI_RESERVE)
...
static int *ptr_pre_allocated_memory_size_current = NULL;
#if defined(CONFIG_MP_MM_CMA) || defined(CONFIG_MP_MM_MALI_RESERVE)
static AllocationList *pre_allocated_memory_miu[MSTAR_MIU_NUM];
static int pre_allocated_memory_size_current_miu[MSTAR_MIU_NUM];
#else
...
static int _get_pre_allocated_memory_size(void)
{
	int size;
#if defined(CONFIG_MP_MM_CMA) || defined(CONFIG_MP_MM_MALI_RESERVE)
	int i;
	...
	{
		size += pre_allocated_memory_size_current_miu[i];
	}
#else
	size = pre_allocated_memory_size_current;
#endif
	return size;
}
...
#endif
{
	...
	if (0 == nr)
	{
		return _get_pre_allocated_memory_size() / PAGE_SIZE;
	}

	if (0 == _get_pre_allocated_memory_size())
	{
		/* No pages availble */
		return 0;
	}
	...
	for (i = 0 ; i < MSTAR_MIU_NUM ; ++i )
	{
		pre_allocated_memory = pre_allocated_memory_miu[i];
		ptr_pre_allocated_memory_size_current = &pre_allocated_memory_size_current_miu[i];
#endif
		while (pre_allocated_memory && nr > 0)
		{
			...
			*ptr_pre_allocated_memory_size_current -= PAGE_SIZE;
			--nr;
		}
#if defined(CONFIG_MP_MM_CMA) || defined(CONFIG_MP_MM_MALI_RESERVE)
		pre_allocated_memory_miu[i] = pre_allocated_memory;
	}
#endif
...
#if defined(CONFIG_MP_MM_CMA) || defined(CONFIG_MP_MM_MALI_RESERVE)
int mali_osk_get_mstar_miu_id(u32 cpu_phys)
{
	if (cpu_phys < MSTAR_MIU1_BUS_BASE)
	{
		return 0;
	}
	else
...
void mali_osk_low_level_mem_init(void)
{
	...
	for ( i = 0 ; i < MSTAR_MIU_NUM ; i++)
	{
		pre_allocated_memory_miu[i] = (AllocationList*) NULL;
		pre_allocated_memory_size_current_miu[i]= 0;
	}
	
	ptr_pre_allocated_memory_size_current = (int*) NULL;
#else
	pre_allocated_memory_size_current = 0;
	ptr_pre_allocated_memory_size_current = &pre_allocated_memory_size_current;
#endif
...
#if defined(CONFIG_MP_MM_CMA) || defined(CONFIG_MP_MM_MALI_RESERVE)
		pre_allocated_memory_miu[i] = NULL;
#endif 
		*ptr_pre_allocated_memory_size_current  = 0;
#if defined(CONFIG_MP_MM_CMA) || defined(CONFIG_MP_MM_MALI_RESERVE)
	}
#endif
}
...
static u32 _kernel_page_allocate(void)
{
...
#elif defined(CONFIG_MP_MM_MALI_RESERVE)
	new_page = alloc_page(GFP_HIGHUSER | __GFP_ZERO | __GFP_REPEAT | __GFP_NOWARN | __GFP_MALI);
#elif defined(MIPS) && defined(HIGH_MEMORY_NOT_SUPPORTED)
	...
	new_page = alloc_page(GFP_HIGHUSER | __GFP_ZERO | __GFP_REPEAT | __GFP_NOWARN | __GFP_COLD | __GFP_NORMAL_MIU1);
#else
	new_page = alloc_page(GFP_HIGHUSER | __GFP_ZERO | __GFP_REPEAT | __GFP_NOWARN | __GFP_COLD);
#endif
...
static AllocationList * _allocation_list_item_get(void)
{
...
#if defined(CONFIG_MP_MM_CMA) || defined(CONFIG_MP_MM_MALI_RESERVE)
	pre_allocated_memory = pre_allocated_memory_miu[mali_strategy];
	ptr_pre_allocated_memory_size_current = &pre_allocated_memory_size_current_miu[mali_strategy];
#endif
	if ( pre_allocated_memory )
	{
		...
		*ptr_pre_allocated_memory_size_current -= PAGE_SIZE;
#if defined(CONFIG_MP_MM_CMA) || defined(CONFIG_MP_MM_MALI_RESERVE)
		pre_allocated_memory_miu[mali_strategy] = pre_allocated_memory;
#endif
...
static void _allocation_list_item_release(AllocationList * item)
{
	unsigned long flags;
#if defined(CONFIG_MP_MM_CMA) || defined(CONFIG_MP_MM_MALI_RESERVE)
	unsigned long mali_miu = mali_osk_get_mstar_miu_id(item->physaddr);
#endif

	spin_lock_irqsave(&allocation_list_spinlock,flags);
#if defined(CONFIG_MP_MM_CMA) || defined(CONFIG_MP_MM_MALI_RESERVE)
	pre_allocated_memory = pre_allocated_memory_miu[mali_miu];
	ptr_pre_allocated_memory_size_current = &pre_allocated_memory_size_current_miu[mali_miu];
#endif
	if ( *ptr_pre_allocated_memory_size_current < pre_allocated_memory_size_max)
	{
		item->next = pre_allocated_memory;
		pre_allocated_memory = item;
		*ptr_pre_allocated_memory_size_current += PAGE_SIZE;
#if defined(CONFIG_MP_MM_CMA) || defined(CONFIG_MP_MM_MALI_RESERVE)
		pre_allocated_memory_miu[mali_miu] = pre_allocated_memory;
#endif
...
#ifdef MSTAR
#ifdef MIPS
	virt = dma_alloc_coherent(NULL, size, phys, GFP_KERNEL | GFP_DMA | __GFP_NOWARN );
#else
	virt = dma_alloc_writecombine(NULL, size, phys, GFP_KERNEL | GFP_DMA | __GFP_NOWARN );
#endif
#else
...
#if defined(MIPS) && !defined(HIGH_MEMORY_NOT_SUPPORTED)
		_dma_cache_wback_inv(((u32)descriptor->mapping) + offset, size);
#endif
