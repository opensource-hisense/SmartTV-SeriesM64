...
#if MSTAR_ADJUST_GPU_CLOCK
#define GPU_MIN_CLOCK 216
#define GPU_MAX_CLOCK 396
#endif
...
void mstar_platform_power_on(void)
{
...
#if MSTAR_ADJUST_GPU_CLOCK
    G3D_REG(REG_GPUPLL_CTRL1_LOW)  = mali_gpu_clock/12;
#else
    ...
    else
    {
        ...
        /* Actually, inside the hardware design, the FIFO depth is only 32 */
        G3D_REG(REG_FIFO_DEPTH) = (G3D_REG(REG_FIFO_DEPTH) & ~MASK_FIFO_DEPTH)
            | ((0x0 << SHIFT_FIFO_DEPTH) & MASK_FIFO_DEPTH);
    }
#else
    /* set read request length to 48 for performance improvement only when GPU clock is 396 MHz */
    /* Actually, inside the hardware design, the FIFO depth is only 32 */
...
#if MSTAR_ADJUST_GPU_CLOCK
void mstar_platform_adjust_gpu_clock(void)
{
    if ((mali_gpu_clock < GPU_MIN_CLOCK) && (mali_gpu_clock > GPU_MAX_CLOCK))
    {
        return;
    }
