...
#if defined(MSTAR_RIU_ENABLED)
/* need mstar_riu_offset to get RIU offset for given Mali offset */
#include <linux/mali/mali_utgard.h>
#endif

#ifndef mstar_riu_offset
...
#ifdef  MSTAR_KAISER
extern unsigned int is_gpu_dual; 
#endif

#ifndef WATCHDOG_MSECS_DEFAULT
...
static u32 mali_get_bcast_id(_mali_osk_resource_t *resource_pp)
{
	...
	{
	case mstar_riu_offset(0x08000):
	case mstar_riu_offset(0x20000): /* fall-through for aliased mapping */
		return 0x01;
	case mstar_riu_offset(0x0A000):
	case mstar_riu_offset(0x22000): /* fall-through for aliased mapping */
		return 0x02;
	case mstar_riu_offset(0x0C000):
	case mstar_riu_offset(0x24000): /* fall-through for aliased mapping */
		return 0x04;
	case mstar_riu_offset(0x0E000):
	case mstar_riu_offset(0x26000): /* fall-through for aliased mapping */
		return 0x08;
	case mstar_riu_offset(0x28000):
		return 0x10;
	case mstar_riu_offset(0x2A000):
		return 0x20;
	case mstar_riu_offset(0x2C000):
		return 0x40;
	case mstar_riu_offset(0x2E000):
		return 0x80;
	default:
		return 0;
	}
...
#ifdef MSTAR_KAISER
	mali_max_pp_cores_group_1 = is_gpu_dual ? 2 : 1;
#endif
...
_mali_osk_errcode_t _mali_ukk_open(void **context)
{
	...
	{
		session->num_pages_allocated_miu[i] = 0;
	}

	/* 
	 * for Napoli mm BW issue,
	 * Must set all of mali memory to miu 0 by mali_strategy.
	 * However surfaceFlinger is out of the control strategy.
	 * We force set surfaceFlnger to miu 0
	 */
	if (_mali_osk_is_surfaceflinger())
	{
		session->use_miu = 0;
	}
	else
	{
		/* MSTAR_MIU_NUM means to use default value*/
		session->use_miu = MSTAR_MIU_NUM;
	}
#endif
