...
#if MSTAR_ADJUST_GPU_CLOCK
        [_MALI_DEVICE_ADJUST_GPU_CLOCK] = "adjust_gpu_clock",
#endif
};
...
#if MSTAR_ADJUST_GPU_CLOCK
	else if (!strncmp(ubuf,mali_power_events[_MALI_DEVICE_ADJUST_GPU_CLOCK],strlen(mali_power_events[_MALI_DEVICE_ADJUST_GPU_CLOCK])))
	{
		mali_dev_pause();
		mali_dev_adjust_gpu_clock();
		mali_dev_resume();
	}
#endif
	*ppos += cnt;
