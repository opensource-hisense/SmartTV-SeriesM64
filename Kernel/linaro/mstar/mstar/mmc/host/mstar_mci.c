...
static unsigned long long gu64_jiffies_tmp, gu64_jiffies_now;
...
)
{
	...
	if(12!=pCmd_st->opcode)
		//0864423: [Sage][eMMC]Sometime kernel EMMC driver will reboot twice
		//Add more delay time(500ms -> 1s) to wait Sage P5 the micron emmc(MTFC16GJV-EC-2M WT) is not busy
		if(eMMC_ST_SUCCESS != mstar_mci_WaitD0High(HW_TIMER_DELAY_1s))	
		{
			u32_ok_cnt = 0;
	...
	struct device_attribute *attr, char *buf)
{
    ...
    gu64_jiffies_now = jiffies_64;
    u32_total_sec = ((U32)(gu64_jiffies_now-gu64_jiffies_tmp))/HZ;
    u32_speed_read = (((gu32_eMMC_read_cnt<<eMMC_SECTOR_512BYTE_BITS)*10)/(0x100000*u32_total_sec)); 
    u32_speed_write = (((gu32_eMMC_write_cnt<<eMMC_SECTOR_512BYTE_BITS)*10)/(0x100000*u32_total_sec));
    printk("\n");
    printk("eMMC: read total count:%xh, %u.%uMBytes/sec ,total_sec:%u\n", gu32_eMMC_read_cnt,(u32_speed_read)/10, (u32_speed_read)%10, u32_total_sec);
    printk("eMMC: write total count:%xh, %u.%uMByte/sec ,total_sec:%u\n", gu32_eMMC_write_cnt, (u32_speed_write)/10, (u32_speed_write)%10, u32_total_sec);
...
static s32 mstar_mci_probe(struct platform_device *pDev_st)
{
    ...
    s32_ret = request_irq(E_IRQEXPH_NFIE, eMMC_FCIE_IRQ, IRQF_SHARED, DRIVER_NAME, pMStarHost_st);
    #else
