...
static long                   _mod_software_ir_ioctl(struct file *filp, unsigned int cmd, unsigned long arg);
#else
static int                      _mod_software_ir_ioctl(struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg);
#endif

SoftwareIRModHandle SIRDev=
{
    ...
    .IRFop=
    {
        ...
        #ifdef HAVE_UNLOCKED_IOCTL
        .unlocked_ioctl = _mod_software_ir_ioctl,
        #else        
...
static int __init mod_software_ir_init(void)
{
    ...
    if (SIRDev.s32IRMajor)
    {
        dev = MKDEV(SIRDev.s32IRMajor, SIRDev.s32IRMinor);
        ret = register_chrdev_region(dev, MOD_SOFTWARE_IR_DEVICE_COUNT, MDRV_NAME_SOFTWARE_IR);
    }
    else
    {
        ret = alloc_chrdev_region(&dev, SIRDev.s32IRMinor, MOD_SOFTWARE_IR_DEVICE_COUNT, MDRV_NAME_SOFTWARE_IR);
        SIRDev.s32IRMajor = MAJOR(dev);
    }
    ...
    cdev_init(&SIRDev.cDevice, &SIRDev.IRFop);
    if (0!= (ret= cdev_add(&SIRDev.cDevice, dev, MOD_SOFTWARE_IR_DEVICE_COUNT)))
    {
...
static void __exit mod_software_ir_exit(void)
{
    ...
    cdev_del(&SIRDev.cDevice);
    unregister_chrdev_region(MKDEV(SIRDev.s32IRMajor, SIRDev.s32IRMinor), MOD_SOFTWARE_IR_DEVICE_COUNT);
