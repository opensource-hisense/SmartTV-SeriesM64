...
static void mmu_update(struct kbase_context *kctx)
{
	...
	/* MSTAR patch begin */
#ifdef MSTAR
	current_setup->transtab = (u64) BUS_TO_MIU_ADDRESS(kctx->pgd) &
...
static void entry_set_ate(u64 *entry, phys_addr_t phy, unsigned long flags)
{
	/* MSTAR patch begin */
#ifdef MSTAR
	page_table_entry_set(entry, (BUS_TO_MIU_ADDRESS(phy) & ~0xFFF) |
#else
...
static void entry_set_pte(u64 *entry, phys_addr_t phy)
{
	/* MSTAR patch begin */
#ifdef MSTAR
	page_table_entry_set(entry, (BUS_TO_MIU_ADDRESS(phy) & ~0xFFF) | ENTRY_IS_PTE);
#else
