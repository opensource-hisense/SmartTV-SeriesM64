...
struct mali_dvfs {
	/* dvfs enable switch */
	u32 dvfs_enable;
	/* indicates current frequency scale */
	u32 current_dvfs_scale;
	/* indictes max currency frequency scale */
	//FIXME: is it required?
	u16 max_dvfs_scale;
	/* this callback sets dvfs switch to enable */
	bool (*enable)(struct kbase_device *);
	/* this callback turn off dvfs mechanism and restore device
	 * frequency to max frequency */
	bool (*disable)(struct kbase_device *);
	/* frequency clock up */
	bool (*gpu_clock_up)(struct kbase_device *);
	/* frequency clock down */
	bool (*gpu_clock_down)(struct kbase_device *);
};
#endif
/* MSTAR patch end */
