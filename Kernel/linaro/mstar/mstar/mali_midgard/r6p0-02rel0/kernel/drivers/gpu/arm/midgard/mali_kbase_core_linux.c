...
#include <../drivers/mstar2/include/mst_devid.h>
#else
...
copy_failed:
	...
	/* MSTAR patch begin */
#if MSTAR_MEM_EXPORT_UMM
	case KBASE_FUNC_MEM_EXPORT_UMM :
		{
			struct kbase_uk_mem_export_umm *mem_export_umm =
					(struct kbase_uk_mem_export_umm *)args;

			if (sizeof(*mem_export_umm) != args_size)
				goto bad_size;

#ifdef CONFIG_DMA_SHARED_BUFFER
			mem_export_umm->fd = mem_block_export_dma_buf(mem_export_umm->base,
					mem_export_umm->size, mem_export_umm->is_bus_address);
#else
			mem_export_umm->fd = -1;
			ukh->ret = MALI_ERROR_FUNCTION_FAILED;
#endif
...
void kbase_os_reg_write(struct kbase_device *kbdev, u16 offset, u32 value)
{
	...
	*(volatile u32*)(kbdev->reg + offset) = value;
#else
...
u32 kbase_os_reg_read(struct kbase_device *kbdev, u16 offset)
{
	/* MSTAR patch begin */
#ifdef MSTAR_RIU_ENABLED
	return *(volatile u32*)(kbdev->reg + offset);
#else
...
/** Show callback for the @c dvfs sysfs file.
 *
 * This function is called to get the contents of the @c dvfs_enable
 * sysfs file.
 *
 * @param devThe device this sysfs file is for
 * @param attrThe attributes of the sysfs file
 * @param bufThe output buffer for the sysfs file contents
 *
 * @return The number of bytes output to @c buf.
 **/

static ssize_t show_dvfs_enable(struct device *dev, struct device_attribute *attr, char *const buf)
{
	struct kbase_device *kbdev;
	ssize_t ret = 0;
	u32 dvfs_enable;
	...
	dvfs_enable = kbdev->dvfs_enable;
	ret += scnprintf(buf, PAGE_SIZE, "%d\n", dvfs_enable);

	return ret;
}

/** Store callback for the @c dvfs_enable sysfs file.
 *
 * This function is called when the @c dvfs_enable sysfs file is
 * written to. It determines the written value is valid and perform
 * corresponding dvfs enabling switch.
 *
 * @param devThe device with sysfs file is for
 * @param attrThe attributes of the sysfs file
 * @param bufThe value written to the sysfs file
 * @param countThe number of bytes written to the sysfs file
 *
 * @return @c count if the function succeeded. An error code on failure.
 **/

static ssize_t set_dvfs_enable(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct kbase_device *kbdev;
	u32 dvfs_enable = 0;
	kbdev = to_kbase_device(dev);
	...
	if (kbdev->dvfs.enable == NULL || kbdev->dvfs.disable == NULL )
	{
			return -EACCES;
	}
	...
	/* set dvfs enable switch */
	if (dvfs_enable == 0)
		kbdev->dvfs.disable(kbdev);
	else
		kbdev->dvfs.enable(kbdev);

	return count;
}

/** The sysfs file @c dvfs_enable
 *
 * This is used for obtaining information about the DVFS enabling status,
 * switching whether to enable DVFS or not.
 **/
static DEVICE_ATTR(dvfs_enable, S_IRUGO | S_IWUSR, show_dvfs_enable, set_dvfs_enable);


/**
 * set_gpu_max_freq - Store callback for the gpu_max_freq sysfs file.
 * @dev:   The device with sysfs file is for
 * @attr:  The attributes of the sysfs file
 * @buf:   The value written to the sysfs file
 * @count: The number of bytes written to the sysfs file
 *
 * This function is called when the gpu_max_freq sysfs file is written to. It
 * checks the data written, and if valid updates the gpu maximum frequency,
 *
 * Return: @c count if the function succeeded. An error code on failure.
 */
static ssize_t set_gpu_max_freq(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct kbase_device *kbdev;
	int ret;
	...
	ret = kstrtoint(buf, 0, &gpu_max_freq);
	if (ret) {
		dev_err(kbdev->dev, "Couldn't process gpu_max_freq write operation.\n");
		return -EINVAL;
	}

	if (gpu_max_freq == kbdev->gpu_max_freq) {
		dev_err(kbdev->dev, "Ignore gpu_max_freq write operation.\n"
			"%d MHz is the same with original value of gpu_max_freq\n", kbdev->gpu_max_freq);
		return -EINVAL;
	}

	if ((gpu_max_freq <= kbdev->gpu_min_freq) || (gpu_max_freq > MALI_MAX_FREQ)) {
		dev_err(kbdev->dev, "Ignore gpu_max_freq write operation.\n"
			"%d MHz is out of boundary of [%d,%d)\n", gpu_max_freq, kbdev->gpu_min_freq, MALI_MAX_FREQ);
		return -EINVAL;
	}

	kbdev->gpu_max_freq = gpu_max_freq;
	dev_dbg(kbdev->dev, "gpu_max_freq: %d MHz\n", gpu_max_freq);

#ifdef MSTAR
	if (mali_gpu_clock > gpu_max_freq) {
		dev_err(kbdev->dev, "gpu_clock_down.\n"
			"mali_gpu_clock: %d > gpu_max_freq: %d\n", mali_gpu_clock, gpu_max_freq);
		kbdev->dvfs.gpu_clock_down(kbdev);
	}
#endif
	return count;
}
...
/**
 * show_gpu_max_freq - Show callback for the gpu_max_freq sysfs entry.
 * @dev:  The device this sysfs file is for.
 * @attr: The attributes of the sysfs file.
 * @buf:  The output buffer to receive the GPU information.
 *
 * This function is called to get the gpu maximum frequency.
 *
 * Return: The number of bytes output to buf.
 */
static ssize_t show_gpu_max_freq(struct device *dev,
		struct device_attribute *attr, char * const buf)
{
	struct kbase_device *kbdev;
	ssize_t ret;
...
static DEVICE_ATTR(gpu_max_freq, S_IRUGO | S_IWUSR, show_gpu_max_freq, set_gpu_max_freq);


/**
 * set_gpu_min_freq - Store callback for the gpu_min_freq sysfs file.
 * @dev:   The device with sysfs file is for
 * @attr:  The attributes of the sysfs file
 * @buf:   The value written to the sysfs file
 * @count: The number of bytes written to the sysfs file
 *
 * This function is called when the gpu_min_freq sysfs file is written to. It
 * checks the data written, and if valid updates the gpu minimum frequency,
 *
 * Return: @c count if the function succeeded. An error code on failure.
 */
static ssize_t set_gpu_min_freq(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct kbase_device *kbdev;
	int ret;
	...
	ret = kstrtoint(buf, 0, &gpu_min_freq);
	if (ret) {
		dev_err(kbdev->dev, "Couldn't process gpu_min_freq write operation.\n");
		return -EINVAL;
	}

	if (gpu_min_freq == kbdev->gpu_min_freq) {
		dev_err(kbdev->dev, "Ignore gpu_min_freq write operation.\n"
			"%d MHz is the same with original value of gpu_min_freq\n", kbdev->gpu_min_freq);
		return -EINVAL;
	}

	if ((gpu_min_freq >= kbdev->gpu_max_freq) || (gpu_min_freq < MALI_MIN_FREQ)) {
		dev_err(kbdev->dev, "Ignore gpu_min_freq write operation.\n"
			"%d MHz is out of boundary of (%d,%d]\n", gpu_min_freq, MALI_MIN_FREQ, kbdev->gpu_max_freq);
		return -EINVAL;
	}

	kbdev->gpu_min_freq = gpu_min_freq;
	dev_dbg(kbdev->dev, "gpu_min_freq: %d MHz\n", gpu_min_freq);

#ifdef MSTAR
	if (mali_gpu_clock < gpu_min_freq) {
		dev_err(kbdev->dev, "gpu_clock_up.\n"
			"mali_gpu_clock: %d < gpu_min_freq: %d\n", mali_gpu_clock, gpu_min_freq);
		kbdev->dvfs.gpu_clock_up(kbdev);
	}
#endif
...
/**
 * show_gpu_min_freq - Show callback for the gpu_min_freq sysfs entry.
 * @dev:  The device this sysfs file is for.
 * @attr: The attributes of the sysfs file.
 * @buf:  The output buffer to receive the GPU information.
 *
 * This function is called to get the gpu minimum frequency.
 *
 * Return: The number of bytes output to buf.
 */
static ssize_t show_gpu_min_freq(struct device *dev,
		struct device_attribute *attr, char * const buf)
{
	struct kbase_device *kbdev;
	ssize_t ret;
...
static DEVICE_ATTR(gpu_min_freq, S_IRUGO | S_IWUSR, show_gpu_min_freq,
		set_gpu_min_freq);
...
static int kbase_common_reg_map(struct kbase_device *kbdev)
{
	...
	/* MSTAR patch begin */
#ifdef MSTAR_RIU_ENABLED
	kbdev->reg = (mstar_riu_address)(uintptr_t)kbdev->reg_start;
#else
...
/* MSTAR patch begin */
#if MSTAR_USE_FIXED_DEVID
static int kbase_device_register(struct mali_device *mdev)
{
	int err;
	dev_t dev = MKDEV(MDRV_MAJOR_MALI, 0);

	KBASE_DEBUG_ASSERT(NULL != mdev);

	/* register device numbers */
	err = register_chrdev_region(dev, 1, kbase_drv_name);
	...
	/* add our char device */
	cdev_init(&mdev->cdev, &kbase_fops);
	mdev->cdev.owner = THIS_MODULE;
	mdev->cdev.ops = &kbase_fops;
	...
	/* create mali class */
	mdev->mali_class = class_create(THIS_MODULE, kbase_drv_name);

	if (IS_ERR(mdev->mali_class))
	{
		err = PTR_ERR(mdev->mali_class);
		goto out_class_create;
	}

	/* create mali device */
	mdev->dev = device_create(mdev->mali_class, NULL, dev, NULL, kbase_drv_name);

	if (IS_ERR(mdev->dev))
	{
		err = PTR_ERR(mdev->dev);
...
static void kbase_device_deregister(struct mali_device *mdev)
{
	dev_t dev = MKDEV(MDRV_MAJOR_MALI, 0);

	KBASE_DEBUG_ASSERT(NULL != mdev);

	device_destroy(mdev->mali_class, dev);
	class_destroy(mdev->mali_class);
	cdev_del(&mdev->cdev);
	unregister_chrdev_region(dev, 1);
}
...
static int kbase_common_device_init(struct kbase_device *kbdev)
{
    ...
    /* MSTAR patch begin */
#if MSTAR_USE_FIXED_DEVID
    err = kbase_device_register(&kbdev->mdev);
#else
...
#ifdef MSTAR_ENABLE_DVFS
	if (device_create_file(kbdev->dev, &dev_attr_dvfs_enable)) {
		dev_err(kbdev->dev, "Couldn't create dvfs_enable sysfs file\n");
		goto out_file_dvfs_enable;
	}

	if (device_create_file(kbdev->dev, &dev_attr_gpu_max_freq)) {
		dev_err(kbdev->dev, "Couldn't create gpu_max_freq sysfs file\n");
		goto out_file_gpu_max_freq;
	}

	if (device_create_file(kbdev->dev, &dev_attr_gpu_min_freq)) {
		dev_err(kbdev->dev, "Couldn't create gpu_min_freq sysfs file\n");
		goto out_file_gpu_min_freq;
	}
#endif
	...
	/* MSTAR patch begin */
#ifdef MSTAR_ENABLE_DVFS
out_file_dvfs_enable:
	device_remove_file(kbdev->dev, &dev_attr_dvfs_enable);
out_file_gpu_max_freq:
	device_remove_file(kbdev->dev, &dev_attr_gpu_max_freq);
out_file_gpu_min_freq:
	device_remove_file(kbdev->dev, &dev_attr_gpu_min_freq);
#endif
	/* MSTAR patch end */
...
out_file:
	/* MSTAR patch begin */
#if MSTAR_USE_FIXED_DEVID
	kbase_device_deregister(&kbdev->mdev);
#else
...
static int kbase_platform_device_probe(struct platform_device *pdev)
{
	...
	/* MSTAR patch begin */
#ifdef CONFIG_ARM64
	kbdev->reg_start = reg_res->start + mstar_pm_base;
#else
...
/* MSTAR patch begin */
#ifdef MSTAR
/** Freeze callback from the OS.
 *
 * This is called by Linux when the device should freeze.
 *
 * @param dev  The device to freeze
 *
 * @return A standard Linux error code
 */
static int kbase_device_freeze(struct device *dev)
{
	struct kbase_device *kbdev = to_kbase_device(dev);
...
/** Thaw callback from the OS.
 *
 * This is called by Linux when the device should thaw from freeze.
 *
 * @param dev  The device to thaw
 *
 * @return A standard Linux error code
 */
static int kbase_device_thaw(struct device *dev)
{
	struct kbase_device *kbdev = to_kbase_device(dev);
...
/** Restore callback from the OS.
 *
 * This is called by Linux when the device should restore.
 *
 * @param dev  The device to restore
 *
 * @return A standard Linux error code
 */
static int kbase_device_restore(struct device *dev)
{
	struct kbase_device *kbdev = to_kbase_device(dev);
...
static const struct dev_pm_ops kbase_pm_ops = {
	...
	/* MSTAR patch begin */
#ifdef MSTAR
	.freeze = kbase_device_freeze,
	.thaw = kbase_device_thaw,
	.restore = kbase_device_restore,
#endif
	/* MSTAR patch end */
#ifdef CONFIG_PM_RUNTIME
...
module_param(mali_gpu_utilization, int, S_IRUGO); /* r--r--r-- */
