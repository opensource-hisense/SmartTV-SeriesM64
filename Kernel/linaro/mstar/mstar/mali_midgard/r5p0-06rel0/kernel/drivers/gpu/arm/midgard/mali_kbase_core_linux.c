...
static ssize_t set_dvfs_enable(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(!kbdev->dvfs_op.enable || !kbdev->dvfs_op.disable)
		return -EACCES;
	...
	if (dvfs_enable == 0)
		kbdev->dvfs_op.disable(kbdev);
	else
		kbdev->dvfs_op.enable(kbdev);
...
static int kbase_common_device_init(struct kbase_device *kbdev)
{
	...
	/* MSTAR patch begin */
#if MSTAR_USE_FIXED_DEVID
	if (kbase_device_register(&kbdev->mdev)) {
#else
	...
	if (device_create_file(kbdev->dev, &dev_attr_dvfs_enable)) {
		dev_err(kbdev->dev, "Couldn't create enable sysfs file\n");
