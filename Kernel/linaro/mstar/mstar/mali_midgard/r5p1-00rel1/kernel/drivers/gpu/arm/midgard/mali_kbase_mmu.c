...
phys_addr_t kbase_mmu_alloc_pgd(struct kbase_context *kctx)
{
	...
	/* MSTAR patch begin */
#ifdef MSTAR
	kbase_update_max_pages(&kctx->used_pages, &kctx->max_used_pages);
	kbase_update_max_pages(&kctx->kbdev->memdev.used_pages,&kctx->kbdev->memdev.max_used_pages);
#endif
	/* MSTAR patch end */
...
static phys_addr_t mmu_pte_to_phy_addr(u64 entry)
{
	...
	/* MSTAR patch begin */
#ifdef MSTAR
	return MIU_TO_BUS_ADDRESS(entry & ~0xFFF);
#else
	return entry & ~0xFFF;
#endif
	/* MSTAR patch end */
...
static u64 mmu_phyaddr_to_pte(phys_addr_t phy)
{
	/* MSTAR patch begin */
#ifdef MSTAR
	return (BUS_TO_MIU_ADDRESS(phy) & ~0xFFF) | ENTRY_IS_PTE;
#else
...
static u64 mmu_phyaddr_to_ate(phys_addr_t phy, u64 flags)
{
	/* MSTAR patch begin */
#ifdef MSTAR
	return (BUS_TO_MIU_ADDRESS(phy) & ~0xFFF) | (flags & ENTRY_FLAGS_MASK) | ENTRY_IS_ATE;
#else
