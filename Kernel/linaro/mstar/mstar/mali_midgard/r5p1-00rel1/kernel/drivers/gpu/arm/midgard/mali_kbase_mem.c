...
/* MSTAR patch begin */
#ifdef MSTAR
#include <mali_kbase_mem_mstar.h>
#endif
/* MSTAR patch end */
...
void kbase_mmu_update(struct kbase_context *kctx)
{
	...
	/* MSTAR patch begin */
#ifdef MSTAR
	current_setup->transtab = (u64) BUS_TO_MIU_ADDRESS(kctx->pgd) & ((0xFFFFFFFFULL << 32) | AS_TRANSTAB_ADDR_SPACE_MASK);
#else
	...
	size_t nr_pages_requested)
{
	...
	/* MSTAR patch begin */
#ifdef MSTAR
	kbase_update_max_pages(&alloc->imported.kctx->used_pages, &alloc->imported.kctx->max_used_pages);
	kbase_update_max_pages(&alloc->imported.kctx->kbdev->memdev.used_pages,&alloc->imported.kctx->kbdev->memdev.max_used_pages);
#endif
	/* MSTAR patch end */
