...
static enum hrtimer_restart dvfs_callback(struct hrtimer *timer)
{
	...
	/* MSTAR patch begin */
#ifdef CONFIG_MALI_MIDGARD_DVFS
	if (metrics->kbdev->dvfs_enable != 0) {
		switch (action)
		{
			case KBASE_PM_DVFS_NOP:
				break;
			case KBASE_PM_DVFS_CLOCK_UP:
				metrics->kbdev->dvfs_op.gpu_clock_up(metrics->kbdev);
				break;
			case KBASE_PM_DVFS_CLOCK_DOWN:
				metrics->kbdev->dvfs_op.gpu_clock_down(metrics->kbdev);        
				break;
		}
	}
#endif
...
out:
	...
	/* MSTAR patch begin */
#ifdef MSTAR
	mali_gpu_utilization = utilisation;
#endif
	/* MSTAR patch end */
