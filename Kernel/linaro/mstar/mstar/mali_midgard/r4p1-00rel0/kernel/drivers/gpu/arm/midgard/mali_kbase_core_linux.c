...
bad_type:
	...
	case KBASE_FUNC_MEM_ALIAS: {
			...
			/* MSTAR patch begin */
			ai = kmalloc(sizeof(*ai) * alias->nents, GFP_KERNEL);
			/* MSTAR patch end */
...
static int kbase_common_reg_map(kbase_device *kbdev)
{
	...
	/* MSTAR patch begin */
#ifdef MSTAR_RIU_ENABLED
	kbdev->reg = (mstar_riu_address)kbdev->reg_start;
#else
...
static int kbase_common_device_init(kbase_device *kbdev)
{
	...
	/* MSTAR patch begin */
#ifdef MSTAR
	/* set dma mask init value for using DS-5 */
	u64 mask = 0xffffffff;
	dev_set_drvdata(kbdev->dev, kbdev);

	if (dma_set_coherent_mask(kbdev->dev, mask) != 0) {
		dev_err(kbdev->dev, "Set dma mask fail\n");
	}
#else
	dev_set_drvdata(kbdev->dev, kbdev);
#endif
...
static int kbase_device_freeze(struct device *dev)
{
	...
	kbase_pm_suspend(kbdev); /* call suspend */
	return 0;
}
...
static int kbase_device_thaw(struct device *dev)
{
	...
	kbase_pm_resume(kbdev); /* call resume */
	return 0;
}
