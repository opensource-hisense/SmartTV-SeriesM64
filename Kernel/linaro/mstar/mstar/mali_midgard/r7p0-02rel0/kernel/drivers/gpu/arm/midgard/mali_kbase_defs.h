...
struct mstar_dvfs
{
	/* states */
	u32 enabled;
	u32 current_scale;	/* current frequency scale */

	/* callbacks */
	bool (*enable)(struct kbase_device *);		/* enable DVFS */
	bool (*disable)(struct kbase_device *);		/* disable DVFS. the frequency will be restored to the max freqency */
...
struct kbase_device {
	...
	/* the following data should be kept in the above "platform_context",
	 * but we place them here to avoid additional memory allocation/free operations
	 */
#ifdef MSTAR
#ifdef CONFIG_MALI_MIDGARD_DVFS
	struct mstar_dvfs dvfs;
#endif
#endif
