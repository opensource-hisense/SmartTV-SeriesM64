...
/* MStar: module parameter to control log level */
#ifdef MSTAR
...
/* MStar: GPU utilization */
#ifdef MSTAR_GPU_UTILIZATION
...
#if MSTAR_UMM_IMPORT
	case KBASE_FUNC_MEM_UMM_IMPORT:
		{
			struct kbase_uk_mem_umm_import *import = (struct kbase_uk_mem_umm_import *)args;

			if (sizeof(*import) != args_size)
				goto bad_size;

#ifdef CONFIG_DMA_SHARED_BUFFER
			import->fd = mem_block_import_dma_buf(import->base,
												  import->size,
												  import->is_bus_address);
#else
			import->fd = -1;
			ukh->ret = MALI_ERROR_FUNCTION_FAILED;
#endif
...
static ssize_t set_dvfs_enable(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if ((NULL == kbdev->dvfs.enable) || (NULL == kbdev->dvfs.disable))
	{
		return -EACCES;
	}

	dvfs_enabled = kbdev->dvfs.enabled;
...
#ifdef CONFIG_OF /* MStar: add to avoid the compiler warning of unused variables */
...
#if defined(MSTAR) && defined(CONFIG_MALI_MIDGARD_DVFS)
