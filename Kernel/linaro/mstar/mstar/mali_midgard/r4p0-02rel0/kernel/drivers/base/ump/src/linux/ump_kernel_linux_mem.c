...
int umpp_phys_commit(umpp_allocation * alloc)
{
	...
	for (i = 0; i < alloc->blocksCount; i++)
	{
		...
		struct page * page = alloc_page(GFP_HIGHUSER | __GFP_NORETRY | __GFP_REPEAT | __GFP_COLD);
#elif defined(CONFIG_ZONE_NORMAL_MIU0)  /* allocate memory from MIU0 */
#warning "review the combination of GFP_USER and kmap"
		struct page * page = alloc_page(GFP_USER | __GFP_NORETRY | __GFP_REPEAT | __GFP_COLD | __GFP_NORMAL_MIU0);
#elif defined(CONFIG_ZONE_NORMAL_MIU1)  /* allocate memory from MIU1 */
		struct page * page = alloc_page(GFP_HIGHUSER | __GFP_NORETRY | __GFP_REPEAT | __GFP_COLD | __GFP_NORMAL_MIU1);
#else
