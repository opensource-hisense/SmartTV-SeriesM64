...
MBX_Result _MHAL_MBXINT_SetHostCPU(MBX_CPU_ID eHostCPU)
{
	...
	s32Err = request_irq(E_FIQEXPL_SECURE_R2_TO_ARM, _MHAL_MBXINT_INTHandler, SA_INTERRUPT, "MBX_FIQ_R2toMIPS", NULL);
            if(s32Err<0)
            {
...
MBX_Result MHAL_MBXINT_Fire (MBX_CPU_ID dstCPUID, MBX_CPU_ID srcCPUID)
{
    ...
    switch(srcCPUID)
    {
        ...
        case E_MBX_CPU_MIPS: //HK to PM
		    ...
		    {
				//Note: (very import)
				//Temporarily use "host0 to host1" to notify host1  replacing with "host2 to host1" 
                //HW designer must CEO for new version chip

           	    CPU_INT_REG(REG_INT_PMFIRE) = INT_PM_AEON; //to R2
				mb();
           	    CPU_INT_REG(REG_INT_PMFIRE) = 0;
				mb();
