...
{
    MS_BOOL bRet = FALSE;
#if  (CONFIG_RIUISP ||CONFIG_DUAL_READ || CONFIG_QUAL_READ ||CONFIG_BURST_READ)
...
HAL_SERFLASH_Write_return:
    ...
    if( (gReadMode==E_QUAD_MODE)||(gReadMode==E_DUAL_AD_MODE))
   {
	 ...
	 PM_WRITE_MASK(REG_PM_SPI_IS_GPIO, PM_SPI_WP_IS_GPIO, PM_SPI_WP_GPIO_MASK);
    	...
    	bRet =HAL_FSP_Write_Burst(u32Addr, u32Size, pu8Data); 	   
...
MS_BOOL HAL_SERFLASH_Read(MS_U32 u32Addr, MS_U32 u32Size, MS_U8 *pu8Data)
{
	...
	{
		case  E_FAST_MODE:
			Ret = HAL_FSP_BurstRead(u32Addr, u32Size, pu8Data);
		break;
		case E_DUAL_AD_MODE:
		case E_QUAD_MODE:
			  _HAL_BDMA_READ(u32Addr, u32Size, pu8Data);
...
HAL_SERFLASH_ReadREMS4_return:
    ...
    MsOS_ReleaseMutex(_s32SERFLASH_Mutex);
#else
    //HAL_FSP_ReadREMS4(pu8Data, u32Size);
    return TRUE;
#endif
return bRet;
...
MS_BOOL HAL_FSP_WriteProtect(MS_BOOL bEnable)
{
   ...
   // _HAL_SERFLASH_ActiveFlash_Set_HW_WP(0);
    ...
    bRet &= _HAL_FSP_WaitDone();
      ...
      MS_U8  u8StatusReg;
	HAL_FSP_ReadStatusReg(&u8StatusReg);
	printk("@@@@@@status%x\n",u8StatusReg);
...
#endif
    HAL_FSP_EX_WriteStatusReg((WRITE_STATUS_BYTE)TWO_BYTE,0xffff);
