...
{
        unsigned int WCR;
        unsigned int u32Wvr = 0;
        unsigned int u32Wcr = 0;
        ...
        u32Wcr |= (info.mask << 24); /*shift 24 is because the mask control bit is defined there*/
        u32Wvr = info.u32AddrVirt;

        /* Set watchpoint link to a breakpoint for context matching,
           or the watchpoing may trigger by inocent processes Since it uses virtual address */
        if(!info.global) {
            /* HMC/BAS/PMC Set Breakpoint state control to all modes */
            /* PMC must set to 0b11 if BT set to linked Context Matching */
            /* E Set to 0b1 to enable the breakpoint */
            /*                     BT               HMC           BAS             PMC           E       */
            unsigned int u32Bcr = (0b0011 << 20) | (0b1 << 13) | (0b1111 << 5) | (0b11 << 1) | (0b1 << 0);
            unsigned int u32Bvr = 0;

            do {
                asm volatile("mrc p15, 0, %0, c13, c0, 1" : "=r" (u32Bvr));   // Get CONTEXTIDR, write into u32Bvr
            } while (0);

            /* Use DBGBCR/DBGBVR here, since breakpoint n in <0> ~ <N-1>
                has context ID comparison capability only if n >= 4.*/
            #if LINUX_VERSION_CODE == KERNEL_VERSION(3,8,13)
                ARM_DBG_WRITE(c0, c4, 4, u32Bvr); // write 5th DBGBVR
                ARM_DBG_WRITE(c0, c4, 5, u32Bcr); // write 5th DBGBCR
            #else
                ARM_DBG_WRITE(c4, 4, u32Bvr); // write 5th DBGBVR
                ARM_DBG_WRITE(c4, 5, u32Bcr); // write 5th DBGBCR
            #endif

            // Set Watchpoint link to breakpoint 4 for Context Matching
            /*          LBN              WT */
            u32Wcr |= ((0b0100 << 16) | (0b1 << 20));
        }

        #if LINUX_VERSION_CODE == KERNEL_VERSION(3,8,13)
            ARM_DBG_WRITE(c0, c0, 6, u32Wvr); // read 0th DBGWVR
            ARM_DBG_WRITE(c0, c0, 7, u32Wcr); // read 0th DBGWCR
        #else
            ARM_DBG_WRITE(c0, 6, u32Wvr); // read 0th DBGWVR
            ARM_DBG_WRITE(c0, 7, u32Wcr); // read 0th DBGWCR
        #endif
