...
#endif
#ifdef CONFIG_SPI_MSPI
#include <linux/spi/spi.h>
#include <linux/spi/mspi_spi.h>
#endif
#ifdef CONFIG_SPI_ISP
#include <linux/spi/spi.h>
#include <linux/spi/isp_spi.h>
#endif
...
int ms_mstar_set_vbus(unsigned int on)
{
#if 0	//fix it
	printk("[OTG] mstar_set_vbus %d\n", on);
...
static struct platform_device *chip_devices[] __initdata = {
    ...
    &Mstar_iic_device,
#endif
#if defined(CONFIG_MSTAR_GPIO_MODULE) || defined(CONFIG_MSTAR_GPIO)
    &Mstar_gpio_device,
#endif
...
#ifdef CONFIG_SPI_MSPI
static struct mspi_spi_info mspi_spi_platdata={
     .num_cs = 1,
     .bus_num = 0,
};

static struct spi_board_info mspi_spi_board={
       .modalias = "spidev",
       .bus_num = 0,
       .chip_select = 0,
...
#ifdef CONFIG_SPI_ISP
static struct isp_spi_info isp_spi_platdata = {
    .num_cs = 1,
    .bus_num = 0,
};

static struct spi_board_info isp_spi_board={
     .modalias = "gp50x",
     .bus_num = 0,
     .chip_select = 0,
...
#ifdef CONFIG_SPI_MSPI
    Mstar_mspi_device.dev.platform_data = &mspi_spi_platdata;
    spi_register_board_info(&mspi_spi_board, 1); 
#endif

#ifdef CONFIG_SPI_ISP
    Mstar_isp_device.dev.platform_data = &isp_spi_platdata;
    spi_register_board_info(&isp_spi_board, 1); 
#endif
