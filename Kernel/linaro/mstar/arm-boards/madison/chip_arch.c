...
void __init chip_map_io(void)
{
    ...
    twd_base = ((void __iomem *) PERI_ADDRESS(0x16000600)); //private timer and watchdog base
#endif
...
void __init serial_init(void)
{
    ...
    memset(&s, 0, sizeof(s));
    s.type = PORT_16550;
    s.iobase = (0xFD000000 + (0x100980 << 1));  //virtual RIU_BASE = 0xFD000000
    s.irq = E_IRQ_UART0;

#ifdef CONFIG_MSTAR_ARM_BD_FPGA
    s.uartclk = 12000000;   //FPGA use 12MHz
#else
    s.uartclk = 123000000;  //ASIC use 123MHz
#endif
    s.iotype = 0;
    s.regshift = 0;
    ...
    memset(&u, 0, sizeof(u));
    u.type = PORT_16550;
    u.iobase = (0xFD000000 + (0x110600 << 1));  //0xFD220C00;
    ...
    memset(&k, 0, sizeof(k));
    k.type = PORT_16550;
    k.iobase = (0xFD000000 + (0x110680 << 1));  //0xFD220D00;
...
static irqreturn_t chip_xiu_timeout_interrupt(int irq, void *dev_id)
{
    ...
    printk("Address is 0x%x\n",*(volatile short*)(0xFD000000 + (0x100112 << 1)));
...
static int recovery_sys_notify_reboot(struct notifier_block *this, unsigned long code, void *x)
{
    if(x && strlen((char*) x))
    {
        mm_segment_t    old_fs;
...
static void __init mstar_reserve(void)
{
...
static void __init chip_board_init(void)
{
    ...
    uncached_buf=dma_alloc_coherent(NULL, 4, &buf_phy_addr, GFP_KERNEL);
...
#ifdef CONFIG_CACHE_L2X0
{
    ...
    *(volatile unsigned short *)(0xfd000000 + (0x100100 << 1)) = 0x0001;
    *(volatile unsigned short *)(0xfd000000 + (0x100128 << 1)) = 0xffff;
    *(volatile unsigned short *)(0xfd000000 + (0x10012a << 1)) = 0xffff;
...
inline void _chip_flush_miu_pipe(void)
{
    ...
    *(volatile unsigned short *)(0xfd000000 + (0x10188a << 1)) &= ~(0x0001);
    *(volatile unsigned short *)(0xfd000000 + (0x10188a << 1)) |= 0x0001;

    do
    {
        dwReadData = *(volatile unsigned short *)(0xfd000000 + (0x1018a0 << 1));
...
unsigned int Chip_Query_Rev(void)
{
    ...
    dwRevisionId = *(volatile unsigned int*)(0xfd000000 + (0x001ece << 1));
    dwRevisionId >>= 8;
...
//control error code
#define ERR_CTRL_RECORD_FOUND       (0x0600 | 0x01)
#define ERR_CTRL_RECORD_INVALID     (0x0600 | 0x02)
#define ERR_CTRL_RECORD_FAIL        (0x0600 | 0x03)
#define ERR_CTRL_RECORD_NOFOUND     (0x0600 | 0x04)
#define ERR_CTRL_RECORD_NOAVLBLK    (0x0600 | 0x05)
#define ERR_CTRL_RECORD_WRITEFAIL   (0x0600 | 0x06)
#define ERR_TBL_RECORD_POWERCUT     (0x0600 | 0x07)
#define ERR_TBL_RECORD_NOAVLBLK     (0x0600 | 0x08)
...
MACHINE_START(MADISON, "Madison")
    .boot_params    = PHYS_OFFSET + 0x00000100,
    .map_io         = chip_map_io,
#ifdef CONFIG_ANDROID_RAM_CONSOLE
     .reserve       = mstar_reserve,
#endif
