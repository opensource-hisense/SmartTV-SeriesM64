...
void  mstar_irq_eoi(struct irq_data *d)
{
    if(d->irq >= MSTAR_INT_BASE) 
        chip_irq_ack( d->irq - MSTAR_INT_BASE);
}

void  mstar_irq_mask(struct irq_data *d)
{
    if(d->irq >= MSTAR_INT_BASE) 
        chip_irq_mask( d->irq - MSTAR_INT_BASE);
}

void  mstar_irq_unmask(struct irq_data *d)
{
    if(d->irq >= MSTAR_INT_BASE) 
        chip_irq_unmask( d->irq - MSTAR_INT_BASE);
}
...
struct irq_chip gic_arch_extn = {
	.irq_eoi	= mstar_irq_eoi,
	.irq_mask	= mstar_irq_mask,
	.irq_unmask	= mstar_irq_unmask,
        .irq_enable     = mstar_irq_unmask,
        .irq_disable    = mstar_irq_mask,
	.irq_retrigger	= NULL,
	.irq_set_type	= NULL,
	.irq_set_wake	= NULL,
};
