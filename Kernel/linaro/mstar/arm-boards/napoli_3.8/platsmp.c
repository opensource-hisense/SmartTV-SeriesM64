...
void __init chip_smp_prepare_cpus(unsigned int max_cpus)
{
	...
	for (i = 0; i < max_cpus; i++)
		...
		//writew(BSYM(virt_to_phys(vexpress_secondary_startup)), (void*)SECOND_START_ADDR_LO);
    		writel(0xbabe, (void*)SECOND_MAGIC_NUMBER_ADRESS);
		writel(BSYM(virt_to_phys(vexpress_secondary_startup)) & 0xFFFF, (void*)SECOND_START_ADDR);
		writel(BSYM(virt_to_phys(vexpress_secondary_startup)) >> 16, (void*)(SECOND_START_ADDR + 4));
}
