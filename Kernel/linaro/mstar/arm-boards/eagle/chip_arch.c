...
struct map_desc chip_io_desc[] __initdata =
{
...
void __init serial_init(void)
{
...
#if defined(CONFIG_MSTAR_EAGLE_BD_FPGA) || \
	defined(CONFIG_MSTAR_ARM_BD_FPGA)
	s.uartclk = 12000000; //FPGA
#elif defined(CONFIG_MSTAR_EAGLE_BD_GENERIC) || \
	  defined(CONFIG_MSTAR_ARM_BD_GENERIC)
	...
	u.type = PORT_16550;
	u.iobase = 0xFD220C00;
    u.membase = (unsigned char __iomem *) 0xFD220c00;
    u.mapbase = 0xFD220c00;
	u.irq = E_IRQEXPL_UART1;
    u.uartclk = 123000000; //real chip
	u.iotype = 0;
	u.regshift = 0;
            ...
            unsigned long code, void *x)
{
    if(x && strlen((char*)x))
    {
        ...
        if(fd)
        {
            ...
            printk(KERN_ERR "write to /dev/block/platform/mstar_mci.0/by-name/misc %ld bytes", t);
            vfs_fsync(fd, 0);
            filp_close(fd, NULL);
        }
        else
...
#if defined(CONFIG_MEMSTICK_EAGLE_MS) || defined(CONFIG_MEMSTICK_CHIP_MS)
