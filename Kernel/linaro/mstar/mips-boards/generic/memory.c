...
unsigned long PM51IAddr=0, PM51DAddr=0, PM51ISize=0, PM51DSize=0, PM51Run=0;
...
EXPORT_SYMBOL(PM51IAddr);
EXPORT_SYMBOL(PM51DAddr);
EXPORT_SYMBOL(PM51ISize);
EXPORT_SYMBOL(PM51DSize);
EXPORT_SYMBOL(PM51Run);
...
static int __init PM51_ADDR_setup(char *str)
{
    if( str != NULL )
    {
		//static unsigned long =0, PM51DAddr=0; PM51ISize=0; PM51DSize=0; PM51Run=0;
        sscanf(str,"%lx", &PM51IAddr);
		PM51DSize = PM51ISize = 0x8000;
		PM51DAddr = PM51IAddr + PM51ISize;
		PM51Run   = PM51IAddr + 0x400;
    }
    return 0;
}
...
early_param("PM51_ADDR", PM51_ADDR_setup);
...
void __init prom_meminit(void)
{
	...
	lx_mem_addr = linux_memory_address;	// add this here to ensure the lx_mem_addr is the real LX_MEM start addr
