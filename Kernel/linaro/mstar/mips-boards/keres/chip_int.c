...
#define irq_set_chip_and_handler_name(a,b,c,d)  (set_irq_chip_and_handler(a,b,c))
#endif
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,0,20)
static unsigned int startup_Mstar_Chip_irq(struct irq_data *data)
{
	//enable_lasat_irq(irq);
    Mstar_Chip_EnableInterrupt((InterruptNum)data->irq);
	return 0; /* never anything pending */
}

static void shutdown_Mstar_Chip_irq(struct irq_data *data)
{
	//enable_lasat_irq(irq);
    Mstar_Chip_DisableInterrupt((InterruptNum)data->irq);
	//return 0; /* never anything pending */
}

static void enable_Mstar_Chip_irq(struct irq_data *data)
{
	//enable_lasat_irq(irq);
    Mstar_Chip_EnableInterrupt((InterruptNum)data->irq);
	//return 0; /* never anything pending */
...
static void disable_Mstar_Chip_irq(struct irq_data *data)
{
	//enable_lasat_irq(irq);
    Mstar_Chip_DisableInterrupt((InterruptNum)data->irq);
	//return 0; /* never anything pending */
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,0,20)
static unsigned int startup_Mstar_Chip_fiq(struct irq_data *data)
{
	//enable_lasat_irq(irq);
    Mstar_Chip_EnableInterrupt((InterruptNum)data->irq);
	return 0; /* never anything pending */
...
static void shutdown_Mstar_Chip_fiq(struct irq_data *data)
{
	//enable_lasat_irq(irq);
    Mstar_Chip_DisableInterrupt((InterruptNum)data->irq);
	//return 0; /* never anything pending */
...
static void enable_Mstar_Chip_fiq(struct irq_data *data)
{
	//enable_lasat_irq(irq);
    Mstar_Chip_EnableInterrupt((InterruptNum)data->irq);
	//return 0; /* never anything pending */
...
static void disable_Mstar_Chip_fiq(struct irq_data *data)
{
	//enable_lasat_irq(irq);
    Mstar_Chip_DisableInterrupt((InterruptNum)data->irq);
	//return 0; /* never anything pending */
...
static struct hw_interrupt_type Mstar_Chip_irq_type = {
	.name = "Mstar_Chip IRQ",
	.irq_startup = startup_Mstar_Chip_irq,
	.irq_shutdown = shutdown_Mstar_Chip_irq,
	.irq_enable = enable_Mstar_Chip_irq,
	.irq_disable = disable_Mstar_Chip_irq,
	.irq_ack = mask_and_ack_Mstar_Chip_irq,
	.irq_mask = disable_Mstar_Chip_irq,
	.irq_mask_ack = disable_Mstar_Chip_irq,
	.irq_unmask = enable_Mstar_Chip_irq,
	.irq_eoi = enable_Mstar_Chip_irq,
	//.irq_end = (void (*)(unsigned int irq))end_Mstar_Chip_irq
};
static struct hw_interrupt_type Mstar_Chip_fiq_type = {
	.name = "Mstar_Chip IRQ",
	.irq_startup = startup_Mstar_Chip_fiq,
	.irq_shutdown = shutdown_Mstar_Chip_fiq,
	.irq_enable = enable_Mstar_Chip_fiq,
	.irq_disable = disable_Mstar_Chip_fiq,
	.irq_ack = mask_and_ack_Mstar_Chip_fiq,
	.irq_mask = disable_Mstar_Chip_fiq,
	.irq_mask_ack = disable_Mstar_Chip_fiq,
	.irq_unmask = enable_Mstar_Chip_fiq,
	.irq_eoi = enable_Mstar_Chip_fiq,
...
void __init arch_init_irq(void)
{
    ...
    for (i = MSTAR_INT_BASE; i <= (MSTAR_CHIP_INT_END+MSTAR_INT_BASE); i++)
    {
        if (i < (64 + MSTAR_INT_BASE))
            irq_set_chip_and_handler_name(i, &Mstar_Chip_irq_type, handle_level_irq, NULL);
        else
			irq_set_chip_and_handler_name(i, &Mstar_Chip_fiq_type, handle_level_irq, NULL);
    }
...
#else
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,0,20)
	// check if cpu supports vector interrupt, and move externel interrupt entry point from EBASE+0x180
    // to EBASE+0x200,there are eight seperate entry points corresponding to IP0-IP7 from EBASE + 0x200,
    // no need dispatch code to decide which IP cause externel interrupt to get faster interrupt responding time
    // "Mstar_Chip_hw0_irqdispatch" and "Mstar_Chip_hw0_fiqdispatch" are attach to IP2 and IP3 separately
    // cpu_has_vint is decided in arch/mips/include/asm/cpu-features.h line 208
	if (cpu_has_vint)
	{
    	set_vi_handler(2, Mstar_Chip_hw0_irqdispatch);
        set_vi_handler(3, Mstar_Chip_hw0_fiqdispatch);
    ...
    irq_set_chip_and_handler_name(0, &Mstar_Chip_fiq_type, handle_level_irq, NULL);
	irq_set_chip_and_handler_name(1, &Mstar_Chip_fiq_type, handle_level_irq, NULL);
