...
const char display_string[] = "        LINUX ON Kronus     ";
...
#if defined(CONFIG_MSTAR_KRONUS_BD_FPGA)
#define KRONUS_UART_CLK_FREQ    12000000
#endif

#if defined(CONFIG_MSTAR_KRONUS_BD_GENERIC)
#define KRONUS_UART_CLK_FREQ    123000000
#endif
...
spinlock_t l2prefetch_lock = __SPIN_LOCK_UNLOCKED(l2prefetch_lock);
...
static int __init Kronus_setup(void)
{
    // int i;
    ...
    *((char*)0xbf200e00) = *((char*)0xbf200e00) | 0x28;  // Dean
    ...
    printk("Begin Kronus_setup\n");

#if defined(CONFIG_MSTAR_KRONUS)

#if defined(CONFIG_MSTAR_KRONUS_MMAP_32MB_32MB) || \
    defined(CONFIG_MSTAR_KRONUS_MMAP_64MB_32MB) || \
    defined(CONFIG_MSTAR_KRONUS_MMAP_64MB_64MB) || \
    defined(CONFIG_MSTAR_KRONUS_MMAP_128MB_64MB) || \
    defined(CONFIG_MSTAR_MMAP_128MB_128MB_DEFAULT) || \
    defined(CONFIG_MSTAR_KRONUS_MMAP_128MB_128MB)
...
static void __init serial_init(void)
{
	...
	s.uartclk   = KRONUS_UART_CLK_FREQ;
	s.iotype = 0;
	s.regshift = 0;
...
#define KRONUS_UART_BASE        0xbf201300

#define KRONUS_UART_RX          (0x00*4 + KRONUS_UART_BASE)
#define KRONUS_UART_TX          (0x00*4 + KRONUS_UART_BASE)
#define KRONUS_UART_DLL         (0x00*4 + KRONUS_UART_BASE)
#define KRONUS_UART_DLM         (0x02*4 + KRONUS_UART_BASE)
#define KRONUS_UART_IER         (0x02*4 + KRONUS_UART_BASE)
#define KRONUS_UART_IIR         (0x04*4 + KRONUS_UART_BASE)
#define KRONUS_UART_FCR         (0x04*4 + KRONUS_UART_BASE)
#define KRONUS_UART_LCR         (0x06*4 + KRONUS_UART_BASE)
#define KRONUS_UART_MCR         (0x08*4 + KRONUS_UART_BASE)
#define KRONUS_UART_LSR         (0x0A*4 + KRONUS_UART_BASE)
#define KRONUS_UART_MSR         (0x0C*4 + KRONUS_UART_BASE)


//Baud rate
#ifdef CONFIG_MSTAR_KRONUS_BD_FPGA
#define KRONUS_UART_BAUD_RATE  38400
#else
#define KRONUS_UART_BAUD_RATE  115200
#endif
...
//Divisor
#define KRONUS_UART_DIVISOR     (KRONUS_UART_CLK_FREQ/(16 * KRONUS_UART_BAUD_RATE))
...
int kronus_putc(int c)
{
    unsigned char u8Reg;

    do
    {
        u8Reg = UART_REG(KRONUS_UART_LSR) & 0xff;
    ...
    if (c == '\n')
    {
        ...
        UART_REG(KRONUS_UART_TX)='\r';
    }
    else
    {
        UART_REG(KRONUS_UART_TX) = c & 0xffff;
    }
...
void uart_clk_scan(void)
{
    ...
    {
        unsigned short divisor= (clk[i]/(16 * KRONUS_UART_BAUD_RATE));
        //UART1 INIT:

        //Disable all UART interrupts
        UART_REG(KRONUS_UART_IER) = 0x00;

        //Reset receiver and transmitr
        UART_REG(KRONUS_UART_FCR) = 0x07;

        //Set 8 bit char, 1 stop bit, no parity
        UART_REG(KRONUS_UART_LCR) = 0x03;

        UART_REG(KRONUS_UART_LCR) = 0x83;
        UART_REG(KRONUS_UART_DLM) = (divisor >> 8) & 0x00ff;
        UART_REG(KRONUS_UART_DLL) = divisor & 0x00ff;
        UART_REG(KRONUS_UART_LCR) = 0x03;

        UART_REG(KRONUS_UART_TX) = '0'+ i;
    }
}
...
void pool_getc(void)
{
    ...
    do
    {
        if ( ((UART_REG(KRONUS_UART_LSR) & UART_LSR_DR) == UART_LSR_DR) || nflag )
        {
            break;
        }
    } while (1);

    u8Ch = (char)UART_REG(KRONUS_UART_RX);      //get char
    kronus_putc(u8Ch);
    return ;
}
...
#else //CONFIG_MSTAR_KRONUS_BD_FPGA
...
//// Switch PAD RS232 end///////////////////////////////////////////////////
...
#endif//CONFIG_MSTAR_URANUS4_BD_FPGA
    ...
    UART_REG(KRONUS_UART_LCR) |= 0x80;
    UART_REG(KRONUS_UART_DLM) = (KRONUS_UART_DIVISOR >> 8) & 0x00ff;
    UART_REG(KRONUS_UART_DLL) = KRONUS_UART_DIVISOR & 0x00ff;
    UART_REG(KRONUS_UART_LCR) &= ~(0x80) ;

    // Enable receiver data available interrupt
    //UART_REG(KRONUS_UART_IER) = 0x01;

    //UART_REG(KRONUS_UART_LCR) &= ~(0x8000);

    //UART_REG(KRONUS_UART_SCCR_RST) = 0x10 ;
...
inline void _chip_flush_miu_pipe(void)
{
    ...
    *(volatile unsigned int *)(0xBF203104) = 0x12;  //0x101841[4]: Start to cache operation
    ...
    while (1)
    {
        ...
        if ((*(volatile unsigned int *)(0xBF203104) & BIT9) != 0)
        {
            break;
        }
    }
...
void Chip_L2_cache_wback_inv( unsigned long addr, unsigned long size)
{
    ...
    if ((*(volatile unsigned int *)(0xbf000000+0x203100)) & BIT1)  // H40
    {
        ...
        // flush pipe
        spin_lock_irqsave(&l2prefetch_lock, flag);
        temp = *(volatile unsigned int *)(0xBF003C00) ; // dummy read
...
void Chip_L2_cache_wback( unsigned long addr, unsigned long size)
{
    ...
    else
    {
        ...
        *(volatile unsigned int *)(0xbf000000+0x203104) =  0x3; // H41 all
        temp = *(volatile unsigned int *)(0xBF003C00) ; // dummy read
        *(volatile unsigned int *)(0xbf000000+0x203104) = BIT4 | 0x3; // H41 all
        do
        {
            temp = *(volatile unsigned int *)(0xBF003C00) ; // dummy read
            temp = *(volatile unsigned int *)(0xbf000000+0x203104);
...
void Chip_Query_L2_Config(void)
{
    ...
    if ((*(volatile unsigned int *)(0xbf203100+0x203100)) & BIT1)  // H40
    {
...
unsigned int Chip_Query_MIPS_CLK(void)
{
    ...
    } else if (uSpeed_L == 0xC9) {
        u32Count = uSpeed_H *48 * 1000000;
    } else if (uSpeed_H == 0x17) {
        u32Count = 552000000;
    } else if (uSpeed_H == 0x19) {
        u32Count = 600000000;
    } else if (uSpeed_H == 0x1D) {
        u32Count = 696000000;
    }
