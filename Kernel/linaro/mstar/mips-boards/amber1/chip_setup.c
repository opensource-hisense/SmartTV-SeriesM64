...
#if defined(CONFIG_MSTAR_AMBER1_BD_FPGA)
...
#if (USE_UART0==1)
    #if (USE_ICE == 1)
        *(volatile volatile unsigned int*)(0xbf001c24) |= 0x0800; //decided by MBoot, not Kernel
...
unsigned int Chip_Query_MIPS_CLK(void)
{
    ...
    dwMipsFreq = ((dwReadRegister & 0x00FF) * 12000000 / 2);
