...
const char display_string[] = "        LINUX ON Kenya     ";
...
#define MIPS_L2_WB_INV_TH    			(256*1024) //0x100000    // 1MB
#define MIPS_L2_WB_TH        			(256*1024) //0x100000    // 1MB
#define MIPS_L2_INV_TH       			(256*1024) //0x10000000  // 256MB
/******************************************************************************/
/* Macro for bitwise                                                          */
/******************************************************************************/
...
void Chip_L2_cache_wback_inv(unsigned long addr, unsigned long size)
{
    ...
    else
    {
		...
		{
		if(1)
        	//if(size >= MIPS_L2_WB_INV_TH)	// if size >= 256KB, flush all
        	{
            	spin_lock_irqsave(((spinlock_t*)&l2prefetch_lock), flag);
...
void Chip_L2_cache_inv(unsigned long addr, unsigned long size)
{
    ...
    else
    {
		...
		{
		       if(1)
			//if(size >= MIPS_L2_INV_TH)	// if size >= 256KB, invalid all
        	{
            	spin_lock_irqsave(((spinlock_t*)&l2prefetch_lock), flag);
...
unsigned int Chip_Query_MIPS_CLK(void)
{
    ...
    unsigned int uReg_mipspll_input_div1 = 0;
    unsigned int uReg_mipspll_loop_div1 = 0;
    unsigned int uReg_mipspll_loop_div2 = 0;
    unsigned int uReg_mipspll_output_div1 = 0;
    ...
    uReg_mipspll_input_div1 = 2;
    uReg_mipspll_loop_div1 = 2;

    uReg_mipspll_loop_div2 = *(volatile u32*)(0xBF221868);
    uReg_mipspll_loop_div2 = (uReg_mipspll_loop_div2 & (0xFF));

    uReg_mipspll_output_div1 = *(volatile u32*)(0xBF221864);
    uReg_mipspll_output_div1 = (uReg_mipspll_output_div1 & (0x30));

    u32Count = 1000000*24/(1<<uReg_mipspll_input_div1)*(1<<uReg_mipspll_loop_div1)*uReg_mipspll_loop_div2/2/(1<<uReg_mipspll_output_div1);
