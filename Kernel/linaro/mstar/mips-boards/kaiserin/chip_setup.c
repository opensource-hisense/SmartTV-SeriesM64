...
const char display_string[] = "        LINUX ON Kaiserin     ";
...
#define KAISERIN_UART_CLK_FREQ    12000000
#endif

#if defined(CONFIG_MSTAR_KAISERIN_BD_GENERIC)
#define KAISERIN_UART_CLK_FREQ    123000000
#endif
...
static int __init Kaiserin_setup(void)
{
    ...
    *((char*)0xbf200e00) = *((char*)0xbf200e00) | 0x28;  // Dean
    ...
    printk("Begin Kaiserin_setup\n");

#if defined(CONFIG_MSTAR_KAISERIN)

#if defined(CONFIG_MSTAR_KAISERIN_MMAP_128MB_128MB)
    // for MIU assignment
...
static void __init serial_init(void)
{
	...
	s.uartclk   = KAISERIN_UART_CLK_FREQ;
	s.iotype = 0;
	s.regshift = 0;
	...
	v.type = PORT_16550;
	v.iobase = 0xbf220D00;
	v.irq = E_IRQEXPH_UART2MCU;

	v.uartclk   = 12000000;
	v.iotype = 0;
	v.regshift = 0;
	v.fifosize = 16 ; // use the 8 byte depth FIFO well
	v.line = 1 ; // use the 8 byte depth FIFO well

	if (early_serial_setup(&v) != 0) {
		printk(KERN_ERR "Serial setup failed!\n");
	}
#endif
}
...
#define KAISERIN_UART_BASE        0xbf201300

#define KAISERIN_UART_RX          (0x00*4 + KAISERIN_UART_BASE)
#define KAISERIN_UART_TX          (0x00*4 + KAISERIN_UART_BASE)
#define KAISERIN_UART_DLL         (0x00*4 + KAISERIN_UART_BASE)
#define KAISERIN_UART_DLM         (0x02*4 + KAISERIN_UART_BASE)
#define KAISERIN_UART_IER         (0x02*4 + KAISERIN_UART_BASE)
#define KAISERIN_UART_IIR         (0x04*4 + KAISERIN_UART_BASE)
#define KAISERIN_UART_FCR         (0x04*4 + KAISERIN_UART_BASE)
#define KAISERIN_UART_LCR         (0x06*4 + KAISERIN_UART_BASE)
#define KAISERIN_UART_MCR         (0x08*4 + KAISERIN_UART_BASE)
#define KAISERIN_UART_LSR         (0x0A*4 + KAISERIN_UART_BASE)
#define KAISERIN_UART_MSR         (0x0C*4 + KAISERIN_UART_BASE)

//Baud rate
#ifdef CONFIG_MSTAR_KAISERIN_BD_FPGA
#define KAISERIN_UART_BAUD_RATE  38400
#else
#define KAISERIN_UART_BAUD_RATE  115200
#endif
...
//Divisor
#define KAISERIN_UART_DIVISOR    (KAISERIN_UART_CLK_FREQ/(16 * KAISERIN_UART_BAUD_RATE))
...
int kaiserin_putc(int c)
{
    unsigned char u8Reg;

    do
    {
        u8Reg = UART_REG(KAISERIN_UART_LSR) & 0xff;
    ...
    if (c == '\n')
    {
        ...
        UART_REG(KAISERIN_UART_TX)='\r';
    }
    else
    {
        UART_REG(KAISERIN_UART_TX) = c & 0xffff;
    }
...
void uart_clk_scan(void)
{
    ...
    {
        unsigned short divisor= (clk[i]/(16 * KAISERIN_UART_BAUD_RATE));
        //UART1 INIT:

        //Disable all UART interrupts
        UART_REG(KAISERIN_UART_IER) = 0x00;

        //Reset receiver and transmitr
        UART_REG(KAISERIN_UART_FCR) = 0x07;

        //Set 8 bit char, 1 stop bit, no parity
        UART_REG(KAISERIN_UART_LCR) = 0x03;

        UART_REG(KAISERIN_UART_LCR) = 0x83;
        UART_REG(KAISERIN_UART_DLM) = (divisor >> 8) & 0x00ff;
        UART_REG(KAISERIN_UART_DLL) = divisor & 0x00ff;
        UART_REG(KAISERIN_UART_LCR) = 0x03;

        UART_REG(KAISERIN_UART_TX) = '0'+ i;
    }
}
...
void pool_getc(void)
{
    ...
    do
    {
        if ( ((UART_REG(KAISERIN_UART_LSR) & UART_LSR_DR) == UART_LSR_DR) || nflag )
        {
            break;
        }
    } while (1);

    u8Ch = (char)UART_REG(KAISERIN_UART_RX);      //get char
    kaiserin_putc(u8Ch);
    return ;
}
...
#else //CONFIG_MSTAR_KAISERIN_BD_FPGA
...
//// Switch PAD RS232 end///////////////////////////////////////////////////
...
#endif//CONFIG_MSTAR_KAISERIN_BD_FPGA
    ...
    UART_REG(KAISERIN_UART_LCR) |= 0x80;
    UART_REG(KAISERIN_UART_DLM) = (KAISERIN_UART_DIVISOR >> 8) & 0x00ff;
    UART_REG(KAISERIN_UART_DLL) = KAISERIN_UART_DIVISOR & 0x00ff;
    UART_REG(KAISERIN_UART_LCR) &= ~(0x80) ;

    // Enable receiver data available interrupt
    //UART_REG(KAISERIN_UART_IER) = 0x01;

    //UART_REG(KAISERIN_UART_LCR) &= ~(0x8000);

    //UART_REG(KAISERIN_UART_SCCR_RST) = 0x10 ;
...
void Chip_L2_cache_wback_inv(unsigned long addr, unsigned long size)
{
    ...
    else
    {
        ...
        else // range flush
        {
            ...
            *(volatile unsigned int *)(0xbf000000+0x203110) = (addr) & 0xFFFF; // H44 range start Lo
            *(volatile unsigned int *)(0xbf000000+0x203114) = ((addr>>16)) & 0xFFFF; // H44 range start hi
            *(volatile unsigned int *)(0xbf000000+0x203118) = ((addr+size-1)) & 0xFFFF; // H44 range start Lo
            *(volatile unsigned int *)(0xbf000000+0x20311C) = ((addr+size-1)>>16) & 0xFFFF; // H44 range start hi
            *(volatile unsigned int *)(0xbf000000+0x203104) = 0x6; // H41 all
            *(volatile unsigned int *)(0xbf000000+0x203104) = _BIT(4) | 0x6; // H41 all
            do
            {
                temp = *(volatile unsigned int *)(0xbf000000+0x203104);
                temp &= _BIT(11);//all
...
void Chip_Flush_Memory_Range(unsigned long pAddress, unsigned long size)
{
    ...
    if (*(volatile unsigned int *)(0xbf000000 + 0x203100) & BIT2) {
		/* dummy read */
        temp = *(volatile unsigned int *)(0xbf000000 + 0x201600); 
...
{
    Chip_Read_Memory_Range(0, 0x20000); /* 128KB */
}
...
unsigned int Chip_Query_MIPS_CLK(void)
{
    ...
    unsigned int uSpeed_Loop_DIV_FIRST = 0, uSpeed_LOOP_DIV_SECOND = 0;
    unsigned int u_Loop_DIV_FIRST_Value;
    // MPLL_CLK_VCO_OUT=(MPLL_CLK_IN*LOOP_DIV_FIRST*LOOP_DIV_SECOND)/2

    uSpeed_Loop_DIV_FIRST = *(volatile u32*)(0xBF221864);
    uSpeed_LOOP_DIV_SECOND = *(volatile u32*)(0xBF221868);

    uSpeed_Loop_DIV_FIRST = ((uSpeed_Loop_DIV_FIRST & (0xC)) >> 2);

    uSpeed_LOOP_DIV_SECOND = (uSpeed_LOOP_DIV_SECOND & (0xFF));


    switch (uSpeed_Loop_DIV_FIRST)
    {
        case 0:
            u_Loop_DIV_FIRST_Value = 1;
        break;
        case 1:
            u_Loop_DIV_FIRST_Value = 2;
        break;
        case 2:
            u_Loop_DIV_FIRST_Value = 4;
        break;
        case 3:
            u_Loop_DIV_FIRST_Value = 8;
        break;
        default:
            u_Loop_DIV_FIRST_Value = 1;
        break;
    ...
    u32Count = 12*uSpeed_LOOP_DIV_SECOND*u_Loop_DIV_FIRST_Value*1000000;
