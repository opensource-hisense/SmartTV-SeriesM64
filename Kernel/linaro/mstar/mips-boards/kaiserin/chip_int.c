...
#else
static unsigned int startup_Kaiserin_irq(struct irq_data *data)
{
    Kaiserin_EnableInterrupt((InterruptNum)data->irq);
	return 0; /* never anything pending */
}

static void shutdown_Kaiserin_irq(struct irq_data *data)
{
    Kaiserin_DisableInterrupt((InterruptNum)data->irq);
}
#endif
...
#else
static unsigned int startup_Kaiserin_fiq(struct irq_data *data)
{
    Kaiserin_EnableInterrupt((InterruptNum)data->irq);
	return 0; /* never anything pending */
}

void shutdown_Kaiserin_fiq(struct irq_data *data)
{
    Kaiserin_DisableInterrupt((InterruptNum)data->irq);
}
#endif
...
static struct irq_chip Kaiserin_irq_type = {
	.name = "Kaiserin IRQ",
	.irq_startup = startup_Kaiserin_irq,
	.irq_shutdown = shutdown_Kaiserin_irq,
	.irq_enable = (void (*)(struct irq_data *))enable_Kaiserin_irq,
	.irq_disable = disable_Kaiserin_irq,
	.irq_ack = mask_and_ack_Kaiserin_irq,
	.irq_mask = disable_Kaiserin_irq,
	.irq_mask_ack = disable_Kaiserin_irq,
	.irq_unmask = (void (*)(struct irq_data *))enable_Kaiserin_irq,
	.irq_eoi = (void (*)(struct irq_data *))enable_Kaiserin_irq,
};

static struct irq_chip Kaiserin_fiq_type = {
	.name = "Kaiserin FIQ",
	.irq_startup = startup_Kaiserin_fiq,
	.irq_shutdown = shutdown_Kaiserin_fiq,
	.irq_enable = (void (*)(struct irq_data *))enable_Kaiserin_fiq,
	.irq_disable = disable_Kaiserin_fiq,
	.irq_ack = mask_and_ack_Kaiserin_fiq,
	.irq_mask = disable_Kaiserin_fiq,
	.irq_mask_ack = disable_Kaiserin_fiq,
	.irq_unmask = (void (*)(struct irq_data *))enable_Kaiserin_fiq,
	.irq_eoi = (void (*)(struct irq_data *))enable_Kaiserin_fiq,
};
#endif
...
/* for compatibility */
#if LINUX_VERSION_CODE > KERNEL_VERSION(3,1,10)
#define set_irq_chip_and_handler(irq, irq_chip, handle) \
	do { \
