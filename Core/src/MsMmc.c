...
int do_emmc_mkbin(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    if(argc > 4)
    {
        ...
        {
            blk = (ulong) (ALIGN(simple_strtoull(argv[4], NULL, 16), 512) / 512);
            printf("offset = 0x%x, blk = %d\n", (int)simple_strtoull(argv[4], NULL, 16), (int)blk);
        }
    }
    if(argc > 5)
...
static int cmd_write(cmd_tbl_t *cmdtp_t, int flag_t, int argc_t, char * const argv_t[])
{
    ...
    s32 blk = -1;
    struct mmc *mmc;
    unsigned char *buf_write = (unsigned char *)malloc(512 * sizeof(unsigned char));
    unsigned char *buf_read = (unsigned char *)malloc(512 * sizeof(unsigned char));
...
static int cmd_writeprotect_boot2(cmd_tbl_t *cmdtp_t, int flag_t, int argc_t, char * const argv_t[])
{
	...
	if(!strcmp(wkh->magic, magic))
	{
		...
		{
			mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL, 173, 0x93);
			write_boot2 = 2;
		}
		else if(!strncmp(wkh->update_sts, PEREND, 8))
		{
			mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL, 173, 0xCC);
			write_boot2 = 2;
		}
		else if(!strncmp(wkh->update_sts, NWPEND, 8))
		{
			mmc_switch(mmc,EXT_CSD_CMD_SET_NORMAL, 173, 0x50);
			write_boot2 = 2;
		}
		else
		{
...
int do_eMMCops(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    else if (strcmp(argv[1], "rpmb") == 0)
    {
        ...
        else if(strcmp(argv[2], "read")==0)
        {
            ...
            u32_data_length = (u32)simple_strtoul(argv[4], NULL, 16);
            u16_blk_addr = (u16)simple_strtoul(argv[5], NULL, 16);
            u32_err = eMMC_RPMB_Read_data(addr_data,u32_data_length,u16_blk_addr);
            if(u32_err==0)
               printf("RPMB read ok\n");
            else
