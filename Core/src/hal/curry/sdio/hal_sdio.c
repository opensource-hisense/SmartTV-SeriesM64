...
void HalSdio_DumpRegister(void)
{
    ...
    printk("\n\nclockgen2 reg:");
	HalSdio_DumpRegisterBank(RIU_BANK_CLKGEN2, 0, 0x80);
...
E_IO_STS HalSdio_SendCommandEx(CMD_RSP_INFO * pCmdRspInfo)
{
    ...
    {
		if(i==0)
			printk("%02X ", pCmdRspInfo->Command.Array[i+3]|TRANSMIT_BIT); // will add transmitter bit later
        else
	...
	else // data transfer command
	{
		...
		if(pCmdRspInfo->ReadWriteDir==DIR_R)
		{
			...
			if(pCmdRspInfo->DataPath==PATH_ADMA) {
				...
				if(!pCmdRspInfo->ReadWriteTimeOut) printk("SdErr, Not proper timeout value set!\n");
    ...
    else // data transfer command
    {
		if(pCmdRspInfo->ReadWriteDir==DIR_R) // read adtc
		{
			...
			{
				printk("SD Err: wait read timeout %04Xh\n", IoSts);
				goto ErrorHandle;
			}
	...
	if(IoSts)
	{
		...
		//printk("parsing error found err %04Xh", IoSts);
		goto ErrorHandle;
	}
	#endif
...
// do clock downgrade and driving adjust
static unsigned char gu8ReadFailTimes = 0;
static unsigned char gu8WriteFailTimes = 0;
unsigned char gu8DowngradeClock = 0;
unsigned char gu8AdjustDriving = 0;

extern int gu32_Skew4__50M;
extern int gu32_Skew4_100M;

E_IO_STS HalSdio_SendCommand(CMD_RSP_INFO * pCmdRspInfo)
{
	E_IO_STS IoSts;
	static unsigned char clcok_change_to_100 = 0;
	static unsigned char clcok_change_to__50 = 0;
	static unsigned char driving_changed = 0;

	//static unsigned char read_test = 0;
	//static unsigned char write_test = 0;

	if(pCmdRspInfo->Command.Cmd.Index == 0)
	{
		gu8ReadFailTimes = 0;
		gu8WriteFailTimes = 0;
		gu8DowngradeClock = 0;
		gu8AdjustDriving = 0;
		clcok_change_to_100 = 0;
		clcok_change_to__50 = 0;
		driving_changed = 0;
	}

	if(pCmdRspInfo->CardClock==200000) // SDR104 mode
	{
		if( gu8ReadFailTimes>=1 || gu8WriteFailTimes>=2 )
		{
			gu8DowngradeClock = 1;
		}

		if( gu8ReadFailTimes>=2 || gu8WriteFailTimes>=3 )
		{
			gu8DowngradeClock = 2;
		}

		if(gu8WriteFailTimes>=1)
		{
			gu8AdjustDriving = 1;
		}

		if(gu8DowngradeClock) // slow down the clock
		{
			//printk(LIGHT_RED"slow down the clock\n"NONE);

			if(gu8DowngradeClock==1)
			{
				pCmdRspInfo->CardClock = 100000; // need to config for each request
				if(!clcok_change_to_100)
				{
					HalSdio_SetSkew(4, gu32_Skew4_100M); // only need once
					clcok_change_to_100 = 1;
					printk(LIGHT_PURPLE"downgrade clock to 100MHz\n"NONE);
				}
			}
			else if(gu8DowngradeClock==2)
			{
				pCmdRspInfo->CardClock = 50000; // need to config for each request
				if(!clcok_change_to__50)
				{
					HalSdio_SetSkew(4, gu32_Skew4__50M); // only need once
					clcok_change_to__50 = 1;
					printk(LIGHT_PURPLE"downgrade clock to 50MHz\n"NONE);
				}
			}
		}
		...
		if(gu8AdjustDriving)
		{
			if(!driving_changed)
			{
				HalSdio_SetDrivingStrength(4); // only need once
				driving_changed = 1;
				printk(LIGHT_PURPLE"enhance driving\n"NONE);
			}
		}
	...
	IoSts = HalSdio_SendCommandEx(pCmdRspInfo);


	#if 0
	//if( (pCmdRspInfo->Command.Cmd.Index == 18) && (pCmdRspInfo->Command.Cmd.Arg==U32BEND2LEND(0x00004000)) )
	if( pCmdRspInfo->Command.Cmd.Index == 25 )
	{
		write_test++;
		if(write_test%100==99)
		{
			printk(LIGHT_PURPLE"do crack here\n"NONE);
			IoSts = IO_W_DATA_STS_ERROR;
		}
	}
	if( pCmdRspInfo->Command.Cmd.Index == 18 )
	{
		read_test++;
		if(read_test%100==99)
		{
			printk(LIGHT_PURPLE"do crack here\n"NONE);
			IoSts = IO_R_DATA_CRC_ERROR;
		}
	}
	#endif

	if(IoSts==IO_R_DATA_CRC_ERROR)
	{
		gu8ReadFailTimes++;
	}
	else if(IoSts==IO_W_DATA_STS_ERROR)
	{
		gu8WriteFailTimes++;
	}
...
E_IO_STS HalSdio_SendCommand(CMD_RSP_INFO * pCmdRspInfo)
{
	return 	HalSdio_SendCommandEx(pCmdRspInfo);
}
