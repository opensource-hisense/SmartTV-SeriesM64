...
U32 nand_pads_switch(U32 u32EnableFCIE)
{
    ...
    REG_CLR_BITS_UINT16(reg_emmc_config, BIT0);                //clean EMMC config 0x5D
    REG_CLR_BITS_UINT16(reg_sd_config, BIT9|BIT8|BIT7);
    ...
    else if(NAND_PAD_ONFI_SYNC_MODE == u32EnableFCIE)
    {
        ...
        {
            REG_WRITE_UINT16(reg_emmcpll_rx03, 0x0060);//avoid ce low
            REG_CLR_BITS_UINT16(reg_emmcpll_rx60, BIT0|BIT1|BIT2);
...
U32 nand_clock_setting(U32 u32ClkParam)
{
    ...
    {
        #if defined(FIX_MPLL_SETTING) && FIX_MPLL_SETTING
        REG_CLR_BITS_UINT16(reg_ckg_mpll, BIT_CKG_MPLL_GATING|BIT_CKG_MPLL_INVERSE|BIT_CLKGEN2_CLK_SEL_MASK);
