...
U32 eMMC_pads_switch(U32 u32Mode)
{
    ...
    // chiptop
    REG_FCIE_CLRBIT(reg_chiptop_0x57, BIT_SD_CONFIG);	//clr sd
    REG_FCIE_CLRBIT(reg_chiptop_0x50, BIT_NAND_MODE);	//clr nand
    REG_FCIE_CLRBIT(reg_chiptop_0x50, BIT_ALL_PAD_IN);	//clr all pad in
    ...
    REG_FCIE_CLRBIT(reg_emmcpll_0x1a, BIT10|BIT5|BIT4|BIT0);
    ...
    REG_FCIE_CLRBIT(REG_1210_0x07,BIT6);      //control eMMC reset pin with eMMC mode = 0
    REG_FCIE_SETBIT(REG_1210_0x08,BIT6);

	//emmc mode
	if((REG_FCIE(reg_stat_bond_sw)& BIT_BOND_BGA)== BIT_BOND_BGA)	//mode1
	{
        REG_FCIE_SETBIT(reg_chiptop_0x6E, BIT_EMMC_CONFIG_EMMC_MODE_1);
	}
	else	//mode2
	{
		REG_FCIE_SETBIT(reg_chiptop_0x1D, BIT_EMMC_CONFIG_EMMC_MODE_2|BIT_SD_CONFIG_ECO);
		REG_FCIE_W(reg_chiptop_0x7C, 0xfffb);
		REG_FCIE_SETBIT(reg_chiptop_0x0C, 0xff);
		//only gpio pad
    ...
    switch(u32Mode)
    {
        ...
        case FCIE_eMMC_SDR:
            ...
            REG_FCIE_SETBIT(reg_emmcpll_0x1c, BIT9);            //meta issue of clk4x and skewi
        ...
        case FCIE_eMMC_HS200:
            ...
            REG_FCIE_SETBIT(reg_emmcpll_0x1a, BIT5|BIT4|BIT0);
        ...
        case FCIE_eMMC_HS400_AIFO_5_1:
                eMMC_debug(eMMC_DEBUG_LEVEL_LOW, 0, "FCIE_eMMC_HS400_AIFO_5_1\n");
...
U32 eMMC_clock_setting(U16 u16_ClkParam)
{
	...
	REG_FCIE_SETBIT(reg_ckg_fcie, BIT8);
...
U32 eMMC_PlatformInit(void)
{
    ...
    eMMC_pll_setting(FCIE_DEFAULT_CLK);//Brian set pll first @ beginning to avoid clock glitch
