...
// this file is use only for maxim project 2016.5.13
...
{
	#ifdef CONFIG_SD
	unsigned long ulSDIO_CONFIG_NUM = 3;
	#endif
	...
	if(ulSDIO_CONFIG_NUM==1) // PCM PAD
	{
		...
		FCIE_RIU_16_ON(CHIPTOP_0Bh, REG_PCM_PE_34);
		FCIE_RIU_16_ON(CHIPTOP_0Ah, REG_PCM_PE_27_24);
	}
	else if(ulSDIO_CONFIG_NUM==2) // PAD_PCM2_RESET, PAD_PCM2_WAIT_N, PAD_TS2_CLK, PAD_TS2_SYNC, PAD_TS2_VLD, PAD_TS2_D0
	{
		FCIE_RIU_16_ON(CHIPTOP_7Bh, REG_SDIO_MODE_2);
		FCIE_RIU_16_OF(CHIPTOP_64h, REG_CIADCONFIG|REG_PCM2CTRLCONFIG|REG_PCMADCONFIG);
		//FCIE_RIU_16_OF(CHIPTOP_5Ah, REG_SD_MODE_MSK); // check below, do NOT clear directly
		FCIE_RIU_16_OF(CHIPTOP_5Ah, REG_TS2_CONFIG); // clear TS2 config
		FCIE_RIU_16_ON(CHIPTOP_0Bh, REG_PCM_PE_34);

		if((FCIE_RIU_R16(CHIPTOP_5Ah)&REG_SD_MODE_MSK)==REG_SD_MODE_1)
		{
			printk(LIGHT_RED"Error! sdio_mode 2, conflict with sd_mode 1\n"NONE);
		}
	}
	else if(ulSDIO_CONFIG_NUM==3) // EMMC IO 12 ~ 17
	{
		FCIE_RIU_16_ON(CHIPTOP_7Bh, REG_SDIO_MODE_3);
		if(FCIE_RIU_R16(CHIPTOP_6Fh)&(REG_NAND_MODE_MSK|REG_NAND_CS1_EN))
		{
			printk(LIGHT_RED"Error! sdio mode 3 cannot share pin with nand\n"NONE);
			FCIE_RIU_16_OF(CHIPTOP_6Fh, REG_NAND_MODE_MSK|REG_NAND_CS1_EN);
		}
		if((FCIE_RIU_R16(CHIPTOP_5Ah)&REG_SD_MODE_MSK)==REG_SD_MODE_3)
		{
			printk(LIGHT_RED"Error! sdio_mode 3, conflict with sd_mode 3\n"NONE);
		}
