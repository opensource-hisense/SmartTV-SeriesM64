...
U32 nand_pads_init(void)
{
    ...
    if(u16_NandMode == NAND_MODE1)
    {
        ...
        REG_WRITE_UINT16(reg_nand_ad_ps, 0xDAFF);  
    }
    ...
    else if(u16_NandMode == NAND_MODE3)
    {
        REG_SET_BITS_UINT16(reg_pcm_a_pe, 0xFF);
        REG_WRITE_UINT16(reg_nand_ad_ps, 0xDEFF);  
    }
...
void nand_DumpPadClk(void)
{
    ...
    nand_debug(0, 1, "  reg_nand_ad_ps(0x%08X): 0x%04X\n", reg_nand_ad_ps, REG(reg_nand_ad_ps));
