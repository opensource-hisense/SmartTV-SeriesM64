...
void HalSdio_Platform_InitChiptop(void)
{
	...
	if(FCIE_RIU_R16(CHIPTOP_5Ah)&REG_SD_MODE_MSK)
	{
		printk(LIGHT_RED"SdErr: sd_mode enable %04Xh!\n"NONE, FCIE_RIU_R16(CHIPTOP_5Ah));
		FCIE_RIU_16_OF(CHIPTOP_5Ah, REG_SD_MODE_MSK);
	}

	if(FCIE_RIU_R16(CHIPTOP_6Fh)&REG_NAND_MODE_MSK)
	{
		printk(LIGHT_RED"SdErr: nand_mode enable %04Xh!\n"NONE, FCIE_RIU_R16(CHIPTOP_6Fh));
		FCIE_RIU_16_OF(CHIPTOP_6Fh, REG_NAND_MODE_MSK);
	}

	if(ulSDIO_CONFIG_NUM==1) // PAD_SD
	{
		FCIE_RIU_16_ON(CHIPTOP_7Bh, REG_SDIO_MODE_1);
		FCIE_RIU_16_ON(CHIPTOP_43h, REG_SD_PE); // pad pull enable, always pull up
	}
	else
	{
	...
	// MIU select by SDIO
	FCIE_RIU_16_ON(MIU2_79, MIU_SELECT_BY_SDIO);
}
