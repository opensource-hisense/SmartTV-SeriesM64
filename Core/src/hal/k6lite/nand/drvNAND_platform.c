...
U32 nand_pads_switch(U32 u32EnableFCIE)
{
    ...
    REG_CLR_BITS_UINT16(reg_sd_config, BIT8);
    REG_CLR_BITS_UINT16(reg_emmc_config, BIT0);
    ...
    {
    	REG_CLR_BITS_UINT16(reg_nc_ddr_onfi, BIT0);
		REG_CLR_BITS_UINT16(reg_nc_ddr_toggle, BIT1);	
		REG_CLR_BITS_UINT16(reg_nc_pad_from_ddr, BIT3);
        REG_CLR_BITS_UINT16(reg_nand_en, BIT0);
        REG_CLR_BITS_UINT16(reg_ddr_io_mode, BIT0);
    ...
    {
		REG_CLR_BITS_UINT16(reg_nc_ddr_onfi, BIT0);
		REG_SET_BITS_UINT16(reg_nc_ddr_toggle, BIT1);	
		REG_SET_BITS_UINT16(reg_nc_pad_from_ddr, BIT3);
        REG_SET_BITS_UINT16(reg_nand_en, BIT0);
        REG_SET_BITS_UINT16(reg_ddr_io_mode, BIT0);
    ...
    {
		REG_SET_BITS_UINT16(reg_nc_ddr_onfi, BIT0);
		REG_CLR_BITS_UINT16(reg_nc_ddr_toggle, BIT1);	
		REG_SET_BITS_UINT16(reg_nc_pad_from_ddr, BIT3);
        REG_SET_BITS_UINT16(reg_nand_en, BIT0);
        REG_SET_BITS_UINT16(reg_ddr_io_mode, BIT0);
...
U32 NC_FCIE5SetInterface(U8 u8_IfDDR, U8 u8_dqs_mode, U8 u8_dqs_delaycell, U8 u8_rd_ddr_timing)
{
    ...
    if(u8_IfDDR)
    {
 	...
 	REG_SET_BITS_UINT16(NC_SM_STS, pNandDrv->u16_EmmcPllReg6c_DqsSel & BIT_DQS_MODE_MASK);		
 	/*
 	@fix me
 	// set DQS_DELAY_CELL
