...
static void MDrv_IR_SW_Isr(MHAL_SavedRegisters *pHalReg, U32 u32Data)
{
    ...
    u8ir_pressed = 1;
        ...
        if(u8IRRes<=MSRET_OK)
        {
            ...
            g_u8IRRes = u8IRRes;
            #if CONFIG_DISPLAY_PICTURE_UPDATE
            if((g_u8IRKey==IRKEY_POWER)||(g_u8IRKey==IRKEY_PAGE_DOWN))
            {
                printf("\n[IR] Key Code = 0x%02X, Repeat=%d\n",g_u8IRKey,g_u8IRRpt);
...
BOOLEAN MDrv_CheckSWIRKey(void)
{
    ...
    if(RxInProcess)
    {
        if((CurrentTime-RxTimeOutCount)>6)
        {
              ...
              /*
              putchar('X');
              putcharvalue((BuffeRX>>8));
              putcharvalue(BuffeRX&0xFF);
              putchar('X');
              putcharvalue(keyDatArrayWTIdx);
              putchar('X');
              */
              BufIdx=0;
              BuffeRX=0;
