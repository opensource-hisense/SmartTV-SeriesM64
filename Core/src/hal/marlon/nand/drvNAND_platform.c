...
U32 nand_pads_init(void)
{
    ...
    {
        REG_READ_UINT16(reg_nand_share_mode, u16_NandMode);
        if(u16_NandMode & BIT_NAND_SHARE_MODE)
        {
            REG_SET_BITS_UINT16(reg_pcm_a_pe, 0xFF);
        }
		else if(u16_NandMode & BIT_NAND_SHARE_TCON)
        {
            //nothing to do
        }
    }
...
U32 nand_pads_switch(U32 u32EnableFCIE)
{
	...
	nand_debug(UNFD_DEBUG_LEVEL_LOW, 0,"reg_nand_share_mode(%08X)=%04X\n", reg_nand_share_mode, REG(reg_nand_share_mode));
