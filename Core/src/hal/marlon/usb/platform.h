...
//---- 3. 240's phase as 120's clock
	...
	//#define UTMI_240_AS_120_PHASE_ECO_INV_IF_REV_0
#endif
...
//---- 6. Not yet support MIU lower bound address subtraction ECO (for chips which use ENABLE_USB_NEW_MIU_SLE)
//#define DISABLE_MIU_LOW_BOUND_ADDR_SUBTRACT_ECO

//---- 7. inverse TX timing select latch path bit if revision is zero
//#define UTMI_TX_TIMING_SEL_LATCH_PATH_INV_IF_REV_0

//------ UTMI disconnect level parameters ---------------------------------
// 0x00: 550mv, 0x20: 575, 0x40: 600, 0x60: 625
#if (ENABLE_MACAN) || (ENABLE_KASTOR) || (ENABLE_MUSTANG) || (ENABLE_MARLON)
	// for 28 nm
...
/****** Porting environment setting ******/
	...
	/* virtual address to physical address translation */
			...
			#define MIU1_PHY_BASE_ADDR	0x80000000UL
                #elif (ENABLE_MUSTANG)
                        #define ENABLE_USB_NEW_MIU_SEL  1       /* Unit: 2GB / 16 = 128MB */
                        #define USB_MIU_SEL0    0x70    /* MIU0: 1GB */
                        #define USB_MIU_SEL1    0xF8    /* MIU1: 1GB */
                        #define USB_MIU_SEL2    0xEF
                        #define USB_MIU_SEL3    0xEF
                        #define MIU0_BUS_BASE_ADDR      CONFIG_MIU0_BUSADDR
                        #define MIU0_PHY_BASE_ADDR      0x00000000UL
...
/* If RAM size >= 512MB (0x2~), we must do VA to PA for MIPS; otherwise, 0xA~ will be 0x2~, not 0x0~.
...
#define MSTAR_UHC_BASE   (HW_BASE+(0x2400 << 1))
	...
	(ENABLE_MANHATTAN) || (ENABLE_MESSI) || (ENABLE_MASERATI) || \
	(ENABLE_MACAN) || (ENABLE_MUSTANG) || (ENABLE_MILAN) || \
	(ENABLE_MARLON)
	#define MSTAR_BC_BASE	(HW_BASE+(0x23600 << 1))
#elif (ENABLE_KRATOS) || (ENABLE_KIWI)
	#define MSTAR_BC_BASE	(HW_BASE+(0x20500 << 1))
#elif (ENABLE_KANO) || (ENABLE_CURRY) || (ENABLE_KASTOR) || (ENABLE_KERES) || (ENABLE_KIRIN) || (ENABLE_KRIS)
...
#if (ENABLE_MONACO) || (ENABLE_MIAMI) || (ENABLE_MUJI) || (ENABLE_CELTICS) || \
	(ENABLE_MONET) || (ENABLE_MUNICH) || (ENABLE_MANHATTAN) || \
	(ENABLE_MESSI) || (ENABLE_MASERATI) || (ENABLE_MACAN) || \
	(ENABLE_MUSTANG) || (ENABLE_MILAN) || (ENABLE_MARLON)
...
#elif (ENABLE_KASTOR) || (ENABLE_KERES) || (ENABLE_KIRIN) || (ENABLE_KRIS)
...
#if defined(ENABLE_MILAN) || defined(ENABLE_MARLON)
	#define ENABLE_UTMI_DPDM_SWAP_P1_FOR_MARLON_P2P_MILAN
	#define _MSTAR_BONDING_OPTION_BASE (HW_BASE+(0x1E00 << 1))
#endif

#define MSTAR_COMP1			XHC_COMP_NONE

/* Port2 */
#if (ENABLE_MUNICH) || (ENABLE_MUSTANG)
...
#elif (ENABLE_MUNICH) || (ENABLE_MUSTANG) // EHCI only
...
#elif (ENABLE_MANHATTAN) || (ENABLE_MACAN)
