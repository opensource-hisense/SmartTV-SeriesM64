...
struct mmc_ops{
       char *name;
	int (*op)(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[]);
};
...
{
	char magic[16];
	unsigned char update_sts[8];
	unsigned char reserved[8];
	unsigned char customer_reserved1[16];
	unsigned char customer_reserved2[16];
}write_key_header;
